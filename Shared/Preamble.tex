...
% 空集符号取自 AMS 符号字体（msbm10）第 63 槽位，即传统 LaTeX 中
% \varnothing 的原始来源，字形为正圆带一杠。unicode-math 会把 \varnothing
% 指向 Latin Modern Math 的 U+2205（本项目字体下呈瘦高的“0 加陡斜线”），
% 且该字体不含 U+00D8 码位，故改从 AMS 符号字体的字符槽位取用——
% 字体仍属数学符号字体，与 \mathbb 同源。
\DeclareSymbolFont{analysisamssym}{U}{msb}{m}{n}
\DeclareMathSymbol{\varnothing}{\mathord}{analysisamssym}{63}
...
% 保留 amsthm 的证明间距与终止符，只将标题字形改为正体。
\makeatletter
\renewenvironment{proof}[1][\proofname]{\par
  \pushQED{\qed}%
  \normalfont \topsep6\p@\@plus6\p@\relax
  \trivlist
  \item[\hskip\labelsep\upshape #1\@addpunct{.}]\ignorespaces
}{%
...
% 与定理引用“定理 x.y”一致。章号必须是汉字（如“第八章”），故章引用取
% \chinese{chapter} 而非 \ref；节与小节取阿拉伯编号。正文不再另写“第”“节”。
% 中文与数字之间的空隙由 xeCJK 的 \CJKecglue 产生；\ref 是控制序列故不触发，
% 而 \hyperref 的链接组会使它出现，因此在此局部清空该空隙，保持紧排。
\NewDocumentCommand{\secref}{m}{%
  \begingroup\xeCJKsetup{CJKecglue={}}\hyperlink{#1}{第\ref*{#1}节}\endgroup}
\NewDocumentCommand{\subsecref}{m}{%
  \begingroup\xeCJKsetup{CJKecglue={}}\hyperlink{#1}{第\ref*{#1}小节}\endgroup}
\NewDocumentCommand{\chapref}{m}{%
  \begingroup\xeCJKsetup{CJKecglue={}}\hyperlink{#1}{\CTEXthechapter}\endgroup}

...
\NewDocumentCommand{\BookAppendixPart}{m}{%
  ...
  }{}{\PackageError{analysis}{无法为附录编题页写入目录与标签}{请检查当前文档类的 \string\@endpart 定义。}}%
  \part*{附录}%
  \endgroup
  \thispagestyle{empty}%
  \null
...
% 中文“弱∗”术语使用数学轴上居中的星号；局部抑制星号两侧的
% 中西文胶，使它与前后的汉字连写。字号随上下文变化，书签保留
% 同一居中字形的 Unicode 文本。正文、标题及索引统一使用此宏。
\NewDocumentCommand{\WeakStar}{}{%
  \texorpdfstring{%
    \begingroup\xeCJKsetup{CJKecglue={}}%
    弱\kern0pt\(\mathord{\ast}\)\kern0pt{}%
    \endgroup
  }{弱∗}}
\pdfstringdefDisableCommands{\def\WeakStar{弱∗}}
% 兼容首次重编时尚未更新的旧目录和索引辅助文件。
\NewDocumentCommand{\WeakStarJoin}{}{\texorpdfstring{\kern0pt{}}{}}

...
% 章号用 \ref* 取标签中的真实编号：不可用 \thechapter（那是当前计数器，
% 在编导读、前言或跨章引用时会给出错误章号，表现为“第零章”）。
% 章号目前为阿拉伯数字。汉字章号（第五章）需要另行记录汉字号，见提交
% 6fdbe63 的说明：在 \@chapter 上打补丁、或逐章加标签后用 \edef 组合，
% 都会与 ctex 的章标题机制或 hyperref 的 \edef 展开冲突，暂不采用。
\NewDocumentCommand{\secref}{m}{%
  \begingroup\xeCJKsetup{CJKecglue={}}\hyperref[#1]{第\ref*{#1}节}\endgroup}
\NewDocumentCommand{\subsecref}{m}{%
  \begingroup\xeCJKsetup{CJKecglue={}}\hyperref[#1]{第\ref*{#1}小节}\endgroup}
\NewDocumentCommand{\chapref}{m}{%
  \begingroup\xeCJKsetup{CJKecglue={}}\hyperref[#1]{第\ref*{#1}章}\endgroup}

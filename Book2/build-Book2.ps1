# 分析学 · 第二卷 一键编译（专用）
#
# 放在本卷目录内，专用于 Book2。一键整卷编译顺序为：
#     xelatex → biber → 三类索引 → xelatex ×2 → 校验 → 更新同目录预览与根目录成品
# 单独运行 XeLaTeX 不会显示参考文献与索引，必须走完整流程。
#
# TeXstudio 无需另建用户命令：Book2.tex 顶部的 TXS-program 魔法注释会在按“编译”
# 时调用 build-Book2.bat -NoPause；TeXstudio 以根文档目录为工作目录。若刚修改过
# 魔法注释，请重新打开根文档使其生效。“构建并查看”会打开同目录 Book2.pdf，
# 并使用同目录 Book2.synctex.gz 在源码与 PDF 之间定位。
#
# 命令行用法（在本目录下，会等待回车以便查看结果）：
#     powershell -ExecutionPolicy Bypass -File .\build-Book2.ps1
#
...
Write-Host ""
Write-Host "=== 分析学 第二卷 编译（$book）===" -ForegroundColor Cyan
Say "源码目录：$bookDir"
Say "输出目录：$buildDir"

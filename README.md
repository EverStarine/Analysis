...
- 第二卷已开始写作，第一章“多元分析与偏微分方程预备”完成四节正文、八题及全解，并补齐第一编导读；当前为待审核初稿，正式审核轮数为0。

- 第三卷第一章“全纯函数与幂级数”已完成四节正文、八题及全解，并补齐第一编导读；当前为待审核初稿，正式审核轮数为0。

- 第一卷的首次写作与第一轮系统编辑已经告一段落；这不等同于全章定稿，正式状态仍以 [审核进度](审核进度.md) 的逐章表和定稿门槛为准。
- 中文统一写作“弱∗拓扑、弱∗收敛、弱∗紧性”等，不加连字符，星号在行中居中，源码统一使用 `\WeakStar{}`；首次定义处说明“弱星”文字异名。英文原文继续保留 `weak-*`，数学记号与选读星号分别处理。

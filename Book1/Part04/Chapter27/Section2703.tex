% !TeX root = ../../Book1.tex
\section{有界变差函数的余面积公式}
\label{b1:sec:2703}

一个实值函数可以由全部超水平集重建。对于 Sobolev 函数，\secref{b1:sec:1802}用梯度长度补偿水平集上的面积；对于有界变差函数，
...
\[
 P(E,U)\coloneqq V(\mathbf1_E,U).
\]
若 \(P(E,\Omega)<\infty\)，则称 \(E\) 为 \(\Omega\) 内的%
\term[youxian-zhouchang-jihe]{有限周长集合}[set of finite perimeter]。
...
\[
 P(E,A)\coloneqq|D\mathbf1_E|(A).
\]
全空间周长简记为 \(P(E)\coloneqq P(E,\mathbb R^d)\)。
\end{definition}
...
\[
 ...
 \bigl|\nabla H_\varepsilon(v-t)\bigr|
   =\varepsilon^{-1}\cdot\mathbf1_{\{t<v<t+\varepsilon\}}\cdot|\nabla v|
       \quad\text{几乎处处}.
\]
...
\begin{theorem}[有界变差函数的余面积公式]\label{b1:thm:bv-coarea}
...
令
\[
 Z=\{t\in\mathbb R:P(E_t,\Omega)=\infty\}.
\]
则 \(Z\) 是 Lebesgue 零测集。对 \(t\notin Z\) 令
\(\mu_t=|D\mathbf1_{E_t}|\)，对 \(t\in Z\) 令 \(\mu_t=0\)。于是对任意 Borel 集
\(A\subseteq\Omega\)，都有
\begin{equation}\label{b1:eq:bv-coarea-measure}
 |Du|(A)=\int_{\mathbb R}\mu_t(A)\,\mathrm dt.
\end{equation}
辅助测度在零测集 \(Z\) 上怎样补值不影响积分；这里取零只为统一记号，并不把例外层实际存在的局部分布导数改定义为零。
\end{theorem}
\begin{proof}
...
取 \(U=\Omega\)，右端有限，所以定理中定义的 \(Z\) 是 Lebesgue 零测集。

还须将开集等式推广到 Borel 集，而不为每个集合另选一组例外层值。
按定理中的定义，\(\mu_t\) 都是 \(\Omega\) 上的有限 Borel 测度，且对每个开集 \(U\)，
\(t\mapsto\mu_t(U)\) 可测。
...
对于一般有界变差函数，几乎所有层集具有有限周长并不意味着每个层集都有光滑边界，
也不意味着例外层值不存在。

\subsection{Cavalieri 型公式}
\label{b1:subsec:270304}

普通的 Cavalieri 分层公式分解体积，有界变差函数的余面积公式分解变差。
...
例如取 \(u=c\cdot\mathbf1_E\)，其中 \(\mathbf1_E\in BV(\Omega)\)、\(c\ne0\)。
...
\begin{equation}\label{b1:eq:bv-weighted-coarea}
 ...
 =\int_{\mathbb R}\left(\int_\Omega g\,\mathrm d\mu_t\right)\,\mathrm dt.
\end{equation}
...
\(A\subseteq\Omega\)，
\[
 |D F_{a,b}(u)|(A)=\int_a^b\mu_t(A)\,\mathrm dt.
\]
\end{proposition}
\begin{proof}
截断有有符号表示
...
\cref{b1:fig:visual-f19}直接计算一个分层模型中的 BV 余面积公式。
\input{Figures/F19}

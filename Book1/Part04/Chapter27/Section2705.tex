...
\begin{equation}\label{b1:eq:relative-isoperimetric-ball}
 ...
 \le C_d\cdot \mu_E\bigl(B(x,r)\bigr).
\end{equation}
\end{lemma}
\begin{proof}
...
\[
 \|\chi_E-a\|_{L^{d/(d-1)}(B)}
 \le C_d\cdot \mu_E(B),\quad a=\frac{m_d(E\cap B)}{m_d(B)}.
\]
若 \(a\le1/2\)，左侧至少为
\(\tfrac12\cdot m_d(E\cap B)^{(d-1)/d}\)；若 \(a\ge1/2\)，则改在补集上估计。这就证明了相对等周不等式。
\end{proof}

\begin{lemma}[约化点附近的周长与两侧体积估计]
\label{b1:lem:reduced-boundary-scale-estimates}
设 \(d\ge2\)，\(x\in\partial^*E\)。则存在 \(r_x>0\) 及只依赖于维数的常数 \(c_d,C_d>0\)，使对 \(0<r<r_x\)，
\[
 \mu_E\bigl(B(x,r)\bigr)\le C_dr^{d-1},
\]
且
\[
 \min\Bigl(m_d\bigl(E\cap B(x,r)\bigr),
            m_d\bigl(B(x,r)\setminus E\bigr)\Bigr)
 \ge c_dr^d.
\]
\end{lemma}
\begin{proof}
...
\[
 ...
 \ge\tfrac12\cdot \mu\bigl(B(0,r)\bigr).
\]
...
\[
 M(r)^{(d-1)/d}
 \le C_d\cdot \mu\bigl(B(0,r)\bigr)
 \le C'_d\min(V'(r),W'(r))
 \le C'_d M'(r)
\]
...
\begin{lemma}[约化点的缩放极限]
\label{b1:lem:reduced-boundary-set-blowup}
设 \(d\ge2\)，\(x\in\partial^*E\)，并记
\[
 E_{x,r}=\{z:x+rz\in E\},\qquad
 H_x=\{z:z\cdot\nu_E(x)<0\}.
\]
则每个趋零半径序列 \((r_j)\) 都有一个子列，使
\[
 \chi_{E_{x,r_j}}\longrightarrow\chi_{H_x}
 \quad\text{于 }L^1_{\mathrm{loc}}(\mathbb R^d).
\]
特别地，任意存在的局部 \(L^1\) 缩放子极限都只能是 \(H_x\)。
\end{lemma}
\begin{proof}
仍可设 \(x=0\)、\(\nu_E(0)=e_d\)，并简记 \(\mu=\mu_E\)、\(\sigma=\sigma_E\)。
对任意趋零序列 \(r_j\)，作变量代换可得
\[
 |D\chi_{E_{0,r_j}}|(B(0,R))
 =r_j^{1-d}\cdot \mu\bigl(B(0,r_jR)\bigr)\le C_dR^{d-1}
\]
对每个固定 \(R\) 及充分大的 \(j\) 成立；这里使用了\cref{b1:lem:reduced-boundary-scale-estimates}。零阶 \(L^1\) 范数也受球体积控制。逐球使用\cref{b1:thm:bv-local-compactness}并对角抽列，得到
...
对任意固定 \(s>0\)，将\eqref{b1:eq:reduced-two-sided-volume}伸缩并传给极限，球 \(B(0,s)\) 中 \(F\) 及其补集的体积都至少为 \(c_ds^d\)。若 \(b\ne0\)，其中也包括 \(b=\pm\infty\)，取足够小的 \(s\)，其中一侧就会为空，矛盾。因此 \(b=0\)。恢复平移与旋转，所取子列的极限就是 \(H_x\)。
\end{proof}

\cref{b1:fig:reduced-boundary-blowup}把约化边界处的结论画成“放大后变平”。左图的曲面只表示局部几何；正式结论是示性函数的局部 \(L^1\) 收敛以及周长测度的缩放收敛。
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[>=stealth]
    \begin{scope}[shift={(-2.7,0)}]
      \begin{scope}
        ...
        \fill[gray!25] (-1.7,-1.7)--(-1.7,0.48)
          .. controls (-1.1,0.20) and (-0.45,0) .. (0,0)
          .. controls (0.45,0) and (1.1,0.20) .. (1.7,0.48)
          --(1.7,-1.7)--cycle;
      \end{scope}
      \draw[thick] (-1.45,0.42)
        .. controls (-0.95,0.16) and (-0.40,0) .. (0,0)
        .. controls (0.40,0) and (0.95,0.16) .. (1.45,0.42);
      \draw (0,0) circle (1.45);
      \fill (0,0) circle (1.4pt) node[below right] {$x$};
      \draw[->,thick] (0,0.05)--(0,1.15) node[right] {$\nu_E(x)$};
      \node[below=1.65cm] at (0,0) {原尺度};
    \end{scope}
    \draw[->,thick] (-0.7,0)--(0.7,0) node[midway,above] {$r\downarrow0$};
    \begin{scope}[shift={(2.7,0)}]
      \begin{scope}
        \clip (0,0) circle (1.45);
        \fill[gray!25] (-1.6,-1.6) rectangle (1.6,0);
      \end{scope}
      \draw[thick] (-1.45,0)--(1.45,0);
      \draw (0,0) circle (1.45);
      \draw[->,thick] (0,0.05)--(0,1.15) node[right] {$\nu_E(x)$};
      \node[below=1.65cm] at (0,0) {极限半空间 \(H_x\)};
    \end{scope}
  \end{tikzpicture}
  \caption[约化边界的半空间缩放]{约化边界的半空间缩放。灰色表示集合一侧；缩小尺度后，边界趋向 \(\nu_E(x)^\perp\)，集合趋向相应半空间。}
  \label{b1:fig:reduced-boundary-blowup}
\end{figure}

\begin{theorem}[约化边界处的半空间缩放]\label{b1:thm:reduced-boundary-blowup}
设 \(d\ge2\)，\(x\in\partial^*E\)，并采用\cref{b1:lem:reduced-boundary-set-blowup}中的 \(E_{x,r}\) 与 \(H_x\)。则当 \(r\downarrow0\) 时，
...
\end{theorem}
\begin{proof}
若\eqref{b1:eq:reduced-boundary-halfspace}不成立，就能选取一个趋零半径序列，使相应示性函数在某个固定球上的 \(L^1\) 距离始终离开零。\cref{b1:lem:reduced-boundary-set-blowup}却从该序列抽出趋于 \(\chi_{H_x}\) 的子列，矛盾。因此全部半径下的集合缩放收敛成立。

为证明周长测度收敛，经平移、旋转设 \(x=0\)、\(\nu_E(0)=e_d\)，并简记 \(\mu=\mu_E\)、\(\sigma=\sigma_E\)。记缩放周长测度为
\(\mu_r(A)=r^{1-d}\cdot \mu(rA)\)。
...
\]
\cref{b1:lem:reduced-boundary-scale-estimates}给出的紧集上一致质量界允许用光滑函数一致逼近一般 \(C_c\) 测试函数，恢复坐标后得到\eqref{b1:eq:perimeter-flat-blowup}。平面测度不给单位球面质量，故用内外连续截断逼近球的示性函数，可得\eqref{b1:eq:reduced-perimeter-unit-density}。集合密度 \(1/2\) 则直接来自半空间的体积及 \(L^1\) 收敛。
\end{proof}
...
\]
这里还须核对这些确实是 Borel 条件。先选取 \(E\) 的 Borel 代表；零测修改不改变上述体积。由\cref{b1:prop:reduced-boundary-carries-perimeter}，\(\nu_E\) 在 \(\partial^*E\) 上可取 Borel 代表，把它在补集上延为一个固定单位向量，记为 \(\bar\nu_E\)。对固定 \(r>0\)，函数
\[
 (x,y)\longmapsto
 \mathbf1_{B(x,r)}(y)
 \left|
  \mathbf1_E(y)-
  \mathbf1_{\{z:(z-x)\cdot\bar\nu_E(x)<0\}}(y)
 \right|
\]
非负且联合 Borel，故由\cref{b1:prop:parameter-integral-measurability}，对 \(y\) 积分所得的半空间误差是 \(x\) 的 Borel 函数。同理，\(x\mapsto m_d(E\cap B(x,r))\) 及其补集体积也是 Borel 函数。对固定 \(x\)，上述三个量随 \(r\) 连续，因为两个半径之间的差至多为相应球壳的体积。因此，对全部半径的要求等价于只检查有理半径；距离函数还使 \(B(x,1/m)\subset\Omega\) 成为 Borel 条件。这样才得到所需的可数 Borel 分片。再按法向所落入的球面小邻域分片，使一片上
...
\end{equation}
每个 \(x\in\partial^*E\) 的集合密度为 \(1/2\)。当 \(d\ge2\) 时，在\chapref{b1:ch:19}的近似切平面意义下，
\[
...
\]
由已完整证明的\cref{b1:prop:rectifiable-weight-density}，\(\mu\) 的 \(k\) 维密度在 \(\mathcal H^k\)-几乎处处等于 \(\theta\)。而\eqref{b1:eq:reduced-perimeter-unit-density}说明该密度在每个 \(S\) 中的点都为 \(1\)，所以 \(\theta=1\) 几乎处处，证明了测度表示。特别地，未加权的约化边界面积现在也已知局部有限。将测度表示代入\eqref{b1:eq:perimeter-flat-blowup}，正好得到\chapref{b1:ch:19}的单位密度平面缩放极限；由\cref{b1:prop:approximate-tangent-properties}，相应的近似切平面方向为 \(\nu_E(x)^\perp\)。
...
分片，并与内部紧穷竭相交。球质量的 Borel 性及有理半径逼近说明所得各片 Borel；每片上都有任意小半径满足 \(\mu\bigl(B(x,r)\bigr)\ge r^k/j\)。固定其中一片 \(N_j\)，它是 \(\mu\)-零集，并包含在某个内部紧集内。由\cref{b1:prop:radon-outer-regularity}中“紧集内 Borel 集的局部外正则性”，给定 \(\varepsilon>0\)，可取开集 \(G\supset N_j\)，使 \(\mu(G\setminus N_j)<\varepsilon\)，从而 \(\mu(G)<\varepsilon\)。再对每个点选取任意小、包含于 \(G\) 且满足上述下界的球。由\cref{b1:lem:five-r-covering}选出不交球 \(B_i=B(x_i,r_i)\)，其五倍球覆盖该片。因此在任意小的 Hausdorff 覆盖尺度上，
\[
 \mathcal H^k_{\delta}(N_j)
 \le C_d\sum_i r_i^k
 \le C_d\cdot j\cdot\sum_i\mu(B_i)
 \le C_d\cdot j\cdot\varepsilon.
\]
...
证明的主干可与 Simon 的《Lectures on Geometric Measure Theory》中 De Giorgi 定理的证明对读\cite[\S14, 14.3 THEOREM, pp. 72--76]{Simon1983Lectures}。其中“约化点处的方向集中—缩放紧性—半空间极限”的顺序尤其重要。本书另外把图分片构造、密度识别及测度论边界余项的覆盖论证展开，以便这些结论只依赖已经建立的工具。
...
这一表示还说明，示性函数的导数是集中在余维一可整流集上的测度；一般有界变差函数的变化则由全部水平集共同组成。与\cref{b1:thm:bv-coarea}合起来，函数的总变差便可以理解为各层约化边界面积的累积。不过本节没有断言所有有界变差函数的导数都由单独一张超曲面上的面积表示：不同水平的边界可以形成完全不同的整体分布。

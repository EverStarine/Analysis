...
对 Sobolev 函数，磨光可以同时逼近函数及其弱梯度；对一般 BV 函数，导数可能含有集中在零测集上的部分，光滑梯度不能按总变差范数逼近这种测度。本节保留函数的 \(L^1\) 收敛和导数的总量收敛，并用导数测度控制平移，得到局部紧性。所有函数均取实值，\(|Du|\) 使用前一节的欧氏全变差。

Simon 的《Lectures on Geometric Measure Theory》\cite[第6节，6.2--6.3]{Simon1983Lectures}以磨光及变差对偶式组织 BV 的逼近与紧性。下面的全域严格逼近另外展开单位分解的误差控制；局部紧性的抽选步骤则调用已经证明的 \(L^1\) 平移判据。
...
\]
则称 \(u_j\) 在 \(BV(\Omega)\) 中\term[yange-shoulian]{严格收敛}[strict convergence]到 \(u\)。
\end{definition}
...
\begin{theorem}[BV 局部紧性]\label{b1:thm:bv-local-compactness}
...
若原序列在全域 BV 范数中一致有界，局部紧性得到的极限也属于全域
...
\]
这同时是一个 BV 有界列，却没有全域 \(L^1\) 收敛子列。对有界
Lipschitz 区域，若要通过延拓证明无附加条件的 BV 全局紧性，还须先建立 BV 延拓定理；第22章的 \(W^{1,1}\) 延拓算子并未定义在一般 BV 函数上，不能直接套用。
...
设 \(u_j,u\in BV(\Omega)\)。若 \(u_j\to u\) 于 \(L^1(\Omega)\)，且有限向量测度 \(Du_j\) 对全部 \(C_0(\Omega;\mathbb R^d)\) 测试弱-*收敛到 \(Du\)，则称这种收敛为\term[bv-ruoxing-shoulian]{BV 弱-*收敛}[weak-* convergence in BV]。
\end{definition}

因此，在 BV 有界列上，一旦已经取得全域 \(L^1\) 收敛，导数测度的弱-*收敛就随之成立。只取得局部 \(L^1\) 收敛时，仍有前一命题的测度测试结论，却不能省去本定义中的全域 \(L^1\) 条件。
...
\[
 ...
 |Du_j|(\Omega)=1.
\]
这里 \(C_0\bigl((0,1)\bigr)\) 测试在边界附近趋零；常数函数 \(1\) 不属于这个测试空间。若改用全部 \(C_b\) 测试，就换成了第十二章的另一种测度收敛。

...
设 \(u_j,u\in BV(\Omega)\)。若 \(u_j\to u\) 于 \(L^1(\Omega)\)，且有限向量测度 \(Du_j\) 对全部 \(C_0(\Omega;\mathbb R^d)\) 测试\WeakStar{}收敛到 \(Du\)，则称这种收敛为\term[youjie-biancha-kongjian-ruoxing-shoulian]{有界变差空间中的\WeakStar{}收敛}[weak-* convergence in BV]。
\end{definition}

因此，对于有界变差函数空间中的有界列，一旦已经取得全域 \(L^1\) 收敛，导数测度的\WeakStar{}收敛就随之成立。只取得局部 \(L^1\) 收敛时，仍有前一命题的测度测试结论，却不能省去本定义中的全域 \(L^1\) 条件。

\ProseParagraphBreak

\WeakStar{}收敛不保证总变差数值收敛。边界逃逸给出一个例子：在
...
\(\Omega=(0,2\pi)\)、\(u_j(x)=j^{-1}\cdot \sin(jx)\)。函数在 \(L^1\) 中趋零，导数为 \(\cos(jx)\,m_1\)，故由前一命题\WeakStar{}趋零，但
\[
 |Du_j|(\Omega)=\int_0^{2\pi}|\cos(jx)|\,\mathrm dx=4.
\]
这些变差测度在边界宽度为 \(\delta\) 的区域内质量至多 \(2\delta\)，所以即使控制了边界质量，也不能从导数的\WeakStar{}收敛推出总变差收敛。严格逼近定理额外取得的，正是后一个数值极限。

...
设 \(u\in BV_{\mathrm{loc}}(\Omega)\)、\(\eta\in C_c^\infty(\Omega)\)，把 \(\eta\cdot u\) 在 \(\Omega\) 外补零，记为 \(v\)。则 \(v\in BV(\mathbb R^d)\)，且
\begin{equation}\label{b1:eq:bv-cutoff-derivative}
 Dv=\eta\cdot Du+u\cdot\nabla\eta\,m_d,
 \quad
 |Dv|(\mathbb R^d)
...
\(\eta\cdot\Phi\) 的支集紧含于 \(\Omega\)。乘积公式给出
\[
 \begin{aligned}
 \int_{\mathbb R^d}v\,\operatorname{div}\Phi
 &=\int_\Omega u\,\operatorname{div}(\eta\cdot\Phi)
       -\int_\Omega u\cdot\nabla\eta\cdot\Phi\\
 &=-\int_\Omega\eta\cdot\Phi\cdot\mathrm dDu
       -\int_\Omega u\cdot\nabla\eta\cdot\Phi.
 \end{aligned}
\]
...
\end{theorem}
\begin{proof}
给定 \(\varepsilon>0\)。取\cref{b1:cor:precompact-smooth-partition}给出的局部有限开覆盖与光滑单位分解，使
\[
 \psi_i\geq0,\quad \sum_i\psi_i=1,\quad
 K_i=\operatorname{supp}\psi_i\Subset V_i\Subset\Omega.
\]
每个紧集只遇到有限多个 \(V_i\)。

令 \(f_i=\psi_i\cdot u\)，并补零到全空间。由前一引理，
\[
 Df_i=\mu_i+g_i\,m_d,\quad
 \mu_i=\psi_i\cdot Du,\quad g_i=u\cdot\nabla\psi_i.
\]
每个 \(f_i,g_i\) 都属于全空间 \(L^1\)，且支集包含于 \(K_i\)。选择足够小的半径 \(\delta_i>0\)，同时满足
\[
 K_i+\overline B(0,\delta_i)\subset V_i,\quad
...
\(\eta_m\in C_c^\infty(U_{m+1})\)、\(0\leq\eta_m\leq1\)，在 \(U_m\) 上等于 \(1\)。把 \(\eta_m\cdot u_j\) 补零到全空间，记为 \(v_{m,j}\)。由截断引理及假设，
\[
...
仅有“\(\Omega\) 有界”不能代替这些条件。\cref{b1:ex:25-infinitely-many-components}中，取 \(p=1\)，在越来越小的不同连通分量上放置归一化常数，得到
\[
 \|u_j\|_{L^1(\Omega)}=1,\quad Du_j=0,\quad
 \|u_j-u_k\|_{L^1(\Omega)}=2\quad(j\ne k).
\]
这同时是有界变差函数空间中的有界列，却没有全域 \(L^1\) 收敛子列。对有界 Lipschitz 区域，标准的有界变差函数延拓定理会给出无附加尾部条件的紧嵌入
\[
 BV(\Omega)\Subset L^1(\Omega).
\]
本章尚未建立这一延拓定理，所以这里只证明适用于任意开集的尾部条件版本。第22章的 \(W^{1,1}\) 延拓算子并未定义在一般有界变差函数上，不能直接套用。换言之，附加尾部条件是本章当前证明路线的范围，不是对规则区域上标准紧性定理的否定。
...
设 \(u_j,u\in BV(\Omega)\)。若 \(u_j\to u\) 于 \(L^1(\Omega)\)，且有限向量测度 \(Du_j\) 对全部 \(C_0(\Omega;\mathbb R^d)\) 测试弱*\WeakStarJoin{}收敛到 \(Du\)，则称这种收敛为\term[youjie-biancha-kongjian-ruoxing-shoulian]{有界变差空间中的弱*\WeakStarJoin{}收敛}[weak-* convergence in BV]。
\end{definition}

因此，对于有界变差函数空间中的有界列，一旦已经取得全域 \(L^1\) 收敛，导数测度的弱*\WeakStarJoin{}收敛就随之成立。只取得局部 \(L^1\) 收敛时，仍有前一命题的测度测试结论，却不能省去本定义中的全域 \(L^1\) 条件。

\ProseParagraphBreak

弱*\WeakStarJoin{}收敛不保证总变差数值收敛。边界逃逸给出一个例子：在
...
\(\Omega=(0,2\pi)\)、\(u_j(x)=j^{-1}\cdot \sin(jx)\)。函数在 \(L^1\) 中趋零，导数为 \(\cos(jx)\,m_1\)，故由前一命题弱*\WeakStarJoin{}趋零，但
\[
 |Du_j|(\Omega)=\int_0^{2\pi}|\cos(jx)|\,\mathrm dx=4.
\]
这些变差测度在边界宽度为 \(\delta\) 的区域内质量至多 \(2\delta\)，所以即使控制了边界质量，也不能从导数的弱*\WeakStarJoin{}收敛推出总变差收敛。严格逼近定理额外取得的，正是后一个数值极限。

导数测度的收敛、其总量收敛及其差的总变差收敛并不等价，见\cref{b1:tab:visual-t15}。
\input{Tables/T15}

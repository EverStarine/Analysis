...
得到 De Giorgi 结构定理及 Gauss–Green 公式。最后把 BV 的 Sobolev 端点估计与集合的等周估计联系起来。
\end{chapterabstract}

\begin{learninggoals}
\begin{enumerate}
\item 从向量测度的欧氏全变差和测试场对偶式识别 BV，区分它与 \(W^{1,1}\) 的导数类型。
\item 完整掌握严格光滑逼近中的单位分解误差，区分局部紧性、全域紧性、弱-*收敛和范数收敛。
\item 从层集截断推导 BV 余面积公式，说明它为何不预先要求光滑边界或结构定理。
\item 区分测度论边界、约化边界和任意代表的拓扑边界，从半空间缩放恢复法向及周长面积。
\item 理解函数与集合不等式之间的双向传递，不把非最优常数的证明误认为尖锐等周定理。
\end{enumerate}
\end{learninggoals}

在一维，阶跃函数虽然没有可积的弱导数，却有一个十分明确的导数：跳跃点上的点质量。
在高维，两个常值区域之间的一道界面也应当贡献变化，而不是因为经典梯度几乎处处为零就被忽略。
BV 空间保留了这类变化，把导数放进有限测度而非仅仅可积函数的范围。
因此它既与前面的 Sobolev 理论连续衔接，又能直接处理不连续函数和集合边界。

本章的证明次序依赖这一选择。先建立变差的分析工具，再用示性函数定义周长；
余面积公式只凭这些工具就能完成。此后才讨论周长究竟集中在哪一种边界上，
不能把尚未证明的“周长等于面积”反过来放进前面的定义或逼近证明。
结构定理中最重要的桥梁是两侧体积下界：方向集中虽然指向一个半空间，
但若没有这项下界，缩放极限仍可能退化为全空间或空集。

Simon 的《Lectures on Geometric Measure Theory》%
\cite[第6节、第14节]{Simon1983Lectures}是本章局部变差及边界结构的对读资料。
本书展开其中需要承接的单位分解、平移紧性和图分片步骤；
更系统的 BV 理论可参阅 Ambrosio、Fusco 与 Pallara 的%
...
BV 函数以有限导数测度控制变化，既容纳可积梯度，也容纳跳跃及更分散的奇异变化。
光滑逼近保持函数的 \(L^1\) 极限和总变差数值，却通常不能保持导数的变差范数极限；
紧性则依赖平移控制，并在全域结论中继续要求质量不逃向边界或无穷远。

余面积公式把导数变差分解为水平集周长。对每个有限周长集合，
De Giorgi 定理进一步把周长识别成约化边界的余维一面积，
其证明先取得非退化半空间极限，再构造可数 Lipschitz 图并辨认密度。
Gauss–Green 的外法向负号与分布导数的约定相配合，不能随文献记号一起忽略。

全空间 BV 端点不等式与有限体积集合的等周不等式具有相同的常数传递机制。
本章证明了它们的有效估计，没有计算最佳常数。
一般 BV 函数并非单个集合的示性函数；各层边界的累积还会形成怎样的导数，
将由下一章的精细结构解释。
\end{chaptersummary}

\BookExercises

% 第二十七章习题临时稿；不另设 BookExercises 入口。
% 八题均由本章与前章已证明工具自拟，逐题注明实质增量，不冒认教材原题号。
% 不调用第28章的一般BV链式法则、跳跃集结构或SBV理论。
...
\begin{exercise}\label{b1:ex:27-translation-bv-norm-characterization}
...
\begin{exercise}\label{b1:ex:27-finite-valued-strict-approximation}
...
\begin{exercise}\label{b1:ex:27-confined-bv-minimization}
设 \(d\ge2\)，考虑
\[
...
\]
由带尾部控制的 BV 紧性推论%
\ref{b1:cor:bv-global-compactness-with-tails}，可取全空间 \(L^1\) 收敛子列，
...
由 BV 的 Sobolev 端点估计\ref{b1:thm:bv-sobolev-endpoint}便有 \(\|v\|_{d/(d-1)}=0\)，
与 \(\int v=1\) 矛盾。

...
\end{theorem}
\begin{proof}
取 Sobolev 有界列。前一命题给出在 \(L^p\) 中收敛的子列。若 \(q<p\)，有限测度上的 Hölder 不等式
\[
...
当 \(p<d\) 时，由第24章的域上 Sobolev 嵌入可取 \(r=p^*=dp/(d-p)\)。当 \(p=d\ge2\) 时，第\ref{b1:sec:2405}节已经证明任意有限指数的连续嵌入，任选有限的 \(r>q\) 即可。若不读该选读节，也可先在有限测度的 \(\Omega\) 上降到 \(p_0<d\)，选 \(p_0\) 足够接近 \(d\) 使 \(dp_0/(d-p_0)>q\)，再用次临界嵌入获得同一 \(L^r\) 界。

剩下 \(d=p=1\)。利用一阶延拓取得共同有界支集的 \(W^{1,1}(\mathbb R)\) 函数；其绝对连续代表满足
\[
...
设 \(\Omega\subset\mathbb R^d\) 为任意有界开集。对 \(W_0^{1,p}(\Omega)\)，%
\cref{b1:thm:rellich-subcritical}的全部有限目标指数结论仍成立。
当 \(d<p\le\infty\) 时，补零后的连续代表限制到 \(\overline\Omega\)，还给出%
\cref{b1:thm:rellich-holder}中的紧嵌入。
\end{corollary}
\begin{proof}
...
当 \(p>d\) 时，对全空间补零函数使用 Morrey 估计；\(p=\infty\) 时使用全空间 Lipschitz 代表。于是有限网抽列及 Hölder 插值的证明都直接用于紧集 \(\overline\Omega\)。
\end{proof}
...
内部紧性则完全不需要全域边界条件。若 \((u_j)\) 在 \(W^{1,p}_{\mathrm{loc}}(\Omega)\) 中局部有界，固定一个 \(\overline U\Subset\Omega\)，取等于一于 \(\overline U\) 附近的光滑紧支集截断。乘积在全空间补零后属于一个有界的 Sobolev 族，因而在 \(U\) 上具有上述相应强收敛子列。用可数内部穷尽和对角抽选，可以取得一个在所有内部子域上收敛的共同子列。它不保证原区域的全局范数收敛，因为质量仍可能靠近边界或移向无穷远。
...
若关心更高的积分指数，可在上述证明中对每个低阶导数使用第24章的高阶连续嵌入，再与已获得的强 \(L^p\) 收敛插值。例如，在该命题的任一种边界条件下，若 \(kp<d\)，则
\[
 W^{k,p}(\Omega)\subset L^q(\Omega),
 \qquad 1\le q<\frac{dp}{d-kp}
\]
为紧嵌入；在零边界情形把左端换成 \(W_0^{k,p}\)。右端的严格不等式仍不能省去。

同样，若 \(k-d/p=m+\alpha>0\) 且 \(m\) 为非负整数、\(0<\alpha<1\)，第24章给出全部不超过 \(m\) 阶导数的统一连续界及 \(m\) 阶导数的统一 Hölder 界。对这有限多个分量使用有限网抽列和\eqref{b1:eq:holder-interpolation-compact}，便得到每个 \(0<\beta<\alpha\) 对应的 \(C^{m,\beta}(\overline\Omega)\) 紧嵌入。极限分量的导数关系由内部线段积分通过一致极限确认，与第24章相同。正整数余量的情形，则在该章已证明的低于1的指数之间再留出严格余量。

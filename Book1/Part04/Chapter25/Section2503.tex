...
\[
 ...
 \right)<\infty.
\]
由\cref{b1:prop:sobolev-separable-reflexive}与\cref{b1:cor:sobolev-weak-subsequence}，可抽取子列，使
\[
...
在两个端点，上述自反性理由不再适用。当 \(p=1\) 时，有界导数的积分可能集中；\(p=\infty\) 时，利用 \(L^1\) 测试得到的弱*\WeakStarJoin{}收敛也不等于由整个连续对偶测试的弱收敛。前者的集中问题可回看\secref{b1:sec:1005}，后者的区别见\cref{b1:prop:sobolev-weak-components}后的说明。
...
已有弱极限时，紧性不只是再给出一条强收敛子列。它能确定像序列的全部极限，从而使整列收敛。\cref{b1:ex:09-compact-improves}中的抽列方法在此成为使用紧嵌入的基本步骤。
...
要恢复原 Sobolev 范数的收敛，可以补充范数大小的信息。下面把所需的标量事实明确记为 \(L^p\) 的\term[radon-riesz-xingzhi]{Radon–Riesz 性质}[Radon--Riesz property]。

\begin{lemma}[Radon–Riesz]\label{b1:lem:lp-weak-norm-strong}
...
\[
 \left|\int_0^{2\pi}\psi(x)\cdot\cos(jx)\,\mathrm dx\right|
 =\frac1j\left|\int_0^{2\pi}\psi'(x)\cdot\sin(jx)\,\mathrm dx\right|
...
下面在一个具体能量上实施\secref{b1:sec:0905}的变分直接法。先区分两类紧性机制：对本节的闭仿射约束和凸能量，自反性给出的弱紧性与凸泛函的弱下半连续性已经足够；若约束并非弱闭，或能量含有可由较弱范数连续控制的非凸低阶项，才通常需要 Rellich 紧性恢复强收敛。当前模型中的零阶正项同时控制函数大小，因此不需要借助 Poincaré 不等式，也不必为区域增加边界正则性。
...
\[
 ...
 \int_\Omega\bigl(
  |\nabla u|^{p-2}\nabla u\cdot\nabla(v-u)
       +|u|^{p-2}\cdot u\cdot(v-u)-f\cdot(v-u)
 \bigr)\,\mathrm dx=0.
 \end{split}
\]

...
\]
\[
 \mathcal C=\{\,e^{inx}\mathbf1_{[0,2\pi]}:n\ge1\,\}.
\]
...
\[
 \tau_{h_n}\bigl(e^{inx}\mathbf1_{[0,2\pi]}\bigr)
 =e^{in(x-h_n)}=-e^{inx},
\]
因而
\[
 \bigl\|\tau_{h_n}\bigl(e^{inx}\mathbf1_{[0,2\pi]}\bigr)
       -e^{inx}\mathbf1_{[0,2\pi]}\bigr\|_2^2
 \ge4(2\pi-h_n).
\]
虽然 \(h_n\to0\)，这些平移误差不趋于零，所以一致平移连续性失败。又由区间上的指数函数正交性，对 \(n\ne m\) 有
\[
 \|e^{inx}\mathbf1_{[0,2\pi]}-
       e^{imx}\mathbf1_{[0,2\pi]}\|_2^2=4\pi,
\]
故同样没有 Cauchy 子列。
\end{solution}
...
\item 取 \(u_n(x)=n^{-1}\sin(nx_1)\)。证明 \(u_n\rightharpoonup0\) 于 \(W^{1,2}(\Omega)\)，但不在该空间中强收敛。再说明增加哪一项范数信息即可恢复强收敛。
\end{enumerate}
\end{exercise}

...
阅读\secref{b1:sec:2501}时可以暂时忘记弱导数，只关心怎样用有限个平均值逼近整族函数。\secref{b1:sec:2502}再将这个判据用于 Sobolev 空间，所需延拓已在第22章建立。其中 \(d=p=1\) 的端点只调用主线\secref{b1:sec:1604}中的实值绝对连续函数理论，不依赖随后关于度量曲线的选读部分。最后一节接回第9章的自反性与弱下半连续性。处理近似问题时，先辨认已有的有界估计，再判断通过极限所需的收敛方式。

\ProseParagraphBreak

平移判据可对照 Hanche-Olsen 与 Holden 的《The Kolmogorov--Riesz compactness theorem》\cite[Section 3]{HancheOlsenHolden2010Compactness}阅读；本章把其有限维投影路线中的网格平均误差和远处截断分别展开。Sobolev 紧嵌入及变分方法的系统伴读可选 Adams 与 Fournier 的《Sobolev Spaces》\cite{Adams2003Sobolev}和 Brezis 的《Functional Analysis, Sobolev Spaces and Partial Differential Equations》\cite{Brezis2011Functional}。本章只使用前文已经建立的工具；第二卷的偏微分方程理论不是前置。
...
边界规则性不是所有版本都需要的条件。一般全边界函数可借 Lipschitz 延拓，零边界闭包可以补零，局部问题可以先截断；只有一般高阶全边界结论需要真正控制相应阶导数的延拓，零边界高阶结论仍可直接使用补零。临界指数处不再保留插值余量，集中例子说明这会实质性地破坏紧性。
...
% 十题均按本章及前章已建立的工具自拟，来源与实质增量逐题说明，不冒认教材原题号。
% 不使用容量、谱完备性或尚未建立的偏微分方程正则性。

\begin{exercise}\label{b1:ex:25-fk-model-families}
在复空间 \(L^2(\mathbb R)\) 中考察下列三个函数族：
\[
 \mathcal A=\{\,\mathbf1_{[0,t]}:0\le t\le1\,\},\qquad
 \mathcal B=\{\,\mathbf1_{[n,n+1]}:n\ge1\,\},
\]
\[
 \mathcal C=\{\,e^{inx}\cdot\mathbf1_{[0,2\pi]}:n\ge1\,\}.
\]
逐一检查 Fréchet–Kolmogorov 定理中的范数有界、尾部一致控制和一致平移连续性，并据此判断三个函数族是否相对紧。对不相对紧的函数族，再直接给出没有 Cauchy 子列的理由。
\end{exercise}
% 来源：自拟基础辨析题；用区间参数、整体平移和固定支集高频振荡分别呈现判据的三种典型行为。
\begin{solution}
三个函数族的范数都一致有界。对 \(\mathcal A\)，所有函数支集于 \([0,1]\)，故尾部一致为零；若 \(|h|\le1\)，则
\[
 \|\tau_h\mathbf1_{[0,t]}-\mathbf1_{[0,t]}\|_2^2
 =m_1\bigl((h,t+h)\mathbin{\triangle}(0,t)\bigr)
 \le2|h|,
\]
端点的取舍不影响等价类。因此 \(\mathcal A\) 满足三个条件并相对紧。也可直接看到
\[
 \|\mathbf1_{[0,t]}-\mathbf1_{[0,s]}\|_2^2=|t-s|,
\]
所以它是紧区间在连续映射下的像。

对 \(\mathcal B\)，同一对称差计算给出
\[
 \sup_{n\ge1}\|\tau_h\mathbf1_{[n,n+1]}-
                    \mathbf1_{[n,n+1]}\|_2^2\le2|h|
 \quad(|h|\le1),
\]
但尾部条件失败：给定 \(R\)，取足够大的 \(n\)，整个区间 \([n,n+1]\) 都落在 \(\{\,|x|>R\,\}\) 中，球外范数仍为 \(1\)。不同成员的支集至多交于零测端点，故其两两距离为 \(\sqrt2\)，不存在 Cauchy 子列。

对 \(\mathcal C\)，所有函数具有共同支集且范数均为 \(\sqrt{2\pi}\)，所以范数与尾部条件成立。令 \(h_n=\pi/n\)。在 \([h_n,2\pi]\) 上有
\[
 \tau_{h_n}\bigl(e^{inx}\cdot\mathbf1_{[0,2\pi]}\bigr)
 =e^{in(x-h_n)}=-e^{inx},
\]
因而
\[
 \bigl\|\tau_{h_n}\bigl(e^{inx}\cdot\mathbf1_{[0,2\pi]}\bigr)
       -e^{inx}\cdot\mathbf1_{[0,2\pi]}\bigr\|_2^2
 \ge4(2\pi-h_n).
\]
虽然 \(h_n\to0\)，这些平移误差不趋于零，所以一致平移连续性失败。又由区间上的指数函数正交性，对 \(n\ne m\) 有
\[
 \|e^{inx}\cdot\mathbf1_{[0,2\pi]}-
       e^{imx}\cdot\mathbf1_{[0,2\pi]}\|_2^2=4\pi,
\]
故同样没有 Cauchy 子列。
\end{solution}

\begin{exercise}\label{b1:ex:25-rellich-regimes}
令 \(\Omega=(0,2\pi)\times(0,1)^2\subset\mathbb R^3\)，并设 \((u_n)\) 在 \(W^{1,2}(\Omega)\) 中有界。
\begin{enumerate}
\item 判断包含映射到 \(L^1(\Omega)\)、\(L^2(\Omega)\)、\(L^5(\Omega)\) 和 \(L^6(\Omega)\) 时，哪些由本章定理保证紧，哪些只保证连续而一般不紧。
\item 若进一步有 \(u_n\rightharpoonup u\) 于 \(W^{1,2}(\Omega)\)，证明整列在每个 \(1\le q<6\) 的 \(L^q(\Omega)\) 中强收敛到 \(u\)。
\item 取 \(u_n(x)=n^{-1}\cdot \sin(nx_1)\)。证明 \(u_n\rightharpoonup0\) 于 \(W^{1,2}(\Omega)\)，但不在该空间中强收敛。再说明增加哪一项范数信息即可恢复强收敛。
\end{enumerate}
\end{exercise}
% 来源：自拟基础范围题；把三维临界指数、紧算子对弱收敛整列的作用及标准范数判据放在同一例中比较。
\begin{solution}
这里 \(d=3\)、\(p=2\)，故临界指数为 \(p^*=6\)。Rellich–Kondrachov 定理给出
\[
 W^{1,2}(\Omega)\longrightarrow L^q(\Omega)
 \quad(1\le q<6)
\]
为紧嵌入，所以 \(q=1,2,5\) 都属于紧的范围。到 \(q=6\) 时，第24章只给出连续嵌入；临界集中族可以放进 \(\Omega\) 内的一个小球，表明该嵌入一般不紧，具体缩放见\cref{b1:ex:25-critical-mass-concentration}。

若 \(u_n\rightharpoonup u\) 于 \(W^{1,2}\)，对每个固定的 \(q<6\)，上述包含映射是紧线性算子。由\cref{b1:prop:compact-weak-to-strong}，无需再抽子列便有
\[
 u_n\longrightarrow u\quad\text{于 }L^q(\Omega).
\]

对第三问，\(u_n\to0\) 于 \(L^2(\Omega)\)，而
\[
 \partial_1u_n=\cos(nx_1),\qquad
 \partial_2u_n=\partial_3u_n=0.
\]
余弦列在 \(L^2(\Omega)\) 中弱收敛到零：先对光滑函数沿 \(x_1\) 分部积分，再用光滑函数在 \(L^2\) 中的稠密性和余弦列的一致范数界推广到全部测试函数。因此 \(u_n\rightharpoonup0\) 于 \(W^{1,2}(\Omega)\)。然而
\[
 \|\partial_1u_n\|_{L^2(\Omega)}^2
 =\int_0^{2\pi}\cos^2(nx_1)\,\mathrm dx_1=\pi,
\]
所以梯度不可能强收敛到零，原 Sobolev 范数中的强收敛失败。若另知本书标准范数满足
\[
 \|u_n\|_{W^{1,2}(\Omega)}\longrightarrow
 \|u\|_{W^{1,2}(\Omega)},
\]
则\cref{b1:prop:sobolev-weak-norm-strong}利用 Radon–Riesz 性质恢复 \(W^{1,2}\) 强收敛。
\end{solution}
...
\(R,\delta\) 决定，不依赖 \(f\)，故得到一致界。再结合%
\cref{b1:thm:frechet-kolmogorov-lp}，这两个条件本身已经足以推出相对紧性。
\end{solution}
...
\]
\cref{b1:thm:frechet-kolmogorov-lp}的三个条件均已满足，所求相对紧性随之成立。

加权项并不要求任何一个函数具有紧支集；它要求所有函数在远处放置的质量都受到同一个界限制。导数界约束局部振荡，而这个额外条件阻止质量整体移向无穷远，两者承担不同的任务。
\end{solution}

\begin{optionalexercise}\label{b1:ex:25-critical-mass-concentration}
设 \(d\ge2\)、\(1\le p<d\)、\(p^*=dp/(d-p)\)，取非零
\(\varphi\in C_c^\infty(B(0,1))\)。对 \(0<\varepsilon<1\)，令
\[
 u_\varepsilon(x)=\varepsilon^{1-d/p}\cdot \varphi(x/\varepsilon),
...
\[
 u_\varepsilon(x)=\varepsilon^\alpha\cdot \varphi(x/\varepsilon).
\]
...
\begin{solution}
与\cref{b1:ex:25-critical-mass-concentration}相同的变量代换给出
\[
...
最后，任何一个 \(u_n\) 都不属于 \(W_0^{1,p}(\Omega)\)。否则%
\cref{b1:thm:zero-boundary-poincare}会使
\(\|u_n\|_p\le C\|\nabla u_n\|_p=0\)，与其范数等于 \(1\) 矛盾。
因此这个例子检验的是不加边界条件且不加边界规则性时的失败，不涉及零边界版本的反例。
\end{solution}

\begin{optionalexercise}\label{b1:ex:25-nonlinear-reaction-limit}
...
\(W^{1,p}(\Omega)\to L^q(\Omega)\) 是紧线性算子。\cref{b1:prop:compact-weak-to-strong}因而给出整列
...
\begin{optionalexercise}\label{b1:ex:25-first-dirichlet-eigenvalue}
...
\cref{b1:thm:rellich-subcritical}与\cref{b1:thm:rellich-holder}的零边界版本给出
\(H_0^1(\Omega)\to L^2(\Omega)\) 的紧性：在 \(d>2\) 时
\(2<2d/(d-2)\)，在 \(d=2\) 时使用临界的有限指数结论；在 \(d=1\) 时，紧 Hölder 嵌入先给出一致收敛，再由区域有界性推出 \(L^2\) 收敛。
结合\cref{b1:prop:compact-weak-to-strong}，该子列在 \(L^2\) 中强收敛到
\(u\)，所以 \(\|u\|_2=1\)。
...
\begin{optionalexercise}\label{b1:ex:25-recover-weak-limit}

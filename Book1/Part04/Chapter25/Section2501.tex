...
范数有界只把函数族限制在 \(L^p\) 的一个范数球内；它既不能阻止质量向无穷远移动，也不能阻止固定区域内的高频振荡。强紧性需要同时限制这些变化。本节用远处的积分尾部和小平移误差刻画它，并把证明落实为有限个网格平均值的逼近；暂不要求函数具有弱导数。
...
\[
 ...
 \leq2^{d/p}\cdot \omega_{\mathcal F}(\sqrt d\,\delta).
\]
\end{lemma}
\begin{proof}
...
\cref{b1:fig:frechet-kolmogorov-mechanism}显示充分性证明的组织：三个一致条件共同产生有限维近似，再由有限维有界集的全有界性回到 \(L^p\) 紧性。
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[>=stealth,every node/.style={align=center}]
    \node[draw,rounded corners,minimum width=2.7cm,minimum height=0.85cm] (norm) at (-4.1,1.15) {范数一致有界};
    \node[draw,rounded corners,minimum width=2.7cm,minimum height=0.85cm] (tail) at (0,1.15) {远处尾部一致小};
    \node[draw,rounded corners,minimum width=2.7cm,minimum height=0.85cm] (trans) at (4.1,1.15) {小平移误差一致小};
    \node[draw,rounded corners,minimum width=3.8cm,minimum height=0.9cm] (finite) at (0,-0.45) {有限个网格上的\\有限维平均近似};
    \node[draw,rounded corners,minimum width=3.2cm,minimum height=0.85cm] (compact) at (0,-2.0) {\(L^p\) 中相对紧};
    \draw[->,thick] (norm)--(finite);
    \draw[->,thick] (tail)--(finite);
    \draw[->,thick] (trans)--(finite);
    \draw[->,thick] (finite)--(compact);
  \end{tikzpicture}
  \caption[平移紧性机制]{平移紧性机制。三个条件分别控制平均值大小、需要保留的空间范围和网格尺度。}
  \label{b1:fig:frechet-kolmogorov-mechanism}
\end{figure}
...
\(\|\mathbf1_{\{|x|>R\}}\cdot g_j\|_p\to0\)。网点有限，可以选同一个 \(R\)，使这些尾部范数都小于 \(\varepsilon\)。给定 \(u\in\mathcal F\)，选取
\(\|u-g_j\|_p<\varepsilon\) 的网点，则
\[
 \|\mathbf1_{\{|x|>R\}}\cdot u\|_p
 \leq\|u-g_j\|_p+\|\mathbf1_{\{|x|>R\}}\cdot g_j\|_p<2\varepsilon.
\]
由此得到尾部条件。
...
\[
 \sup_{u\in\mathcal F}\|\mathbf1_{\{|x|>R\}}\cdot u\|_p<\varepsilon/3.
\]
保留所有与 \(B(0,R)\) 相交的网格，指标集合记为 \(J_{\delta,R}\)，它是有限集；令
\[
 S_{\delta,R}=\bigcup_{k\in J_{\delta,R}}Q_{\delta,k},
 \quad A_{\delta,R}u=\mathbf1_{S_{\delta,R}}\cdot P_\delta u.
\]
...
\]
且范数一致有界。具体地，若 \(v=\sum_{k\in J_{\delta,R}}a_k\cdot\mathbf1_{Q_{\delta,k}}\)，则
\[
...
证明始终先对原函数作全空间网格平均，然后截去平均函数的远处部分。\eqref{b1:eq:cell-projection-tail}控制了第二步；没有把“先截断函数、再平移”产生的边界差当成零。证明中显式使用范数一致有界来控制保留下来的有限个平均值，尾部与平移条件则决定需要保留多少网格、网格应当多细。为保持与常见表述一致，定理仍把范数有界列为一个条件；不过在当前全空间版本中，它事实上可由尾部与一致平移连续性推出，见\cref{b1:ex:25-fk-redundant-boundedness}。因此这里的三个条件便于分别识别障碍，却不是逻辑上相互独立的三项假设。
...
\[
 u_n(x)=\mathbf1_Q(x)\cdot(-1)^{\lfloor2^n x_1\rfloor}.
\]
...
\cref{b1:fig:visual-f17}展示平移紧性证明中的有限维近似对象。
\input{Figures/F17}

...
给定 \(\overline U\Subset V\)，取固定 \(\chi\in C_c^\infty(V)\)，使它在 \(\overline U\) 的一个邻域内等于 \(1\)。将 \(\chi\cdot u\) 补零到整个空间。由弱乘积公式及\cref{b1:lem:compact-sobolev-zero-extension}，所得 \(w\) 满足
\[
...
\[
 u_\varepsilon(x)=\varepsilon^\alpha\cdot\phi(x/\varepsilon),
 \quad 0<\varepsilon<1.
\]
换元直接给出
\[

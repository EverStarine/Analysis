...
本章以第\ref{b1:ch:21}章的弱导数运算和代表理论为基础，反复使用第\ref{b1:ch:22}章的内部逼近、零边界闭包及一阶延拓。第\ref{b1:ch:23}章的分数阶空间用于高阶一节末尾，迹则帮助解释零边界条件。这里先直接证明均值 Poincaré 不等式，不把下一章的紧嵌入提前作为工具。
...
% 第二十四章习题临时稿；不另设 BookExercises 入口。
% 八题均为自拟的正文工具变式；各解答展开新增计算或构造，不冒认教材原题编号。
% 依赖止于本章，不调用第二十五章的紧嵌入或平移紧性判据。
...
\]
由定理\ref{b1:thm:connected-domain-poincare}及弱导数范数的有限维等价性，有
\[
...
\begin{solution}
由推论\ref{b1:cor:morrey-domain}，
\[
 \|w\|_\infty\le C_M\|w\|_{W^{1,p}(\Omega)}.
\]
所以 \(u,v\) 都本性有界，可以使用命题\ref{b1:prop:sobolev-product}的乘积公式。
函数项和梯度项分别满足
\[
...
\(F(u)=1/u\)。由定理\ref{b1:thm:sobolev-chain}，
\[
 \nabla(1/u)=-u^{-2}\nabla u
       \quad\text{几乎处处},
\]
...
由推论\ref{b1:cor:morrey-domain}，这个空间的元素有 \(\alpha\)-Hölder 连续代表。它与题中已有的连续函数几乎处处相同，因而处处相同。可是对任意 \(\beta>\alpha\)，
\[
...
\(W^{1,d}\)，由命题\ref{b1:prop:compact-sobolev-smooth-approximation}的内部紧支集逼近，得到 \(u_L\in W_0^{1,d}(B(0,1))\)。

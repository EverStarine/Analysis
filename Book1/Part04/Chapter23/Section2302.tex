...
分解
\[
 a(x)\cdot u(x)-a(y)\cdot u(y)
 =a(x)\cdot\bigl(u(x)-u(y)\bigr)+u(y)\cdot\bigl(a(x)-a(y)\bigr).
\]
...
\[
 ...
 =\int\rho_\varepsilon(z)\cdot D_su(x-z,h)\,\mathrm dz
\]
...
\[
 ...
 =\bigl(1-\chi_R(x)\bigr)\Delta_hu(x)
   +u(x-h)\cdot\bigl(\chi_R(x)-\chi_R(x-h)\bigr).
\]
...
\[
 ...
 \end{aligned}
\]
等式先在 \(x\) 中使用平移不变性，再令 \(h=Rz\)。最后的核积分在原点附近由 \(|z|^{p-d-sp}\) 控制，在远处由 \(2^p|z|^{-d-sp}\) 控制，故有限。因 \(sp>0\)，这一项也趋于零。合并两项并使用 \(|a+b|^p\leq2^{p-1}(|a|^p+|b|^p)\)，得到 \([w_R]_{s,p}\to0\)。所以 \(\chi_R\cdot u\to u\) 于 \(W^{s,p}\)。

给定误差 \(\eta>0\)，先取 \(R\) 使截断误差小于 \(\eta/2\)，再用前一命题磨光 \(\chi_R\cdot u\)，使磨光误差小于 \(\eta/2\)。卷积的支集包含于 \(\operatorname{supp}\chi_R+\overline B(0,\varepsilon)\)，仍为紧集。所得函数属于 \(C_c^\infty\)，三角不等式完成证明。
\end{proof}
...
\end{corollary}
\begin{proof}
取\cref{b1:cor:precompact-smooth-partition}给出的光滑单位分解 \((\psi_i)\) 与局部有限开覆盖 \((V_i)\)，其中 \(\psi_i\in C_c^\infty(V_i)\)、\(V_i\Subset\Omega\)。每个 \(\psi_i\) 是有界 Lipschitz 函数，故由乘子命题，\(\psi_i\cdot u\in W^{s,p}(\Omega)\)，且在紧集 \(\operatorname{supp}\psi_i\) 外几乎处处为零。给定 \(\eta>0\)，前一推论允许取 \(v_i\in C_c^\infty(V_i)\)，使
\[
 \|v_i-\psi_i\cdot u\|_{W^{s,p}(\Omega)}<\eta\cdot2^{-i-1}.
\]
局部有限和 \(v=\sum_i v_i\) 在 \(\Omega\) 内光滑。另一方面，由完备性，误差级数 \(\sum_i(v_i-\psi_i\cdot u)\) 在 \(W^{s,p}(\Omega)\) 中收敛到某个 \(e\)，且 \(\|e\|_{W^{s,p}}\leq\eta/2\)。在每个内部相对紧开集上，级数只有有限个非零项，故其和等于 \(v-u\)；范数收敛蕴含局部 \(L^p\) 收敛，因而 \(e=v-u\) 几乎处处。于是 \(v\in W^{s,p}(\Omega)\)，并达到所需误差。
\end{proof}

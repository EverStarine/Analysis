...
Di Nezza、Palatucci 与 Valdinoci 的《Hitchhiker's guide to the fractional Sobolev spaces》\cite[第2节及引理5.1--5.3]{DiNezza2012Hitchhikers}给出双积分定义、指数比较及局部化工具。本节固定 \(0<s<1\)、\(1\leq p<\infty\)，函数可以取实值或复值；\(\Omega\subseteq\mathbb R^d\) 为非空开集，\(d\geq1\)。所有积分均使用 Lebesgue 测度，不在双积分前加入依赖于 \(s\) 的归一化系数。
...
Di Nezza 等在所引文章第2节将同一双积分称为 Gagliardo semi-norm，亦即以 Gagliardo 命名的半范数。\index[foreign]{Gagliardo semi-norm}这里的两个名称指向同一个差分条件；本书保持上述主称及不附加归一化系数的约定。
...
对 \(u\in L^p(\mathbb R^d)\)，置 \(\Delta_hu(x)=u(x+h)-u(x)\)。则
...
\[
 \|u\|_{W^{s,p}(\Omega)}
 =\left(\|u\|_{L^p(\Omega)}^p+[u]_{s,p;\Omega}^p\right)^{1/p}.
\]
\end{definition}
...
设 \(u\in W^{s,p}(\mathbb R^d)\)，沿用 \(\tau_zu(x)=u(x-z)\) 的平移约定。则
\[
 \|\tau_zu-u\|_{W^{s,p}(\mathbb R^d)}\longrightarrow0\quad(z\to0).
\]
对第\ref{b1:sec:2003}节的磨光函数，还成立
\[
...
在 \(h\ne0\) 时定义
\[
 D_su(x,h)=\frac{u(x+h)-u(x)}{|h|^{d/p+s}},
\]
...
卷积的光滑性来自第\ref{b1:sec:2003}节。差分与积分相交换，给出
\[
...
\end{corollary}
\begin{proof}
采用\cref{b1:thm:meyers-serrin}证明中的单位分解 \((\psi_i)\)，同时保留各支集所在的局部有限开族 \((V_i)\)。每个 \(\psi_i\) 是有界 Lipschitz 函数，故由乘子命题，\(\psi_i u\in W^{s,p}(\Omega)\)，且具有内部紧支集。给定 \(\eta>0\)，前一推论允许取 \(v_i\in C_c^\infty(V_i)\)，使
\[

...
Di Nezza、Palatucci 与 Valdinoci 的《Hitchhiker's guide to the fractional Sobolev spaces》\cite{DiNezza2012Hitchhikers}第2节末介绍了这一联系，第3.8命题用 Fourier 方法证明二次可积情形。本节改用差分、一个一维积分不等式及尺度卷积，直接处理 \(1<p<\infty\)。
...
先设 \(d\ge2\)，记 \(n=d-1\)，并令
\(\sigma=\mathcal H^n\llcorner\partial\Omega\)。取第\ref{b1:sec:2203}节的有限内外图表：在刚性坐标中，
\[
...
\[
 ...
 =\left(\sum_{j=1}^N\|f_j\|_{W^{s,p}(\mathbb R^n)}^p\right)^{1/p}.
\]
\end{definition}
...
对一般 \(u\in W^{1,p}(H_+)\)，先偶反射，再在整个空间中作光滑紧支集逼近，最后限制回半空间。这是第\ref{b1:sec:2301}节构造 \(L^p\) 迹时使用的逼近。上述估计作用于两项之差，使光滑迹成为 \(W^{s,p}(\mathbb R^n)\) 中的 Cauchy 列；其极限在 \(L^p\) 中又等于既有的 \(\operatorname{Tr}u\)。完备性于是给出结论，并保留同一个范数界。
\end{proof}
...
仅有迹估计还不能说明每个边界函数都能实现。若在每个高度使用同一个磨光尺度，通常不能同时恢复原数据并控制竖向导数；尺度应随高度一起趋零。
...
\[
 (K_t*f)(x)=\int K_t(h)\bigl(f(x-h)-f(x)\bigr)\,\mathrm dh.
\]
Hölder 不等式给出
\[
 \begin{split}
...
\[
 ...
 &\le C\int_{|h|<2}\|\Delta_{-h}f-f\|_p^p
                  \int_{|h|}^2t^{-n-p}\,\mathrm dt\,\mathrm dh\\
 &\le C_{n,p}\int_{\mathbb R^n}
        \frac{\|\Delta_hf-f\|_p^p}{|h|^{n+p-1}}\,\mathrm dh
 =C_{n,p}[f]_{s,p}^p.
 \end{split}
\]
...
\end{theorem}
\begin{proof}
先证明有界性。对每个固定边界系数 \(\chi_j\)，将 \(\chi_j u\) 拉回 \(D_j\cap H_+\)，再在侧面和顶面之外补零，得到整个半空间上的函数 \(v_j\)。第\ref{b1:sec:2203}节局部图表延拓证明中的测试分解适用于此处：支集与侧面、顶面有正距离，但允许贴着 \(t=0\)。因此
\[
 \|v_j\|_{W^{1,p}(H_+)}
...
把 \(q_j\) 经 \(F_j^{-1}\) 推回 \(\Omega\cap F_j(D_j)\)，再在 \(\Omega\) 的其余部分补零，记结果为 \(Q_jf\)。侧面和顶面的支集余量再次保证局部测试恒等式可以拼接；这里始终只在 \(\Omega\) 内补零，并没有跨越 \(\partial\Omega\) 作零延拓。故
\[
 \|Q_jf\|_{W^{1,p}(\Omega)}
...
这个迹空间允许的边界不连续性取决于 \(p\)。例如在二维半空间上取边界函数 \(f=\mathbf1_{(0,1)}\)。习题\ref{b1:ex:23-interval-characteristic}的差分计算表明，它属于 \(W^{s,p}(\mathbb R)\) 当且仅当 \(sp<1\)。代入 \(s=1-1/p\)，就知道它能作为 \(W^{1,p}(H_+)\) 函数的迹，当且仅当 \(1<p<2\)。因此不能从内部具有一阶弱导数，直接推断边界数据连续。

本节没有把两个端点纳入分数阶结论。对 \(p=1\)，第\ref{b1:sec:2301}节的 \(L^1\) 迹仍可使用，但这里的 \(W^{s,p}\) 定义要求 \(s>0\)，也没有给出同样的线性右逆证明。对 \(p=\infty\)，既不能代入有限 \(p\) 的双积分范数，也不能用该范数的稠密性取极限；应回到局部 Lipschitz 代表与边界几何讨论。

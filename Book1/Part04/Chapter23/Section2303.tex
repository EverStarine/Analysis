...
\[
 f_j(z)=\chi_j\bigl(\Gamma_j(z)\bigr)\cdot f\bigl(\Gamma_j(z)\bigr)
 \quad(z\in B_j)
\]
...
\[
 I=\frac p{p-1}\int_0^\infty
       \left(\frac{A(r)}r\right)^{p-1}\cdot a(r)\,\mathrm dr
 \le\frac p{p-1}I^{(p-1)/p}\|a\|_p.
\]
若 \(I>0\)，约去相应幂次即得结论；\(I=0\) 时也成立。对一般 \(a\)，取
\[
 a_m=\min\{a,m\}\cdot\mathbf1_{(1/m,m)},
\]
再由单调收敛定理取极限。
\end{proof}
...
\[
 ...
 \le C_p\cdot\left(\int_0^r a(t)\,\mathrm dt\right)^p
       +C_p\cdot r^p\cdot b(r)^p,
\]
...
\[
 \rho_t(h)=t^{-n}\cdot \rho(h/t),\quad
 v(x,t)=(\rho_t*f)(x),\quad
 (R_+f)(x,t)=\eta(t)\cdot v(x,t).
\]
由卷积不等式，\(\|v(\cdot,t)\|_p\le\|f\|_p\)，所以零阶项和竖向求导产生的 \(\eta'(t)\cdot v(x,t)\) 都有由 \(\|f\|_p\) 控制的全域 \(L^p\) 范数。

对切向导数，核为 \(t^{-n-1}(\partial_i\rho)(h/t)\)；对高度导数，核为
\[
 \partial_t\rho_t(h)=t^{-n-1}\cdot \psi(h/t),\quad
 \psi(z)=-n\cdot \rho(z)-z\cdot\nabla\rho(z).
\]
这些核都支撑在 \(|h|<t\) 内，绝对值不超过 \(Ct^{-n-1}\)，且积分为零。最后一点对 \(\psi\) 由紧支集函数的分部积分得到。若 \(K_t\) 是其中任一导数核，因而可以写
\[
 (K_t*f)(x)=\int K_t(h)\cdot\Delta_hf(x)\,\mathrm dh.
\]
Hölder 不等式给出
\[
 \begin{split}
...
\[
 q_j(z,t)=\beta_j(z,t)\cdot w_j(z,t),\quad t>0,
\]

...
\end{theorem}
\begin{proof}
先证明有界性。对每个固定边界系数 \(\chi_j\)，将 \(\chi_j u\) 拉回 \(D_j\cap H_+\)，再在侧面和顶面之外补零，得到整个半空间上的函数 \(v_j\)。\secref{b1:sec:2203}局部图表延拓证明中的测试分解适用于此处：支集与侧面、顶面有正距离，但允许贴着 \(t=0\)。因此
\[
 \|v_j\|_{W^{1,p}(H_+)}
...
本节没有把两个端点纳入分数阶结论。对 \(p=1\)，\secref{b1:sec:2301}的 \(L^1\) 迹仍可使用，但这里的 \(W^{s,p}\) 定义要求 \(s>0\)，也没有给出同样的线性右逆证明。对 \(p=\infty\)，既不能代入有限 \(p\) 的双积分范数，也不能用该范数的稠密性取极限；应回到局部 Lipschitz 代表与边界几何讨论。

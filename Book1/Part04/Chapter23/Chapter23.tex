...
本章从一个边界值问题出发：域内几乎处处等价的 Sobolev 函数，怎样决定边界上的数据？先通过沿线端点与范数估计构造迹算子，证明有限指数下零迹与紧支集光滑闭包等价；再以点对差分定义分数阶 Sobolev 空间。当 \(1<p<\infty\) 时，以此刻画一阶迹的精确目标，并构造有界线性右逆；\(p=1\) 时只使用连续的 \(L^1\) 迹与零迹刻画。最后讨论平坦边界上的高阶数据相容性、法向导数在折角处的限制，以及 Besov 空间与本章差分范数的关系。
\end{chapterabstract}
...
中文伴读可选王明新的《索伯列夫空间》\cite{Wang2013SobolevChinese}，先对照第2.4节、第2.10节的边界迹，再读第3.5节中的实指数空间与迹。该书书名采用人名的中文音译，本书正文仍沿用 Sobolev 原拼。Di Nezza、Palatucci 与 Valdinoci 的《Hitchhiker's guide to the fractional Sobolev spaces》\cite{DiNezza2012Hitchhikers}适合伴读本章的差分部分；原文只陈述结论或采用不同前置工具之处，各节另行说明，本章所需的论证仍在正文中展开。
...
% 十题均为自拟的基础训练或工具变式；来源为已建立的正文接口与此处独立展开的计算，不冒认教材原题编号。
...
\begin{exercise}\label{b1:ex:23-interval-affine-trace}
设 \(I=(0,1)\)、\(1\le p<\infty\)，并始终采用 \(W^{1,p}(I)\) 元素的闭区间绝对连续代表。
\begin{enumerate}
 \item 证明端点估计
 \[
  |u(0)|+|u(1)|
  \le 2\bigl(\|u\|_{L^p(I)}+\|u'\|_{L^p(I)}\bigr).
 \]
 \item 对 \((a,b)\in\mathbb R^2\)，令
 \[
  R(a,b)(x)=(1-x)a+xb.
 \]
 证明 \(R:\mathbb R^2\to W^{1,p}(I)\) 是端点迹
 \(\gamma_Iu=(u(0),u(1))\) 的有界线性右逆。
 \item 令 \(v(x)=x(1-x)\)。直接构造 \(v_m\in C_c^\infty(I)\)，使 \(v_m\to v\) 于 \(W^{1,p}(I)\)，从而具体核对 \(v\in\ker\gamma_I=W_0^{1,p}(I)\)。
\end{enumerate}
\end{exercise}
% 来源：自拟；一维绝对连续积分表示、端点迹和边界截断的基础训练。
\begin{solution}
对每个 \(t\in(0,1)\)，绝对连续积分表示给出
\[
 |u(0)|\le |u(t)|+\int_0^1|u'(s)|\,\mathrm ds.
\]
对 \(t\) 积分，并在长度为一的区间上使用 Hölder 不等式，得到
\[
 |u(0)|\le\|u\|_p+\|u'\|_p.
\]
从右端点作同样估计，再相加即得第一问。
...
映射 \(R\) 显然线性，而且
\[
 \|R(a,b)\|_{L^p(I)}\le |a|+|b|,
 \qquad
 \|(R(a,b))'\|_{L^p(I)}=|b-a|\le |a|+|b|.
\]
因此它有界；又有 \(R(a,b)(0)=a\)、\(R(a,b)(1)=b\)，故
\(\gamma_I R=\operatorname{id}_{\mathbb R^2}\)。

最后，取 \(\eta_m\in C_c^\infty(I)\)，使 \(0\le\eta_m\le1\)，在
\([2/m,1-2/m]\) 上等于 \(1\)，在两个端点的 \(1/m\) 邻域为零，并满足
\(\|\eta_m'\|_\infty\le Cm\)。令 \(v_m=\eta_mv\)。函数项与
\((1-\eta_m)v'\) 都只留在长度趋零的边界层上，故其 \(L^p\) 范数趋零。在左边界层上 \(|v(x)|\le x\le2/m\)，在右边界层上同理有
\(|v(x)|\le1-x\le2/m\)，所以 \(|\eta_m'v|\le2C\)，且它的支集测度趋零。于是
\[
 \|v_m-v\|_{W^{1,p}(I)}\longrightarrow0.
\]
每个 \(v_m\) 都属于 \(C_c^\infty(I)\)，这直接验证了结论。
\end{solution}

\begin{exercise}\label{b1:ex:23-half-space-product-trace}
设 \(d\ge2\)、\(1\le p<\infty\)，取
\(\phi\in C_c^\infty(\mathbb R^{d-1})\)，并在
\(H_+=\mathbb R^{d-1}\times(0,\infty)\) 上令
\[
 u(x',t)=e^{-t}\cdot \phi(x').
\]
证明 \(u\in W^{1,p}(H_+)\)，逐项计算 \(u\) 及其一阶弱导数的 \(L^p\) 范数，并由切片刻画验证
\(\operatorname{Tr}_+u=\phi\)。
\end{exercise}
% 来源：自拟；半空间迹、切片收敛与乘积函数范数的直接计算。
\begin{solution}
函数在 \(H_+\) 中光滑，且
\[
 \partial_tu=-e^{-t}\phi,
 \qquad
 \partial_i u=e^{-t}\partial_i\phi\quad(1\le i<d).
\]
由于 \(\int_0^\infty e^{-pt}\,\mathrm dt=1/p\)，Tonelli 定理给出
\[
 \|u\|_{L^p(H_+)}^p
 =\|\partial_tu\|_{L^p(H_+)}^p
 =\frac1p\|\phi\|_{L^p(\mathbb R^{d-1})}^p,
\]
以及
\[
 \|\partial_i u\|_{L^p(H_+)}^p
 =\frac1p\|\partial_i\phi\|_{L^p(\mathbb R^{d-1})}^p.
\]
故 \(u\in W^{1,p}(H_+)\)。对每个 \(t>0\)，
\[
 \|u(\cdot,t)-\phi\|_p
 =|e^{-t}-1|\,\|\phi\|_p\longrightarrow0
 \quad(t\downarrow0).
\]
由\eqref{b1:eq:trace-slice-convergence}的切片刻画，迹正是 \(\phi\)。
\end{solution}

\begin{optionalexercise}\label{b1:ex:23-smooth-integer-limit}
...
\[
 ...
 \le\frac{(1-s)2^p\cdot \sigma(\mathbb S^{d-1})}{sp}\cdot\|u\|_p^p
 \longrightarrow0.
\]
...
\[
 (1-s)\int_0^1r^{p(1-s)-1}\cdot B(r)\,\mathrm dr.
\]
...
\[
 ...
 =4|a|\cdot|b|\cdot\mathbf1_{\{ab<0\}}.
\]
两边积分得到
\[
 [u]_{s,2;\Omega}^2-[\,|u|\,]_{s,2;\Omega}^2
 =4\iint_{\{u(x)\cdot u(y)<0\}}
...
\[
 u_\varepsilon(x',t)=f(x')\cdot\chi(t/\varepsilon).
\]
...
下面给出非零的选择。记 \(\sigma=\mathcal H^{d-1}\llcorner\partial\Omega\)，取非零 \(w\in C_c^\infty(\Omega)\)，并令
\[
...
\begin{optionalexercise}\label{b1:ex:23-circle-energy}
...
\begin{optionalexercise}\label{b1:ex:23-higher-zero-endpoints}
...
\]
一维代表估计，即\cref{b1:prop:sobolev-interval-representative}，使这个收敛也成为闭区间上的一致收敛。由于 \(u_n^{(j)}\) 的两端值都是零，极限的两端值也为零，必要性得证。
...
\(t^{k-r-1}\cdot\mathbf1_{(0,a)}(t)/(k-r-1)!\) 的 \(L^1\) 范数为
...
每个 \(\eta_\varepsilon u\) 都有包含在 \(I\) 内部的紧支集。由%
\cref{b1:prop:compact-sobolev-smooth-approximation}，它可以在 \(W^{k,p}(I)\) 中由
\(C_c^\infty(I)\) 函数逼近。先选 \(\varepsilon\) 控制截断误差，再作光滑逼近，便得到
\(u\in W_0^{k,p}(I)\)。这里有限 \(p\) 保证了边界层上的最高阶导数范数趋零；证明并未涵盖 \(p=\infty\)。
\end{solution}

\begin{optionalexercise}\label{b1:ex:23-smooth-normal-data}
...
\[
 U(x',t)=\chi(t)\cdot\bigl(a(x')-t\cdot b(x')\bigr),\quad t\ge0.
\]

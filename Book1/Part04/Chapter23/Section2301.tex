...
当 \(d\ge2\) 时，先在 \(H_+=\mathbb R^{d-1}\times(0,\infty)\) 上说明边界估计的来源。横向变量写作 \(x'\)，纵向变量写作 \(t\)。沿几乎每条竖线，Sobolev 函数具有绝对连续代表，因而有机会通过 \(t\downarrow0\) 确定边界值。一维有界区间的端点迹已在\subsecref{b1:subsec:220204}中单独构造，不在这里引入 \(\mathbb R^0\) 与 \(L^p(\mathbb R^0)\) 的额外约定。

\ProseParagraphBreak

\cref{b1:fig:half-space-trace}把半空间迹看成内部切片向边界的 \(L^p\) 极限。图中的箭头表示 \(t\downarrow0\)，不是对任意可测代表作逐点限制。
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[>=stealth]
    \fill[gray!12] (-3,0) rectangle (3,1.8);
    \draw[thick] (-3,0)--(3,0) node[right] {$\partial H_+$};
    \draw[thick] (-2.65,1.3)--(2.65,1.3) node[right] {$t=t_1$};
    \draw[thick] (-2.65,0.75)--(2.65,0.75) node[right] {$t=t_2$};
    \draw[thick] (-2.65,0.32)--(2.65,0.32) node[right] {$t=t_3$};
    \foreach \x in {-1.8,0,1.8}
      \draw[->] (\x,1.18)--(\x,0.08);
    \node[below] at (0,0) {$\operatorname{Tr}_+u$};
    \node[left] at (-3,1.05) {$H_+$};
  \end{tikzpicture}
  \caption[半空间迹]{半空间迹。内部切片 \(u(\cdot,t)\) 随 \(t\downarrow0\) 在 \(L^p\) 中趋向边界数据 \(\operatorname{Tr}_+u\)。}
  \label{b1:fig:half-space-trace}
\end{figure}

\begin{theorem}[半空间的 \(L^p\) 迹]\label{b1:thm:half-space-lp-trace}
设 \(d\ge2\)、\(1\le p<\infty\)。存在唯一有界线性算子
\[
...
对 \(u\in W^{1,p}(\Omega)\)，先将 \(\eta_j\cdot u\) 拉回正半盒。它在侧面及顶面附近为零，可以像局部延拓引理那样补零为 \(H_+\) 上的 \(W^{1,p}\) 函数 \(v_j\)。这里不跨过 \(t=0\) 补零，故不需要预先假设零边界值。定义
\[
...
\[
 T_ju=(\eta_j|_{\partial\Omega})\cdot(u|_{\partial\Omega}).
\]
...
还需说明局部相容性并不限于本次选定的分割函数。给定一个图表及支集留在其内的光滑截断 \(\chi\)，取全空间光滑限制 \(u_m\to u\) 于 \(W^{1,p}(\Omega)\)。弱乘积与双 Lipschitz 换元保证 \((\chi\cdot u_m)\circ F\to(\chi\cdot u)\circ F\) 于相应正半盒。对各 \(u_m\)，两种端点解释一致；两边的 \(L^p\) 迹映射都连续，因此取极限仍一致。在任意较小边界片选取 \(\chi=1\)，即可得到所述局部端点刻画。
...
\[
 \operatorname{Tr}(\chi\cdot u)
 =(\chi|_{\partial\Omega})\cdot\operatorname{Tr}u
...
设 \(d\ge2\)。对 \(u\in W^{1,p}(H_+)\)、\(1\le p<\infty\)，若 \(\operatorname{Tr}_+u=0\)，则零延拓 \(\widetilde u\) 属于 \(W^{1,p}(\mathbb R^d)\)，并有
\[
...
为得到内部光滑逼近，令 \(F=\widetilde u\)，取向内平移 \(F_h(x)=F(x-he_d)\)。函数及其弱导数的 \(L^p\) 平移连续性给出 \(F_h\to F\) 于 \(W^{1,p}(\mathbb R^d)\)，而 \(F_h=0\) 于 \(\{x_d<h\}\)。固定 \(h\)，用远处截断 \(\chi_R\) 令 \(\chi_R\cdot F_h\to F_h\)；这与全空间稠密性证明中的尾部估计相同。再以小于 \(h/2\) 的尺度磨光，所得函数的紧支集留在 \(H_+\) 内，且在全空间 Sobolev 范数中逼近 \(\chi_R\cdot F_h\)。依次令平移、尾部及磨光误差趋零，再限制回半空间，即得 \(W_0^{1,p}\) 的闭包条件。
\end{proof}
...
反过来，设 \(\operatorname{Tr}u=0\)。维数为一的情形已由区间零端点刻画证明；以下设 \(d\ge2\)。仍用有限分解 \(u=\eta_0\cdot u+\sum_{j=1}^N\eta_j\cdot u\)。内部项具有内部紧支集，故由有限指数的内部磨光属于 \(W_0^{1,p}(\Omega)\)。

对一个边界项，将 \(\eta_j\cdot u\) 拉回正半盒并在半空间内补零，记为 \(v_j\)。迹的乘法及图表相容性说明 \(\operatorname{Tr}_+v_j=0\)，所以前一引理给出 \(v_j\in W_0^{1,p}(H_+)\)。取 \(a_m\in C_c^\infty(H_+)\)，使 \(a_m\to v_j\) 于 \(W^{1,p}(H_+)\)。

由边界项的构造，存在紧集 \(K_j\Subset D_j\)，使 \(v_j=0\) 几乎处处成立于 \(H_+\setminus K_j\)；\(K_j\) 可以接触 \(t=0\)。选 \(\zeta\in C_c^\infty(D_j)\)，使它在 \(K_j\) 的邻域等于一。于是 \(\zeta\cdot v_j=v_j\) 几乎处处；由光滑乘法的连续性，\(\zeta\cdot a_m\to v_j\) 于半空间 Sobolev 范数。每个 \(\zeta\cdot a_m\) 的支集同时避开 \(t=0\) 与外盒边界。

将 \(\zeta\cdot a_m\) 经 \(F_j^{-1}\) 推回，并在图表外补零，得到 \(\Omega\) 内部紧支集的 \(W^{1,p}\) 函数 \(b_m\)。虽然 Lipschitz 坐标变换不保证它光滑，但内部紧支集逼近保证 \(b_m\in W_0^{1,p}(\Omega)\)。由双 Lipschitz 换元的范数估计，\(b_m\to\eta_j\cdot u\) 于 \(W^{1,p}(\Omega)\)。闭性于是给出 \(\eta_j\cdot u\in W_0^{1,p}(\Omega)\)。有限个边界项与内部项相加，完成证明。
\end{proof}
...
零边界的几种表述只有在相应条件下才能互换，见\cref{b1:tab:visual-t11}。
\input{Tables/T11}

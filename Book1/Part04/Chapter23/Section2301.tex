...
由边界项的构造，存在紧集 \(K_j\Subset D_j\)，使 \(v_j=0\) 几乎处处成立于 \(H_+\setminus K_j\)；\(K_j\) 可以接触 \(t=0\)。选 \(\zeta\in C_c^\infty(D_j)\)，使它在 \(K_j\) 的邻域等于一。于是 \(\zeta v_j=v_j\) 几乎处处；由光滑乘法的连续性，\(\zeta a_m\to v_j\) 于半空间 Sobolev 范数。每个 \(\zeta a_m\) 的支集同时避开 \(t=0\) 与外盒边界。

...
初读可以先沿“弱导数—Sobolev 空间—光滑逼近—基本嵌入与紧性”的路线前进。这条函数空间主线大部分只需要前两编的积分与泛函分析，以及\chapref{b1:ch:13}的卷积逼近，不必先掌握完整的几何测度论。但一维绝对连续代表会用到\secref{b1:sec:1604}；第21章的点态代表还会调用\chapref{b1:ch:13}的极大函数、\chapref{b1:ch:14}的 Lebesgue 点，以及\chapref{b1:ch:16}的 Rademacher 定理和 Lipschitz 分片，双 Lipschitz 坐标变换则使用\chapref{b1:ch:17}的加权面积公式。迹与后续有界变差函数的几何结构还会使用第三编的覆盖、面积，以及\chapref{b1:ch:19}前 3 节建立的可整流性与近似切平面。这里应区分“定义弱导数所需的起点”和“精细代表与几何结论所需的后续输入”；第 19.4 节的结构分解、密度判据与 Preiss 理论仍可留待第二遍阅读。

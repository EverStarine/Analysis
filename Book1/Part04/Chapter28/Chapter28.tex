% !TeX root = ../../Book1.tex
\OptionalChapter[有界变差函数的精细结构]{有界变差函数的\\精细结构}{b1:ch:28}
\BookChapterNumber{b1:ch:28}
\begin{chapterabstract}
本章把有界变差函数的导数分成体积上的绝对连续部分、界面上的跳跃部分与剩余的 Cantor 部分。
通过近似上下极限、单侧值和跳跃法向，把等价类的点值恢复与测度分解联系起来。
最后介绍特殊有界变差函数空间，并在给定的值域与增长条件下推导 Mumford–Shah 型能量的极小元存在性。
精细迹、弥散结构及链律作为深层输入，在相应位置标明；其完整证明不在本章展开。
\end{chapterabstract}
...
\item 理解特殊有界变差函数为何排除 Cantor 部分，以及有界变差空间中的严格收敛为何仍不保证特殊有界变差函数空间闭合。
...
本章是选读章，也是本卷从实分析通向变分问题的收束。
近似极限的基础已在\chapref{b1:ch:14}建立，这里增加上下极限与等价类代表的组织，
不重新改变“近似连续”对指定点值的含义。
读者应熟悉\chapref{b1:ch:06}的测度分解、\chapref{b1:ch:19}的可整流性，以及上一章有界变差函数的逼近与余面积公式。
若暂时只想掌握可使用的结论，可以先读各结构定理的准确陈述及一维例子，
再循证明概要了解精细迹与切片工具所承担的作用。
...
该文篇幅简短，基础精细结构另引专著。
已有工具在本章用于推导可整流性、测度分解后果、固定面积界下的闭合桥、
跳跃面积下半连续性和极小值论证；其余深层输入另行标明。
...
\input{Part04/Chapter28/Section2805}

\begin{chaptersummary}
有界变差函数的导数分成三部分，分别由体积、跳跃界面与不充余维一有限面积集的奇异变化承载。
...
特殊有界变差函数排除了 Cantor 部分，但其空间对有界变差空间中的严格收敛并不自动闭合。
超线性梯度界与跳跃面积界分别防止不同形式的变化集中。
在有界值域内，这些界配合链律和下半连续性，给出 Mumford–Shah 型极小元。
存在性结论给出函数等价类及其跳跃结构；唯一性、闭裂纹表示与界面正则性不在其范围内。
至此，本卷的积分、微分、覆盖、可整流性和紧性已经汇入同一个变分问题。
由此继续前进，还需分别调用本卷已证工具与后续精细理论。
\end{chaptersummary}

\BookExercises

% \chapref{b1:ch:28}习题临时稿；由根任务接在唯一的BookExercises之后，不另建入口。
% 九题均为自拟；所用有界变差函数结构、Volpert链律、特殊有界变差函数的紧性来自本章明确陈述的版本，
% 不把其深层证明移入基础习题。逐题注释记录训练目标与相对正文的增量。

\begin{exercise}\label{b1:ex:28-basic-approximate-limits}
设 \(d\ge2\)、\(\Omega=B(0,1)\subset\mathbb R^d\)、
\(0<\alpha<d-1\)，并在原点任取一个有限值定义下列含尖点的函数：
\[
 u_1=\mathbf1_{\{x_d>0\}},\qquad
 u_2=\mathbf1_{\mathbb Q^d\cap\Omega},\qquad
 u_3(x)=|x|^{-\alpha}\ (x\ne0),\qquad
 u_4=u_1+u_3.
\]
分别求四个函数的 \(u_i^\wedge,u_i^\vee,G_{u_i},S_{u_i},J_{u_i}\)。
在跳跃点上还要写出两侧值与跳跃法向，并说明哪一个例子满足
\(S_{u_i}\setminus J_{u_i}\ne\varnothing\)。
最后说明这些集合和跳跃数据中哪些只依赖几乎处处等价类，
而指定代表在一点是否近似连续是否具有同样的不变性。
\end{exercise}
% 来源：自拟；用跳跃、稠密零测修改、无穷尖点及二者叠加，训练本章最初两节的全部基本对象。
\begin{solution}
对 \(u_1\)，平面外的函数在一个邻域内为常数；平面上两个半球各占一半体积。
因此
\[
 u_1^\wedge=u_1^\vee=u_1\quad\text{于 }\Omega\setminus\{x_d=0\},
\]
而在 \(\{x_d=0\}\cap\Omega\) 上有
\[
 u_1^\wedge=0,\quad u_1^\vee=1,\quad
 u_1^-=0,\quad u_1^+=1,\quad \nu_{u_1}=e_d.
\]
从而
\[
 G_{u_1}=\Omega\setminus\{x_d=0\},\qquad
 S_{u_1}=J_{u_1}=\Omega\cap\{x_d=0\}.
\]

有理点集为零测集，所以 \(u_2=0\) 几乎处处。
任意球中所有阈值密度都与零函数相同，故
\[
 u_2^\wedge=u_2^\vee=0,\qquad
 G_{u_2}=\Omega,\qquad S_{u_2}=J_{u_2}=\varnothing.
\]
这里指定代表在无理点近似连续，在有理点不近似连续：
近似极限始终为零，却未必等于该点实际取值。

函数 \(u_3\) 在每个非零点连续，所以两个近似极限都等于 \(|x|^{-\alpha}\)。
在原点，对每个有限 \(M\)，充分小球内除原点外都有 \(u_3>M\)，因而
\[
 u_3^\wedge(0)=u_3^\vee(0)=+\infty.
\]
所以
\[
 G_{u_3}=\Omega\setminus\{0\},\qquad
 S_{u_3}=\{0\},\qquad J_{u_3}=\varnothing.
\]
原点的指定有限值不改变这些结论，也不可能使函数在那里近似连续。

对 \(u_4\)，在坐标平面之外仍是连续函数。
若 \(x\in\Omega\cap\{x_d=0\}\setminus\{0\}\)，两侧值为
\[
 u_4^-(x)=|x|^{-\alpha},\qquad
 u_4^+(x)=|x|^{-\alpha}+1,\qquad
 \nu_{u_4}(x)=e_d.
\]
原点附近的 \(|x|^{-\alpha}\) 在密度一意义下趋于 \(+\infty\)，
加上有界的 \(u_1\) 不改变两个无穷近似极限。于是
\[
 S_{u_4}=\Omega\cap\{x_d=0\},\qquad
 J_{u_4}=\bigl(\Omega\cap\{x_d=0\}\bigr)\setminus\{0\},
\]
并且 \(S_{u_4}\setminus J_{u_4}=\{0\}\)。

近似上下极限、\(G_u,S_u,J_u\) 以及跳跃的两侧值和法向都由球中或半球中的
体积与积分条件定义，因而只依赖几乎处处等价类。
指定代表在 \(x\) 近似连续还要求有限近似极限等于实际点值；
只改动 \(u(x)\) 就可能改变这一性质，所以它不是同样的等价类不变量。
\end{solution}
...
\[
 D^au_a=
 \left(2x\cdot\mathbf1_{\{|x|<1\}}
       +\frac{2x}{|x|}\cdot\mathbf1_{\{1<|x|<2\}}\right)m_2,
\]
\[
...
\[
 \Phi(u(x))=(1+|x|)^2.
\]
因此 \(\Phi(u)=(1+|x|)^2\) 几乎处处；它具有 Lipschitz 代表
\[
 v(x)=(1+|x|)^2.
\]
指定代表 \(\Phi(u)\) 在原点的取值依赖于原先的任意补值，未必连续，但这不影响其弱导数：
\[
 D\bigl(\Phi(u)\bigr)
 =Dv
...
这里消除的是两侧迹的跳跃，而不是任意指定的原点值与共同迹之间可能存在的点态不连续。
...
沿用\cref{b1:ex:28-cantor-cylinder-and-atoms}的一维 Cantor 分布函数。
...
\(\mathcal H^{d-1}\llcorner K\) 为 \(\sigma\) 有限。
给定 \(u\in BV(\Omega)\)，假设
\[
...
\]
证明 \(u\in W^{1,1}(\Omega)\)。
分别利用\cref{b1:ex:28-radial-piecewise-decomposition}和%
\cref{b1:ex:28-cantor-cylinder-and-atoms}说明：
...
若删除跳跃条件，取\cref{b1:ex:28-radial-piecewise-decomposition}中 \(a\ne-1\) 的函数，并令 \(K=\partial B(0,1)\)。
它在 \(K\) 外分片光滑，\(K\) 的长度有限，却有非零界面导数。
若删除 \(\sigma\) 有限条件，取\cref{b1:ex:28-cantor-cylinder-and-atoms}中的 \(u(x,y)=C(x)\) 及 Cantor 柱 \(K\)。
它连续，故没有跳跃；在 \(K\) 外局部常于 \(x\)，因而光滑，
却仍有非零 Cantor 导数。\cref{b1:ex:28-cantor-cylinder-and-atoms}已经证明这时 \(K\) 确实不是长度的 \(\sigma\) 有限集。
\end{solution}
...
将这一结论与\cref{b1:ex:28-chain-jump-cancellation}中的消跳现象比较。
\end{exercise}
...
\[
 J_v=\rho J_u,\quad
 \nu_v(\rho x)=\operatorname{sgn}(s)\cdot\nu_u(x).
\]
...
\begin{solution}
在常数函数中，最优常数为 \(c=0\)，对应最小能量为 \(2\lambda La^2\)，因为
\[
 \lambda\int_{-L}^L|c-g|^2\,\mathrm dx
 =2\lambda\cdot L(c^2+a^2).
\]
这正好验证了上述最小能量。

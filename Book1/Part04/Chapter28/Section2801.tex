...
本节的基础定义适用于一般实值可测函数；涉及 BV 的余维一结构时，再明确加入有限变差条件。

Alberti 与 Mantegazza 的《A Note on the Theory of SBV Functions》%
\cite[注1.2]{AlbertiMantegazza1997SBV}把近似极限与导数分解放在一起介绍。
...
\end{proposition}
\begin{proof}
先把 \(u\) 换成几乎处处相等的 Borel 代表。任意这样的替换都只在零测集内改变每个阈值集，
...
第十四章的“近似连续”还要求这个等式成立，属于指定代表的性质；
...
由第十四章的 Lebesgue 微分定理，局部可积函数的几乎每点满足
\[
...
第\ref{b1:sec:1405}节已构造可积尖峰的反例：非零部分在原点的密度趋零，
而球中归一化积分不趋零。因此这里不重复该构造，也不把近似极限存在自动当作一个
\(L^1\) 平均极限。BV 在余维一尺度上还具有更强的性质，证明这点需要下一节的结构输入。

...
再用上一章的层集与周长结构，说明 BV 函数的跳跃点为何由可数个曲面承载。
...
因此这里的 \(u^*\) 与第十四章由球平均定义的精确代表在 \(J_u\) 上相同；
不另用这个记号表示一个冲突的点值约定。
...
\]
与命题\ref{b1:prop:smooth-domain-perimeter}一致。不能将集合外法向直接代入
排序后的函数跳跃公式而漏掉这个负号。
...
\end{proposition}
\begin{proof}
先选取 \(u\) 的 Borel 代表；这不改变任何跳跃数据。
由命题\ref{b1:prop:approximate-limits-basic}，集合
\[
...
跳跃方向并不预设曲面，但 BV 条件将这些局部方向约束在可数个可整流集合上。
...
\end{proposition}
\begin{proof}
由 BV 余面积公式，几乎每个 \(t\in\mathbb R\) 都使
...
\]
De Giorgi 结构定理\ref{b1:thm:de-giorgi-structure}说明，
...
\begin{theorem}[BV 跳跃结构]\label{b1:thm:bv-jump-structure}
若 \(u\in BV(\Omega)\)，则
...
在它们之间选稠密好层值，由命题\ref{b1:prop:approximate-oscillation-level-boundary}，
便把这些点覆盖在好层集的测度论边界上。
由上一章，这些边界模零由可数个可整流曲面承载。

第二项输入是 BV 函数在可整流曲面上的两侧迹定理：
...
本节不展开非有限近似值的余维一估计，以及 BV 曲面两侧迹与其分部积分公式。
它们需要进一步的 BV 切片和精细代表理论，不能把第23章的 Sobolev 迹定理
直接套在一般 BV 函数上来替代。
\end{proofsketch}

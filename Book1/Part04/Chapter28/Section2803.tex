...
跳跃值与法向沿用第\ref{b1:sec:2802}节的约定：
\(u^+>u^-\)，\(\nu_u\) 从低值侧指向高值侧。
...
\cite[第3章]{Ambrosio2000Functions}。
...
尚未证明 \(g\) 与逐点的微分有什么联系。
\symidx{\(D^au\)：BV 导数测度的绝对连续部分}
\symidx{\(D^su\)：BV 导数测度的奇异部分}

\begin{theorem}[BV 函数的近似梯度]
...
上述结论在所引短文 Remark 1.2、式(1.2)之后明确陈述，
该处将完整论证指向 Evans--Gariepy 原版的第6.1节。
\end{proofsketch}
...
再加上 BV 定义中的 \(u\in L^1(\Omega)\)，恰好满足 \(W^{1,1}\) 的定义。
\end{proof}
...
\end{definition}
\symidx{\(D^cu\)：BV 导数测度的 Cantor 部分}

\begin{theorem}[BV 导数测度的三部分分解]
...
\]
因此 \(D(u-F)=0\)。
第\ref{b1:sec:2004}节证明的零弱导数判别说明 \(u-F=c\) 几乎处处。

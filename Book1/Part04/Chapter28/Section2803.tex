...
\begin{equation}\label{b1:eq:bv-lebesgue-decomposition}
 ...
 |D^su|\perp m_d.
\end{equation}
记 \(D^au\coloneqq g\,m_d\)，称它为导数测度的绝对连续部分。
...
\begin{theorem}[有界变差函数的近似梯度（深层结构定理）]
...
\[
 D^ju\coloneqq Du\mathbin{\llcorner}J_u,
\]
...
例如在 \((0,1)\) 内令
\[
 v(x)=\sum_{n=1}^{\infty}2^{-n}\cdot\mathbf1_{(2^{-n},1)}(x).
\]
...
\begin{theorem}[有界变差函数的导数测度三分解（深层结构定理）]
...
\cref{b1:tab:bv-derivative-three-parts}把三项按承载尺度与密度信息并列。它是\eqref{b1:eq:bv-three-part-decomposition}的检索表，不替代各项的正式定义和深层结构定理。


\cref{b1:fig:visual-f20}与导数分解表互补：一个展示模型函数，一个列出三类测度的性质。
\input{Figures/F20}

\begin{table}[htbp]
  \centering
  \caption[有界变差函数导数的三部分]{有界变差函数导数的三部分。三项分别记录体积变化、界面跳跃和剩余的奇异连续变化。}
  \label{b1:tab:bv-derivative-three-parts}
  \begin{tabularx}{\textwidth}{@{}p{2.5cm}p{4.6cm}X@{}}
    \toprule
    分量 & 表示或承载 & 记录的变化 \\
    \midrule
    \(D^au\) & \(\nabla u\,m_d\) & 体积上可由近似梯度描述的变化 \\
    \(D^ju\) & \((u^+-u^-)\nu_u\,\mathcal H^{d-1}\llcorner J_u\) & 可整流界面上的有向跳跃 \\
    \(D^cu\) & 与体积测度奇异，且不充任何 \(\sigma\)-有限 \(\mathcal H^{d-1}\) 集 & 既非体积密度、又非跳跃面承载的变化 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\[
 \int_a^b F(x)\cdot\varphi'(x)\,\mathrm dx
...
例如，\cref{b1:ex:14-cantor-function}中的 \(F_C\) 满足
\[
...
\begin{theorem}[Volpert 链律的 \(C^1\) 版本（深层结构定理）]

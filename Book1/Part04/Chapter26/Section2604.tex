...
\[
 ...
 \coloneqq\inf\{\,\operatorname{cap}_p(G,D):
...
相对容量关于 \(E\) 单调且次可加，关于外域 \(D\) 反向单调。容许函数可以限制为 \(0\le v\le1\)。对紧集 \(K\Subset D\)，还有
...
集合单调性来自容许类的包含；外域增大时，利用\cref{b1:prop:w0-zero-extension}把容许函数补零即可。证明次可加性时，只须考虑容量之和有限的情形。为各集选取开覆盖及近乎最优的容许函数 \(v_j\in[0,1]\)，使梯度能量之和有限，并令 \(w_N=\max_{j\le N}v_j\)。格公式控制其梯度，\cref{b1:thm:zero-boundary-poincare}控制其零阶项；而
\[
...
这也说明本定义与经典势论先在紧集上定义容量的口径一致。具体地，取开集 \(G\) 的紧穷竭 \((K_j)\)，使
\[
 K_j\subseteq\operatorname{int}K_{j+1},\qquad
 \bigcup_{j=1}^{\infty}\operatorname{int}K_j=G.
\]
记 \(L=\sup_j\operatorname{cap}_p(K_j,D)\)。若 \(L=+\infty\)，所需等式由单调性立即成立；以下设 \(L<\infty\)。由\eqref{b1:eq:relative-compact-capacity}，可为每个 \(j\) 选取 \(\varphi_j\in C_c^\infty(D)\)，使它在 \(K_j\) 的某个开邻域上至少为一，并且
\[
 \int_D|\nabla\varphi_j|^p\,\mathrm dx\le L+2^{-j}.
\]
零边界 Poincaré 不等式使 \((\varphi_j)\) 在 \(W_0^{1,p}(D)\) 中有界。抽取弱收敛子列，记其极限为 \(\varphi\)。固定 \(m\)。当 \(j\ge m\) 时，\(\varphi_j\ge1\) 于 \(\operatorname{int}K_m\)；而
\[
 \mathcal C_m=\{\,v\in W_0^{1,p}(D):v\ge1\text{ 几乎处处于 }\operatorname{int}K_m\,\}
\]
是 \(W_0^{1,p}(D)\) 中的范数闭凸集。凸性由逐点不等式保持；若 \(v_j\in\mathcal C_m\) 且 \(v_j\to v\) 于 \(W_0^{1,p}(D)\)，则
\[
 \|(1-v)_+\cdot\mathbf1_{\operatorname{int}K_m}\|_{L^p(D)}
 \le\|v-v_j\|_{L^p(D)}\longrightarrow0,
\]
所以 \(v\in\mathcal C_m\)。由\cref{b1:thm:convex-weak-closure}，\(\mathcal C_m\) 在 \(W_0^{1,p}(D)\) 中弱闭。故 \(\varphi\ge1\) 几乎处处于每个 \(\operatorname{int}K_m\)，也就几乎处处于 \(G\)。梯度能量的弱下半连续性给出
\[
 \operatorname{cap}_p(G,D)\le\int_D|\nabla\varphi|^p\,\mathrm dx\le L.
\]
反向不等式来自单调性，所以
\(\operatorname{cap}_p(G,D)=\sup_{K\Subset G}\operatorname{cap}_p(K,D)\)。最后再对任意集取开外正则化。弱极限中保留的是开集 \(\operatorname{int}K_m\) 上的几乎处处约束，而不是可能为零测集的 \(K_m\) 上的约束；此处也不需要先定义某个 Sobolev 代表在任意集合上的逐点值。
...
\begin{definition}\label{b1:def:p-quasiopen-set}
设 \(U\subseteq\mathbb R^d\)。若对每个 \(\varepsilon>0\)，都存在开集 \(G_\varepsilon\subseteq\mathbb R^d\)，使
\[
 C_p(G_\varepsilon)<\varepsilon,\qquad
 U\cup G_\varepsilon\text{ 为欧氏开集},
\]
则称 \(U\) 为 \(p\)-\term[ni-kaiji]{拟开集}[quasiopen set]。
\end{definition}

\begin{theorem}[拟开集的细拓扑刻画]\label{b1:thm:p-quasiopen-fine-characterization}
设 \(1<p<\infty\)，\(U\subseteq\mathbb R^d\)。集合 \(U\) 是 \(p\)-拟开集，当且仅当存在 \(p\)-细开集 \(V\) 与容量零集 \(N\)，使
\[
 U=V\cup N.
\]
这里不要求该并互不相交；等式表达的是拟开集与细开集只相差一个容量零的附加部分。
\end{theorem}
\begin{proofsketch}
这是本节使用的外部势论输入，不由前面的拓扑公理推出。Björn、Björn 和 Latvala 的《The Cartan, Choquet and Kellogg properties for the fine topology on metric spaces》\cite[Theorem 1.4 and Section 8]{Bjorn2018Cartan}证明了拟开集恰好是细开集与容量零集之并；欧氏空间及本章的 Sobolev 容量属于该结果的经典情形。本书在这里只调用这一集合刻画，不重建其所需的 Cartan、Choquet 与 Kellogg 性质。
\end{proofsketch}

\begin{corollary}\label{b1:thm:quasicontinuous-fine-continuity}
设 \(1<p<\infty\)。每个有限实值的 \(p\)-拟连续函数都在 \(p\)-拟处处细连续。特别地，\cref{b1:thm:sobolev-quasicontinuous-representative}所构造的 Sobolev 代表具有这一性质。
\end{corollary}
\begin{proof}
设 \(v\) 拟连续。对每个实数 \(a\)，集合 \(\{v>a\}\) 与 \(\{v<a\}\) 都是拟开集。事实上，给定 \(\varepsilon>0\)，取容量小于 \(\varepsilon\) 的开集 \(G\)，使 \(v\) 在 \(G^c\) 上相对连续；相应水平集与 \(G\) 的并便是欧氏开集。

对所有有理数 \(a\) 应用前一定理，分别写成细开集与容量零集之并。由次可加性，可将这些例外集合并为一个容量零集 \(N\)。固定 \(x\notin N\) 及 \(\varepsilon>0\)，选有理数
\[
 v(x)-\varepsilon<a<v(x)<b<v(x)+\varepsilon.
\]
在 \(N^c\) 中，与 \(\{v>a\}\) 和 \(\{v<b\}\) 对应的两个细开部分都含有 \(x\)。它们同细开集 \(N^c\) 的交仍是含 \(x\) 的细开邻域，并且其上有 \(a<v<b\)。故 \(v\) 在 \(x\) 处细连续。最后应用\cref{b1:thm:sobolev-quasicontinuous-representative}即可。
\end{proof}

\OptionalSubsection{Wiener 判据与边界正则性}{b1:subsec:260403}
...
\[
 \Delta_pu\coloneqq\operatorname{div}\bigl(|\nabla u|^{p-2}\nabla u\bigr).
\]
...
下面引用的原始结果采用同一口径：Kilpeläinen 与 Malý 直接以满足 \(u-g\in W_0^{1,p}(\Omega)\) 的变分解能否在边界点恢复 \(g\) 来定义正则性\cite[p. 137 and THEOREM 1.1]{Kilpelainen1994Wiener}。因此这里不需要先把本书的变分正则性同另一套 Perron 正则性作等价辨认。
...
必要性更深：当补集薄时，借助相对容量的极小函数、非线性势的点态估计及 Cartan 性质，构造在该点不能恢复指定边值的解。Kilpeläinen 与 Malý 的《The Wiener test and potential estimates for quasilinear elliptic equations》\cite[THEOREM 1.1 and Section 5]{Kilpelainen1994Wiener}给出上述完整范围，并说明 Cartan 性质、比较方法和 Wiener 判别之间的联系。本节不展开支撑必要性的非线性势估计；所引定理本身已经使用上文的变分正则性口径，并不要求本书另行建立 Perron 方法。定理在这里用于指出容量理论通向边界正则性的方向，而不是宣称这些势论工具已在前文证明。
\end{proofsketch}
...
\OptionalSubsection{Cartan 性质与势论联系}{b1:subsec:260404}
...
\begin{theorem}[Cartan 分离]\label{b1:thm:p-cartan-characterization}
...
Kilpeläinen 与 Malý 的《The Wiener test and potential estimates for quasilinear elliptic equations》\cite[THEOREM 1.3 and Section 5]{Kilpelainen1994Wiener}建立了这一点态分离。完整证明依赖本节尚未展开的非线性势估计，故这里保留为外部定理。
\end{proofsketch}

\begin{corollary}[细邻域刻画]\label{b1:cor:p-fine-neighborhood-characterization}
设 \(U\subseteq\mathbb R^d\) 且 \(x_0\in U\)。集合 \(U\) 包含 \(x_0\) 的一个细开邻域，当且仅当 \(\mathbb R^d\setminus U\) 在 \(x_0\) 处是 \(p\)-薄的。
\end{corollary}
\begin{proofsketch}
若 \(U\) 已经包含一个细开邻域，结论由薄性对取子集保持立即得到。反向的细邻域提取是 Cartan 分离的非平凡推论；Heinonen、Kilpeläinen 和 Martio 的《Fine topology and quasilinear elliptic equations》\cite[3.2. THEOREM]{Heinonen1989Fine}给出完整证明。这里把它单列，是为了不把“补集在一点薄”误当成细开集定义本身已经包含的结论。
\end{proofsketch}

\begin{theorem}[细拓扑的超调和刻画]\label{b1:thm:p-fine-topology-superharmonic}
在每个欧氏开集上，\(p\)-细拓扑是使所有局部 \(p\)-超调和函数连续的最弱拓扑；函数允许取 \(+\infty\) 时，值域采用通常的扩充实数拓扑。
\end{theorem}
\begin{proofsketch}
这一结论既需要证明每个 \(p\)-超调和函数细连续，也需要证明任何具有该连续性的拓扑都包含由薄性刻画的细邻域；两步都超出前面的拓扑公理。上述论文\cite[pp. 293--299 and 3.2. THEOREM]{Heinonen1989Fine}以超调和函数生成的拓扑为起点，并用细邻域与薄性的等价把它同本章定义的细拓扑辨认。本书在此只记录该外部刻画。
\end{proofsketch}
...
\cref{b1:tab:capacity-exception-scales}把本章的几个相近说法按“允许忽略什么”分开。后两行涉及函数的连续性，不应仅由前两行的零集包含关系推出。
\begin{table}[htbp]
  \centering
  \caption[容量控制的例外尺度]{容量控制的例外尺度。Lebesgue 零测、容量零和细邻域分别服务于不同层次的点态陈述。}
  \label{b1:tab:capacity-exception-scales}
  \begin{tabularx}{\textwidth}{@{}p{2.8cm}p{4.6cm}X@{}}
    \toprule
    说法 & 允许忽略的对象 & 典型用途 \\
    \midrule
    几乎处处 & Lebesgue 零测集 & 识别 \(L^p\) 与 Sobolev 等价类 \\
    拟处处 & \(p\)-容量零集 & 确定拟连续代表的唯一性与逐点子列 \\
    拟连续 & 对每个 \(\varepsilon>0\) 删除开集 \(G\)，且 \(C_p(G)<\varepsilon\) & 在 \(G^c\) 上恢复通常的相对连续性 \\
    细连续 & 采用 \(p\)-细拓扑的邻域 & 把容量控制的局部障碍纳入连续性概念 \\
    \bottomrule
  \end{tabularx}
\end{table}

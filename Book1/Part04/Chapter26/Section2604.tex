...
\end{theorem}
\begin{proofsketch}
需要的深层输入是以下集合刻画：若一个集合在加入容量任意小的适当开集后总能成为开集，则它可写成一个细开集与一个容量零集的并。这是非线性势论中的结果，其证明依赖 Cartan 型分离、Choquet 性质和相应的容量例外集控制；这些步骤不在本节展开。Björn、Björn 和 Latvala 的《The Cartan, Choquet and Kellogg properties for the fine topology on metric spaces》\cite[定理1.4，第8节]{Bjorn2018Cartan}给出该刻画与本定理的联系，这里仅使用其欧氏空间情形。
...
必要性更深：当补集薄时，借助相对容量的极小函数、非线性势的点态估计及 Cartan 性质，构造在该点不能恢复指定边值的解。Kilpeläinen 与 Malý 的《The Wiener test and potential estimates for quasilinear elliptic equations》\cite[定理1.1，第5节]{Kilpelainen1994Wiener}给出上述完整范围，并说明 Cartan 性质、比较方法和 Wiener 判别之间的联系。本节不展开支撑必要性的非线性势估计，也不另行建立 Perron 方法及其与变分解的对应。定理在这里用于指出容量理论通向边界正则性的方向，而不是宣称这些势论工具已在前文证明。
\end{proofsketch}
...
Kilpeläinen 与 Malý 的《The Wiener test and potential estimates for quasilinear elliptic equations》\cite[定理1.3，第5节]{Kilpelainen1994Wiener}建立这种点态分离；由超调和函数定义的拓扑与薄性邻域的等价刻画，可参阅 Heinonen、Kilpeläinen 和 Martio 的《Fine topology and quasilinear elliptic equations》\cite[定理3.2]{Heinonen1989Fine}。这些结论包含了前文尚未证明的邻域提取步骤。若一个集合已经含有细开邻域，其补集薄只需单调性；从仅在一个点成立的薄性反过来提取细开邻域，才是此处需要势论的方向。
\end{proofsketch}

...
\begin{equation}\label{b1:eq:quasicontinuous-bad-open-sets}
 ...
 \leq\sum_{j\geq N}2^{jp}\cdot E_p(\varphi_{j+1}-\varphi_j)
...
\[
 z\in W^{1,p},\quad
 E_p(z)\leq a^{-p}\cdot E_p(u)+E_p(w_G)
 <a^{-p}\cdot E_p(u)+2\varepsilon.
\]
在 \(G\) 上，函数 \(w_G\) 几乎处处等于 \(1\)；在 \(O\setminus G\) 上，由 \(u=v\) 几乎处处可知 \(|u|/a>1\) 几乎处处。因此 \(z\) 是开集 \(O\cup G\) 的容许函数，进而
\[
 C_p\bigl(\{\,|v|>t\,\}\bigr)
 \leq C_p(O\cup G)
 \leq a^{-p}\cdot E_p(u)+2\varepsilon.
\]
...
\[
 ...
 \leq t^{-p}\cdot E_p(u_n-u)\longrightarrow0.
\]

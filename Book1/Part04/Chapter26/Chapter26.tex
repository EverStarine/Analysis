...
本章属于选读。主线固定 \(1<p<\infty\)，因为次可加性和若干极限步骤使用自反性；本章没有覆盖端点 \(p=1\) 的另一套细节。前三节主要依赖第9章的 Mazur 定理与凸泛函弱下半连续性、第13章的 \(5r\) 覆盖、第15章的 Hausdorff 测度、第16章和第21章的一维绝对连续理论与 ACL 代表，以及第24章的 Poincaré 和 Morrey 不等式；弱导数、光滑稠密性与 Sobolev 弱紧性仍是贯穿其中的基础。最后一节把这些结果放入非线性势论的图景，其中 Wiener 判据、Cartan 性质及超调和函数刻画均作为有明确来源的外部输入，不当作前三节已经证明的推论。
...
\begin{enumerate}
 ...
 \]
 \item 分别举出一个零容量的 Lebesgue 零测集和一个正容量的 Lebesgue 零测集，并说明这不与
 \(m_d^*(E)\le C_p(E)\) 矛盾。
\end{enumerate}
\end{exercise}
...
当 \(1<p\le d\) 时，单点是 Lebesgue 零测且容量为零的集合。若 \(d\ge2\)，平面片
\[
 [0,1]^{d-1}\times\{0\}
\]
的 \(d\) 维 Lebesgue 测度为零，但由竖线上的一维点值估计具有正容量。若 \(d=1\)，本题的固定范围必有 \(p>d\)，此时任意单点已经是零测而正容量的例子。不等式 \(m_d^*(E)\le C_p(E)\) 只说明零容量蕴含零测；它并未断言零测蕴含零容量，所以没有矛盾。
\end{solution}
...
\begin{solution}
固定 \(x\) 与 \(r>0\)。取一个在 \(B(x,r)\) 上等于一、支集含于 \(B(x,2r)\) 的光滑截断 \(\eta\)。若 \(u\) 是覆盖 \(N\) 的全空间容许函数，则 \(\eta u\) 在 \(B(x,2r)\) 中零边界，并覆盖 \(N\cap B(x,r)\)。乘积估计给出
\[

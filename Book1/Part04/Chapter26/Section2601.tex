...
\begin{equation}\label{b1:eq:sobolev-outer-capacity}
 C_p(A)\coloneqq\inf\{C_p(G): A\subseteq G,\ G\text{ 开}\}.
\end{equation}
\end{definition}
...
至此，\(C_p\) 具有外测度的三个公理。不过它一般不是通常意义的可加测度；不能因其定义在全部集合上，就认定所有 Borel 集自动满足 Carathéodory 可测性。\cref{b1:ex:26-one-dimensional-exact-capacity}将具体算出可加性的失败。我们在后续只使用已经证明的单调性、次可加性和开外正则性。
...
\[
 C_p(\{|v|>t\})\le t^{-p}\cdot E_p(v).
\]
\end{lemma}
\begin{proof}
连续性使超水平集开；函数 \(\min(1,|v|/t)\) 在其上几乎处处等于一，且能量不超过 \(t^{-p}\cdot E_p(v)\)，因而是所需容许函数。
\end{proof}
...
\[
 ...
 =\sigma_{d-1}\cdot \log(R/\varepsilon)^{1-d}\longrightarrow0.
\]
...
\]
对 \(s\) 平均，再用 Hölder 不等式。随后对 \(x'\) 积分，得到 \(1\le C_p\cdot E_p(u)\)。因此 \(C_p(S)>0\)，尽管 \(m_d(S)=0\)。邻域厚度可以任意小，保持片上值所需的能量却不能消失。
...
容量的两种能量项及其尺度可从\cref{b1:fig:visual-f18}辨认。
\input{Figures/F18}

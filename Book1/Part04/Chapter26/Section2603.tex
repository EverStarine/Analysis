...
\(\|u_n\|_p^p\leq C_d\cdot M\cdot\delta_n^p\to0\)。再从能量有界列
...
最后，若 \(E=\bigcup_\ell E_\ell\)，每个 \(E_\ell\) 的临界外测度有限，就由刚证的结论及容量的次可加性得到 \(C_p(E)=0\)。
\end{proof}
...
\[
 ...
 &\leq c_s\cdot10^s\cdot\sum_i r_i^s\\
 &\leq\frac{c_s\cdot10^s}{M}\cdot\sum_i\mu(B_i)
 \leq\frac{c_s\cdot10^s}{M}\cdot\mu(\mathbb R^d).
 \end{aligned}
\]

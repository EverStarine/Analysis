...
本章的起点是第\ref{b1:ch:20}章的弱导数及局部卷积。第\ref{b1:sec:2102}节使用第八至十章的 Banach 空间、Hahn--Banach 定理与 \(L^p\) 对偶；第\ref{b1:sec:2104}节末尾再接上极大函数和第\ref{b1:sec:1603}节的 Lipschitz 分片。这些工具的调用都发生在开集内部，暂不要求边界光滑，也不先使用下一章的全域光滑逼近。
...
% 第二十一章习题临时稿；由章聚合文件已有的 BookExercises 入口统一接入。
...
\]
因此与任意有限区间阶梯函数相乘后的积分都趋于零。这些阶梯函数在 \(L^2(I)\) 中稠密：先用第\ref{b1:sec:1002}节的连续紧支集函数逼近，再在有限区间上利用一致连续性作阶梯逼近。又因 \(\|\cos(nx)\|_2=\sqrt\pi\)，对 \(h\in L^2(I)\) 及一个阶梯函数 \(s\)，有
\[

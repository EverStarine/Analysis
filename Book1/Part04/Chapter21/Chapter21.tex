...
本章的起点是\chapref{b1:ch:20}的弱导数及局部卷积。\secref{b1:sec:2102}使用第八至十章的 Banach 空间、Hahn--Banach 定理与 \(L^p\) 对偶；\secref{b1:sec:2103}前三类局部运算沿用第二十章的卷积，而坐标变换还调用\cref{b1:thm:rademacher}和\cref{b1:thm:weighted-area-formula}。代表理论使用\secref{b1:sec:1604}的一维绝对连续微积分，通常可微性调用\chapref{b1:ch:14}的 Lebesgue 点与 Rademacher 定理，近似可微性再接上\chapref{b1:ch:13}的 Hardy--Littlewood 极大函数和\secref{b1:sec:1603}的 Lipschitz 分片。由此，定义与空间结构只依赖前两编及卷积工具，点态代表和坐标变换则确实使用第三编；这些调用仍发生在开集内部，暂不要求边界光滑，也不先使用下一章的全域光滑逼近。
...
\begin{exercise}[\(\star\) 选读]\label{b1:ex:21-scaling-concentration}
...
\begin{exercise}[\(\star\) 选读]\label{b1:ex:21-endpoint-structure}
\begin{enumerate}
...
\begin{exercise}[\(\star\) 选读]\label{b1:ex:21-coordinate-change-boundaries}
\begin{enumerate}
...
\begin{exercise}[\(\star\) 选读]\label{b1:ex:21-highest-derivative-kernel}

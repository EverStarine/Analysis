...
\end{proposition}
\begin{proof}
局部刻画及唯一性就是\cref{b1:thm:one-dimensional-weak-ac}。在有界区间上，由 Hölder 不等式，\(u,u'\in L^1(I)\)。固定内点 \(x_0\)，积分表示
\[
...
\cref{b1:fig:acl-coordinate-lines}示意二维时的量词结构。每个方向都允许删去一族横向参数零测的直线；定义要求存在同一个代表，在其余截线上分别绝对连续。
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}
    \draw[thick] (-3,-1.7) rectangle (3,1.7);
    \node[above] at (0,1.7) {$\Omega$ 内的矩形块};
    \foreach \x in {-2.2,-1.1,1.1,2.2}
      \draw[gray!70] (\x,-1.5)--(\x,1.5);
    \foreach \y in {-1.1,0,1.1}
      \draw[gray!70] (-2.8,\y)--(2.8,\y);
    \draw[dashed,thick] (0,-1.5)--(0,1.5);
    \node[right] at (0,1.25) {允许的异常截线};
    \draw[->] (-2.2,-1.25)--(-2.2,1.25) node[above] {$e_2$};
    \draw[->] (-2.55,-1.1)--(2.55,-1.1) node[right] {$e_1$};
  \end{tikzpicture}
  \caption[ACL 坐标截线]{ACL 坐标截线。实线示意良好截线，虚线只表示某个允许排除的截线；真正的例外族由横向零测集参数化。}
  \label{b1:fig:acl-coordinate-lines}
\end{figure}
...
反向由\cref{b1:thm:lipschitz-weak-derivative}得到。最后对局部 Lipschitz 代表使用 Rademacher \cref{b1:thm:rademacher}；经典偏导与弱偏导几乎处处一致，故全微分的系数正是弱梯度。
\end{proof}
...
\[
 \begin{gathered}
  ...
  \Longrightarrow\operatorname{ap}D\,u=\nabla u.
 \end{gathered}
\]
...
\[
 |w(x)-w_{Q(x,r)}|\leq C_dr\cdot Mg(x).
\]
在 \(y\) 处也有同样的估计。
...
\begin{proof}
局部 \(L^p\) 包含于局部 \(L^1\)，所以先把 \(u\) 看成 \(W^{1,1}_{\mathrm{loc}}\) 函数。固定 \(K\Subset\Omega\)，选 \(\chi\in C_c^\infty(\Omega)\)，使 \(\chi=1\) 于 \(K\) 的某个邻域。由光滑乘法公式，将 \(w=\chi\cdot u\) 及其弱导数补零到 \(\mathbb R^d\)，得到 \(w\in W^{1,1}(\mathbb R^d)\)。这一步的补零没有边界项，因为 \(\chi\) 紧支撑于 \(\Omega\)。

令 \(g=\sum_i|\partial_iw|\in L^1(\mathbb R^d)\)。由\hyperref[b1:thm:hl-weak-11]{Hardy--Littlewood 弱型不等式\ref*{b1:thm:hl-weak-11}}，\(Mg\) 几乎处处有限。从 \(K\) 删去零测例外，使剩下的点都是 \(w\) 的 Lebesgue 点，且所选的 \(u\) 值等于该点的 Lebesgue 值。再按 \(Mg(x)\leq n\) 分层，得可测集 \(E_n\)，其并覆盖 \(K\) 中几乎所有点。由\eqref{b1:eq:sobolev-maximal-difference}，\(u|_{E_n}\) 为 \(2C_dn\)-Lipschitz 函数。
...
还要识别近似微分，而不只是证明它存在。把 \(u|_{K_j}\) 延拓为全空间 Lipschitz 函数 \(F_j\)。由\cref{b1:thm:lipschitz-weak-derivative}，\(F_j\) 的弱梯度等于其几乎处处经典梯度。又因 \(u-F_j=0\) 于 \(K_j\)，\cref{b1:prop:sobolev-gradient-on-level-sets}给出
\[
 \nabla u=\nabla F_j=DF_j
 \quad\text{几乎处处于 }K_j.
\]
其中最后一项按行向量与线性泛函的对应理解。在 \(K_j\) 的密度点且 \(F_j\) 可微的点上，沿 \(K_j\) 两函数相等，所以密度一集合判据给出 \(\operatorname{ap}D\,u=DF_j\)。合并这些等式，再合并可数个零测例外，便得所需识别。
\end{proof}

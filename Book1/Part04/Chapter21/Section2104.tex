...
每个 \(E_n\) 体积有限。由紧内正则性，可在其中选择可数个紧集，除去一个零测集后覆盖 \(E_n\)。再用可数个 \(K\Subset\Omega\) 覆盖 \(\Omega\)，就得到几乎处处覆盖定义域的紧集族 \((K_j)\)，每个 \(u|_{K_j}\) 都 Lipschitz。由定理\ref{b1:thm:approximate-lipschitz-pieces}，\(u\) 几乎处处近似可微。

...
对任意子列，再抽取一条使函数值几乎处处收敛于 \(u\) 的子列。在 \(u\ne0\) 的点，两个指标函数最终相同；在 \(u=0\) 的点，后一项几乎处处为零。因此后一项沿该子列几乎处处趋零，其绝对值的 \(p\) 次幂又受 \(|\partial_i u|^p\) 控制。控制收敛定理给出这条子列上的 \(L^p\) 收敛。由于原序列的任意子列都可这样再抽取，完整序列的后一项也在 \(L^p\) 中趋零。对有限多个坐标合并，便得 \(u_n^+\to u^+\) 于 \(W^{1,p}\)。负部由 \(u^-=(-u)^+\) 得到，绝对值再由 \(|u|=u^++u^-\) 得到。
\end{proof}

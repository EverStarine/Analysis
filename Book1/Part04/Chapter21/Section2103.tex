...
经典微积分的运算公式能否保留，要同时回答两个问题：等式在分布意义下是否成立，右端是否具有所要求的可积性。下面先在开集内部把函数光滑化，再用局部积分收敛传递公式。乘积、复合与截断这前三类局部运算主要依靠\chapref{b1:ch:20}的卷积；坐标变换还需要\cref{b1:thm:rademacher}的几乎处处可微性和\cref{b1:thm:weighted-area-formula}的加权面积公式。这里不依赖下一章的全域光滑稠密性或延拓定理。
...
后面的证明还会使用这样的取极限方式：若 \(u_j\to u\)、\(g_{i,j}\to g_i\) 于 \(L^1_{\mathrm{loc}}\)，且 \(\partial_i u_j=g_{i,j}\)，则 \(\partial_i u=g_i\)。只需对固定测试函数在分部积分恒等式两端取极限，正如\secref{b1:sec:2004}已说明的那样。
...
只知道 \(u,v\in W^{1,p}\) 时，不能无条件断言乘积仍属于 \(W^{1,p}\)。弱导数公式并不自动提高 \(u\partial_i v\) 的可积性；\chapref{b1:ch:24}的嵌入定理以后会提供另外一些充分条件。\cref{b1:ex:21-product-integrability-loss}则直接构造乘积失去原有可积性的例子。
...
当 \(p=\infty\) 时，先在局部使用 \(p=1\) 的结论，再用所得公式的本性界。若 \(u\) 仅局部本性有界，对固定区域取包含其值域和卷积值域的紧球，用光滑截断把 \(F\) 改成该球邻域上与其相等、导数全域有界的映射，即归结为已证情形。
\end{proof}

全域结论还需要零阶项可积。若 \(u\in W^{1,p}(\Omega)\)，\(DF\) 有界且 \(F(0)=0\)，则 \(F(u)\in W^{1,p}(\Omega)\)。若 \(p<\infty\) 且 \(m_d(\Omega)<\infty\)，允许 \(F(0)\ne0\) 也可以；但在无限测度域上，\(u=0\)、\(F(t)=1+t\) 已经说明不能遗漏这个条件。当 \(p=\infty\) 时，全域本性有界的 \(u\) 经连续 \(F\) 映射后仍本性有界。
...
\]
函数在每个内部有界区域趋于零，右端则由控制收敛在局部 \(L^1\) 中趋于 \(\mathbf1_{\{u=c\}}\cdot\partial_i u\)。弱导数关系的闭性说明后一个极限是零函数的弱导数，因此等于零。这个证明也包括 \(p=\infty\)，并没有使用尚待证明的沿直线代表或近似可微性。
\end{proof}
...
\[
 ...
 \nabla T_M(u)=\mathbf1_{\{|u|<M\}}\cdot\nabla u
 \quad\text{几乎处处}.
\]
正部 \(u_+=\max\{u,0\}\)、负部 \(u_-=\max\{-u,0\}\) 和绝对值也属于同一局部空间，且
\[
 \nabla u_+=\mathbf1_{\{u>0\}}\cdot\nabla u,\quad
 \nabla u_-=-\mathbf1_{\{u<0\}}\cdot\nabla u,\quad
...
\[
 F_\varepsilon'(u)\nabla u
 \longrightarrow\mathbf1_{\{|u|<M\}}\cdot\nabla u
 \quad\text{于 }L^1_{\mathrm{loc}}.
\]
由链式法则和弱导数的闭性得到所求关系；右端的局部 \(L^p\) 控制给出空间归属。

对 \(t_+\) 作同样卷积，并减去卷积在零点的值，使近似函数仍在零点为零。其导数在 \(t\ne0\) 处趋于 \(\mathbf1_{(0,\infty)}(t)\)，在零水平集上仍由梯度为零消除折点的不确定性。于是正部公式成立。负部及绝对值由 \((-u)_+\) 和 \(u_++u_-\) 得到。

最后，\(|T_M(u)|\)、\(u_+\)、\(u_-\) 和 \(|u|\) 均不超过 \(|u|\)，各导数的绝对值也由原导数控制，故全域情形不会丢失零阶或一阶可积性。
\end{proof}

\begin{proposition}[正负部的强连续性]
\label{b1:prop:sobolev-lattice-strong-continuity}
设 \(\Omega\subseteq\mathbb R^d\) 为开集、\(1\le p<\infty\)，并设实值函数
\(u_n,u\in W^{1,p}(\Omega)\)。若 \(u_n\to u\) 于 \(W^{1,p}(\Omega)\)，则
\[
 (u_n)_+\longrightarrow u_+,\qquad
 (u_n)_-\longrightarrow u_-,\qquad
...
\end{proposition}
\begin{proof}
先处理正部。实数正部映射满足 \(|a_+-b_+|\le|a-b|\)，所以
\[
 \|(u_n)_+-u_+\|_p\le\|u_n-u\|_p\longrightarrow0.
\]
由前一命题及水平集上的梯度公式，
\[
 \begin{aligned}
 \partial_i((u_n)_+)-\partial_i(u_+)
 &=\mathbf1_{\{u_n>0\}}\cdot(\partial_i u_n-\partial_i u)\\
 &\quad+
   (\mathbf1_{\{u_n>0\}}-\mathbf1_{\{u>0\}})\cdot\partial_i u,
 \end{aligned}
\]
而 \(\partial_i u=0\) 几乎处处于 \(\{u=0\}\)。第一项在 \(L^p\) 中趋于零。

对任意子列，再抽取一条使函数值几乎处处收敛于 \(u\) 的子列。在 \(u\ne0\) 的点，两个指标函数最终相同；在 \(u=0\) 的点，后一项几乎处处为零。因此后一项沿该子列几乎处处趋零，其绝对值的 \(p\) 次幂又受 \(|\partial_i u|^p\) 控制。控制收敛定理给出这条子列上的 \(L^p\) 收敛。由于原序列的任意子列都可这样再抽取，完整序列的后一项也在 \(L^p\) 中趋零。对有限多个坐标合并，便得 \((u_n)_+\to u_+\) 于 \(W^{1,p}\)。负部由 \(u_-=(-u)_+\) 得到，绝对值再由 \(|u|=u_++u_-\) 得到。
\end{proof}
...
\begin{equation}\label{b1:eq:sobolev-truncation-convergence}
 ...
 \quad(M\to\infty).
\end{equation}
事实上，函数误差由 \(|u|\cdot\mathbf1_{\{|u|>M\}}\) 控制，导数误差由 \(|\partial_i u|\cdot\mathbf1_{\{|u|\ge M\}}\) 控制，对它们的 \(p\) 次幂分别用控制收敛即可。在全域 \(W^{1,\infty}\) 中，对一个固定 \(u\)，只要 \(M\ge\|u\|_\infty\)，截断已经与原函数几乎处处相等；这不意味着正部等非线性运算对 \(W^{1,\infty}\) 范数总是连续，习题会区分这两个问题。
...
高阶空间需要更强的坐标条件。在上述双 Lipschitz 假设下，若 \(\Phi\) 进一步为 \(C^k\) 微分同胚，并且 \(1\) 至 \(k\) 阶的各导数有界，则相同方法给出 \(u\circ\Phi\in W^{k,p}(U)\) 及相应有界估计：对光滑 \(u_j\)，逐次求导后，每一项都是某个 \((\partial^\beta u_j)\circ\Phi\) 乘以有限个 \(\Phi\) 的导数，且 \(|\beta|\le k\)；系数有统一界，故逐项用换元估计取极限即可。若还要得到反向的 \(W^{k,p}\) 有界映射，同样要控制 \(\Phi^{-1}\) 的高阶导数。仅有双 Lipschitz 性只控制一阶，不能代替这些条件。

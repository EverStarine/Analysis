...
第\ref{b1:subsec:130403}小节已经用可积核构造函数的逼近恒等。本节把平均推广到分布：把核的平移看成测试函数，由分布作用于它。所得对象虽然是普通光滑函数，其导数却能保留原分布的微分信息。整个构造先在远离边界的区域进行，不预设分布可以延拓到开集之外。
...
\[
 ...
 \quad \rho_\varepsilon(x)=\varepsilon^{-d}\rho(x/\varepsilon).
\]
满足上述条件的核称为本章的\term[moguang-hanshu]{磨光函数}[mollifier]。名称可对照 Oleinik 的《偏微分方程讲义》\cite[第1.2节]{Oleinik2008PDEChinese}；中文资料也使用“磨光子”这一名称。\termidx[moguangzi]{磨光子}\footnote{北京大学数学学院公开的《实变函数》前言，在介绍第四章的 \(L^p\) 与 Sobolev 空间工具时采用“磨光子”。该用词说明见\href{https://math101.pku.edu.cn/docs/2025-04/b74312fec0b54f0c90ff66270f2bded8.pdf}{学校公布的前言}；这里仅提示名称，不据前言推断其核函数的全部约定。}

例如可将 \(|x|<1\) 上的 \(\exp\bigl(-1/(1-|x|^2)\bigr)\) 在球外补零，再除以其正积分；光滑性已在第\ref{b1:sec:2001}节的截断构造中说明。这是\cref{b1:def:approximate-identity}中的一种特殊核，光滑性和紧支集是本节新增的要求，偶性则用来简化反射记号。
...
\[
 (T*\eta)(x)=\langle T_y,\eta(x-y)\rangle.
\]
...
\[
 (T_f*\eta)(x)=\int_\Omega f(y)\eta(x-y)\,\mathrm dy.
\]
...
\[
 |\langle T,\psi\rangle|\le C p_{H,m}(\psi)
...
\[
 q(y)=\int_{\mathbb R^d}\varphi(x)\eta(x-y)\,\mathrm dx
 =(\check\eta*\varphi)(y)
\]
...
\[
 q_N(y)=\sum_i m_d(Q_i)\varphi(x_i)\eta(x_i-y)
\]
...
\[
 ...
 =\int\rho(z)\bigl(D^\beta\varphi(y+\varepsilon z)-D^\beta\varphi(y)\bigr)\,\mathrm dz.
\]
...
\end{proposition}
\begin{proof}
由第\ref{b1:sec:2001}节的光滑截断，取 \(\chi\in C_c^\infty(\Omega)\)，使它在 \(K\) 的一个邻域上等于 \(1\)。将 \(\chi f\) 补零到全空间；当 \(\varepsilon\) 充分小时，它与 \(f\) 的卷积在 \(K\) 上相同。有限 \(p\) 的结论因此直接归结为\cref{b1:prop:approximate-identity-lp}。

...
\end{theorem}
\begin{proof}
先在 \(u\) 为 \(L\)-Lipschitz 的有界开球 \(B\Subset\Omega\) 内证明。由\cref{b1:thm:rademacher}，经典微分几乎处处存在；把异常点处的偏导数补零，得到可测函数 \(g_i\)，满足 \(|g_i|\leq L\)。将 \(u|_B\) 在球外补零为 \(u_0\)。对 \(\varphi\in C_c^\infty(B)\)，平移变量给出
\[

...
\begin{exercise}[\(\star\) 选读]\label{b1:ex:20-locally-finite-unbounded-order}
在 \(\mathbb R\) 上定义线性泛函
\[
...
\begin{exercise}[\(\star\) 选读]\label{b1:ex:20-polynomial-annihilator}
...
\begin{exercise}[\(\star\) 选读]\label{b1:ex:20-principal-value-logarithm}
...
\begin{exercise}[\(\star\) 选读]\label{b1:ex:20-distribution-first-order-ode}
...
\begin{exercise}[\(\star\) 选读]\label{b1:ex:20-smoothing-variable-coefficient}

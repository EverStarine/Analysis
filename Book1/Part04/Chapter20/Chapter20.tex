...
从\secref{b1:sec:2001}到\secref{b1:sec:2004}，测试空间、其连续对偶以及对偶中仍由函数表示的部分依次出现。测试空间的局部凸拓扑只展开到本章需要的有限阶估计；读者第一次阅读时可以先掌握共同紧支集的收敛判据，再回看其拓扑构造。积分、\(L^p\) 平移连续性与\chapref{b1:ch:16}的绝对连续理论是主要前置工具。

...
分部积分把求导转移到测试函数上，使不可微函数乃至点质量也能拥有导数。本章构造紧支集光滑测试空间，定义分布及其基本操作，证明分布的局部光滑化，再从中识别能够由局部可积函数表示的弱导数。跳跃函数、尖角和 Cantor 函数将说明：经典导数几乎处处存在，并不足以保证弱导数存在。
\end{chapterabstract}
...
从第\ref{b1:sec:2001}节到第\ref{b1:sec:2004}节，测试空间、其连续对偶以及对偶中仍由函数表示的部分依次出现。测试空间的局部凸拓扑只展开到本章需要的有限阶估计；读者第一次阅读时可以先掌握共同紧支集的收敛判据，再回看其拓扑构造。积分、\(L^p\) 平移连续性与第\ref{b1:ch:16}章的绝对连续理论是主要前置工具。

Hunter 与 Nachtergaele 的《Applied Analysis》\cite[第11章及第12.9节]{Hunter2001Applied}适合对照对偶配对、导数和典型例子，其中第11章主要面向温和分布，本章则在一般开集上使用紧支集测试函数，不施加无穷远增长条件。Schwartz 的《广义函数论》\cite[第一至三章]{Schwartz2010DistributionsChinese}把测试空间、局部化和导数系统地联系起来，可用作进一步阅读。Sobolev 空间的应用路线可接 Evans 的《Partial Differential Equations》\cite[第5章]{Evans2010Partial}；本章先完成这条路线所需的弱导数语言。
...
% 第二十章习题临时稿；不另设 BookExercises 入口。
% 八题均为依据本章已建立工具自拟的变式，不冒认教材原题编号。
...
\]
然而取 \(z_0\) 使 \(\psi(z_0)\ne0\)，则 \(x_n=1/n+z_0/n^2\) 为非零取值点，且 \(x_n\to0\)。\((0,1)\) 的任一紧子集都与端点保持正距离，所以不存在共同紧集包含全部支集。由\cref{b1:prop:test-sequence-convergence}，测试函数收敛不成立。振幅再小，也不能消除支集逃向边界的问题。
...
\begin{exercise}\label{b1:ex:20-locally-finite-unbounded-order}
在 \(\mathbb R\) 上定义线性泛函
\[
...
\begin{exercise}\label{b1:ex:20-polynomial-annihilator}
...
\begin{exercise}\label{b1:ex:20-principal-value-logarithm}
...
\begin{exercise}\label{b1:ex:20-distribution-first-order-ode}
...
\begin{exercise}\label{b1:ex:20-smoothing-variable-coefficient}
...
\]
其绝对值不超过 \(2\varepsilon^{a+1}\|\varphi'\|_\infty\)，趋于零。两侧积分由绝对可积性传到极限，证明 \(u_a'=g_a\) 为弱导数。\(a=0\) 时弱导数为零。

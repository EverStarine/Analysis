...
\cref{b1:fig:half-space-even-reflection}显示反射的几何动作。上下两点使用同一函数值，因此沿法向的绝对连续代表能在边界处接合。
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[>=stealth]
    \fill[gray!14] (-3,0) rectangle (3,1.75);
    \draw[thick] (-3,0)--(3,0) node[right] {$t=0$};
    \draw[->] (0,-1.65)--(0,1.9) node[above] {$t$};
    \fill (1.2,1.05) circle (1.5pt) node[right] {$(x',t)$};
    \fill (1.2,-1.05) circle (1.5pt) node[right] {$(x',-t)$};
    \draw[<->,thick] (1.2,-0.94)--(1.2,0.94);
    \node at (-1.8,1.15) {$H_+$};
    \node at (-1.55,-1.05) {偶反射补出的部分};
  \end{tikzpicture}
  \caption[半空间偶反射]{半空间偶反射。定义 \(E_+u(x',-t)=u(x',t)\)，函数值跨边界接合，而法向导数改变符号。}
  \label{b1:fig:half-space-even-reflection}
\end{figure}
...
局部构造分成五步：先在物理空间乘光滑截断，再把边界拉平；随后只越过图表盒的侧面和顶面作零延拓，此时并不跨越代表真实边界的超平面 \(t=0\)；第四步才关于 \(t=0\) 作偶反射；最后把函数推回物理空间，并在一个固定紧集以外补零到全空间。第三步排除的是图表人工边界上的跳跃，第四步处理的才是真实边界，两者不能合并成一次无条件补零。
...
\]
存在一个只依赖于 \(\eta,F\) 的固定紧集 \(K_{\eta,F}\Subset U\)，使每个 \(E_\eta u\) 在 \(K_{\eta,F}\) 外几乎处处为零。
\end{lemma}
\begin{proof}
光滑乘法的弱 Leibniz 公式给出
\[
 \partial_i(\eta\cdot u)=\eta\cdot\partial_i u+(\partial_i\eta)\cdot u.
\]
因此乘法在这里的 Sobolev 范数下有界，不要求 \(u\) 本性有界。由\cref{b1:thm:sobolev-bilipschitz-change}，函数
\[
 v=(\eta\cdot u)\circ F\in W^{1,p}(D^+)
\]
也有相应范数控制。它在 \(D^+\setminus\overline{D_1}\) 上为零，其弱导数在这一开集上同样为零。

将 \(v\) 在 \(H_+\setminus D^+\) 上补零，记为 \(\widehat v\)。这里的非零部分可能贴着 \(t=0\)，不能把它当作在某个紧含于 \(D^+\) 的集合外几乎处处为零的内部函数。不过，开集
\[
...
\]
集合 \(S\) 是 \(D_1\) 中的紧集，\(w\) 在 \(S\cup\mathcal R(S)\) 之外几乎处处为零。因此反射后的非零部分仍留在 \(D_1\) 的一个紧子集中。再在 \(U\) 上把 \(w\) 推回物理空间：
\[
 e=w\circ F^{-1}.
\]
双 Lipschitz 换元保证 \(e\in W^{1,p}(U)\)，且它在固定紧集
\(F\bigl(S\cup\mathcal R(S)\bigr)\Subset U\) 外几乎处处为零。此时可用\cref{b1:lem:compact-sobolev-zero-extension}把 \(e\) 补零到整个空间，所得函数就是 \(E_\eta u\)。

在 \(\Omega\cap U\) 上，拉回坐标的最后一分量为正，所以反射并未改变原函数，故 \(E_\eta u=\eta\cdot u\)；在 \(\Omega\setminus U\) 上两者都为零。各次乘法、复合、反射与补零都是固定的线性操作。依次使用它们的范数界，就得到引理中的估计。
\end{proof}

\subsection{有界 Lipschitz 区域上的延拓}
...
\end{equation}
存在一个只依赖于 \(\Omega\) 及所选图表的固定紧集 \(K\subset\mathbb R^d\)，使所有 \(Eu\) 均在 \(K\) 外几乎处处为零。
\end{theorem}
\begin{proof}
...
\[
 ...
 \end{split}
\]
常数包含有限个图表的 Lipschitz 界与截断函数的一阶导数界，这就证明\eqref{b1:eq:lipschitz-domain-extension-bound}。各项都在预先固定的紧集外几乎处处为零，有限个这样的紧集之并仍紧。整个公式不随 \(p\) 改变，因此在不同指数空间的交上也给出同一个延拓。
\end{proof}

...
\begin{solution}
取实值 \(\varphi_n\in C_c^\infty(\Omega)\)，使 \(\varphi_n\to u\) 于 \(W^{1,p}\)，并令 \(s_n=\varphi_n^+\)。由\cref{b1:ex:21-positive-part-continuity}，\(s_n\to u^+=u\) 于同一空间。再令
\[
...
由于 \(-s_n\le v_n\le s_n\)，函数 \(v_n\) 在 \(\operatorname{supp}\varphi_n\) 之外几乎处处为零。它虽不一定光滑，却有内部紧支集。由\cref{b1:prop:compact-sobolev-smooth-approximation}，每个 \(v_n\) 都能由内部紧支集光滑函数在 \(W^{1,p}\) 中逼近，故 \(v_n\in W_0^{1,p}(\Omega)\)。闭包空间本身是闭的，再由 \(v_n\to v\) 得到所求结论。

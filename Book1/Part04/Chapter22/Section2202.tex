...
\[
 ...
 \Subset\Omega,
\]
而 \(v_\varepsilon\) 在 \(\Omega\setminus K_\varepsilon\) 上几乎处处为零。因此\cref{b1:prop:compact-sobolev-smooth-approximation}给出 \(v_\varepsilon\in W_0^{1,p}(\Omega)\)。又因 \(|T_\varepsilon(u)|\le\varepsilon\) 且 \(\Omega\) 有限测度，\(T_\varepsilon(u)\to0\) 于 \(L^p\)。它的弱梯度为 \(\mathbf1_{\{|u|<\varepsilon\}}\nabla u\)；在 \(u=0\) 的水平集上梯度几乎处处为零，故控制收敛定理说明这部分也趋于零。于是 \(v_\varepsilon\to u\) 于 \(W^{1,p}\)，再用 \(W_0^{1,p}\) 的闭性即得结论。复值情形分别对连续代表的实部与虚部应用实值结论。
\end{proof}

...
\[
 W_0^{1,p}(\Omega)
 \coloneqq\overline{C_c^\infty(\Omega)}^{\,W^{1,p}(\Omega)}.
\]
也就是说，\(u\in W_0^{1,p}(\Omega)\) 当且仅当存在 \(\varphi_j\in C_c^\infty(\Omega)\)，使 \(\|u-\varphi_j\|_{W^{1,p}(\Omega)}\to0\)。另记 \(H_0^1(\Omega)\coloneqq W_0^{1,2}(\Omega)\)。
\end{definition}
...
当 \(p=\infty\) 时，本书也严格使用上述范数闭包。即使在有界区间这样的规则区域上，这个闭包也可能严格小于“具有连续零边界值的 Lipschitz 函数”所组成的比较类，不能把有限 \(p\) 的稠密性结论直接搬到这个端点。需要时，应明确其他文献所用 \(W_0^{1,\infty}\) 是否采用同一定义。
...
\]
补零后所得等价类由全实轴上的帐篷函数 \((1-|x|)_+\) 表示；仅在原点处填的值不同，不影响等价类，所以 \(E_0u\in W^{1,p}(\mathbb R)\) 对全部 \(p\) 成立。然而若 \(\varphi_j\in C_c^\infty(\Omega)\) 依 \(W^{1,p}(\Omega)\) 逼近 \(u\)，限制到 \((0,1)\)，由\secref{b1:sec:2104}的一维上确界估计，绝对连续代表必须在 \([0,1]\) 上一致收敛。近似函数在 \(0\) 处都为零，极限代表在 \(0\) 处却为一，矛盾。
...
\]
这不是端点函数值的问题，而是强无穷范数还要求导数在靠近端点时也能被一致控制。因此，即使在最规则的一维区域上，连续零边界值也不能刻画本书按范数闭包定义的 \(W_0^{1,\infty}\)。
...
\[
 ...
 &=\mathbf1_{\{|\varphi_j|<M\}}\cdot(\partial_i\varphi_j-\partial_i u)\\
 &\quad+
  \bigl(\mathbf1_{\{|\varphi_j|<M\}}-\mathbf1_{\{|u|<M\}}\bigr)\cdot\partial_i u.
 \end{aligned}
\]
第一项在 \(L^p\) 中趋零。沿任意一条 \(\varphi_j\to u\) 几乎处处的子列，第二项在 \(|u|\ne M\) 处几乎处处趋零；在 \(|u|=M\) 上，水平集梯度为零。控制收敛于是给出该项的 \(L^p\) 收敛。对任意子列再抽取这样的子列，可把结论传回全列，正如\cref{b1:prop:sobolev-lattice-strong-continuity}证明中的论证。

每个 \(T_M(\varphi_j)\) 都是内部紧支集的 \(W^{1,p}\) 函数。由\cref{b1:prop:compact-sobolev-smooth-approximation}，它属于 \(W_0^{1,p}\)。再用闭性，得到 \(T_M(u)\in W_0^{1,p}\)。当 \(M\to\infty\) 时的强收敛已由\eqref{b1:eq:sobolev-truncation-convergence}证明。正部、负部和绝对值用相同的紧支集逼近，并使用\cref{b1:prop:sobolev-lattice-strong-continuity}，即可得到。
...
对有界开集，还有一个不要求边界规则的充分条件。

\begin{proposition}[连续零边界的充分条件]
\label{b1:prop:continuous-zero-boundary-w0}
设 \(\Omega\subseteq\mathbb R^d\) 为有界开集、\(1\le p<\infty\)。若实值或复值函数 \(u\in W^{1,p}(\Omega)\) 有代表 \(U\in C(\overline\Omega)\)，且 \(U=0\) 于 \(\partial\Omega\)，则 \(u\in W_0^{1,p}(\Omega)\)。这里不要求 \(\partial\Omega\) 具有任何正则性。
\end{proposition}
\begin{proof}
先设 \(u\) 为实值，并仍以 \(u\) 表示给定的连续代表。对 \(\varepsilon>0\) 取
\[
...
\[
 K_\varepsilon
 =\{\,x\in\overline\Omega\mid |u(x)|\ge\varepsilon\,\}
 \Subset\Omega,
\]
而 \(v_\varepsilon\) 在 \(\Omega\setminus K_\varepsilon\) 上几乎处处为零。因此\cref{b1:prop:compact-sobolev-smooth-approximation}给出 \(v_\varepsilon\in W_0^{1,p}(\Omega)\)。又因 \(|T_\varepsilon(u)|\le\varepsilon\) 且 \(\Omega\) 有限测度，\(T_\varepsilon(u)\to0\) 于 \(L^p\)。它的弱梯度为 \(\mathbf1_{\{|u|<\varepsilon\}}\cdot\nabla u\)；在 \(u=0\) 的水平集上梯度几乎处处为零，故控制收敛定理说明这部分也趋于零。于是 \(v_\varepsilon\to u\) 于 \(W^{1,p}\)，再用 \(W_0^{1,p}\) 的闭性即得结论。复值情形分别对连续代表的实部与虚部应用实值结论。
\end{proof}

...
设 \(\Omega\subseteq\mathbb R^d\) 为非空开集，\(k\in\mathbb N_0\)。仍使用\secref{b1:sec:2003}的非负偶磨光函数 \(\rho\)，其支集包含于 \(\overline B(0,1)\)，积分等于 \(1\)，并记 \(\rho_\varepsilon(x)=\varepsilon^{-d}\cdot \rho(x/\varepsilon)\)。对 \(u\in W^{k,p}_{\mathrm{loc}}(\Omega)\)，卷积 \(u*\rho_\varepsilon\) 只先定义在
\[
 \Omega_\varepsilon
...
\end{theorem}
\begin{proof}
证明较一般的陈述。取\cref{b1:cor:precompact-smooth-partition}给出的光滑单位分解 \((\psi_i)_{i\geq1}\) 及局部有限开覆盖 \((V_i)_{i\geq1}\)。于是
\[
 \psi_i\geq0,
 \quad\sum_i\psi_i=1,
 \quad\operatorname{supp}\psi_i\Subset V_i\Subset\Omega.
\]
令 \(w_i=\psi_i\cdot u\)，并在 \(\Omega\) 外补零。前面的乘积讨论及补零引理保证 \(\widetilde w_i\in W^{k,p}(\mathbb R^d)\)。

对每个 \(i\)，使用\cref{b1:prop:compact-sobolev-smooth-approximation}，选取半径 \(\varepsilon_i>0\)，使
\[
 v_i=\rho_{\varepsilon_i}*\widetilde w_i\in C_c^\infty(V_i),
 \quad
 \|v_i-w_i\|_{W^{k,p}(\Omega)}<\eta2^{-i-1}.
\]
这里先要求 \(\varepsilon_i<\operatorname{dist}\bigl(\operatorname{supp}\psi_i,V_i^c\bigr)\)，再进一步缩小半径以达到误差条件。两个要求可以同时实现。

函数族 \((v_i)\) 仍局部有限，因为每个紧集只遇到有限多个 \(V_i\)。因此
\[
...
\]
由\cref{b1:thm:sobolev-complete}及\cref{b1:prop:banach-series-criterion}，级数 \(\sum_i e_i\) 在 \(W^{k,p}(\Omega)\) 中收敛到某个 \(e\)，且 \(\|e\|_{W^{k,p}}\leq\eta/2\)。在任意内部相对紧开集上，\(v_i\) 和 \(w_i\) 都只有有限个非零项；足够长的部分和因而几乎处处等于
\[
...
证明没有宣称 \(\sum_{i=1}^N\psi_i\cdot u\) 在全域 Sobolev 范数中趋于 \(u\)。单位分解的导数可能随位置增大；局部有限只保证局部运算合法，并不自动给出全域范数收敛。逐项磨光后的误差另有可和估计，完备性用在这组误差上。
...
\]
第一项的 \(L^p\) 范数不超过
\(\|\mathbf1_{\mathbb R^d\setminus B(0,R)}\cdot\partial^\alpha u\|_p\)，由有限 \(p\) 的可积尾部趋于零。余下每项的范数不超过
\[
 \binom\alpha\beta R^{-|\beta|}
 \|\partial^\beta\chi\|_\infty\cdot\|\partial^{\alpha-\beta}u\|_p,
\]
也趋于零。有限个多重指标合起来给出 \(\chi_R\cdot u\to u\) 于 \(W^{k,p}(\mathbb R^d)\)。

给定 \(\eta>0\)，先取 \(R\) 使截断误差小于 \(\eta/2\)。函数 \(\chi_R\cdot u\) 具有紧支集，再由\cref{b1:prop:compact-sobolev-smooth-approximation}取 \(v\in C_c^\infty(\mathbb R^d)\)，使 \(\|v-\chi_R\cdot u\|_{W^{k,p}}<\eta/2\)。三角不等式完成逼近。
\end{proof}

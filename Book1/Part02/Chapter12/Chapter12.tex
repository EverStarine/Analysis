...
本章需要\chapref{b1:ch:05}的积分极限定理、\chapref{b1:ch:09}的弱-*紧性与度量化思想，
以及\chapref{b1:ch:11}的连续截断和 Riesz--Markov 表示。
乘积测度习题还会调用\chapref{b1:ch:07}的 Fubini 定理。
...
\cite[Second Edition]{Billingsley1999Convergence}作为后续专书。
...
\begin{exercise}[\(\star\) 选读]\label{b1:ex:12-product-limits}
...
\(f\in C_b(S)\)、\(g\in C_b(T)\)，函数 \((x,y)\mapsto f(x)g(y)\) 有界连续，故
\[
 \begin{aligned}
 \int_{S\times T}f(x)g(y)\,\mathrm d\lambda
 &=\lim_{j\to\infty}\int_{S\times T}f(x)g(y)\,
...
\begin{exercise}[\(\star\) 选读]\label{b1:ex:12-holder-paths}
...
\begin{exercise}[\(\star\) 选读]\label{b1:ex:12-joint-marginals}

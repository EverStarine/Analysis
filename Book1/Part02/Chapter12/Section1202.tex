...
\begin{proof}
先设弱收敛成立。对非空闭集 \(F\)，令
\(h_k(x)=\max\{1-k\cdot d(x,F),0\}\)。这些函数属于 \(C_b(X)\)，并递减到 \(\mathbf1_F\)。
...
\[
 ...
 \longrightarrow\sum_{j=0}^{m-1}t_j\cdot\mu(A_j)=\int_X s\,d\mu.
\]
...
\[
 g_k(x)=\min\{1,k\cdot d(x,X\setminus G)\}.
\]
...
便得到分布函数在极限分布连续点处的收敛。
这一条件反过来也能推出弱收敛，具体证明留在\cref{b1:ex:12-distribution-functions}中。

...
拓扑方面只预设开闭集、连续性和紧性的基本概念。本章所需的邻域缩小、
Urysohn 截断与有限连续分解均在\secref{b1:sec:1101}证明；附录只作系统整理，不承担本章的前置证明。
...
\secref{b1:sec:1104}的两个定理，再返回构造证明。这一阅读次序只服务于预览，证明依赖仍按正文次序展开。
...
这一约定使后面测度的弱收敛有统一的对象，而开外正则性不能无条件加入结论。
...
若 \(x_n\rightarrow x\)，证明 \(\delta_{x_n}\) 弱-*\WeakStarJoin{}收敛到 \(\delta_x\)。据此给出弱-*\WeakStarJoin{}收敛但不按全变差范数收敛的例子。
\item\label{b1:item:11-dirac-escape}
证明 \(\mathbb R\) 上的 \(\delta_n\) 在 \(C_0(\mathbb R)^*\) 中弱-*\WeakStarJoin{}收敛到零，尽管每个测度的总质量均为 \(1\)。说明若把测试函数扩大到全部有界连续函数，这个结论为何不再成立。
\end{enumerate}
\end{exercise}
...
\]
这正是指定原空间为 \(C_0(X)\) 的弱-*\WeakStarJoin{}收敛。在 \(X=\mathbb R\) 中取 \(x_n=1/n\)、\(x=0\)，全变差距离却始终为 \(2\)。

最后，对 \(f\in C_0(\mathbb R)\)，无穷远处消失保证 \(f(n)\rightarrow0\)，故 \(\delta_n\) 弱-*\WeakStarJoin{}趋于零。常数函数 \(1\) 不属于 \(C_0(\mathbb R)\)，但属于有界连续函数空间，且 \(\int1\,\mathrm d\delta_n=1\)。因此总质量并不是此处弱-*\WeakStarJoin{}拓扑下的连续测试。
\end{solution}

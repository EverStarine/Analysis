...
本章用到\chapref{b1:ch:02}的 Carathéodory 外测度构造、\chapref{b1:ch:06}的全变差与极分解，
以及\chapref{b1:ch:08}、的连续泛函与稠密逼近。
拓扑方面只预设开闭集、连续性和紧性的基本概念；这里实际需要的邻域缩小、
Urysohn 截断与有限连续分解都在\secref{b1:sec:1101}证明，不借尚未写出的附录补齐关键步骤。
...
\secref{b1:sec:1104}的两个定理，再返回构造证明；不能把这一阅读次序当作证明中的依赖次序。
...
\cite[Volume 4, \S\S411, 416, and 436--437; Appendix, \S4A2]{Fremlin2016Measure}。
...
下一章将据此研究测度列的极限：用 \(C_c\) 或 \(C_0\) 测试可能看不见逃往无穷远处的质量；
例如质量恒为 \(1\) 的 \(\delta_n\) 在 \(C_0(\mathbb R)^*\) 中仍弱-*\WeakStarJoin{}收敛到零。
...
\begin{exercise}[基础模型：离散空间中的构造链]\label{b1:ex:11-discrete-construction}
...
\[
 a(U)=\sum_{n\in U}w_n
 :=\sup_{\substack{F\subseteq U\\F\text{ 有限}}}\sum_{n\in F}w_n.
\]
...
\begin{exercise}[\(\star\) 选读]\label{b1:ex:11-extreme-states}
...
\begin{exercise}[\(\star\) 选读]\label{b1:ex:11-norm-attainment}
\begin{enumerate}

...
\cref{b1:fig:riesz-markov-construction}把这条存在性证明链压缩成五个节点；每个箭头只表示下一步的构造来源，并不表示这些对象可以彼此无条件等同。
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[>=stealth, every node/.style={align=center}]
    \node[draw,rounded corners,minimum width=1.7cm,minimum height=0.9cm] (a) at (-5.2,0) {$a(U)$\\开集质量};
    \node[draw,rounded corners,minimum width=1.7cm,minimum height=0.9cm] (os) at (-2.6,0) {$\mu_0^*$\\外测度};
    \node[draw,rounded corners,minimum width=1.7cm,minimum height=0.9cm] (m0) at (0,0) {$\mu_0$\\Borel 测度};
    \node[draw,rounded corners,minimum width=1.7cm,minimum height=0.9cm] (m) at (2.7,0) {$\mu$\\Radon 测度};
    \node[draw,rounded corners,minimum width=2.2cm,minimum height=0.9cm] (rep) at (5.55,0) {$\Lambda(f)=\displaystyle\int f\,\mathrm d\mu$};
    \draw[->,thick] (a)--node[above]{开覆盖} (os);
    \draw[->,thick] (os)--node[above]{限制} (m0);
    \draw[->,thick] (m0)--node[above]{紧内正则化} (m);
    \draw[->,thick] (m)--node[above]{识别} (rep);
  \end{tikzpicture}
...
\end{lemma}
\begin{proof}
首先，\(\mu_0^*(\varnothing)=0\)，且 \(\mu_0^*\) 单调。为证可数次次可加性，只需考虑 \(\sum_n\mu_0^*(A_n)<\infty\) 的情形。给定 \(\varepsilon>0\)，逐个选择开集 \(U_n\supseteq A_n\)，使 \(a(U_n)<\mu_0^*(A_n)+\varepsilon2^{-n}\)。由前一引理，
\[
...
现在表示测度已经构造完毕；余下只需识别它确实表示 \(\Lambda\)。先设 \(f\in C_c^+(X)\)，令 \(K=\operatorname{supp}f\)、\(M=\|f\|_\infty\)。当 \(M=0\) 时无须证明，以下设 \(M>0\)。给定正整数 \(n\)，令 \(\delta=M/n\)，并对 \(1\leq j\leq n\) 置 \(U_j=\{f>j\delta\}\)。若分别取 \(h_j\in\mathcal T(U_j)\)，则逐点有 \(\delta\sum_{j=1}^n h_j\leq f\)。由正性，分别对有限个 \(h_j\) 取上确界，得到
\[

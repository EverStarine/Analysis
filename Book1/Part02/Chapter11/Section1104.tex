...
以下始终设 \(X\) 为局部紧 Hausdorff 空间，\(\mathbb K=\mathbb R\) 或 \(\mathbb C\)。Radon 测度沿用本章的 Borel 约定，不额外要求完备或全局外正则。D.~H. Fremlin 的《Measure Theory》\cite[第 416F、436J--K、437I 节]{Fremlin2016Measure}分别讨论了这一 Borel 约定与完备版本的联系、正泛函表示，以及实连续函数空间的对偶；下面的复值推广直接使用\chapref{b1:ch:06}的全变差与极分解。
...
\[
 ...
 |\operatorname{Im}\nu|\leq|\nu|,
\]
所以同一论证适用；极分解则直接调用\chapref{b1:ch:06}的结果。

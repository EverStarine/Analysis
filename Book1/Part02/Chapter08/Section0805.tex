...
本节构成一个完整的进阶模块。第一次阅读可以把它连同章末带星号的四道习题整体暂缓，不影响前四节形成的 Banach--Hilbert 基础主线；后续需要紧算子或谱工具时再回读即可。

\ProseParagraphBreak

关于紧算子的基本运算和紧自伴谱定理，可伴读 Axler 的%
《Measure, Integration \& Real Analysis》\cite[Sections 10C and 10D]{Axler2020Measure}。
...
设 \(X,Y\) 为赋范空间，\(T:X\to Y\) 为线性算子。若对 \(X\) 中每个有界列 \((x_n)\)，
...
设 \(T\) 是 Hilbert 空间 \(H\) 上的有界线性算子。若 \(T=T^*\)，则称 \(T\) 为%
...
\[
 \left\|\int_0^1L(\,\cdot\,,y)\cdot f(y)\,dy\right\|_2
...
\[
 ...
 Af(t)=\int_t^1 f(y)\,dy-\int_0^1y\cdot f(y)\,dy.
\]
...
\[
 u(x)=(1-x)\cdot\int_0^x y\cdot f(y)\,dy
       +x\int_x^1(1-y)\cdot f(y)\,dy.
\]
...
函数 \(e_n(x)=\sqrt2\cdot \sin(n\pi x)\)，\(n\geq1\)，组成 \(L^2(0,1)\) 的标准正交基，

...
设 \(X\) 是 \(\mathbb K\) 上的向量空间，给定映射 \(\|\cdot\|:X\rightarrow[0,\infty)\)。若对所有 \(x,y\in X\)、\(a\in\mathbb K\) 都有
\[
...
设 \(\|\cdot\|_a\)、\(\|\cdot\|_b\) 是同一向量空间 \(X\) 上的两个范数。若存在常数 \(c,C>0\)，使
\[
...
设 \((x_n)\) 是赋范空间 \(X\) 中的序列。若对每个 \(\varepsilon>0\)，都存在 \(N\)，使 \(n,m\geq N\) 时 \(\|x_n-x_m\|<\varepsilon\)，则称 \((x_n)\) 为\term[cauchy-lie]{Cauchy 列}[Cauchy sequence]。若每个这样的序列都收敛于 \(X\) 内的元素，则称 \(X\) 为\term[banach-kongjian]{Banach 空间}[Banach space]。
\end{definition}
...
若 \(X\) 是 Banach 空间，\(M\subset X\) 是闭线性子空间，则商范数使 \(X/M\) 成为 Banach 空间。商映射 \(Q\) 是线性连续满射，并满足 \(\|Qx\|_{X/M}\leq\|x\|\)。
\end{theorem}
\begin{proof}

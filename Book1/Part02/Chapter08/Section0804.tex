...
\]
并且 \(\langle x,x\rangle\geq0\)，等号当且仅当 \(x=0\)，则称该映射为\term[neiji]{内积}[inner product]。配备内积的空间称为\term[neiji-kongjian]{内积空间}[inner product space]。记 \(\|x\|=\sqrt{\langle x,x\rangle}\)。
\end{definition}
...
对任意 \(x,y\in H\)，有 \(|\langle x,y\rangle|\leq\|x\|\cdot\|y\|\)，等号当且仅当 \(x,y\) 线性相关。内积给出的 \(\|\cdot\|\) 是范数，并满足\term[pingxing-sibianxing-hengdengshi]{平行四边形恒等式}[parallelogram identity]：
...
有限维空间中，正交坐标同时计算向量和长度。无限维情形还要决定无穷求和的意义，并检查坐标是否遗漏了整个子空间。Axler 的《Measure, Integration \& Real Analysis》\cite[Section 8C]{Axler2020Measure}把正交展开与最佳逼近并行讨论，适合与本小节对读；以下不预设空间可分。

\begin{definition}\label{b1:def:orthonormal-system}
一族两两正交的非零向量称为\term[zhengjiao-xi]{正交系}[orthogonal system]；若每个向量的范数均为 \(1\)，则称为\term[biaozhun-zhengjiao-xi]{标准正交系}[orthonormal system]。若标准正交系 \((e_\alpha)_{\alpha\in I}\) 满足
\[
...
\]
有限部分和的内积具有这个形式，随后由内积连续性取极限即可。标量级数绝对收敛，因为对任意有限指标集，坐标绝对值乘积的和由有限维 Cauchy--Schwarz 不等式控制在 \(\|x\|\cdot\|y\|\) 以内。若只有标准正交系，坐标则可能遗漏分量：\(\ell^2\) 中所有偶数坐标方向构成标准正交系，但第一单位向量的全部相应系数都为零。

...
因此，若 \(C\subset L^1\) 非空、凸、范数闭且一致可积，则 \(C\) 弱紧：闭凸性给出弱闭性，Dunford--Pettis 定理给出相对弱紧性。这使第九章的直接法仍有用武之地。对于 \(C\) 上有下界且至少在一点有限的弱下半连续泛函，选取极小化序列，抽取弱收敛子列，再比较极限处的泛函值，就能得到极小点。这里不需要假设整个 \(L^1\) 空间自反；提供弱紧性的是具体可行族的一致可积性。

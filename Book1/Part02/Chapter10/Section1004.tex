...
设 \(\mu\) 为 \(\sigma\)-有限正测度，\(1<p<\infty\)。则实或复空间 \(L^p(\mu)\) 自反。
\end{theorem}
\begin{proof}
...
同样，\(\ell^\infty\) 含有闭线性子空间 \(c_0\)，而第\ref{b1:sec:0904}节已说明 \(c_0\) 不自反；由\cref{b1:cor:closed-subspace-reflexive}，\(\ell^\infty\) 也不自反。这不意味着每个 \(L^1\) 或 \(L^\infty\) 空间都不自反：有限集合上各点质量为正且有限时，它们都有限维。端点所缺少的是对一般测度空间统一成立的自反性结论。

...
\end{theorem}
\begin{proof}
由\cref{b1:thm:lp-dual}，映射 \(D_p:g\mapsto\Lambda_g\) 是从 \(L^q(\mu)\) 到 \((L^p(\mu))^*\) 的线性等距满射。任取
\(\Phi\in(L^p(\mu))^{**}\)，令
\[
 \Psi(g)=\Phi(D_pg)\quad(g\in L^q(\mu)).
\]
因为 \(|\Psi(g)|\le\|\Phi\|\|g\|_q\)，所以 \(\Psi\in(L^q(\mu))^*\)。指数 \(q\) 也在 \((1,\infty)\) 中，其共轭指数为 \(p\)。再次应用对偶表示，取得 \(f\in L^p(\mu)\)，使
\[
 \Phi(D_pg)=\Psi(g)=\int_Xgf\,\mathrm d\mu
 =(D_pg)(f)\quad(g\in L^q(\mu)).
\]
...
\[
 \mathcal F(f)=\int_X\phi(f(x))\,\mathrm d\mu(x)
 \quad(f\in L^r(\mu;\mathbb K))
\]
是强下半连续、弱下半连续的凸泛函。
\end{proposition}
\begin{proof}

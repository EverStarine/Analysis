...
若序列 \((x_n)\subset X\) 满足：对每个 \(f\in X^*\)，标量列 \(\bigl(f(x_n)\bigr)\) 有界，则 \(\sup_n\|x_n\|<\infty\)。特别地，每个弱收敛列都在范数下有界。这里不要求 \(X\) 完备。
\end{proposition}
\begin{proof}
...
若对每个 \(f\in X^*\)，标量列 \(\bigl(f(x_n)\bigr)\) 都是 Cauchy 列，则称 \((x_n)\) 为\term[ruo-cauchy-xulie]{弱 Cauchy 序列}[weakly Cauchy sequence]。
\end{definition}
...
趋于零的标量序列空间 \(c_0\) 关于一致范数是 Banach 空间。每个 \(f\in c_0^*\) 都唯一表示为
\[

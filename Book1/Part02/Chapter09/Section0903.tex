% !TeX root = ../../Book1.tex
\section{弱*\WeakStarJoin{}紧性、Banach--Alaoglu 与 Goldstine 定理}
\label{b1:sec:0903}

在范数拓扑中，无穷维空间的闭单位球不紧；换用弱*\WeakStarJoin{}拓扑以后，对偶单位球却总是紧的。
...
\]
其中 \(\pi_i\) 是第 \(i\) 个坐标投影。因此 \(\Phi\) 把弱*\WeakStarJoin{}基本邻域变成像空间中的乘积基本邻域。
这正是弱*\WeakStarJoin{}拓扑与逐点测试之间的关系，而不是一种另加的紧性假设。
...
设 \(X\) 是赋范空间。对偶闭单位球 \(B_{X^*}\) 在弱*\WeakStarJoin{}拓扑 \(\sigma(X^*,X)\) 下是紧的。
\end{theorem}
\begin{proof}
...
\(\Phi\) 是弱*\WeakStarJoin{}拓扑到像的同胚，所求紧性成立。
\end{proof}

\term[banach-alaoglu-dingli]{Banach--Alaoglu 定理}[Banach--Alaoglu theorem]保证的是弱*\WeakStarJoin{}紧性；这里的“弱*”不能改成“弱”。
它说的是 \(X^*\) 上由 \(X\) 测试的紧性，而不是由全部 \(X^{**}\) 测试的弱紧性。
闭球与有界性的条件也各有作用：对任意 \(R\geq0\)，半径为 \(R\) 的对偶闭球弱*\WeakStarJoin{}紧，
从而每个范数有界、弱*\WeakStarJoin{}闭的子集弱*\WeakStarJoin{}紧；一般范数闭集却不必弱*\WeakStarJoin{}闭。
...
\end{theorem}
\begin{proof}
右边的球由闭条件 \(|x^{**}(f)|\leq\|f\|\) 描述，故弱*\WeakStarJoin{}闭，并包含左边的闭包。
...
这正是 \(x^{**}\) 属于所求弱*\WeakStarJoin{}闭包的邻域判据。
\end{proof}
...
设 \(X\) 是可分赋范空间，\(R>0\)。在 \(RB_{X^*}\) 上，弱*\WeakStarJoin{}拓扑可由一个度量给出。
特别地，每个范数有界的 \(X^*\) 中的序列都有弱*\WeakStarJoin{}收敛子列，其极限仍在 \(X^*\) 中。
\end{theorem}
\begin{proof}
...
先说明它确实给出弱*\WeakStarJoin{}拓扑。给定一个 \(\mathrm{d}\)-球，先使级数尾和小于半个半径，
再限制前面有限多个测试值，就得到包含在球内的弱*\WeakStarJoin{}邻域。
...
于是 \(f\in RB_{X^*}\)，抽出的子列弱*\WeakStarJoin{}收敛到 \(f\)。
\end{proof}
...
所以这一对偶单位球虽然弱*\WeakStarJoin{}紧，却含有无弱*\WeakStarJoin{}收敛子列的序列。

...
记号中保留 \(X\) 十分重要。这里不是任意给 \(X^*\) 降低一个拓扑，而是明确规定只用来自 \(X\) 的求值泛函作为测试。Fremlin 的《Measure Theory》\cite[Appendix to Volume 2, 2A5I]{Fremlin2016Measure}把弱拓扑与弱-*拓扑放在同一框架下讨论，适合用来对照这两种测试族；下面则用序列空间把区别具体化。
...
\end{proposition}
\begin{proof}
这里要说明 \(J_X\) 是单射。而 \(X^{**}\) 中以 \(J_Xx\) 为中心、由有限个 \(f_1,\ldots,f_m\in X^*\) 给出的弱-*基本邻域，在 \(J_X\) 下的反像由
\[

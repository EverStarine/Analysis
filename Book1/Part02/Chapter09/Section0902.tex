...
\]
连续的最弱拓扑。若 \(f\ne g\)，总有一个 \(x\in X\) 使 \(f(x)\ne g(x)\)，故两个小求值邻域可将它们分开，弱-*拓扑也是 Hausdorff 的。估计 \(|(g-f)(x)|\leq\|g-f\|\,\|x\|\) 又说明范数拓扑不弱于弱-*拓扑。这些事实都不要求 \(X\) 完备。
...
\end{proposition}
\begin{proof}
对每个 \(x\in X\)，收敛的标量列 \((f_n(x))\) 有界。把 \(f_n\) 看成从 Banach 空间 \(X\) 到 \(\mathbb K\) 的有界线性算子，\cref{b1:thm:uniform-boundedness}直接给出结论。
\end{proof}
...
\end{proposition}
\begin{proof}
绝对收敛性和估计 \(|F_u(a)|\leq\|u\|_\infty\|a\|_1\) 给出 \(\|F_u\|\leq\|u\|_\infty\)。逐个代入单位坐标向量 \(e_k\)，得到 \(\|F_u\|\geq|u_k|\)，取上确界即有范数相等。
...
自然嵌入 \(J_X\) 把 \((X,\sigma(X,X^*))\) 同胚地映到 \(X^{**}\) 中的像 \(J_X(X)\)，其中像空间采用 \(X^{**}\) 上弱-*拓扑 \(\sigma(X^{**},X^*)\) 的子空间拓扑。
\end{proposition}
\begin{proof}

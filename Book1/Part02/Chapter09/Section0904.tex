...
弱-*\WeakStarJoin{}紧性保证一族有界泛函不会失去全部极限，但把向量看成二次对偶中的泛函之后，极限可能落在原空间之外。自反性排除的正是这种现象。它把上一节的对偶单位球紧性带回原空间，从而为有界近似列提供弱极限。定义及弱紧性判别可参看 Fremlin 的《Measure Theory》\cite[Volume 3, 3A5G]{Fremlin2016Measure}；本节还要解决从紧性到收敛子列之间不能直接跳过的一步。
...
若赋范空间 \(X\) 的自然嵌入 \(J_X:X\rightarrow X^{**}\) 是满射，则称 \(X\) 为\term[zifan-kongjian]{自反空间}[reflexive space]，或称 \(X\) 具有自反性。\termidx[zifanxing]{自反性}%
\footnote{“自反性”的名称与许全华等的《泛函分析讲义》\cite[第9.3节]{Xu2017Fanhan}一致。本书据此统一中文主称，不因译名调整改变自然嵌入的定义。}
...
赋范空间 \(X\) 自反，当且仅当 \(B_X\) 弱紧。
\end{theorem}
\begin{proof}
由\cref{b1:prop:canonical-weak-embedding}，\(J_X\) 把 \(X\) 的弱拓扑与其像上的弱-*\WeakStarJoin{}子空间拓扑对应起来。若 \(X\) 自反，等距性给出 \(J_X(B_X)=B_{X^{**}}\)。把\cref{b1:thm:banach-alaoglu}用于赋范空间 \(X^*\)，可知 \(B_{X^{**}}\) 在 \(\sigma(X^{**},X^*)\) 下紧，从而 \(B_X\) 弱紧。

反之，若 \(B_X\) 弱紧，则 \(J_X(B_X)\) 是弱-*\WeakStarJoin{}紧集。弱-*\WeakStarJoin{}拓扑为 Hausdorff 拓扑，故这个紧集闭；而\cref{b1:thm:goldstine}又说明它在 \(B_{X^{**}}\) 中弱-*\WeakStarJoin{}稠密。因此
\[
 J_X(B_X)=B_{X^{**}}.
\]
...
这里两个对偶拓扑必须分清：对 \(X^{**}\) 使用的是由 \(X^*\) 测试的弱-*\WeakStarJoin{}拓扑，正好对应 \(X\) 中由 \(X^*\) 测试的弱拓扑。若把二次对偶自身的弱拓扑放进证明，Banach--Alaoglu 定理就不能给出所需的结论。
...
若 \(X\) 可分且自反，则 \(X^*\) 关于对偶范数可分。
\end{lemma}
\begin{proof}
由\cref{b1:thm:weak-star-metrizable}，\(B_{X^*}\) 在弱-*\WeakStarJoin{}拓扑下是紧致可度量空间。对一个相容度量，在每个尺度 \(1/n\) 选有限网，把这些有限网合并，可得弱-*\WeakStarJoin{}稠密的可数集 \(D\subset B_{X^*}\)。
...
\]
自反性使 \(\Phi=J_Xx\) 对某个 \(x\in X\) 成立，因而 \(\Phi(f)=f(x)\) 弱-*\WeakStarJoin{}连续。由于 \(D\) 弱-*\WeakStarJoin{}稠密，\(f_0\) 不可能被这个连续泛函与 \(D\subset C\) 严格分开。矛盾说明 \(C=B_{X^*}\)。

由 \(D\) 中有限多个点作有理系数凸组合，所得集合可数。每个有限凸组合的非负系数都可在保持和为 \(1\) 的条件下由有理系数逼近，所以这个可数集合在 \(B_{X^*}\) 中范数稠密。最后取它的正整数倍的并，便得到 \(X^*\) 的可数范数稠密子集。
\end{proof}

弱-*\WeakStarJoin{}稠密本来只控制有限多个向量上的值，不控制对偶范数。上面的分离步骤把两者接起来；自反性用在分离泛函能否由原空间向量表示这一点，而不只是用来增加一个完备性假设。

\begin{theorem}\label{b1:thm:reflexive-weak-subsequence}
自反空间 \(X\) 中的任意有界序列，都有在 \(X\) 中弱收敛的子列。
\end{theorem}
\begin{proof}
...
前面的自反空间子列定理使用了可分闭子空间，而没有把一般拓扑紧性当成序列紧性。\term[eberlein-smulian]{Eberlein--Šmulian 定理}[Eberlein--Šmulian theorem]进一步说明，对 Banach 空间的弱拓扑，这两种紧性确实一致，甚至无需假定整个空间自反。
...
逆向，即从所有序列都有 \(K\) 内的弱极限推出 \(K\) 弱紧，在这里省略。一个完整途径是把 \(X\) 的向量作为对偶弱-*\WeakStarJoin{}单位球上的连续函数，再证明相应逐点拓扑中可数聚点性质能够产生真正的紧性。所需的逐点紧性论证及其回到弱拓扑的步骤，见 Fremlin 的《Measure Theory》\cite[Volume 4, 462B(c) and 462C--D]{Fremlin2016Measure}。该处以实空间叙述；复空间看成实空间后，每个连续实线性泛函 \(g\) 都是复线性泛函 \(f(x)=g(x)-\mathrm i \cdot g(\mathrm i x)\) 的实部，故两种弱拓扑相同，结论也适用。
\end{proofsketch}
...
三种收敛都在比较同一列对象，但测试方式和常用紧性工具不同。\cref{b1:tab:norm-weak-weakstar}把这些入口并列起来；它只列本章最常用的判据，不把拓扑紧性误写成一般的序列紧性。
\begin{table}[htbp]
  \centering
  \caption[三种线性拓扑收敛]{三种线性拓扑收敛。表中的紧性入口均须连同所在空间及本章相应定理的假设使用。}
  \label{b1:tab:norm-weak-weakstar}
  \begin{tabularx}{\textwidth}{@{}p{2.2cm}p{5.2cm}X@{}}
    \toprule
    收敛方式 & 判据 & 常用紧性入口 \\
    \midrule
    范数收敛 & \(\|x_n-x\|\to0\) & 完备性负责极限存在；紧性通常还需额外结构 \\
    弱收敛 & \(f(x_n)\to f(x)\) 对每个 \(f\in X^*\) & 自反空间中的有界列可抽取弱收敛子列 \\
    弱-*\WeakStarJoin{}收敛 & \(f_n(x)\to f(x)\) 对每个 \(x\in X\) & Banach--Alaoglu 给出对偶闭球的弱-*\WeakStarJoin{}紧性 \\
    \bottomrule
  \end{tabularx}
\end{table}

...
若赋范空间 \(X\) 的自然嵌入 \(J_X:X\rightarrow X^{**}\) 是满射，称 \(X\) 为\term[fanshen-kongjian]{反身空间}[reflexive space]，或称 \(X\) 具有反身性。\termidx[fanshenxing]{反身性}%
...
赋范空间 \(X\) 的子集 \(K\) 若在弱拓扑下紧，称为\term[ruo-jin-ji]{弱紧集}[weakly compact set]。若 \(A\subset X\) 的弱闭包是弱紧集，称 \(A\) 为\term[xiangdui-ruo-jin-ji]{相对弱紧集}[relatively weakly compact set]。
\end{definition}
...
\[
 F(y)=\overline{\Phi(Ry)}.
\]
在实情形，共轭不改变数值；在复情形，两个共轭互相抵消，使 \(F\) 线性。又有 \(|F(y)|\le\|\Phi\|\|y\|\)，故再用 Riesz 表示取得 \(h\in H\)，使 \(F(y)=\langle y,h\rangle\)。于是
\[
...
\[
 ...
 \quad x\longmapsto(f_j(x))_{j\ge1}
\]

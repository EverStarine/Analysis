...
若右端 \(y\) 不满足正交条件，也可以考虑最小二乘问题。对任意 \(x\in H\)，
\(x\) 是最小二乘解，当且仅当 \(Tx-y\) 与 \(\operatorname{ran}T\) 正交，
也就是满足正规方程
\[
 T^*Tx=T^*y.
\]
这个刻画本身不要求值域闭。若 \(\operatorname{ran}T\) 闭，则正交投影
\(P_{\operatorname{ran}T}y\) 存在且仍属于值域，因而可以选取 \(x\) 使
\(Tx=P_{\operatorname{ran}T}y\)；所以正规方程对每个 \(y\) 确有解。
若值域不闭，最小残差的下确界可能不取得。

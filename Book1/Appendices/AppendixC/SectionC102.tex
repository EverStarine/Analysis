...
第八章已给出紧算子的定义及其在复合、相加和算子范数极限下的稳定性。
...
\end{proposition}
\begin{proof}
在紧集 \(\overline{T(B_X(0,1))}\) 中取有限个半径小于 \(\varepsilon\) 的球覆盖，
球心为 \(y_1,\ldots,y_m\)，令 \(E\) 为这些向量张成的空间。
对 \(\|x\|\leq1\)，可取一个球心使 \(\|Tx-y_j\|<\varepsilon\)。
正交投影给出最近点，故
\[
 \|Tx-P_ETx\|=\operatorname{dist}(Tx,E)
 \leq\|Tx-y_j\|<\varepsilon.
\]
若最初网半径固定为 \(\varepsilon/2\)，取上确界后仍得严格小于 \(\varepsilon\)。
\end{proof}

...
\cref{b1:ex:08-quotient-dual}中的商范数与商映射：
\[
 \|x+M\|_{X/M}=\operatorname{dist}(x,M),\quad Qx=x+M.
\]
该习题已经证明 \(X/M\) 完备，并把 \((X/M)^*\) 等距识别为
\[
 M^\circ=\{\,f\in X^*:f(m)=0\ \text{对每个 }m\in M\,\}.
\]
本节直接使用这两个结果，不再重新证明商空间的完备性。商空间的定义要求 \(M\) 闭；
若不闭，上式只是半范数，它把 \(\overline M\) 中的所有点也压成距离零。
...
\]
最后，若 \(g_\alpha\) 弱-*收敛于 \(g\)，则对每个固定的 \(x\)，
\((T'g_\alpha)(x)=g_\alpha(Tx)\rightarrow g(Tx)\)。这正是像网弱-*收敛的定义。
\end{proof}
...
\cref{b1:ex:09-double-annihilator}中的分离结论，可取 \(g\) 消去这个子空间而

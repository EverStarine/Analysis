...
以及\chapref{b1:ch:09}的弱拓扑与自然嵌入。商空间完备性及其对偶已经分别由%
\cref{b1:thm:quotient-banach}与\cref{b1:prop:quotient-dual}作为正文结果建立，双零化子关系见%
\cref{b1:prop:double-annihilator}。
本附录沿这些正式结果继续，不把章末习题作为不可跳过的证明节点。
...
\input{Appendices/AppendixC/SectionC101}
\input{Appendices/AppendixC/SectionC102}
\input{Appendices/AppendixC/SectionC103}
\input{Appendices/AppendixC/SectionC104}
\input{Appendices/AppendixC/SectionC105}
\input{Appendices/AppendixC/SectionC106}
...
% 八题均为自拟；逐题注释列出训练增量。
% 不复述\chapref{b1:ch:08}商空间及\chapref{b1:ch:09}双零化子的正文结果；c0对偶与范数不取得的习题只作应用。
...
\begin{solution}
恒有 \(K^2=f(u)\cdot K\)。当 \(1-\alpha\cdot f(u)\ne0\) 时，直接相乘得到
\[
 (I-\alpha K)^{-1}
 =I+\frac{\alpha}{1-\alpha\cdot f(u)}\cdot K,
\]
所以唯一解为
\[
 x=y+\frac{\alpha\cdot f(y)}{1-\alpha\cdot f(u)}\cdot u.
\]
这包括 \(\alpha=0\) 或 \(f(u)=0\) 的情况。

当 \(\alpha\cdot f(u)=1\) 时，对方程作用 \(f\) 得 \(f(y)=0\)。
若该条件满足，则 \(x=y\) 是一解；齐次方程要求 \(x=\alpha\cdot f(x)\cdot u\)，

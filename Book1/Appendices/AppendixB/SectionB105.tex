...
在\chapref{b1:ch:19}的可整流情形，切空间一般只在几乎处处存在；\ref{b1:app:D}将用可测的单位可分解外向量记录方向，而不要求它连续。连续定向与可测定向具有不同的用途，不能把流形的全局可定向性当作所有可整流积分对象的先决条件。
...
这说明无向面积公式中的绝对值和交错型拉回中的带符号行列式并不矛盾。前者改变积分所用的非负测度，后者改变被积对象对有序向量的取值。将定向整体反转，面积测度不动而 \(\tau\) 取负，所以型的积分整体取负。\ref{b1:app:D}把这种积分看作作用于测试形式的线性泛函，边界也将在这一对偶语言中统一处理。

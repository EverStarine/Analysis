...
在\chapref{b1:ch:19}的可整流情形，切空间一般只在几乎处处存在；附录\ref{b1:app:D}将用可测的单位可分解外向量记录方向，而不要求它连续。连续定向与可测定向具有不同的用途，不能把流形的全局可定向性当作所有可整流积分对象的先决条件。
...
若 \(f:U\rightarrow M\) 是覆盖所考察曲面片的正向单射参数图，则\chapref{b1:ch:17}的面积公式给出
\[
 \begin{aligned}
 \int_{(M,\tau)}\omega
 &=\int_U

...
第\ref{b1:ch:17}章已经用子式写出外积，并由 Gram 行列式计算面积伸缩。
...
前三节的代数构造可以独立阅读；体积的测度解释调用第\ref{b1:ch:17}章，
余面积的几何解释调用第\ref{b1:ch:18}章。
若只为查阅 Jacobi 因子的约定，可先看第\ref{b1:sec:B103}节与第\ref{b1:sec:B104}节；
准备阅读流的定义时，还应掌握交错型的配对和第\ref{b1:sec:B105}节的定向。
这里始终限于有限维实空间，不讨论无限维张量积的范数与完备化。

黎景辉、白正简、周国晖的《高等线性代数学》\cite{LiBaiZhou2014AdvancedLinear}%
可用于衔接张量积、张量代数与交错型的中文表述。
Simon 的《Lectures on Geometric Measure Theory》\cite[第25节]{Simon1983Lectures}%
...
\input{Appendices/AppendixB/SectionB101}
\input{Appendices/AppendixB/SectionB102}
\input{Appendices/AppendixB/SectionB103}
\input{Appendices/AppendixB/SectionB104}
\input{Appendices/AppendixB/SectionB105}
...
\[
 ...
 T_{v\otimes w}(\lambda)=\lambda(v)w,
\]
...
\begin{solution}
对固定 \(\lambda\)，求值 \((v,w)\mapsto\lambda(v)w\) 双线性，因而可在张量积上线性化。
对每个 \(\xi\)，所得值又对 \(\lambda\) 线性，所以确实得到 \(T_\xi\)。
若 \(\xi=\sum_{\ell=1}^r v_\ell\otimes w_\ell\)，则
\[
 T_\xi(\lambda)=\sum_{\ell=1}^r\lambda(v_\ell)w_\ell.
\]
...
\[
 T_\xi(\lambda)=\sum_{j=1}^q\ell_j(\lambda)w_j.
\]
有限维双对偶的自然识别给出 \(v_j\in V\)，满足
\(\ell_j(\lambda)=\lambda(v_j)\)：若 \(e_i,e^i\) 互为对偶基，
可直接取 \(v_j=\sum_i\ell_j(e^i)e_i\) 并验证。
...
\]
\[
 F_\theta=(R+t\cos(\theta/2))e_\theta
      -\frac t2\sin(\theta/2)e_r
      +\frac t2\cos(\theta/2)e_z.
\]
...
\]
它连续且处处为单位切向外向量，所以
\(\tau\bigl(F(\theta,0)\bigr)=\sigma(\theta)\eta(\theta)\)，

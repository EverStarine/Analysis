...
本附录中的向量空间均为有限维实向量空间；代数构造本身不要求内积。对偶 \(V^*\) 表示全部实线性泛函，有限维时它与连续对偶一致。直到第\ref{b1:sec:B103}节才给空间增加内积，不能在此前用内积把 \(V\) 和 \(V^*\) 默认为同一个对象。
...
\[
 \widehat B\left(\sum_{\ell=1}^N a_\ell[v_\ell,w_\ell]\right)
   =\sum_{\ell=1}^N a_\ell B(v_\ell,w_\ell).
\]
...
\[
 ...
 \quad \lambda\otimes w\longmapsto
       \bigl[v\longmapsto\lambda(v)w\bigr].
\]
还存在自然同构
\[
 V^*\otimes W^*\longrightarrow (V\otimes W)^*,
 \quad
 (\lambda\otimes\mu)(v\otimes w)=\lambda(v)\mu(w).
\]
\end{proposition}
\begin{proof}
...
\[
 \sum_i e^i(v)Ae_i=A\left(\sum_i e^i(v)e_i\right)=Av.
\]

...
同样，在 \(\mathbb R^4\) 中，\(\xi=e_1\wedge e_2+e_3\wedge e_4\) 的欧氏长度为 \(\sqrt2\)，却不能据此说“两张互相正交的单位面片总面积为 \(\sqrt2\)”。这个代数和只保留一个外向量；几何上的两块面片若作为不同的积分对象相加，还必须说明支集、重数以及怎样测试它们。\ref{b1:app:D}将据此引入流的质量和余质量；在可分解外向量上有关长度相容，但一般外向量上的质量范数与本节欧氏范数不得混用。

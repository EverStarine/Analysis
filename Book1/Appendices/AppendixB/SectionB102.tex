...
\[
 \omega(v_{\sigma(1)},\ldots,v_{\sigma(k)})
   =\operatorname{sgn}(\sigma)\cdot\omega(v_1,\ldots,v_k).
\]
...
\[
 \epsilon^I(v_1,\ldots,v_k)
   \coloneqq\det\bigl(e^{i_a}(v_b)\bigr)_{a,b=1}^k.
\]
...
上述商代数
\[
 \bigwedge V\coloneqq T(V)/I
   =\bigoplus_{k=0}^{n}\bigwedge\nolimits^kV,
 \quad
 \bigwedge\nolimits^kV\coloneqq
   V^{\otimes k}/\bigl(I\cap V^{\otimes k}\bigr)
\]
称为 \(V\) 的\term[waidaishu]{外代数}[exterior algebra]；其中 \(\bigwedge^kV\) 是它的 \(k\) 次分量。商中的乘法记为 \(\wedge\)，称为外积，纯张量的像记为 \(v_1\wedge\cdots\wedge v_k\)。约定 \(\bigwedge^0V\coloneqq\mathbb R\)。
\end{definition}
...
\[
 (\lambda\wedge\mu)(v,w)
   =\lambda(v)\cdot\mu(w)-\lambda(w)\cdot\mu(v),
\]
没有额外的 \(\frac12\)。若把一般 \(r\) 次多线性型 \(T\) 的交错化定义为
\[
 \operatorname{Alt}T(v_1,\ldots,v_r)
  \coloneqq\frac1{r!}\sum_{\sigma\in S_r}
       \operatorname{sgn}(\sigma)
...
类似地，令置换 \(\sigma\in S_k\) 在张量空间上的作用为
\[
...
并定义反对称化算子
\[
 \operatorname{Alt}_{\otimes}
 \coloneqq
 \frac1{k!}\sum_{\sigma\in S_k}
   \operatorname{sgn}(\sigma)P_\sigma.
\]
它满足 \(\operatorname{Alt}_{\otimes}^2=\operatorname{Alt}_{\otimes}\)。事实上，
\(P_\tau\operatorname{Alt}_{\otimes}
=\operatorname{sgn}(\tau)\operatorname{Alt}_{\otimes}\)；反过来，若张量 \(T\) 满足
\(P_\tau T=\operatorname{sgn}(\tau)T\) 对每个 \(\tau\in S_k\) 成立，则
\(\operatorname{Alt}_{\otimes}T=T\)。所以
\[
 \operatorname{im}(\operatorname{Alt}_{\otimes})
 =\{\,T\in V^{\otimes k}:P_\sigma T
      =\operatorname{sgn}(\sigma)T\ \text{对所有 }\sigma\in S_k\,\},
\]
即它的像恰为交错张量子空间。

\ProseParagraphBreak

若两个张量因子相同，交换这两个位置的转置保持该纯张量不变，却使反对称化求和中的符号反转，故各项两两抵消。于是反对称化消去外代数商关系中的全部生成元，下面的公式便给出良定义的线性映射
\(\iota:\bigwedge^kV\rightarrow V^{\otimes k}\)：
\[
 \iota(v_1\wedge\cdots\wedge v_k)
 \coloneqq
 \frac1{k!}\sum_{\sigma\in S_k}
   \operatorname{sgn}(\sigma)
   v_{\sigma(1)}\otimes\cdots\otimes v_{\sigma(k)}.
\]
设 \(\pi:V^{\otimes k}\rightarrow\bigwedge^kV\) 为商映射，则
\(\pi\circ\iota\) 是恒等映射，故 \(\iota\) 单射；同时在纯张量上
\(\iota\circ\pi=\operatorname{Alt}_{\otimes}\)，再由纯张量的张成性可知两边在整个
\(V^{\otimes k}\) 上相等。因此
\[
 \operatorname{im}\iota
 =\operatorname{im}(\operatorname{Alt}_{\otimes}),
\]
这就完整证明了 \(\bigwedge^kV\) 与张量空间中交错部分的自然识别。这个识别中的 \(1/k!\) 与型的行列式求值公式并不冲突，但提醒我们：不能既使用张量子空间的平均定义，又未经换算套用另一种外积归一化。

...
对每个固定 \(R>0\)，当 \(j\) 足够大时，
\[
 \nu_j(B(0,R))=r_j^{-k}\cdot\mu(B(x,Rr_j))\leq bR^k.
\]
...
充分大的 \(j\) 满足 \(\int\varphi\,\mathrm{d}\nu_j\geq a\)，
故 \(\int\varphi\,\mathrm{d}\nu\geq a>0\)。
...
\[
 \int_{E\cap B(x,r)}|\theta(y)-\theta(x)|\,\mathrm{d}\mathcal H^k(y)
     =o(r^k).
\]
...
\[
 ...
 &\left|r^{-k}\int_E
   ...
   \bigl(\theta(y)-\theta(x)\bigr)\,\mathrm{d}\mathcal H^k(y)\right|\\
 &\hspace{1em}\leq
  \|\varphi\|_\infty\,r^{-k}
  \int_{E\cap B(x,Rr)}|\theta(y)-\theta(x)|\,\mathrm{d}\mathcal H^k(y)
  \longrightarrow0.
 \end{split}
\]
由无权放大极限便得到第一式。
...
取非负测试函数 \(\varphi\)，使 \(\int\varphi\,\mathrm{d}\lambda>0\)。

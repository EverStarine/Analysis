...
\cite[30.1 THEOREM]{Simon1983Lectures}的网格选择。
...
取直线同伦 \(h(t,x)=(1-t)\cdot f_1(x)+t\cdot f_2(x)\)。
...
取同一个非负标量单位磨光核 \(\rho\)，令
\(\rho_j(x)=j^n\cdot\rho(jx)\)，并按向量值积分定义
\(f_j=f*\rho_j\)。这等价于用同一个核磨光 \(f\) 的各分量，而且
\[
 \begin{split}
 |f_j(x)-f_j(y)|
 &\leq\int_{\mathbb R^n}\rho_j(z)
          |f(x-z)-f(y-z)|\,\mathrm{d}z\\
 &\leq\operatorname{Lip}(f)|x-y|,
 \end{split}
\]
故 \(\operatorname{Lip}(f_j)\leq\operatorname{Lip}(f)\)。同时
\[
 \sup_x|f_j(x)-f(x)|
 \leq\operatorname{Lip}(f)
       \int_{\mathbb R^n}|z|\rho_j(z)\,\mathrm{d}z
 \longrightarrow0.
\]
因此 \(f_j\rightarrow f\) 一致，且上述 Lipschitz 常数保持精确不增。
...
\cite[\S33]{Simon1983Lectures}以后转向极小流的局部性质；
...
设 \(1\leq k\leq n\)，\(T_0\in\mathbf I_k(\mathbb R^n)\) 紧支撑。
若存在光滑 \((k-1)\) 次形式 \(\eta\)，使光滑闭 \(k\) 次形式
\(\omega=\mathrm{d}\eta\) 满足
\[
...
因 \(\mathrm{d}\chi=0\) 在该邻域，
\[
 (T-T_0)(\omega)
    =(T-T_0)\bigl(\mathrm{d}(\chi\eta)\bigr)
...
虽然命题称作闭形式下界，这里还明确要求 \(\omega=\mathrm{d}\eta\)，
...
\[
 \omega=\mathrm{d}x^1\wedge\cdots\wedge \mathrm{d}x^k
       =\mathrm{d}\bigl(x^1\,\mathrm{d}x^2\wedge\cdots\wedge \mathrm{d}x^k\bigr).
\]
...
最后也可把固定边界换成惩罚项。
设 \(1\leq k\leq n\)，\(K\) 紧，
\(S\in\mathbf I_{k-1}(\mathbb R^n)\) 支撑于 \(K\)，
对固定 \(\lambda>0\) 考虑
\[
 \mathcal E_\lambda(T)=\mathbf M(T)+\lambda\cdot\mathbf M(\partial T-S),
...
\[
 ...
 \leq (1+\lambda^{-1})\cdot\mathcal E_\lambda(T)+\mathbf M(S).
\]
于是最小化列满足积分流紧性；
...
从有界变差函数到流，变分法的解析骨架仍是有界性、紧性和下半连续性。

...
\[
 \int_{\mathbb R}\mathbf M(S_t)\,\mathrm{d}t
...
\[
 ...
 &=-T(\mathrm{d}g_{\varepsilon,t}\wedge\eta)\\
 &=T\!\left(\frac1\varepsilon
       \chi'\!\left(\frac{t-f}{\varepsilon}\right)\mathrm{d}f\wedge\eta\right).
 \end{split}
\]
对一次形式 \(a\) 和交错形式 \(\eta\)，有
...
\[
 ...
 \leq L\int\frac1\varepsilon
           \chi'\!\left(\frac{t-f(x)}{\varepsilon}\right)\,\mathrm{d}\|T\|(x).
\]
对 \(t\) 积分，Tonelli 定理及 \(\int\chi'=1\) 给出
\(\int\mathbf M(S_{\varepsilon,t})\,\mathrm{d}t\leq L\mathbf M(T)\)。
...
因此 Fatou 引理给出
\[
 \int\mathbf M(S_t)\,\mathrm{d}t\leq L\mathbf M(T).
\]
...
在 \(E\) 的近似切空间上，把 \(Df\) 的切向部分按\chapref{b1:ch:19}记为 \(\mathrm{d}^Ef\)，
其对应切向梯度记为 \(\nabla_Ef\)。
当该梯度非零时，截面切空间是
\[
...
并且
\[
 \int_{\mathbb R}\mathbf M(\langle T,f,t\rangle)\,\mathrm{d}t
        =\int_E\theta\,|\nabla_Ef|\,\mathrm{d}\mathcal H^k.
\]
\end{theorem}
\begin{proof}
...
把这些片合并，所得截面流
\[
 R_t(\eta)=\int_{E_t}\theta\langle\eta,\tau_t\rangle\,\mathrm{d}\mathcal H^{k-1}
\]
...
\[
 S_{\varepsilon,t}(\eta)
    =\int_{\mathbb R}\frac1\varepsilon
          \chi'\!\left(\frac{t-s}{\varepsilon}\right)\cdot R_s(\eta)\,\mathrm{d}s.
\]
这里使用配对恒等式
\(\langle \mathrm{d}f\wedge\eta,\tau\rangle

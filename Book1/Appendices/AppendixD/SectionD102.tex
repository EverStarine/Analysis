...
\(\mathrm{d}x^I=\mathrm{d}x^{i_1}\wedge\cdots\wedge \mathrm{d}x^{i_k}\)。

\begin{definition}\label{b1:def:app-d-differential-form}
若
\[
 \omega(x)=\sum_{|I|=k}\omega_I(x)\,\mathrm{d}x^I,
...
\[
 \mathrm{d}\omega=\sum_{|I|=k}\sum_{i=1}^n
       \partial_i\omega_I\,\mathrm{d}x^i\wedge \mathrm{d}x^I.
\]
对最高次形式规定 \(\mathrm{d}\omega=0\)。

\begin{proposition}\label{b1:prop:app-d-exterior-derivative}
外微分是测试形式空间之间的连续线性映射，并满足
\[
 \mathrm{d}^2\omega=0,\quad
 \mathrm{d}(\alpha\wedge\beta)
   =\mathrm{d}\alpha\wedge\beta+(-1)^p\alpha\wedge \mathrm{d}\beta
\]
其中 \(\alpha\) 为 \(p\) 次形式。
\end{proposition}
\begin{proof}
...
支集不扩大。
计算 \(\mathrm{d}^2\omega\) 时，\(i=j\) 的项含 \(\mathrm{d}x^i\wedge \mathrm{d}x^i=0\)；
当 \(i\neq j\) 时，交换 \(i,j\) 不改变二阶偏导，却使外积改变符号，所以两项抵消。
对第二式只需考察
\(\alpha=a\,\mathrm{d}x^I\)、\(\beta=b\,\mathrm{d}x^J\)。
系数乘积求导给出 \(b\,\mathrm{d}a+a\,\mathrm{d}b\)，
再把第二项中的 \(\mathrm{d}b\) 移过 \(p\) 个一次形式，得到因子 \((-1)^p\)。
对所有 \(I,J\) 求和即得结论。
\end{proof}
...
\[
 [M](\omega)=\int_M\langle\omega(x),\tau_M(x)\rangle\,\mathrm{d}\mathcal H^k(x).
\]
...
\[
 [a,b](\omega)
 =\int_0^1\langle\omega(a+t(b-a)),b-a\rangle\,\mathrm{d}t.
\]
...
\[
 (\partial T)(\eta)\coloneqq T(\mathrm{d}\eta),
...
由 \(\mathrm{d}^2=0\)，有 \(\partial^2T=0\)。
若 \(T_j\rightharpoonup T\)，则对固定 \(\eta\)，
\(\partial T_j(\eta)=T_j(\mathrm{d}\eta)\to T(\mathrm{d}\eta)\)，故边界也弱收敛。
...
形式 \(\mathrm{d}x^1\wedge \mathrm{d}x^2+\mathrm{d}x^3\wedge \mathrm{d}x^4\) 的余质量为一，
...
\[
 T(\omega)=\int_U\langle\omega,\vec T\rangle\,\mathrm{d}\|T\|,
...
由对偶不等式，所有允许的测试形式都满足
\(|T(\omega)|\leq\int\|\omega\|_{\mathrm{co}}\,\mathrm{d}\|T\|\)，
所以局部质量不超过 \(\|T\|(W)\)。
...
\[
 (T\llcorner A)(\omega)
       =\int_A\langle\omega,\vec T\rangle\,\mathrm{d}\|T\|.
\]
...
\[
 T(\omega_1\,\mathrm{d}x^1+\omega_2\,\mathrm{d}x^2)
      =\int_{\mathbb R^2}\phi\omega_1\,\mathrm{d}x.
\]

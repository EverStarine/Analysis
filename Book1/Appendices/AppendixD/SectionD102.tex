...
\]
证明可逐系数归结到测试函数空间的连续性。
特别地，零维流就是第\ref{b1:ch:20}章的分布，而不只是点质量的线性组合。
...
附录\ref{b1:app:B}在外空间中使用使标准外积基正交归一的欧氏范数，记为 \(|\xi|\)。
...
\[
 |\xi|_{\mathrm m}
    =\sup_{\|\alpha\|_{\mathrm{co}}\leq1}|\langle\alpha,\xi\rangle|,
\]
称为外向量的质量范数。
\end{definition}
...
\cite{Simon1983Lectures}第26.6条明确说明后改用欧氏形式范数。

...
\appref{b1:app:B}提供外代数、Jacobi 因子与取向约定。
...
\cite[\S\S26--32]{Simon1983Lectures}。
...
\input{Appendices/AppendixD/SectionD101}
\input{Appendices/AppendixD/SectionD102}
\input{Appendices/AppendixD/SectionD103}
\input{Appendices/AppendixD/SectionD104}
\input{Appendices/AppendixD/SectionD105}
\input{Appendices/AppendixD/SectionD106}
...
\[
 r^{-k}\cdot\int\varphi(y/r)\,\mathrm{d}\mu(y)\geq a\cdot r^{-k}\longrightarrow\infty.
\]
...
定义一维流 \(B(g\,\mathrm{d}x)=g(0)\)。其质量为 \(1\)，并且
\(\partial B(\varphi)=B(\mathrm{d}\varphi)=\varphi'(0)=T(\varphi)\)。
...
\[
 T(\varphi\,\mathrm{d}x^1\wedge\cdots\wedge \mathrm{d}x^n)=\int\varphi\,\mathrm{d}\mu.
\]
对省去 \(\mathrm{d}x^i\) 的测试 \(n-1\) 次形式使用 \(\partial T=0\)，
得到 \(\int\partial_i\varphi\,\mathrm{d}\mu=0\)，即 \(\partial_i\mu=0\) 作为分布成立。
...
薄矩形的有向抵消见\cref{b1:fig:visual-f22}，它把质量与平坦范数的区别具体化。
\input{Figures/F22}
...
对测试一形式 \(\omega=a(x,y)\,\mathrm{d}x+b(x,y)\,\mathrm{d}y\)，只有第一项贡献积分。
在每个区间 \([i/j,(i+1)/j]\) 内，以 \(a(i/j,0)\) 代替 \(a(x,0)\)，
两种积分的误差都由该系数在尺度 \(1/j\) 上的连续模控制。
所有区间相加后得到
\[
 T_j(\omega)-\frac12\int_0^1a(x,0)\,\mathrm{d}x\longrightarrow0.
\]
...
若 \(R=0\)，令 \(e=(b-a)/|b-a|\)。
常值一形式 \(\omega=e\cdot \mathrm{d}x=\mathrm{d}(e\cdot x)\) 的余质量为一。

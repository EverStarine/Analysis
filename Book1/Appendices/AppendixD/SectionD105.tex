...
\[
 \mathbb F(T)\coloneqq\inf\{\,\mathbf M(A)+\mathbf M(B):
...
\[
 ...
 \max\!\left\{\sup_x\|\omega(x)\|_{\mathrm{co}},
               \sup_x\|\mathrm{d}\omega(x)\|_{\mathrm{co}}\right\}\mathbb F(T).
\]
因而平坦范数收敛蕴含流的弱收敛。
\end{proposition}
\begin{proof}
...
对同一个分解，
\[
 |T(\omega)|=|A(\omega)+B(\mathrm{d}\omega)|
 \leq \mathbf M(A)\sup_x\|\omega(x)\|_{\mathrm{co}}
       +\mathbf M(B)\sup_x\|\mathrm{d}\omega(x)\|_{\mathrm{co}}.
\]
取下确界即得测试估计。
...
\]
则存在子列 \((T_{j_\ell})\) 弱收敛于支撑包含于 \(K\) 的正规流 \(T\)，并且
\[
 \mathbf M(T)\leq\liminf_{\ell\to\infty}\mathbf M(T_{j_\ell}),\quad
 \mathbf M(\partial T)\leq\liminf_{\ell\to\infty}\mathbf M(\partial T_{j_\ell}).
\]
\end{proposition}
\begin{proof}
...
一致总变差界把收敛推广到整个 \(C(K)\)，
Riesz 表示给出极限分量测度。
将它们组合，得到支撑在 \(K\) 上的有限质量流 \(T\)，且 \(T_{j_\ell}\rightharpoonup T\)。以下均沿这条子列论证。

对任意测试 \(k-1\) 次形式 \(\eta\)，
\[
 \partial T_{j_\ell}(\eta)=T_{j_\ell}(\mathrm{d}\eta)\longrightarrow T(\mathrm{d}\eta)=\partial T(\eta).
\]
...
\[
 T_\varepsilon
    =\int_{\mathbb R^n}\rho_\varepsilon(h)(\tau_h)_\#T\,\mathrm{d}h.
\]
这个积分可按每个测试形式理解，也可按有限维向量测度逐分量理解。
将\cref{b1:prop:app-d-translation-flat}证明中的 \(A_hT\) 逐分量积分，记所得流为 \(B_\varepsilon T\)。
...
这些并不是弱*\WeakStarJoin{}紧性本身的后果。Simon 的
...
质量与边界质量满足
\[
 \mathbf M(T)\leq\liminf_{\ell\to\infty}\mathbf M(T_{j_\ell}),\qquad
 \mathbf M(\partial T)\leq\liminf_{\ell\to\infty}\mathbf M(\partial T_{j_\ell}).
\]
\end{theorem}
\begin{proof}
...
质量、可整流性和边界条件分别限制流的不同方面，见\cref{b1:tab:visual-t18}。
\input{Tables/T18}

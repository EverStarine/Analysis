...
\cite{Simon1983Lectures}第27节专门讨论 integer multiplicity rectifiable currents，
...
\(\mathbb R^n\) 上全体 \(k\) 维积分流构成的集合记为 \(\mathbf I_k(\mathbb R^n)\)。
...
边界便自动整数重数可整流；Simon 第30.3条给出这一结果。
本附录的紧性和变分问题使用上述明示边界条件的定义。

\begin{proposition}\label{b1:prop:app-d-integral-algebra}
积分流对加法、取负和整数倍封闭，且 \(\partial T\) 仍为积分流。
\end{proposition}
\begin{proof}
在两个可整流承载集的交集上，它们的近似切空间几乎处处相同。
...
取负、整数倍相同处理。
最后 \(\partial^2T=0\)，所以边界的边界是零流，满足积分流的定义。
\end{proof}

...
\[
 T(\omega)=\int_E\theta(x)
                 \langle\omega(x),\tau(x)\rangle\,\mathrm{d}\mathcal H^k(x),
\]
...
\[
 ...
 \mathbf M(T)=\int_E\theta\,\mathrm{d}\mathcal H^k.
\]
...
\[
 [E](f\,\mathrm{d}x^1\wedge\cdots\wedge \mathrm{d}x^n)=\int_E f\,\mathrm{d}x.
\]
它的质量等于 \(|E|\)。
...
直接外微分得到 \(\mathrm{d}\eta=(\operatorname{div}X)\,\mathrm{d}x^1\wedge\cdots\wedge \mathrm{d}x^n\)。
由\cref{b1:thm:gauss-green-finite-perimeter}，
\[
 \partial[E](\eta)
   =\int_E\operatorname{div}X\,\mathrm{d}x
   =\int_{\partial^*E}X\cdot\nu_E\,\mathrm{d}\mathcal H^{n-1}.
\]
上述取向满足
...
逐坐标计算给出 \(\mathrm{d}(f^*\eta)=f^*(\mathrm{d}\eta)\)：
一阶系数满足通常链式法则，二阶偏导项因反对称性相消。
于是
\[
 \partial(f_\#T)(\eta)=T(f^*\mathrm{d}\eta)
       =T(\mathrm{d} f^*\eta)=f_\#(\partial T)(\eta).
\]
...
\(\Omega=\mathrm{d}t\wedge\alpha(t,x)+\beta(t,x)\)，
其中 \(\alpha\) 不含 \(\mathrm{d}t\)，\(\beta\) 也不含 \(\mathrm{d}t\)。
定义
\[
 ([0,1]\times T)(\Omega)=\int_0^1T\bigl(\alpha(t,\cdot)\bigr)\,\mathrm{d}t.
\]
...
先验证乘积边界。
把一个 \(k\) 次测试形式写成
\(\eta=\mathrm{d}t\wedge\gamma(t,x)+\alpha(t,x)\)。
其外微分中含 \(\mathrm{d}t\) 的部分为
\[
 \mathrm{d}t\wedge(\partial_t\alpha-\mathrm{d}_x\gamma).
\]
所以
\[
 \begin{split}
 \partial([0,1]\times T)(\eta)
 &=\int_0^1 T(\partial_t\alpha)\,\mathrm{d}t
     -\int_0^1\partial T(\gamma)\,\mathrm{d}t\\

...
第\ref{b1:sec:0903}节已经引入乘积拓扑并证明 Tychonoff 定理。这里补充其使用方法。设 \(X=\prod_{i\in I}X_i\)，坐标投影为 \(\pi_i\)。基本开集是
\[
...
这里可数个坐标使级数权重与对角子列都可用。一般不可数乘积的紧性仍成立，但不能从这一度量公式或可数次抽取推出，这正是 Tychonoff 定理在第九章中不可被简单对角法替代的原因。
...
证明中不是“开集是基本开集之并”就已足够，因为 \(\sigma\)-代数只对可数并封闭。可数基把任意并变成可数并，这是拓扑假设真正进入测度论的位置。第\ref{b1:ch:07}章的乘积测度使用可测矩形，不能在一般空间中不经这一步就把乘积可测集与全部乘积 Borel 集等同。

...
本附录可在\chapref{b1:ch:01}之前作为基础补读，也可按正文需要查阅。度量与紧性部分只需实数完备性和集合运算；乘积部分回顾\secref{b1:sec:0903}已经证明的 Tychonoff 定理，不把其证明转嫁到本附录。最后一节用到的 Baire 纲定理可在\secref{b1:sec:0803}找到。一般点集拓扑可继续读熊金城《点集拓扑讲义》第五版\cite[第3章, 第5--7章]{Xiong2020Topology}；与分析的联系可对照 Hunter、Nachtergaele《Applied Analysis》\cite[Chapters 1, 2, and 4]{Hunter2001Applied}，测度论常用的拓扑事实可查 Fremlin《Measure Theory》\cite[Appendix to Volume 4, \S4A2]{Fremlin2016Measure}。

\input{Appendices/AppendixA/SectionA101}
\input{Appendices/AppendixA/SectionA102}
\input{Appendices/AppendixA/SectionA103}
\input{Appendices/AppendixA/SectionA104}
\input{Appendices/AppendixA/SectionA105}
...
\]
不动点方程为 \(x^2=2\)，故在 \(X\) 中无解。令 \(x_{n+1}=T(x_n)\)，则相邻差被 \(4^{-n}|x_1-x_0|\) 控制，尾部差以几何级数估计，故是 Cauchy 列。这时 \(X\) 的完备化是 \([1,2]\)；连续延拓的极限满足同一方程，因而为 \(\sqrt2\)。这说明不动点论证需要完备性，不能只检查压缩常数。
\end{solution}
...
设 \(Y\) 为非空可分度量空间，\(D\subset Y\) 为可数稠密集，\((S,\mathcal A)\) 为可测空间。证明 \(f:(S,\mathcal A)\rightarrow\bigl(Y,\mathcal B(Y)\bigr)\) 可测，当且仅当对每个 \(a\in D\)，实函数 \(s\mapsto \mathrm{d}(f(s),a)\) 可测。
\end{exercise}
\begin{solution}
...
设 \((X,\mathrm{d})\) 为非空紧度量空间，以 \(\mathcal K(X)\) 表示全部非空紧子集。对 \(A,B\in\mathcal K(X)\) 规定
\[
 \mathrm{d}_H(A,B)\coloneqq\max\left\{\sup_{a\in A}\mathrm{d}(a,B),\ \sup_{b\in B}\mathrm{d}(b,A)\right\}.
\]
证明 \(\mathrm{d}_H\) 为度量，并证明 \(\mathcal K(X)\) 在此度量下紧。
\end{exercise}
\begin{solution}
各上确界有限；若 \(\mathrm{d}_H(A,B)=0\)，闭性说明 \(A\subset B\) 且 \(B\subset A\)。由
\(\mathrm{d}(a,C)\le \mathrm{d}(a,B)+\sup_{b\in B}\mathrm{d}(b,C)\)
及反向同类估计，取上确界得到三角不等式。

给定 \(\varepsilon>0\)，取 \(X\) 的有限 \(\varepsilon/2\)-网 \(\{x_1,\ldots,x_N\}\)。对每个非空紧集 \(A\)，保留那些半径为 \(\varepsilon/2\) 的球与 \(A\) 相交的网点，所得非空有限集 \(F_A\) 满足
\[
 \mathrm{d}_H(A,F_A)\le \frac{\varepsilon}{2}<\varepsilon.
\]
这样的有限集只有有限多个，故它们是 \(\mathcal K(X)\) 的一个有限 \(\varepsilon\)-网，\(\mathcal K(X)\) 全有界。

设 \((A_n)\) 是 \(\mathrm{d}_H\)-Cauchy 列。令
\[
 A=\bigcap_{N=1}^{\infty}\overline{\bigcup_{n\ge N}A_n}.
\]
右侧是紧空间 \(X\) 中递减非空闭集的交，所以 \(A\) 非空且紧。给定 \(\varepsilon>0\)，选 \(N\)，使 \(n,m\ge N\) 时 \(\mathrm{d}_H(A_n,A_m)<\varepsilon/2\)。固定 \(n\ge N\) 与 \(a\in A_n\)，在每个 \(A_m\)、\(m\ge N\)，选 \(a_m\) 距 \(a\) 小于 \(\varepsilon/2\)。取趋于无穷的指标的收敛子列，极限属于 \(A\)，所以 \(\mathrm{d}(a,A)\le\varepsilon/2\)。反过来，每个 \(b\in A\) 可由 \(b_j\in A_{m_j}\)、\(m_j\to\infty\)，逼近；故 \(\mathrm{d}(b,A_n)\le\varepsilon/2\)。于是
\[
 \mathrm{d}_H(A_n,A)\le\frac{\varepsilon}{2}<\varepsilon
 \qquad(n\ge N).
\]
这证明完备，结合全有界性得到紧性。
\end{solution}

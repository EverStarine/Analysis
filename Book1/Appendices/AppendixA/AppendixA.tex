...
设 \(Y\) 为非空可分度量空间，\(D\subset Y\) 为可数稠密集，\((S,\mathcal A)\) 为可测空间。证明 \(f:(S,\mathcal A)\rightarrow\bigl(Y,\mathcal B(Y)\bigr)\) 可测，当且仅当对每个 \(a\in D\)，实函数 \(s\mapsto d(f(s),a)\) 可测。
\end{exercise}
\begin{solution}
...
\[
 d_H(A,B)\coloneqq\max\left\{\sup_{a\in A}d(a,B),\ \sup_{b\in B}d(b,A)\right\}.
\]
...
\[
 d_H(A,F_A)\le \frac{\varepsilon}{2}<\varepsilon.
\]
...
\]
右侧是紧空间 \(X\) 中递减非空闭集的交，所以 \(A\) 非空且紧。给定 \(\varepsilon>0\)，选 \(N\)，使 \(n,m\ge N\) 时 \(d_H(A_n,A_m)<\varepsilon/2\)。固定 \(n\ge N\) 与 \(a\in A_n\)，在每个 \(A_m\)、\(m\ge N\)，选 \(a_m\) 距 \(a\) 小于 \(\varepsilon/2\)。取趋于无穷的指标的收敛子列，极限属于 \(A\)，所以 \(d(a,A)\le\varepsilon/2\)。反过来，每个 \(b\in A\) 可由 \(b_j\in A_{m_j}\)、\(m_j\to\infty\)，逼近；故 \(d(b,A_n)\le\varepsilon/2\)。于是
\[
 d_H(A_n,A)\le\frac{\varepsilon}{2}<\varepsilon
 \qquad(n\ge N).
\]
这证明完备，结合全有界性得到紧性。
\end{solution}

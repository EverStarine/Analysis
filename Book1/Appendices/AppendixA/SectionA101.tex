...
分析中经常先给出一种距离，再谈极限、连续与闭集。但这些概念对距离的依赖并不相同：连续性只看开集，Cauchy 条件却比较尚未知道极限的两个点。本节把这一区别落实到构造中。以下用正体 \(\mathrm{d}\) 表示距离；正文中表示欧氏空间维数的变量 \(d\) 则用斜体。

\subsection{开集与连续性}
\label{b1:subsec:A10101}

\begin{definition}\label{b1:def:app-metric}
设 \(X\) 为集合。若函数 \(\mathrm{d}:X\times X\rightarrow[0,\infty)\) 满足
\[
 \mathrm{d}(x,y)=0\ \Longleftrightarrow\ x=y,\quad
 \mathrm{d}(x,y)=\mathrm{d}(y,x),\quad \mathrm{d}(x,z)\le \mathrm{d}(x,y)+\mathrm{d}(y,z),
\]
则称 \(\mathrm{d}\) 为 \(X\) 上的\term[duliang]{度量}[metric]，称 \((X,\mathrm{d})\) 为度量空间。开球记作 \(B_{\mathrm{d}}(x,r)\coloneqq\{\,y:\mathrm{d}(x,y)<r\,\}\)。若 \(U\subset X\) 的每个点都包含在一个仍位于 \(U\) 内的开球中，则称 \(U\) 为开集。
\end{definition}

赋范空间中的 \(\mathrm{d}(x,y)=\|x-y\|\) 是例子，任意集合上的离散距离也是例子：不同点的距离规定为 \(1\)。反过来，度量空间不必允许向量相加。讨论曲线、测度的状态空间时，这一点尤其有用。
...
连续映射保持序列极限，直接由邻域定义得到。若度量空间间的映射在 \(x\) 不连续，则存在 \(\varepsilon>0\)，对每个 \(n\) 都能取 \(x_n\in B_{\mathrm{d}}(x,1/n)\)，而目标距离满足 \(\mathrm{d}_Y\bigl(f(x_n),f(x)\bigr)\ge\varepsilon\)。这个序列反驳保持极限的性质。
\end{proof}

最后一步依靠可数个球控制一点附近的全部邻域。一般拓扑空间不能不加条件地以序列代替连续性的定义；\secref{b1:sec:0901}引入网，正是为了处理这种局限。

\subsection{相同拓扑与不同的 Cauchy 列}
\label{b1:subsec:A10102}

\begin{definition}\label{b1:def:app-complete}
若对每个 \(\varepsilon>0\)，存在 \(N\)，使 \(m,n\ge N\) 时 \(\mathrm{d}(x_m,x_n)<\varepsilon\)，则称 \((x_n)\) 为 Cauchy 列。若空间中每个 Cauchy 列都收敛到本空间的点，则称它\term[wanbei]{完备}[complete]。
若对每个 \(\varepsilon>0\) 可取不依赖于点的 \(\delta>0\)，使 \(\mathrm{d}_X(x,y)<\delta\) 蕴含 \(\mathrm{d}_Y\bigl(f(x),f(y)\bigr)<\varepsilon\)，则称 \(f\) 一致连续。
\end{definition}

度量 \(\mathrm{d}\) 与 \(\min(1,\mathrm{d})\) 给出相同的开集，也有相同的 Cauchy 列，因为小于 \(1\) 的距离阈值没有改变。相同拓扑本身却不足以推出后一结论。在 \((0,1)\) 上，通常距离与
\[
...
若 \(A\) 完备而 \(x\in\overline A\)，从 \(A\cap B_{\mathrm{d}}(x,1/n)\) 取 \(a_n\)。它是 Cauchy 列，在 \(A\) 中有极限；极限唯一，故 \(x\in A\)。反之，闭子集中的 Cauchy 列先在全空间收敛，闭性再把极限留在子集中。
\end{proof}
...
\[
 (x_n)\sim(y_n)\quad\Longleftrightarrow\quad \mathrm{d}(x_n,y_n)\longrightarrow0.
\]
三角不等式给出传递性，因此这确为等价关系。令 \(\widehat X=\mathcal C/{\sim}\)，定义
\[
 \widehat {\mathrm{d}}\bigl([(x_n)],[(y_n)]\bigr)=\lim_{n\to\infty}\mathrm{d}(x_n,y_n).
\]
由
\[
 |\mathrm{d}(x_n,y_n)-\mathrm{d}(x_m,y_m)|
 \le \mathrm{d}(x_n,x_m)+\mathrm{d}(y_n,y_m)
\]
可知右侧极限存在。再用同一估计比较两组代表，极限不随代表改变。对称性与三角不等式从逐项距离传到极限，零距离的情形恰好是同一个等价类。于是 \(\widehat {\mathrm{d}}\) 是度量。

把 \(x\) 送到常值列的等价类，得到等距嵌入 \(i:X\rightarrow\widehat X\)。若 \(\xi=[(x_n)]\)，则
\[
 \widehat {\mathrm{d}}\bigl(i(x_m),\xi\bigr)=\lim_{n\to\infty}\mathrm{d}(x_m,x_n)
 \longrightarrow0\quad(m\to\infty).
\]
所以 \(i(X)\) 稠密。

为证完备，设 \((\xi_m)\) 是 \(\widehat X\) 中的 Cauchy 列。由稠密性取 \(a_m\in X\)，使 \(\widehat {\mathrm{d}}\bigl(i(a_m),\xi_m\bigr)<1/m\)。三角不等式说明 \((a_m)\) 是 \(X\) 中的 Cauchy 列。令 \(\xi=[(a_m)]\)，上段已证 \(i(a_m)\rightarrow\xi\)，故 \(\xi_m\rightarrow\xi\)。
...
给定 \(\varepsilon>0\)，以 \(\varepsilon/2\) 在 \(A\) 上的一致连续性选取 \(\delta>0\)。若 \(\mathrm{d}_X(x,y)<\delta/3\)，可分别用 \(a_n,b_n\) 逼近它们，尾部满足 \(\mathrm{d}_X(a_n,b_n)<\delta\)。传到极限得 \(\mathrm{d}_Y(F(x),F(y))\le\varepsilon/2<\varepsilon\)。Lipschitz 断言则把原不等式直接传到极限。
\end{proof}

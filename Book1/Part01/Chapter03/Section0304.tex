...
Lebesgue 测度由坐标长方体构造，但最后的答案不应依赖坐标原点或正交标架。本节从覆盖定义出发验证这一点。平移可以直接搬运覆盖；可逆线性变换则可分解为坐标置换、反射、单坐标伸缩与\term[jianqie-bianhuan]{剪切变换}[shear transformation]。%
\footnote{剪切变换也称错切变换\termidx[cuoqie-bianhuan]{错切变换}；两种译名并列见黄进、李剑波《数字图像处理——原理与实现》\cite{Huang2020ShuziTuxiang}第 3.3.5 节，第 41--42 页。这里指下文给出的线性变换，与乘积空间中集合的截面无关。}%
剪切变换是其中唯一不把长方体送成长方体的步骤，需要单独作覆盖估计。奇异线性变换不能由这些可逆变换复合而成，将在最后利用其像落在真子空间中单独处理。
...
m^*(A)
&=m^*(A-a)\\
&=m^*\bigl((A-a)\cap E\bigr)+m^*\bigl((A-a)\setminus E\bigr)\\
&=m^*\bigl(A\cap(E+a)\bigr)+m^*\bigl(A\setminus(E+a)\bigr).
\end{align*}
...
\begin{enumerate}
  ...
  \item \label{b1:item:0304-elementary-2}若 \(S_{ij,t}\) 是剪切变换
  \[
...
\quad \ell_k=b_k-a_k.
\]
把第 \(j\) 个坐标区间等分成 \(N\) 段，便把 \(Q\) 分成 \(N\) 个两两不交的薄长方体。对每个薄长方体，第 \(j\) 个坐标的振幅为 \(\ell_j/N\)，所以剪切变换后第 \(i\) 个坐标的振幅至多
\[
...
\[
m^*(F A)=\gamma\cdot m^*(A)
\]
对所有 \(A\) 成立，其中 \(0<\gamma<\infty\)。若 \(E\in\mathcal L^d\)，则对任意 \(A\subseteq\mathbb R^d\)，
\begin{align*}
m^*(A)
&=\gamma\cdot m^*(F^{-1}A)\\
&=\gamma\cdot m^*(F^{-1}A\cap E)
+\gamma\cdot m^*(F^{-1}A\setminus E)\\
...
m^*(rA)=|r|^d m^*(A).
\]
若 \(r=0\)，则 \(rA\) 是空集或单点，因而 \(m^*(rA)=0\)。在两种情形下，若 \(E\) 是 Lebesgue 可测的，则 \(rE\) 也是 Lebesgue 可测的。
\end{proposition}
\begin{proof}
...
\[
m\bigl(B(0,r)\bigr)=r^d \cdot m\bigl(B(0,1)\bigr).
\]
本章尚不需要计算常数 \(m\bigl(B(0,1)\bigr)\)；重要的是维数 \(d\) 决定了伸缩指数。
...
\(\mathbb R^d\) 的每个真线性子空间都是 Lebesgue 零测集；因此它的任意子集也是 Lebesgue 可测的且测度为零。
\end{proposition}
\begin{proof}
...
剪切的保测度性质可以从截面直接看出，见\cref{b1:fig:visual-f01}。
\input{Figures/F01}

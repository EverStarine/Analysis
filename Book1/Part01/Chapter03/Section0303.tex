...
陶哲轩的《An Introduction to Measure Theory》\cite{Tao2011Introduction}可用于对照 Lebesgue 测度从外测度构造到正则性所得的整体结构。这里得到的 Borel 代表与紧开夹逼，将在\chapref{b1:ch:04}转化为可测函数在大集合上的连续性。

\ProseParagraphBreak

正则性给出的三种逼近并不承担同一任务。\cref{b1:tab:lebesgue-regular-approximations}集中列出逼近方向、误差和所得集合的复杂度，便于以后判断应调用哪一种形式。

\begin{table}[htbp]
\centering
\small
\renewcommand{\arraystretch}{1.16}
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{6.2em} >{\raggedright\arraybackslash}p{11.5em} >{\raggedright\arraybackslash}p{12.5em} >{\raggedright\arraybackslash}X@{}}
\toprule
\textbf{方式} & \textbf{集合关系} & \textbf{误差控制} & \textbf{所得对象} \\
\midrule
外逼近 & \(E\subseteq G\) & \(m(G\setminus E)<\varepsilon\) & 开集；逐次相交可得 \(G_\delta\) 外代表 \\
内逼近 & \(K\subseteq E\) & 有限测度时 \(m(E\setminus K)<\varepsilon\) & 紧集；局部化后也刻画无穷测度 \\
Borel 夹逼 & \(F\subseteq E\subseteq B\) & 两个差集均为零测集 & \(F_\sigma\) 内代表与 \(G_\delta\) 外代表 \\
\bottomrule
\end{tabularx}
\caption[Lebesgue 正则逼近]{Lebesgue 正则逼近。无穷测度时，外逼近仍使用差集估计；内正则性的上确界形式则通过有界局部化取得。}
\label{b1:tab:lebesgue-regular-approximations}
\end{table}

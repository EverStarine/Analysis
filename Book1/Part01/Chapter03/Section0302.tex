...
设集合 \(E\subseteq\mathbb R^d\)。若对任意 \(A\subseteq\mathbb R^d\)，都有
\[
m^*(A)=m^*(A\cap E)+m^*(A\setminus E),
\]
则称 \(E\) 为\term[lebesgue-keceji]{Lebesgue 可测集}[Lebesgue measurable set]。全体 Lebesgue 可测集组成的 \(\sigma\)-代数记为 \(\mathcal L^d\)，而
\[
m\coloneqq m^*|_{\mathcal L^d}
\]
...
连续映射的 Borel 原像仍是 Borel 集；一般连续映射却未必把 Borel 集映成 Borel 集，因而不能沿用上述原像判别来处理像集。这一现象需要描述集合论中的额外工具，本书此处不作证明；线性映射的像将在\secref{b1:sec:0304}直接处理。
...
\subsection{Lebesgue \texorpdfstring{\(\sigma\)}{\AnalysisPdfSigma}-代数}
...
反过来，设 \(B,Z\) 是定理中的 Borel 集。由\cref{b1:thm:borel-lebesgue}，二者都是 Lebesgue 可测的；由完备性，\(Z\) 的任意子集也可测。因为 \(E\mathbin{\triangle}B\subseteq Z\)，集合 \(E\) 可由 \(B\) 增删两个 \(Z\) 的子集得到，故 \(E\in\mathcal L^d\)。这也说明 \(\mathcal L^d\) 恰是限制测度 \(m|_{\mathcal B(\mathbb R^d)}\) 的完备化。
\end{proof}

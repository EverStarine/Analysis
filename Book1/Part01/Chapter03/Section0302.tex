...
连续映射的 Borel 原像仍是 Borel 集；连续映射的 Borel 像却未必是 Borel 集，因而不能沿用上述原像判别来处理像集。一般像集问题需要额外结构，线性映射的像将在第 \ref{b1:sec:0304} 节直接处理。
...
=\frac{1}{3^r}>0.
\]
所以该映射是单射；二元序列全体不可数，故 \(C\) 不可数。集合 \(C\) 是闭集，因为它是递减闭集列 \((C_n)\) 的交；任意区间在足够深的一轮都会遇到被删去的中间区间，故 \(C\) 没有内点。
\end{proof}

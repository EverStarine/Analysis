...
若 \(s=\sum_{j=1}^{r}a_j\cdot\mathbf1_{A_j}\)，其中 \(a_j\geq0\)、\(A_j\in\mathcal A\)，则不论这些集合是否互不相交，都有
\[
I_\mu(s)=\sum_{j=1}^{r}a_j\cdot\mu(A_j).
\]
非负简单函数的积分具有单调性，并且对 \(c,d\geq0\) 满足
\[
I_\mu(cs+dt)=c\cdot I_\mu(s)+d\cdot I_\mu(t).
\]
\end{proposition}
\begin{proof}
...
例如，在实直线上取 \(s=2\cdot\mathbf1_{[0,2]}+3\cdot\mathbf1_{[1,3]}\)。两项的支集重叠，而积分仍为 \(2\cdot2+3\cdot2=10\)。也可以在 \([0,1)\)、\([1,2]\)、\((2,3]\) 上分别使用函数值 \(2,5,3\)，得到同一结果。这里用的是有限可测分割，不要求分块为区间。
...
\[
\int_X f\,\mathrm d\mu
\coloneqq\sup\{\,I_\mu(s)\mid 0\leq s\leq f,\ s\in\mathcal S_+(X,\mathcal A)\,\},
\]
其中 \(\mathcal S_+(X,\mathcal A)\) 表示全部非负可测简单函数。当定义域明确时省略下标 \(X\)。对 \(E\in\mathcal A\)，定义
\[
\int_E f\,\mathrm d\mu\coloneqq\int_X f\cdot\mathbf1_E\,\mathrm d\mu,
\]
其中 \(f\cdot\mathbf1_E\) 在 \(E\) 外取零，即使 \(f\) 在那里等于无穷也如此。
...
\end{lemma}
\begin{proof}
由单调性，\(L=\lim_n I_\mu(s_n)\) 存在且不超过 \(\int f\,\mathrm d\mu\)。固定任意非负简单函数 \(s\leq f\)，写成 \(s=\sum_ja_j\cdot\mathbf1_{A_j}\)，并固定 \(0<c<1\)。令
\[
E_n=\{\,x\in X\mid s_n(x)\geq c\cdot s(x)\,\}.
\]
这些集合递增，且并为 \(X\)：当 \(s(x)=0\) 时每个不等式都成立；当 \(s(x)>0\) 时，\(s_n(x)\to f(x)\geq s(x)>c\cdot s(x)\)，不等式最终成立。于是
\[
I_\mu(s_n)\geq c\cdot I_\mu(s\cdot\mathbf1_{E_n})
=c\cdot\sum_j a_j\cdot\mu(A_j\cap E_n).
\]
测度的从下连续性给出 \(L\geq c\cdot I_\mu(s)\)。若 \(I_\mu(s)=\infty\)，这已经说明 \(L=\infty\)；否则令 \(c\uparrow1\)，得 \(L\geq I_\mu(s)\)。最后对所有 \(s\leq f\) 取上确界，便得反向不等式。
\end{proof}
...
\[
\int(cf+dg)\,\mathrm d\mu
=\lim_n\bigl(c\cdot I_\mu(s_n)+d\cdot I_\mu(t_n)\bigr)
=c\int f\,\mathrm d\mu+d\int g\,\mathrm d\mu.
\]
...
设 \(f\geq0\) 可测。对 \(a>0\)，有
\[
a\cdot\mu(\{f\geq a\})\leq\int f\,\mathrm d\mu.
\]
并且，\(\int f\,\mathrm d\mu=0\) 当且仅当 \(f=0\) 几乎处处；若积分有限，则 \(f\) 几乎处处有限。
\end{proposition}
\begin{proof}
由 \(a\cdot\mathbf1_{\{f\geq a\}}\leq f\) 及单调性，得到第一个估计。若积分为零，则对每个正整数 \(k\)，集合 \(\{f\geq1/k\}\) 为零测集。它们的并为 \(\{f>0\}\)，故 \(f=0\) 几乎处处。
...
\end{corollary}
\begin{proof}
取包含例外点的可测零集 \(N\)。分解 \(f=f\cdot\mathbf1_{X\setminus N}+f\cdot\mathbf1_N\)，再用非负可加性及零积分判别，得到
\(\int f=\int f\cdot\mathbf1_{X\setminus N}\)。对 \(g\) 同样处理，在补集上使用相等或大小关系即可。
\end{proof}
...
\end{theorem}
\begin{proof}
先对简单函数 \(s=\sum_\ell a_\ell\cdot\mathbf1_{A_\ell}\) 考虑
\[
\nu_s(E)=\sum_\ell a_\ell\cdot\mu(A_\ell\cap E).
\]
它是有限个测度的非负线性组合，空集上为零，可数可加性由 \(\mu\) 的可数可加性逐项得到。

对一般 \(f\) 取 \(s_n\uparrow f\)。在每个 \(E\) 上，\(s_n\cdot\mathbf1_E\uparrow f\cdot\mathbf1_E\)，所以\cref{b1:lem:simple-integral-approximation}给出

...
控制收敛定理也能处理连续变化的参数。设 \((X,\mathcal A,\mu)\) 是测度空间，实参数 \(t\) 在区间 \(I\) 中变化。以下各结论分别给出保证参数截面可积，以及允许交换极限、微分与积分的条件；不在节首统一预设每个截面已经可积。

\ProseParagraphBreak

当相应截面可积时，\term[hancan-bianliang-jifen]{含参变量积分}[parameter integral]是参数 \(t\) 的函数，记为%
\footnote{V.~A. Zorich《数学分析》第二卷第四版中译本\cite{Zorich2007AnalysisIIChinese}第 17 章使用这一名称；华东师范大学数学系《数学分析》下册第四版\cite{ECNU2012AnalysisII}第 19 章称为含参量积分\termsee[hancanliang-jifen]{含参量积分}{含参变量积分}。两种译名均指把积分看作参数的函数；本书后文统一使用“含参变量积分”。}%
\[
F(t)=\int_X f(t,x)\,\mathrm d\mu(x).
\]
逐点检查 \(f\) 对参数的变化以后，还要找到对附近参数同时有效的可积控制。控制函数在这里的作用，可对照 Sheldon Axler 的《Measure, Integration \& Real Analysis》\cite[Section 3B]{Axler2020Measure}关于控制收敛的论证。以下允许被积函数取实值或复值；记号 \(\mathrm{d}x\) 表示对一维 Lebesgue 测度积分。
...
\[
t\longmapsto\int_{\mathbb R}e^{itx}\cdot h(x)\,\mathrm dx
\]
连续，因为被积函数的绝对值恒为 \(|h(x)|\)。这里没有要求 \(x\cdot h(x)\) 可积；若要再对参数求导，就需要检查差商能否受到可积函数控制。
...
\[
f_n\cdot\mathbf1_{[a_n,b_n]}
\longrightarrow f\cdot\mathbf1_{[a,b]}
\quad\text{几乎处处}.
\]
...
先考虑实值函数。对 \(x\notin N\)，中值定理给出
\[
|f(t,x)-f(s,x)|\leq |t-s|\cdot g(x)
\quad(s,t\in J).
\]
复值情形分别对实部、虚部使用中值定理，再用三角不等式，得到右侧改为 \(2|t-s|\cdot g(x)\) 的估计。因此两种情形都满足
\[
|f(t,x)|\leq |f(t_*,x)|+2|t-t_*|\cdot g(x),
\]
从而每个截面可积。

固定 \(t\in J\)，取非零实数 \(h_n\to0\) 且 \(t+h_n\in J\)。在 \(X\setminus N\) 上，相应差商趋于 \(\partial_t f(t,x)\)，绝对值不超过 \(2\cdot g(x)\)。把差商在 \(N\) 上置零，就得到可测函数列，故其极限即上述导数也是可测的。由\cref{b1:thm:dominated-convergence}与积分线性性，
\[
...
回到本节开始的指数积分。若除 \(h\in L^1(\mathbb R)\) 外，还有 \(|x|^k\cdot h(x)\in L^1(\mathbb R)\)，则每个 \(j\leq k\) 的参数导数都由 \((1+|x|^k)|h(x)|\) 控制。这是因为 \(|x|^j\leq1+|x|^k\)。于是该含参变量积分属于 \(C^k\)，并且
\[
\dfrac{\mathrm{d}^j}{\mathrm{d}t^j}\int_{\mathbb R}e^{itx}\cdot h(x)\,\mathrm dx
=\int_{\mathbb R}(ix)^j e^{itx}\cdot h(x)\,\mathrm dx.
\]
...
\]
则 \(f(t,\cdot)\to f(t_0,\cdot)\) 在 \(L^1(0,\infty)\) 中成立，含参变量积分也趋于 \(F(t_0)\)。
\end{proposition}
\begin{proof}
...
若附近所有截面都由同一个 \(g\in L^1(0,\infty)\) 控制，则\cref{b1:thm:monotone-convergence}应用于 \(g\cdot\mathbf1_{(0,R)}\) 就给出一致尾部条件。反之，区间族 \(f(t,x)=\mathbf1_{[t,t+1]}(x)\) 在 \(t\to\infty\) 时逐点趋于零，在任意固定有限区间内最终恒为零，而整个积分始终为 \(1\)。它的质量移向无穷远，尾部不能一致舍去。
...
这里的控制只需在每个 \(a_0>0\) 的邻域内成立；当 \(a\downarrow0\) 时，\(L(a)\to\infty\)，不能把同一结论延伸到零参数。\secref{b1:sec:0505}将进一步讨论：一个函数族没有共同的可积上界时，还能用哪些条件控制积分极限。

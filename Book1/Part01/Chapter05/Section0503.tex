...
\begin{lemma}[Fatou]\label{b1:thm:fatou}
...
\[
E_k=\bigcap_{n\geq k}\{\,x\in X\mid f_n(x)\geq c\cdot s(x)\,\}.
\]
集合 \(E_k\) 可测且递增。若 \(s(x)=0\)，则 \(x\) 属于全部 \(E_k\)；若 \(s(x)>0\)，由 \(\liminf_n f_n(x)\geq s(x)>c\cdot s(x)\)，知 \(x\) 最终也属于 \(E_k\)。因此 \(E_k\uparrow X\)。

固定 \(k\)，当 \(n\geq k\) 时有 \(f_n\geq c\cdot s\cdot\mathbf1_{E_k}\)，于是
\[
\liminf_n\int f_n\,\mathrm d\mu\geq c\int_{E_k}s\,\mathrm d\mu.
\]
右端是简单函数积分；对其有限个水平集使用测度从下连续性，令 \(k\to\infty\)，得到右端趋于 \(c\cdot I_\mu(s)\)。若这个积分无穷，所需下界已得；否则再令 \(c\uparrow1\)。最后对全部 \(s\leq h\) 取上确界，便得到结论。
\end{proof}
...
\begin{theorem}[单调收敛]\label{b1:thm:monotone-convergence}
...
设 \(f_n,f:X\rightarrow[0,\infty]\) 均可测。若 \(f_n\downarrow f\) 几乎处处，且 \(\int f_1\,\mathrm d\mu<\infty\)，则
\[
...
\begin{theorem}[控制收敛]\label{b1:thm:dominated-convergence}
...
有限测度空间上，若 \(|f_n|\leq M\) 几乎处处，可以取 \(g=M\cdot\mathbf1_X\)，这就是有界收敛的情形。无限测度空间上常数函数未必可积，所以仅有一致有界不够。而 \(\mathbf1_{[n,n+1]}\) 在实直线上处处趋零，而积分恒为一，正是一个反例。

\subsection{与 Riemann 积分和级数的相容性}
\label{b1:subsec:050304-compatibility}

\begin{proposition}\label{b1:prop:riemann-lebesgue-compatibility}
若有界函数 \(f:[a,b]\rightarrow\mathbb R\) 是 Riemann 可积的，则它是 Lebesgue 可测且可积的，并且两种积分相等。
\end{proposition}
\begin{proof}
由 Riemann 可积性，对每个 \(n\geq1\) 选取有限分割 \(Q_n\)，使 \(U(f,Q_n)-L(f,Q_n)<1/n\)。令 \(P_0=\{a,b\}\)，递归取共同细化 \(P_n=P_{n-1}\cup Q_n\)。细化不会增大上、下 Darboux 和之差，故 \(P_n\) 逐次加细且仍有
\[
U(f,P_n)-L(f,P_n)<1/n.
\]
记相应的下、上阶梯函数为 \(l_n,u_n\)，在各分割端点处统一取零。区间的 Lebesgue 测度等于长度，有限个端点又不影响积分，因此
\[
\int_{[a,b]}l_n\,\mathrm dm=L(f,P_n),
\quad
\int_{[a,b]}u_n\,\mathrm dm=U(f,P_n).
\]
...
\begin{proposition}[反向 Fatou]\label{b1:prop:reverse-fatou}
...
\end{proposition}
\begin{proof}
令 \(Z=\bigcup_{n\geq1}\{f_n>g\}\)，它是可测零集。在 \(Z\) 上把所有 \(f_n\) 改为 \(g\)，并相应重定义 \(h=\limsup_n f_n\)。分块定义保持可测性；由\cref{b1:cor:integral-ae-invariance}，各 \(f_n\) 的积分以及 \(h\) 的正、负部积分均不变。因此可假设 \(f_n\leq g\) 处处成立。

此时 \(h\leq g\)，故 \(h_+\leq g_+\)，所以 \(\int h_+<\infty\)，积分不会成为 \(\infty-\infty\)。对非负函数 \(g-f_n\) 应用 Fatou 引理，得到
\[
\int(g-h)\,\mathrm d\mu
...
(g-h)+g_-+h_+=g_++h_-
\]
积分；除 \(g-h\) 与 \(h_-\) 外，其余项积分均有限。因而
\(\int(g-h)=\int g-\int h\)，包括两边对应无穷的情形。代入上式并移去有限量 \(\int g\)，就得到所需不等式。
\end{proof}

对非负尖峰列 \(n\cdot\mathbf1_{(0,1/n)}\)，反向不等式会要求 \(1\leq0\)，所以它不可能有共同可积上界。使用本节结果时，可以先检查手中的控制是哪一种：单调性给精确的非负极限，单侧可积控制给一个方向的不等式，双侧绝对值控制则给出 \(L^1\) 收敛。

\ProseParagraphBreak

\cref{b1:tab:integral-convergence-theorems}把本节四种常用入口按“函数列的结构—所得结论”排列。它不是定理陈述的替代；应用时仍须回到相应结果核对可测性、几乎处处条件与积分是否允许取无穷。

\begin{table}[htbp]
\centering
\small
\renewcommand{\arraystretch}{1.17}
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{7.1em} >{\raggedright\arraybackslash}p{15.7em} >{\raggedright\arraybackslash}X@{}}
\toprule
\textbf{结果} & \textbf{主要控制} & \textbf{结论形态} \\
\midrule
Fatou 引理 & \(f_n\geq0\) & 极限下方的不等式 \\
单调收敛 & \(0\leq f_n\uparrow f\) & 积分单调趋于极限积分 \\
控制收敛 & \(f_n\to f\) 且 \(|f_n|\leq g\in L^1\) & \(L^1\) 收敛及积分收敛 \\
反向 Fatou & \(f_n\leq g\in L^1\) & 上极限方向的不等式 \\
\bottomrule
\end{tabularx}
\caption[积分收敛的四种入口]{积分收敛的四种入口。单侧控制通常只给一个方向的不等式；单调性或双侧可积控制才把结论加强为等式。}
\label{b1:tab:integral-convergence-theorems}
\end{table}

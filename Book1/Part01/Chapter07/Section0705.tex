...
本节使用多元微积分中的导数、链式法则和沿线段的微积分基本定理。先证明一个立方体覆盖估计，再用它识别微分同胚搬运后的测度，最后推广到函数积分。D.~H. Fremlin 的《Measure Theory》\cite[Volume 2, \S263]{Fremlin2016Measure}讨论了更一般的可微变换；这里保留开集之间的 \(C^1\) 微分同胚假设，使局部误差可以直接由导数的连续性控制。非单射映射的重数与较低正则性的情形留到\chapref{b1:ch:17}。

\subsection{线性换元}
\label{b1:subsec:070501}

以下 \(m_d\) 表示 \(\mathbb R^d\) 上的 Lebesgue 测度。写 \(\mathrm{d}x\) 时即指对它积分；积分区域上的可测性均相对于 Lebesgue \(\sigma\)-代数的迹。

\begin{proposition}\label{b1:prop:affine-integral-change}
设 \(T\) 是可逆的实 \(d\times d\) 矩阵，\(a\in\mathbb R^d\)，且 \(E\subseteq\mathbb R^d\) 是 Lebesgue 可测的。对任意非负可测函数 \(g:\mathbb R^d\rightarrow[0,\infty]\)，
\[
\int_{TE+a}g(y)\,\mathrm{d}y
=|\det T|\int_E g(Tx+a)\,\mathrm{d}x.
\]
...
\[
\int_{FE}\mathbf1_B(y)\,\mathrm{d}y
=m_d\bigl(F(E\cap F^{-1}B)\bigr)
...
\[
\int_{\mathbb R^d}h(Ax)\,\mathrm{d}x
=|\det A|^{-1}\int_{\mathbb R^d}h(y)\,\mathrm{d}y.
\]
...
设 \(U,V\subseteq\mathbb R^d\) 为开集，\(\Phi:U\rightarrow V\) 为双射。若 \(\Phi\) 和 \(\Phi^{-1}\) 都连续可微，则称 \(\Phi\) 为一个 \(C^1\) \term[weifen-tongpei]{微分同胚}[diffeomorphism]。
\end{definition}

由链式法则，
\[
D\Phi^{-1}\bigl(\Phi(x)\bigr)D\Phi(x)=I_d,
\]
所以 \(D\Phi(x)\) 处处可逆。局部导数可逆与全局单射是不同的条件；例如 \((r,\theta)\mapsto(r\cdot \cos\theta,r\cdot \sin\theta)\) 在 \(r>0\) 时的导数可逆，但允许 \(\theta\) 遍历整个实轴便会重复覆盖。
...
\(\|Ax\|_\infty\leq\|A\|_\infty\cdot\|x\|_\infty\)。映射 \(F:D\rightarrow\mathbb R^d\) 若满足
\(\|F(x)-F(y)\|_\infty\leq L\|x-y\|_\infty\)，就满足常数为 \(L\) 的\term[lipschitz-tiaojian]{Lipschitz 条件}[Lipschitz condition]。本节只使用这一具体条件；一般度量空间中的系统讨论见\chapref{b1:ch:16}。
...
\[
m_d^*\bigl(F(E)\bigr)\leq L^d \cdot m_d^*(E).
\]
...
\[
m_d^*\bigl(F(E)\bigr)\leq L^d\sum_jm_d(Q_j)+\eta.
\]
...
\[
\Phi(y)-\Phi(x)=\int_0^1D\Phi\bigl(x+t(y-x)\bigr)(y-x)\,\mathrm{d}t,
\]
于是 \(\Phi\) 在 \(B\) 上满足某个有限常数的 Lipschitz 条件。若 \(N\subseteq U\) 为零测集，\cref{b1:lem:cube-lipschitz-distortion}说明每个 \(\Phi(N\cap B)\) 都为零测集。可数覆盖给出 \(m_d^*\bigl(\Phi(N)\bigr)=0\)。对 \(\Phi^{-1}\) 在 \(V\) 上重复这一论证，得到反向结论。

若 \(E\subseteq U\) 是 Lebesgue 可测的，\cref{b1:thm:lebesgue-completion-borel}给出一个相对 Borel 集 \(B\subseteq U\)，使 \(E\mathbin{\triangle}B\) 为零测集。因为 \(\Phi^{-1}\) 连续，集合 \(\Phi(B)=(\Phi^{-1})^{-1}(B)\) 是 \(V\) 中的 Borel 集；而
\(\Phi(E)\mathbin{\triangle}\Phi(B)=\Phi(E\mathbin{\triangle}B)\) 为零测集。因此 \(\Phi(E)\) 是 Lebesgue 可测的。逆映射的情形相同。
\end{proof}
...
\[
J_{\Psi\circ\Phi}(x)=J_\Psi\bigl(\Phi(x)\bigr)J_\Phi(x),
\quad
J_{\Phi^{-1}}\bigl(\Phi(x)\bigr)=J_\Phi(x)^{-1}.
\]
...
\[
\nu(E)=m_d\bigl(\Phi(E)\bigr),\quad E\subseteq U\ \text{Lebesgue 可测},
\]
在 \(U\) 的 Lebesgue 迹 \(\sigma\)-代数上定义一个 \(\sigma\)-有限正测度。记 \((m_d)_U\) 为\cref{b1:def:subspace-measure}意义下的 Lebesgue 子空间测度，则
\[
\nu\ll(m_d)_U,
\qquad
\frac{\mathrm d\nu}{\mathrm d(m_d)_U}=J_\Phi
\quad\text{在 }U\text{ 上 }(m_d)_U\text{-几乎处处}.
\]
等价地，对每个 Lebesgue 可测集 \(E\subseteq U\)，都有
\[
\nu(E)=\int_E J_\Phi(x)\,\mathrm{d}x.
\]
\end{theorem}

证明分三步。先把像集体积看成 \(U\) 上的测度，并用 Radon--Nikodym 定理写成未知密度 \(h\)。再在每一点 \(a\) 用 \(A=D\Phi(a)\) 归一化映射，使 \(A^{-1}(\Phi-\Phi(a))\) 在小球上接近恒等映射，由此同时控制小球内每个可测子集的像体积。最后把集合估计转成 \(h\) 的几乎处处上下界，以可数子覆盖统一局部例外集，再令误差趋于零。

\begin{proof}
像集的可测性已由\cref{b1:prop:diffeomorphism-measurability}证明。由于 \(\Phi\) 单射，它保持不交并，所以 \(\nu\) 可数可加。用 \(U\) 中可数个紧集 \(K_j\) 覆盖 \(U\)，每个 \(\Phi(K_j)\) 紧且有界，故 \(\nu(K_j)<\infty\)。同一覆盖也说明 \((m_d)_U\) 为 \(\sigma\)-有限测度；上述命题则给出 \(\nu\ll(m_d)_U\)。由\cref{b1:thm:radon-nikodym}，存在非负可测密度 \(h\)，使
\(\nu(E)=\int_Eh\,\mathrm d(m_d)_U=\int_Eh\,\mathrm{d}x\)。
...
\(S(x)=A^{-1}\bigl(\Phi(x)-\Phi(a)\bigr)\)。沿球内线段积分可得
\[
\|S(x)-S(y)-(x-y)\|_\infty
...
\[
(1-\varepsilon)^d\cdot J_\Phi(a)\cdot m_d(E)
\leq\nu(E)
\leq(1+\varepsilon)^d\cdot J_\Phi(a)\cdot m_d(E).
\]
...
证明中的近似并不是对某一个小球计算比值极限，而是在一个小邻域内同时控制所有可测子集。这样的统一估计使密度能够由水平集判定，也避免了此处提前使用\chapref{b1:ch:14}的测度微分定理。

\subsection{换元公式}
\label{b1:subsec:070504}

\begin{theorem}[\(C^1\) 微分同胚的换元公式]\label{b1:thm:c1-change-of-variables}
设 \(\Phi:U\rightarrow V\) 是开集之间的 \(C^1\) 微分同胚，\(E\subseteq U\) 是 Lebesgue 可测的。若 \(g:V\rightarrow[0,\infty]\) 是 Lebesgue 可测的，则
\[
\int_{\Phi(E)}g(y)\,\mathrm{d}y
=\int_E g\bigl(\Phi(x)\bigr)J_\Phi(x)\,\mathrm{d}x.
\]
...
\[
\int_{\Phi(E)}\mathbf1_B(y)\,\mathrm{d}y
=m_d\bigl(\Phi(E\cap\Phi^{-1}B)\bigr)
=\int_E\mathbf1_B\bigl(\Phi(x)\bigr)\cdot J_\Phi(x)\,\mathrm{d}x.
\]
...
\quad
\Phi(r,\theta)=(r\cdot \cos\theta,r\cdot \sin\theta).
\]
...
\]
删去的射线是二维零测集。因此，对非负或可积的 Lebesgue 可测函数 \(g\)，\cref{b1:thm:c1-change-of-variables}、\cref{b1:thm:tonelli}与\cref{b1:thm:fubini}给出
\[
\int_{\mathbb R^2}g(x,y)\,\mathrm{d}x\,\mathrm{d}y
=\int_0^{2\pi}\int_0^\infty
g(r\cdot \cos\theta,r\cdot \sin\theta)\,r\,\mathrm{d}r\,\mathrm{d}\theta.
\]
...
令 \(I=\int_{\mathbb R}e^{-t^2}\,\mathrm{d}t\)。由于 \(|t|\geq1\) 时 \(e^{-t^2}\leq e^{-|t|}\)，有 \(0<I<\infty\)。先用 Tonelli 定理，再作极坐标变换，得到
\[
I^2=\int_{\mathbb R^2}e^{-(x^2+y^2)}\,\mathrm{d}x\,\mathrm{d}y
=2\pi\int_0^\infty e^{-r^2}r\,\mathrm{d}r=\pi.
\]
...
\[
m_1(V)=1,\quad \int_U|\Phi'(t)|\,\mathrm{d}t=2.
\]
若不记录覆盖重数，就不能把前者等同于后者。\chapref{b1:ch:17}的面积公式将把这项重数纳入积分；在本节的微分同胚情形，它恒等于 \(1\)。

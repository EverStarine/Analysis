...
这里，\(\sigma\)-有限性已经在集合截面公式中使用。不能仅因本段简单逼近没有再次写出它，就从定理中删除这一假设。在 \([0,1]\) 的 Borel 集上，一因子取 Lebesgue 测度 \(m\)，另一因子取计数测度 \(\#\)。对角线 \(D=\{(x,x):x\in[0,1]\}\) 闭且可测，却有
\[
...
\term[fubini-dingli]{Fubini 定理}[Fubini's theorem]用绝对可积性保证减法和交换次序均有意义。Sheldon Axler 的《Measure, Integration \& Real Analysis》\cite[Section 5B]{Axler2020Measure}采用先非负、后变号的顺序；实际计算中也宜先检查绝对值的积分，再选择方便的积分次序。

\begin{theorem}[Fubini]\label{b1:thm:fubini}
设实值或复值可测函数 \(f\) 满足 \(\int|f|\,\mathrm d\rho<\infty\)。则对 \(\mu\)-几乎每个 \(x\)，函数 \(f(x,\cdot)\) 关于 \(\nu\) 可积；对 \(\nu\)-几乎每个 \(y\)，函数 \(f(\cdot,y)\) 关于 \(\mu\) 可积。在例外可测零集上把相应的截面积分规定为零，就得到可积函数 \(F,G\)，并有
\[
\int_{X\times Y}f\,\mathrm d\rho
...
先设 \(f\) 实值。Tonelli 定理使两个函数 \(x\mapsto\int_Yf_\pm(x,y)\,\mathrm d\nu(y)\) 可测；它们在 \(N\) 外有限，其差便是 \(F\)，在 \(N\) 上另取零后仍可测。又因 \(|F|\leq A\) 几乎处处，\(F\) 可积。分别对 \(f_+\)、\(f_-\) 的 Tonelli 等式相减，所有积分均有限，得到 \(\int F\,\mathrm d\mu=\int f\,\mathrm d\rho\)。
...
将乘积测度完备化后，截面的可测性本身也需要保留例外集。这里讨论的是先取乘积、再完备化所得的 \(\overline{\mu\otimes\nu}\)，不能未经证明便把它与先将两个因子完备化、再取乘积所得的 \(\overline\mu\otimes\overline\nu\) 混同；二者的构造次序不同，相应的乘积 \(\sigma\)-代数也不必相同。D. H. Fremlin 的《Measure Theory》\cite[Volume 2, \S252]{Fremlin2016Measure}明确区分原可测空间与完备化上的截面；下面把这一点落实到本章的 \(\sigma\)-有限情形。
...
\cref{b1:tab:tonelli-fubini-comparison}概括两条定理的分工。两者共享乘积测度背景，但“非负”与“绝对可积”决定了截面积分的值域以及能否不受限制地交换次序。

\begin{table}[htbp]
\centering
\small
\renewcommand{\arraystretch}{1.17}
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{6.5em} >{\raggedright\arraybackslash}p{11.8em} >{\raggedright\arraybackslash}p{13.2em} >{\raggedright\arraybackslash}X@{}}
\toprule
\textbf{定理} & \textbf{输入条件} & \textbf{截面积分} & \textbf{结论} \\
\midrule
Tonelli & \(f\geq0\) 可测 & 处处定义，可取 \(+\infty\) & 乘积积分与两个迭代积分相等 \\
Fubini & \(f\in L^1(\mu\otimes\nu)\) & 几乎处处绝对可积 & 两种迭代积分有限且可交换 \\
\bottomrule
\end{tabularx}
\caption[Tonelli 与 Fubini]{Tonelli 与 Fubini。前者先处理非负质量，后者以 \(|f|\) 的可积性排除正负无穷抵消。}
\label{b1:tab:tonelli-fubini-comparison}
\end{table}

...
设 \(\nu\) 是 \((X,\mathcal A)\) 上的符号测度。称有限正测度
\[
|\nu|\coloneqq\nu^++\nu^-
\]
为 \(\nu\) 的\term[quan-biancha-cedu]{全变差测度}[total variation measure]。
\symidx{\(\lvert\nu\rvert\)：测度的全变差；正测度时为其自身}
\end{definition}

为统一后文的记号，对任意正测度 \(\rho\)，无论其总质量是否有限，都约定
\[
|\rho|\coloneqq\rho.
\]
若 \(\rho\) 有限，这与把它视为负部为零的符号测度所得的全变差一致；若 \(\rho(X)=\infty\)，则这是把同一记号自然延伸到通常的正测度，而不是声称它属于本章的有限实值符号测度空间。
...
\subsection{有限变差约定与积分诱导测度}
\label{b1:subsec:060202}

当 \(|\nu|(X)<\infty\) 时，称 \(\nu\) 具有\term[youxian-biancha]{有限变差}[finite variation]。按本章的实值约定，每个符号测度都自动满足这一条件，因为\cref{b1:thm:jordan-decomposition}已经证明 \(\nu^+\)、\(\nu^-\) 均有限。这里保留“有限变差”一词是为了与采用更广定义的文献衔接，并未在本章的符号测度中再划出一个子类，也不构成额外假设。对允许取 \(+\infty\) 的一般正测度，有限性则是实质限制：实直线上的 Lebesgue 测度的变差仍是它本身，总变差为无穷。
...
\end{proposition}
\begin{proof}
由\cref{b1:thm:integral-induced-measure}，\(g_+\mu\) 与 \(g_-\mu\) 是有限正测度。它们的差等于 \(\nu\)，故 \(\nu\) 有限且可数可加。选取可测的实值代表 \(g\)，令 \(P=\{g\geq0\}\)、\(N=\{g<0\}\)。可测子集上的积分保序性说明 \(P\)、\(N\) 分别是 \(\nu\) 的正集、负集，所以它们构成 Hahn 分解。按 Jordan 分解的构造，\(\nu^+(E)=\int_{E\cap P}g\,\mathrm d\mu=\int_Eg_+\,\mathrm d\mu\)，负部也有相应等式。两者相加得到全变差公式。
\end{proof}
...
\subsection{符号测度空间的范数}
\label{b1:subsec:060203}

全体实值符号测度按逐集相加与实数乘法构成向量空间，记为 \(\mathcal M(X,\mathcal A;\mathbb R)\)。有限实线性组合保持可数可加性；这里不会出现无穷值相减。定义\term[quan-biancha-fanshu]{全变差范数}[total variation norm]为
\[
...
上述公式是 \(\mathcal M(X,\mathcal A;\mathbb R)\) 上的范数。对符号测度 \(\nu,\eta\) 和实数 \(a\)，还有逐集不等式
\[
|\nu+\eta|\leq|\nu|+|\eta|,\quad |a\nu|=|a|\cdot|\nu|.
\]
\end{proposition}
...
\[
s(\nu)=\max\{\nu^+(X),\nu^-(X)\},\quad
s(\nu)\leq\|\nu\|_{\mathrm{TV}}\leq2\cdot s(\nu).
\]
若 \(\nu(X)=0\)，则 \(\|\nu\|_{\mathrm{TV}}=2\cdot s(\nu)\)。
\end{proposition}
\begin{proof}
...
若范数空间中每个 Cauchy 列都在该范数下收敛于空间内的元素，就称该空间\term[wanbei]{完备}[complete]。这里 Cauchy 条件指：对每个 \(\varepsilon>0\)，存在 \(N\)，使 \(n,m\geq N\) 时 \(\|\nu_n-\nu_m\|_{\mathrm{TV}}<\varepsilon\)。下面直接验证测度空间的完备性；其一般理论将在\cref{b1:ch:08}讨论。

\ProseParagraphBreak

Sheldon Axler 的《Measure, Integration \& Real Analysis》\cite[Section 9A]{Axler2020Measure}中把实测度与复测度统一放进这一范数框架。以下证明只使用实数的完备性、有限分割和正测度的从上连续性。
...
对既有密度加原子的例子，\cref{b1:fig:visual-f03}同时显示抵消与全变差的区别。
\input{Figures/F03}

...
设 \((X,\mathcal A)\) 是可测空间，\(\nu:\mathcal A\rightarrow\mathbb C\) 为集合函数。若 \(\nu\) 可数可加，即对任意两两不交的可测集合列 \((E_n)\)，数值级数收敛，且
\[
\nu\left(\bigcup_{n=1}^{\infty}E_n\right)
=\sum_{n=1}^{\infty}\nu(E_n),
\]
则称 \(\nu\) 为\term[fu-cedu]{复测度}[complex measure]。
\end{definition}
...
复测度 \(\nu\) 的全变差由下式定义：
\[
|\nu|(E)\coloneqq\sup_{\mathcal P}\sum_{A\in\mathcal P}|\nu(A)|,
\]
...
先确定由复值密度产生的测度，其变差如何计算。Sheldon Axler 的《Measure, Integration \& Real Analysis》\cite[Chapter 9]{Axler2020Measure}中以简单函数逼近证明这一公式；下面只逼近密度的方向，使模长保留在积分中。
...
\end{lemma}
\begin{proof}
若各 \(E_n\) 两两不交，则 \(h\cdot\mathbf1_{\bigcup_{n\leq m}E_n}\) 逐点趋于 \(h\cdot\mathbf1_{\bigcup_nE_n}\)，其模不超过可积函数 \(|h|\)。由\cref{b1:thm:dominated-convergence}，有限可加式可取极限，故 \(\nu\) 可数可加。对每个有限分割使用积分三角不等式，立即得到 \(|\nu|(E)\leq\int_E|h|\,\mathrm d\mu\)。

为证明下界，在 \(h\neq0\) 处令 \(v=\overline h/|h|\)，在 \(h=0\) 处令 \(v=1\)。给定 \(\varepsilon>0\)，取足够多个等距分布的单位复数 \(c_1,\ldots,c_N\)，使单位圆上每一点都距其中某一个小于 \(\varepsilon\)。将每个点分配给首个满足 \(|v(x)-c_j|<\varepsilon\) 的序号，得到有限可测分割 \((A_j)\)。简单函数 \(q=\sum_jc_j\cdot\mathbf1_{A_j}\) 满足 \(|q-v|<\varepsilon\)，从而
\[
...
=\sum_{j=1}^{N}\operatorname{Re}\bigl(c_j\cdot\nu(E\cap A_j)\bigr)
\leq\sum_{j=1}^{N}|\nu(E\cap A_j)|
\leq|\nu|(E).
\]
...
\[
E_n^+=\{\operatorname{Re}(u-w)>1/n\}.
\]
若 \(|\nu|(E_n^+)>0\)，则
\[
\operatorname{Re}\int_{E_n^+}(u-w)\,\mathrm d|\nu|
=\int_{E_n^+}\operatorname{Re}(u-w)\,\mathrm d|\nu|
\geq\frac1n|\nu|(E_n^+)>0,
\]
与该复积分等于零矛盾。因此 \(E_n^+\) 是零测集。对 \(\{\operatorname{Re}(u-w)<-1/n\}\) 使用相反的不等式，再对虚部的正、负水平集作同样处理；最后对正整数 \(n\) 取可数并，得到 \(u=w\) 在 \(|\nu|\)-几乎处处成立。
\end{proof}
...
设 \(\mathrm d\nu=u\,\mathrm d|\nu|\) 是极分解，\(f\) 为可测复函数。若 \(f\in L^1(|\nu|)\)，则称 \(f\) 关于 \(\nu\) 可积，并规定
\[
\int f\,\mathrm d\nu\coloneqq\int fu\,\mathrm d|\nu|.
\]
\end{definition}

...
设 \(X\) 是一个非空集合，\(\mathcal P(X)\) 表示 \(X\) 的幂集，并用 \(\varnothing\) 表示空集。以下各集合系都是 \(\mathcal P(X)\) 的子集；未特别说明时，补集均相对于 \(X\) 而言。
\symidx{\(\varnothing\)：空集}
...
设 \(\mathcal R\subseteq\mathcal P(X)\) 是非空集合族。若对任意 \(A,B\in\mathcal R\)，都有
\[
A\setminus B\in\mathcal R,
\quad
A\cup B\in\mathcal R,
\]
则称 \(\mathcal R\) 为\term[jihehuan]{集合环}[ring of sets]。
\end{definition}
...
设集合族 \(\mathcal A\subseteq\mathcal P(X)\)。若 \(X\in\mathcal A\)，且对任意 \(A,B\in\mathcal A\)，都有
\[
A^{\mathrm c}\in\mathcal A,
\quad
A\cup B\in\mathcal A,
\]
则称 \(\mathcal A\) 为\term[jihedaishu]{集合代数}[algebra of sets]。
\end{definition}
...
\subsection{\texorpdfstring{\(\sigma\)}{\AnalysisPdfSigma}-代数}
\label{b1:subsec:010103}

分析中的逼近通常要进行可数多步。若允许的集合只对有限并封闭，取极限时就可能离开原有集合系。而 \(\sigma\)-代数正是把有限封闭提升到可数封闭所得的结构。

\begin{definition}
\label{b1:def:sigma-algebra}
设集合族 \(\mathcal F\subseteq\mathcal P(X)\)。若它满足：
\begin{enumerate}
...
\end{enumerate}
则称 \(\mathcal F\) 为 \term[sigma-daishu]{\(\sigma\)-代数}[sigma-algebra]。
\end{definition}
...
\[
\mathcal F_1=\bigl\{\varnothing,\{1\},\{2,3\},X\bigr\},
\quad
\mathcal F_2=\bigl\{\varnothing,\{2\},\{1,3\},X\bigr\}.
\]

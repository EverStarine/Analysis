...
由集合 \(X\) 与其上的 \(\sigma\)-代数 \(\mathcal A\) 组成的二元组 \((X,\mathcal A)\) 称为\term[kece-kongjian]{可测空间}[measurable space]。而 \(\mathcal A\) 的成员称为该空间的\term[kece-ji]{可测集}[measurable set]。
\end{definition}
...
设 \((X,\mathcal A)\) 是可测空间，\(E\subseteq X\) 是任意子集。所谓 \(E\) 上的\term[ji-sigma-daishu]{迹 \(\sigma\)-代数}[trace sigma-algebra]定义为
\[
\mathcal A|_E
\coloneqq\{\,A\cap E:A\in\mathcal A\,\}.
\]
\end{definition}
...
设 \((X,\mathcal A)\)、\((Y,\mathcal B)\) 是可测空间，\(f:X\to Y\) 是映射。若
\[
f^{-1}(B)\in\mathcal A
\quad
\text{对每个 }B\in\mathcal B\text{ 成立},
\]
则称 \(f\) 为\term[kece-yingshe]{可测映射}[measurable map]。
\end{definition}
...
设 \(A\subseteq X\)。示性函数
\(
\mathbf 1_A:(X,\mathcal A)\to\bigl(\mathbb R,\mathcal B(\mathbb R)\bigr)
\)
...
若 \(\mathbf 1_A\) 可测，则
\(
A=\mathbf 1_A^{-1}\bigl((1/2,3/2)\bigr)\in\mathcal A
\)。反之，若 \(A\in\mathcal A\)，对任意 Borel 集 \(B\subseteq\mathbb R\)，反像 \(\mathbf 1_A^{-1}(B)\) 只能是 \(\varnothing,A,A^{\mathrm c},X\) 四者之一，因而属于 \(\mathcal A\)。
\end{proof}

\subsection{值域取 Borel 结构的可测映射}
\label{b1:subsec:010403}

设 \((X,\mathcal A)\) 是可测空间，\(T\) 是拓扑空间。若映射
\[
f:(X,\mathcal A)\longrightarrow\bigl(T,\mathcal B(T)\bigr)
\]
可测，就称它为 \(T\) 值可测映射，并明确约定值域取 Borel \(\sigma\)-代数；这个说法并不要求源空间来自拓扑。只有当源空间也是拓扑空间 \(S\)，并赋予 \(\mathcal B(S)\) 时，才称 \(f:S\to T\) 为\term[borel-kece]{Borel 可测映射}[Borel measurable map]；当 \(T=\mathbb R\) 时，相应地称为实值 Borel 可测函数。连续映射必然属于后一类，但可测性只记录集合的反像，不要求保持邻近关系，所以它比连续性弱得多。
...
映射 \(f:(X,\mathcal A)\to\bigl(\mathbb R,\mathcal B(\mathbb R)\bigr)\) 可测，当且仅当对每个 \(q\in\mathbb Q\)，
\[
...
\end{proposition}
\begin{proof}
若 \(f\) 可测，则每个开半直线的反像都可测，必要性成立。反过来，有理开半直线生成 \(\mathcal B(\mathbb R)\)，所以充分性由\cref{b1:prop:measurable-generator-test}得到。
...
这个阈值判别也补全了\cref{b1:thm:functional-monotone-class}与本节术语之间的接口：该定理中的反像条件正是实值映射在值域取 Borel 结构时的可测性。

\ProseParagraphBreak

从可测映射、阈值判别到乘积可测结构的另一套紧凑叙述，可参阅 Gerald B. Folland 的《Real Analysis: Modern Techniques and Their Applications》\cite{Folland1999Real}。本书在此只建立后续复值函数与有限维值域所需的乘积结构。
...
设 \((X,\mathcal A)\)、\((Y,\mathcal B)\) 是可测空间。所谓 \(X\times Y\) 上由全部可测矩形 \(A\times B\)（其中 \(A\in\mathcal A\)、\(B\in\mathcal B\)）生成的 \(\sigma\)-代数
\[
\mathcal A\otimes\mathcal B
\coloneqq\sigma\bigl(\{\,A\times B:A\in\mathcal A,\ B\in\mathcal B\,\}\bigr)
\]
...
\coloneqq\sigma\left(\left\{\,\prod_{j=1}^{d}A_j:A_j\in\mathcal A_j\,\right\}\right).
\]
...
\quad
z\longmapsto\bigl(f(z),g(z)\bigr)
\]
可测，当且仅当 \(f\) 与 \(g\) 都可测。
\end{proposition}
\begin{proof}
...
\)
可测，当且仅当每个分量 \(f_j:(X,\mathcal A)\to\bigl(\mathbb R,\mathcal B(\mathbb R)\bigr)\) 可测。
\end{proposition}
\begin{proof}
...
若 \(f\) 可测，则每个分量 \(f_j=p_j\circ f\) 可测。反过来，若每个 \(f_j\) 可测，则任一生成矩形的反像为
\[
...
例如，若实值映射 \(f,g:(X,\mathcal A)\to\bigl(\mathbb R,\mathcal B(\mathbb R)\bigr)\) 可测，则配对映射 \((f,g)\) 可测。连续函数
\[

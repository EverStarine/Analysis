...
必须注意，\(\sigma(\mathcal E)\) 一般不能通过有限多次补与并得到。即使 \(\mathcal E\) 很简单，可数运算仍可能产生无限层次；生成定义给出的首先是最小性，而不是一张可执行的有限运算清单。第\ref{b1:sec:0103}节因此不再尝试枚举生成所得的集合，而是建立从生成元向整个结构推广性质的比较方法。

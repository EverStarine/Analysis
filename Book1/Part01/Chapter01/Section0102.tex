% !TeX root = ../../Book1.tex
\section{生成的 \texorpdfstring{\(\sigma\)}{\AnalysisPdfSigma}-代数}
\label{b1:sec:0102}
...
\subsection{由集合族生成的 \texorpdfstring{\(\sigma\)}{\AnalysisPdfSigma}-代数}
...
\[
\sigma(\mathcal E)
\coloneqq\bigcap\{\,\mathcal F:\mathcal F\text{ 是 }X\text{ 上的 \(\sigma\)-代数，且 }\mathcal E\subseteq\mathcal F\,\}.
\]
...
\[
\sigma(\mathcal E)
\subseteq\sigma\bigl(\sigma(\mathcal G)\bigr)
=\sigma(\mathcal G),
\]
...
必须注意，\(\sigma(\mathcal E)\) 一般不能通过有限多次补与并得到。即使 \(\mathcal E\) 很简单，可数运算仍可能产生无限层次；生成定义给出的首先是最小性，而不是一张可执行的有限运算清单。\secref{b1:sec:0103}因此不再尝试枚举生成所得的集合，而是建立从生成元向整个结构推广性质的比较方法。

\subsection{初始 \texorpdfstring{\(\sigma\)}{\AnalysisPdfSigma}-代数}
...
\[
\sigma(f_i:i\in I)
\coloneqq\sigma\bigl(\{\,f_i^{-1}(B):i\in I,\ B\in\mathcal B_i\,\}\bigr).
\]
\end{definition}

\secref{b1:sec:0104}将把“所有上述反像都属于源空间的 \(\sigma\)-代数”称为映射 \(f_i\) 的可测性。这里的定义本身只使用已经建立的反像与生成运算，不依赖这一术语。
...
“可数生成”不等于“只有可数多个成员”。例如，只要 \(X\) 含有可数个互不相同的点 \(x_n\)，由单点集 \(\{x_n\}\) 生成的 \(\sigma\)-代数就包含
\[
\bigcup_{n\in J}\{x_n\}
\quad(J\subseteq\mathbb N).
\]
不同的 \(J\) 给出不同的集合，所以这个 \(\sigma\)-代数至少有 \(2^{\aleph_0}\) 个成员。可数性约束的是生成信息，而不是生成结果的基数。
...
\subsection{Borel \texorpdfstring{\(\sigma\)}{\AnalysisPdfSigma}-代数}
...
同一论证表明，\(\mathbb R^d\) 的 Borel \(\sigma\)-代数由有理端点开矩形生成，因为这些矩形构成可数拓扑基。\secref{b1:sec:0104}将利用这一事实识别欧氏空间的乘积结构。

\begin{proposition}
\label{b1:prop:real-initial-countable}
若 \(f:X\to\mathbb R\)，则相对于目标空间 \(\bigl(\mathbb R,\mathcal B(\mathbb R)\bigr)\) 有
\[
\sigma(f)
=\sigma\Bigl(\{\,f^{-1}\bigl(( -\infty,q)\bigr):q\in\mathbb Q\,\}\Bigr).
\]
...
这个证明展示了一种以后会反复使用的“生成元判别”：先把具有目标性质的集合收集成一个 \(\sigma\)-代数，再检查它包含生成族。\secref{b1:sec:0103}将把这一策略推广到闭性更弱、因而更容易验证的集合类。

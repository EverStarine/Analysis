% !TeX root = ../../Book1.tex
\section{\texorpdfstring{\(\pi\)}{\AnalysisPdfPi}-系统、\texorpdfstring{\(\lambda\)}{\AnalysisPdfLambda}-系统与单调类}
\label{b1:sec:0103}
...
\subsection{\texorpdfstring{\(\pi\)}{\AnalysisPdfPi}-系统}
...
\subsection{Dynkin \texorpdfstring{\(\lambda\)}{\AnalysisPdfLambda}-系统}
\label{b1:subsec:010302}

而 \(\sigma\)-代数要求任意可数并封闭，很多等式却只能自然地对两两不交的集合求和。Dynkin \(\lambda\)-系统把公理调整为“嵌套差与不交可数并”，因而特别适合承载这类性质。

\begin{definition}
\label{b1:def:lambda-system}
设集合族 \(\mathcal D\subseteq\mathcal P(X)\)。若它满足：
\begin{enumerate}
...
\end{enumerate}
则称 \(\mathcal D\) 为\term[dynkin-lambda-xitong]{Dynkin \(\lambda\)-系统}[Dynkin lambda-system]。
\end{definition}
...
\subsection{\texorpdfstring{\(\pi\)}{\AnalysisPdfPi}-\texorpdfstring{\(\lambda\)}{\AnalysisPdfLambda} 定理}
\label{b1:subsec:010303}

\begin{theorem}[\(\pi\)--\(\lambda\)]
...
而 \(\lambda\)-系统仍然要求处理两两不交的可数并。有些极限论证天然只对递增列或递减列成立，这就引出更弱的单调闭性。
...
单调类本身可能非常弱。例如在有限集合 \(X=\{1,2\}\) 上，集合族 \(\bigl\{\{1\}\bigr\}\) 已是单调类，因为其中的单调集合列只能是常值列；但它既不含 \(X\)，也不对补封闭。因此，下一定理要求起始生成族是集合代数，而不能是任意集合族。
...
集合型定理可以转化为函数上的推广工具。本小节的陈述会先使用下一节才系统说明的实值可测性，其中值域取 Borel \(\sigma\)-代数；第一次阅读时可以先掌握结论，读完\subsecref{b1:subsec:010403}后再返回核对证明。为避免预先使用后章才系统发展的简单函数理论，下面把所需逼近直接写成有限个示性函数之和。

\begin{theorem}[函数型单调类]
...
\[
s_n(x)=2^{-n}\left\lfloor2^n\cdot f(x)\right\rfloor,
\]
...
(f+c)^{-1}(B)=f^{-1}(B-c),
\]
平移 \(x\mapsto x-c\) 是 \(\mathbb R\) 的同胚，故 \(B-c\) 仍是 Borel 集。因此 \(f+c\) 满足同一反像条件。由非负情形，\(f+c\in\mathcal H\)。又因 \(c\cdot\mathbf 1_X\in\mathcal H\)，向量空间性质给出
\(
f=(f+c)-c\cdot\mathbf 1_X\in\mathcal H
\)。
\end{proof}

定理中的反像条件将在下一节表述为实值映射在值域取 Borel 结构时的可测性。它的典型用法是：先对集合代数中的示性函数验证一个等式，再检查等式对线性组合和有界单调极限保持，便能把等式推广到所有有界可测函数。该证明只使用集合论闭性，不依赖积分理论。

\ProseParagraphBreak

本章出现的几类集合系各自只保留后续论证真正需要的闭性。
\cref{b1:tab:set-system-closures}把它们放在同一张表中；表中的“核心用途”说明为什么不总是一开始就取最强的 \(\sigma\)-代数。

\begin{table}[htbp]
\centering
\small
\renewcommand{\arraystretch}{1.16}
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{7.5em} >{\raggedright\arraybackslash}p{15.2em} >{\raggedright\arraybackslash}X@{}}
\toprule
\textbf{集合系} & \textbf{定义性闭性} & \textbf{核心用途} \\
\midrule
集合代数 & 补集、有限并 & 有限集合运算与前测度的原始定义域 \\
\(\sigma\)-代数 & 补集、可数并 & 测度和可测映射的完整定义域 \\
\(\pi\)-系统 & 有限交 & 从简单生成元启动唯一性论证 \\
Dynkin \(\lambda\)-系统 & 全集、嵌套差、不交可数并 & 承载由可数可加性保持的集合性质 \\
单调类 & 递增并、递减交 & 从有限结构推广到单调极限 \\
\bottomrule
\end{tabularx}
\caption[集合系的闭性]{集合系的闭性。每一行列出的只是该类的定义性要求；由这些要求还能推出的闭性在相应命题中证明。}

...
许多自然生成族不对补或并封闭，却对有限交封闭。区间相交仍是区间，矩形相交仍是矩形；这一条简单闭性正是后面论证所需的有限结构。
...
集合型定理可以转化为函数上的推广工具。为避免预先使用后章才系统发展的简单函数理论，下面把所需逼近直接写成有限个示性函数之和。
...
(f+c)^{-1}(B)=f^{-1}(B-c),
\]
平移与补集、可数并交换，而且把开集送到开集；因此满足 \(B-c\in\mathcal B(\mathbb R)\) 的集合 \(B\) 组成一个包含全部开集的 \(\sigma\)-代数。于是 \(B-c\) 仍为 Borel 集，故 \(f+c\) 满足同一反像条件。由非负情形，\(f+c\in\mathcal H\)。又因 \(c\mathbf 1_X\in\mathcal H\)，向量空间性质给出
\(

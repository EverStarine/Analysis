...
\subsection{\texorpdfstring{\(\sigma\)}{\AnalysisPdfSigma}-有限测度}
...
至于 \(\sigma\)-有限性，它并不把总质量变成有限，而是提供一种可数的有限质量局部化。\subsecref{b1:subsec:020403}将看到，它恰好允许我们把有限测度情形的唯一性论证逐块推广到整个空间；没有这一条件，延拓可能不唯一。

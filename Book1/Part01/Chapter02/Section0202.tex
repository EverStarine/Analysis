...
\]
集合 \(B_n\) 可测且两两不交。由于 \(E_n\cap E_k=\varnothing\)，每个交集 \(A_n\cap A_k\) 都包含在 \(Z_n\cup Z_k\) 中；因此 \(A_n\setminus B_n\) 是零测集，\(\mu(B_n)=\mu(A_n)\)。此外，\(\bigcup_nE_n\) 与 \(\bigcup_nB_n\) 的对称差包含在可数个上述零测集之并中。由定义良好与 \(\mu\) 的可数可加性，
\[

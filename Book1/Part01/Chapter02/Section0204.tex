...
这段证明中有三处定义域转换：前测度 \(p\) 定义在 \(\mathcal R\)，外测度 \(p^*\) 定义在 \(\mathcal P(X)\)，所得延拓 \(\mu_{\mathrm C}\) 定义在 \(\sigma(\mathcal R)\)。把三者都写成同一个字母，容易掩盖“哪一步已经具有可数可加性”以及“哪一个集合可以代入函数”。
...
本书用\term*[caratheodory-yantuo-dingli]{Carathéodory 延拓定理}[Carathéodory extension theorem]强调外测度与可测性筛选的构造过程，并把存在性、唯一性合并后的表述称为\term*[hahn-kolmogorov]{Hahn--Kolmogorov 定理}[Hahn--Kolmogorov theorem]。
...
\begin{enumerate}
  ...
  \item\label{b1:item:semiring-intersection} \(\mathcal S\) 对有限交封闭；
...
设 \(\mathcal S\) 是半环，\(p\) 是 \(\mathcal S\) 上的前测度。则 \(\mathcal R(\mathcal S)\) 是集合环，而且公式
\[

...
设 \(\mathcal R\) 是 \(X\) 上的集合环或集合代数，并给定函数
\[
p:\mathcal R\longrightarrow[0,\infty].
\]
若 \(p(\varnothing)=0\)，并且每当 \((A_n)_{n\geq1}\subseteq\mathcal R\) 两两不交且 \(\bigcup_nA_n\in\mathcal R\) 时，都有
\[
p\left(\bigcup_{n=1}^{\infty}A_n\right)
=\sum_{n=1}^{\infty}p(A_n),
\]
则称 \(p\) 为\term[qian-cedu]{前测度}[premeasure]。
\end{definition}
...
% 三列按内容分配宽度：对象列最窄，定义域列略宽，作用列吸收余量并可收缩，
% 避免 X 列独占剩余宽度而在右侧留下大片空白。
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{4.6em} >{\raggedright\arraybackslash}p{8.2em} >{\raggedright\arraybackslash}X@{}}
\toprule
...
\bottomrule
\end{tabularx}
\caption[测度延拓的四层对象]{测度延拓的四层对象。各行依次区分原始前测度、覆盖外测度、Carathéodory 测度及目标可测域上的延拓。}
\label{b1:tab:measure-extension-objects}
\end{table}

\subsection{\texorpdfstring{\(\sigma\)}{\AnalysisPdfSigma}-有限条件下的唯一性}
...
\begin{align*}
\mu\bigl((B\setminus A)\cap X_n\bigr)
&=\mu(B\cap X_n)-\mu(A\cap X_n)\\
&=\nu(B\cap X_n)-\nu(A\cap X_n)\\
&=\nu\bigl((B\setminus A)\cap X_n\bigr).
\end{align*}
故 \(B\setminus A\in\mathcal D_n\)。若 \((E_j)\subseteq\mathcal D_n\) 两两不交，则 \((E_j\cap X_n)\) 也两两不交，由两个延拓的可数可加性，
\begin{align*}
\mu\Biggl(\biggl(\bigcup_jE_j\biggr)\cap X_n\Biggr)
&=\sum_j\mu(E_j\cap X_n)\\
&=\sum_j\nu(E_j\cap X_n)
=\nu\Biggl(\biggl(\bigcup_jE_j\biggr)\cap X_n\Biggr).
\end{align*}
...
设集合族 \(\mathcal S\subseteq\mathcal P(X)\)。若它满足：
\begin{enumerate}
...
\end{enumerate}
则称 \(\mathcal S\) 为\term[banhuan]{半环}[semiring]。
\end{definition}
...
设 \(\mathcal S\) 是半环，并给定函数 \(p:\mathcal S\to[0,\infty]\)。若 \(p(\varnothing)=0\)，并且每当 \((S_n)_{n\geq1}\subseteq\mathcal S\) 两两不交、其并 \(S=\bigcup_nS_n\) 仍属于 \(\mathcal S\) 时，都有
\[
p(S)=\sum_{n=1}^{\infty}p(S_n),
\]
则称 \(p\) 为 \(\mathcal S\) 上的前测度。
\end{definition}
...
=\sum_{k=1}^{m}p(S_k)
\]
定义了 \(\mathcal R(\mathcal S)\) 上唯一的前测度延拓 \(\widetilde p\)。因此 \(\widetilde p\) 至少有一个到 \(\sigma(\mathcal S)=\sigma\bigl(\mathcal R(\mathcal S)\bigr)\) 的测度延拓；若 \(X\) 可由可数个有限 \(p\)-值的 \(\mathcal S\) 中集合覆盖，则该测度延拓唯一。
\end{theorem}
\begin{proof}
...
\sigma(\mathcal S)\subseteq\sigma\bigl(\mathcal R(\mathcal S)\bigr).
\]
反过来，\(\mathcal R(\mathcal S)\) 的每个成员都是有限个 \(\mathcal S\) 中集合的并，故属于 \(\sigma(\mathcal S)\)。因此
\[
\mathcal R(\mathcal S)\subseteq\sigma(\mathcal S)
\quad\Longrightarrow\quad
\sigma\bigl(\mathcal R(\mathcal S)\bigr)\subseteq\sigma(\mathcal S).
\]
两边结合便得到
\(
\sigma(\mathcal S)=\sigma\bigl(\mathcal R(\mathcal S)\bigr)
\)。
...
由此，常见构造并不需要在每一层重新发明测度：先在半环上核验几何大小的可数可加性，再唯一地通过有限不交并扩到集合环，最后用 Carathéodory 构造延拓到生成的 \(\sigma\)-代数。\chapref{b1:ch:03}将以半开长方体及其体积为原始数据，完成 Lebesgue 测度的具体构造。

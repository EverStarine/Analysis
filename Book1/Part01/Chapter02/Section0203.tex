...
下一节将把这个思想用于尚未成为测度的集合函数：先在集合环上给出前测度，再以覆盖生成外测度。Carathéodory 定理正是把覆盖数据转化为测度的中间桥梁。Terence Tao 的《An Introduction to Measure Theory》\cite{Tao2011Introduction}可用来对照外测度与 Carathéodory 可测性的分工；本书则会特别追踪怎样从前测度出发，一步步扩大定义域。

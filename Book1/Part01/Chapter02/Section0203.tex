...
定义要求对所有测试集 \(A\) 成立，而不只要求对 \(A=X\) 成立。前面那个“非空集外测度恒为 \(1\)”的例子说明 Carathéodory 可测域可能很小：若 \(\varnothing\neq E\neq X\)，仅取 \(A=X\) 已经得到 \(1\neq1+1\)，所以只有 \(\varnothing\) 与 \(X\) 可测。不过，这个例子并不能说明“只检验 \(A=X\)”为什么不够，因为其中的失败已经在 \(X\) 上出现。

真正显示全体测试集不可省略，可以取
\[
X=\{a,b,c\},
\quad
\mathcal C=\{\varnothing,\{a\},\{b\},\{c\},\{a,b\},\{a,c\}\},
\]
并令 \(c(\varnothing)=0\)，其余 \(C\in\mathcal C\) 的代价均为 \(c(C)=1\)。由\cref{b1:prop:covering-outer-measure}得到覆盖外测度 \(\mu^*\)。直接计算可知
\[
\mu^*(X)=2,
\qquad
\mu^*(\{a,b\})=\mu^*(\{a,c\})=1,
...
对 \(E=\{b\}\)，测试 \(A=X\) 时确有
\[
\mu^*(X)=2=\mu^*(E)+\mu^*(X\setminus E),
\]
但取 \(A=\{a,b\}\) 就得到
\[
\mu^*(A)=1<2=\mu^*(A\cap E)+\mu^*(A\setminus E).
\]
因此 \(E\) 仍非 Carathéodory 可测；只检验全集会漏掉这个失败。
...
\mu^*(A)
={}&\mu^*(A\cap E\cap F)
+\mu^*\bigl((A\cap E)\setminus F\bigr)\\
&+\mu^*\bigl((A\setminus E)\cap F\bigr)
+\mu^*\bigl((A\setminus E)\setminus F\bigr).
\end{align*}
后三个集合的并恰为 \(A\setminus(E\cap F)\)。由可数次可加性，
\[
\mu^*(A)
\geq\mu^*(A\cap E\cap F)+\mu^*\bigl(A\setminus(E\cap F)\bigr).
\]

...
测度的定义预先要求一个 \(\sigma\)-代数，但在几何构造中，哪些集合可测往往不是事先知道的。外测度把顺序倒过来：先允许测量 \(X\) 的每个子集，代价是暂时只要求可数次可加性；随后再筛出那些能够把任意测试集合无损切开的集合。这个筛选同时给出可测集族和真正的测度。
...
下一节将把这个思想用于尚未成为测度的集合函数：先在集合环上给出前测度，再以覆盖生成外测度。Carathéodory 定理正是把覆盖数据转化为测度的中间桥梁。

...
例如在 \(E=[0,1]\) 上令 \(f_n(x)=x^n\)。它逐点收敛于函数
\[
f(x)=
\begin{cases}
0,&0\leq x<1,\\
...
逐点收敛缺少的正是这种统一尾部控制。在 \(x^n\) 的例子中，每个
\(x<1\) 都有自己的收敛速度；当 \(x\) 越靠近 \(1\)，使 \(x^n\) 变小所需的 \(n\) 越大，因而不存在适用于整个区间的指标。
...
A=(1-\delta,1].
\]
集合 \(A\) 的测度为 \(\delta<\varepsilon\)，而在
\(E\setminus A=[0,1-\delta]\) 上，极限函数恒为零，并且
\[
\sup_{x\in[0,1-\delta]}|f_n(x)-f(x)|
\leq(1-\delta)^n\longrightarrow0.
\]
例外集删去了收敛速度最慢、不断向端点 \(1\) 靠近的区域。

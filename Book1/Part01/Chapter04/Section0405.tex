...
这里不能省去对给定极限 \(f\) 的可测性要求。仍设 \(\mu(E)<\infty\)，若实值或复值可测函数列 \((f_n)\) 几乎处处收敛于同值域函数 \(f\)，但未预先假设 \(f\) 可测，则取可测零集 \(Z\subseteq E\)，使收敛在 \(E\setminus Z\) 上逐点成立，并令
\[
g(x)=
\begin{cases}
...
\end{cases}
\]
由逐点极限保持可测性及\cref{b1:lem:measurable-pasting}，\(g\) 是可测函数，且 \(g=f\) 几乎处处。因此\cref{b1:cor:egoroff-ae}给出 \(f_n\to g\) 几乎一致。若原测度空间完备，则由\cref{b1:thm:complete-null-modification}，给定的 \(f\) 本身可测，也就可以直接对 \(f_n\to f\) 应用上述推论。

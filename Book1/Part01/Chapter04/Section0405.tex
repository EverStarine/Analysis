...
逐点收敛允许收敛开始的时刻随空间点改变，一致收敛则要求同一个时刻对所有点有效。两者通常相差很远。对于实值或复值可测函数列，有限测度使这种差距可以集中到任意小的例外集上：舍去该集合以后，逐点收敛能够提升为一致收敛。
...
回到\subsecref{b1:subsec:040203}的 \(f_n(x)=x^n\)：它在 \([0,1]\) 上逐点收敛于 \(f=\mathbf 1_{\{1\}}\)，但
\[
...
\(\bigl(f_n(x)\bigr)\) 是 \(\mathbb R\) 或 \(\mathbb C\) 中的 Cauchy 列，由完备性收敛到某个值，记为 \(f(x)\)。给定 \(\eta>0\)，在一致 Cauchy 条件中把误差取为 \(\eta/2\)，得到 \(N\)。若 \(m\geq N\)，则对所有 \(n\geq N\) 和 \(x\in E\) 都有
\[
...
在处处逐点收敛的前提下，若只假设每个 \(f_n\) 可测，极限函数 \(f\) 的可测性也会由逐点极限保持可测性得到；定理把它列入假设，是为了让所有误差集合的可测性在陈述中直接可见。整个证明没有使用 \(X\) 上的拓扑，也不要求函数连续。与 Lusin 定理不同，Egoroff 定理是纯测度论结论。
...
在有限测度集 \(E\) 上，对实值或复值可测函数 \(f_n,f\)，几乎处处收敛与几乎一致收敛等价：一个方向已由\cref{b1:prop:almost-uniform-implies-ae}给出，另一个方向由下面的推论得到。前者允许先舍去一个零测集，后者则要求每个给定的正误差预算都留下一个一致收敛的大集合。下面把零测例外并入 Egoroff 定理。

\begin{corollary}[Egoroff，几乎处处版本]
...
这里需要区分给定极限的可测性与大集上的一致收敛本身。仍设 \(\mu(E)<\infty\)，若实值或复值可测函数列 \((f_n)\) 几乎处处收敛于同值域函数 \(f\)，但未预先假设 \(f\) 可测，则取可测零集 \(Z\subseteq E\)，使收敛在 \(E\setminus Z\) 上逐点成立，并令
\[
g(x)=
\begin{cases}
\lim_{n\to\infty}f_n(x),&x\in E\setminus Z,\\
0,&x\in Z.
\end{cases}
\]
由逐点极限保持可测性及\cref{b1:lem:measurable-pasting}，\(g\) 是可测函数，且 \(g=f\) 于 \(E\setminus Z\)。对 \(g\) 应用\cref{b1:cor:egoroff-ae}：给定 \(\varepsilon>0\)，存在可测集 \(A_0\subseteq E\)，使 \(\mu(A_0)<\varepsilon\)，且 \(f_n\to g\) 在 \(E\setminus A_0\) 上一致。令 \(A=A_0\cup Z\)，则 \(\mu(A)<\varepsilon\)，而 \(f=g\) 于 \(E\setminus A\)，所以 \(f_n\to f\) 在同一个 \(E\setminus A\) 上一致。

\ProseParagraphBreak

因此，给定 \(f\) 的可测性并不是上述大集一致收敛现象所必需的。它影响的是本书的术语使用：依照\cref{b1:def:almost-uniform-convergence}，“几乎一致收敛”只用于可测函数列及可测极限。在不完备空间中，给定的 \(f\) 可能不可测，此时可以说 \(f_n\) 几乎一致收敛于与 \(f\) 几乎处处相等的可测代表 \(g\)，并另行保留刚证明的“大集上一致收敛到原函数 \(f\)”的结论。若原测度空间完备，则由\cref{b1:thm:complete-null-modification}，给定的 \(f\) 本身可测，也就可以直接称 \(f_n\to f\) 几乎一致。

\ProseParagraphBreak

对几乎处处有限的扩展实值可测函数，可以先删去无穷值所在的公共零集，再在余下集合上应用上述推论。若每个
\(f_n:E\rightarrow\overline{\mathbb R}\) 及极限 \(f\) 都可测且几乎处处有限，并且
...
至此，Lusin 定理与 Egoroff 定理的分工也清楚了：前者用欧氏正则性把可测函数限制为大集合上的连续函数，后者在有限测度集上，把实值或复值可测函数 \(f_n,f\) 之间的几乎处处收敛加强为几乎一致收敛。Walter Rudin 的《Real and Complex Analysis》\cite{Rudin1987Real}给出了较紧凑的对照叙述；阅读时仍要把这两组假设分开核对。

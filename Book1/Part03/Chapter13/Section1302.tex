...
证明先处理有限球族，再把所得相交数界用于无穷选择。有限几何引理的路线是
\[
 \begin{aligned}
 \text{相交旧球}
 &\longrightarrow\text{把中心径向压入固定球}
 \longrightarrow\text{证明压缩后仍统一分离}\\
 &\longrightarrow\text{用体积装箱限制球数}.
 \end{aligned}
\]
随后按近最大半径贪心选球，并用有限种颜色把相交数界转成不交分族。\cref{b1:fig:besicovitch-radial-compression}示意第一层中的径向压缩；图只帮助辨认几何对象，具体常数仍由下面的估计给出。

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[scale=1.05,line cap=round,line join=round]
  \coordinate (o) at (0,0);
  \coordinate (xi) at (2.65,1.05);
  \coordinate (pi) at (1.95,0.77);
  \draw[thick] (o) circle (1);
  \draw[dashed] (o) circle (2.1);
  \draw[thick] (xi) circle (2);
  \draw[gray] (o)--(2.95,1.17);
  \draw[->,thick] (2.50,0.99)--(2.08,0.82);
  \fill (o) circle (1.4pt);
  \fill (xi) circle (1.4pt);
  \fill (pi) circle (1.4pt);
  \node[below left] at (o) {$0=x_n$};
  \node[below] at (0.55,-0.82) {$B_n$};
  \node[above right] at (xi) {$x_i$};
  \node[below right] at (pi) {$p_i$};
  \node[above left] at (-1.42,1.55) {$\overline B(0,R)$};
  \node[right] at (4.35,1.9) {$B_i$};
\end{tikzpicture}
\caption[径向压缩]{径向压缩。与末球 \(B_n\) 相交的旧球中心 \(x_i\) 若位于 \(\overline B(0,R)\) 外，就沿射线压到 \(p_i\)；证明的核心是这些压缩点仍有统一的分离距离。示意图不按比例。}
\label{b1:fig:besicovitch-radial-compression}
\end{figure}
...
下面五种情况只是在证明同一个目标 \(|p_i-p_j|>s\)：前两种处理至多一个中心被压缩且较远中心的次序固定，第三种处理相反次序，最后两种处理两个中心都被压缩时的径向远近。完成这个统一分离估计后，五种分类便不再进入后续论证。
...
\begin{proof}[\cref{b1:thm:besicovitch-covering}的证明]
...
\[
 N_{\mathcal G}(x)\coloneqq\sum_{B\in\mathcal G}\mathbf1_B(x).
\]
...
\[
 \sum_{B\in\mathcal G}\int_B g\,\mathrm{d}\mu
 \leq5^d\int_{\mathbb R^d}g\,\mathrm{d}\mu.
\]
\end{corollary}
\begin{proof}
...
\[
 \sum_{B\in\mathcal G}\int_Bg\,\mathrm{d}\mu
 =\int_{\mathbb R^d}N_{\mathcal G}g\,\mathrm{d}\mu
 \leq5^d\int_{\mathbb R^d}g\,\mathrm{d}\mu.
\]
\end{proof}
...
\secref{b1:sec:1301}中的两类选择与本节定理承担不同任务：
...
《Lectures on Geometric Measure Theory》\cite[pp. 23--24, 4.5 REMARKS and 4.6 LEMMA]{Simon1983Lectures}对读；
...
\[
 ...
 \leq\left(1-\frac1{2N}\right)\cdot\mu(A\setminus F_m).
\]
...
\[
 ...
 \leq\left(1-\frac1{2N}\right)^m\cdot\mu(A)\longrightarrow0.
\]
有限步骤停止时，同一结论已经成立。
\end{proof}
...
\secref{b1:sec:1303}将把覆盖选择用于极大函数估计；

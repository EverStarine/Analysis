...
本节先证明一般度量空间中的选择引理，再用 Lebesgue 测度的伸缩性质完成 Vitali 定理。Simon 的《Lectures on Geometric Measure Theory》\cite[\S3]{Simon1983Lectures}适合对照选择过程与尾部覆盖；Fremlin 的《Measure Theory》\cite[261B]{Fremlin2016Measure}则给出了允许球只包含待覆盖点的 Lebesgue 版本。
...
在度量空间 \((X,\rho)\) 中，记
\[
B(x,r)\coloneqq\{\,y\in X\mid \rho(x,y)<r\,\},
\quad
\overline B(x,r)\coloneqq\{\,y\in X\mid \rho(x,y)\leq r\,\}.
\]
...
\end{corollary}
\begin{proof}
取左边一点 \(x\)。有限个闭球的并是闭集，且不含 \(x\)，故有 \(\eta>0\)，使 \(B(x,\eta)\) 与该并不交。利用细覆盖，选取含 \(x\)、半径小于 \(\eta/2\) 的
\(B=\overline B(c,r)\in\mathcal F\)。这里 \(B\) 不必以 \(x\) 为中心，但对任意 \(y\in B\)，仍有
\[
 \rho(y,x)\leq\rho(y,c)+\rho(c,x)\leq2r<\eta.
\]
因此 \(B\subseteq B(x,\eta)\)，从而 \(B\) 与 \(\mathcal H\) 中的所有球不交。引理给出的 \(D\in\mathcal G\) 与 \(B\) 相交且 \(B\subseteq5D\)，所以 \(D\notin\mathcal H\)，而 \(x\in5D\)。
\end{proof}
...
\OptionalSubsection{Vitali 覆盖关系}{b1:subsec:130104}

本小节只为\chapref{b1:ch:14}准备一般度量空间中的语言，不参与\secref{b1:sec:1302}--\secref{b1:sec:1303}的主证明链；首次沿本章“覆盖选择—极大估计”主线阅读时可以略过。

\ProseParagraphBreak

上例中，一个点虽然落在许多小圆盘里，却总在圆盘的边缘。若只记录圆盘集合而不记录它围绕哪个点缩小，就会丢失这种区别。覆盖关系把点和用来观察它的集合放在一起；本小节只引入后文所需的闭集型版本。
...
\[
 \operatorname{diam}S
 \coloneqq\sup\{\,\rho(y,z)\mid y,z\in S\,\}.
\]
...
例如，若在所用中心及相关半径上已有 \(\mu\bigl(\overline B(x,2r)\bigr)\leq C_0\cdot\mu\bigl(\overline B(x,r)\bigr)\)，连续应用三次便得到五倍球估计，常数可取 \(C_0^3\)。这是额外的尺度控制，不由局部有限或紧内正则性自动提供。覆盖关系的作用正是把这种几何与测度的配合单独列为条件，供以后的测度微分论反复使用。

5r 选择中“互不相交”与“膨胀后覆盖”是两项不同要求，见\cref{b1:fig:visual-f08}。
\input{Figures/F08}

...
这一步使用的是\chapref{b1:ch:10}已经建立的连续紧支集稠密性，不预借光滑函数稠密性。\(p=\infty\) 时结论不成立：对 \(f=\mathbf1_{(0,1)}\) 和 \(0<|h|<1\)，有 \(\|\tau_hf-f\|_\infty=1\)。
...
调和分析中还会遇到不能由绝对平均直接支配的算子。某些核靠正负抵消才有意义，先把核和函数全部取绝对值反而会破坏关键性质。届时覆盖选择、幅值分解与极大估计仍会使用，但要与抵消条件配合。程民德等的《实分析》\cite[\chapref{b1:ch:03}]{Cheng2008RealAnalysis}将覆盖、极大函数、插值及奇异积分放在相邻各节，提供了一条中文阅读路径；本卷先把覆盖与微分的基础做完，第二卷再进入这些算子的系统理论。

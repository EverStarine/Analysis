...
下面的构造不把 \(\mathcal H^s\llcorner K\) 归一化，因为 \(\mathcal H^s(K)\) 可能为无穷。
...
\]
的泛函；它们都位于对偶单位球中。这些条件在弱*\WeakStarJoin{}拓扑下闭。
\cref{b1:thm:banach-alaoglu}、\cref{b1:thm:weak-star-metrizable}与 \(C(K)\) 的可分性说明，
这族泛函是紧致可度量空间，因而每个序列都有弱*\WeakStarJoin{}收敛子列。
...
由\cref{b1:lem:hausdorff-content-positive}，
\[
...
\[
 I_t(\mu)\coloneqq\iint_{\mathbb R^d\times\mathbb R^d}|x-y|^{-t}\,\mathrm{d}\mu(y)\,\mathrm{d}\mu(x),
\]
并约定对角线上的核值为 \(+\infty\)。
相应的非负函数
\[
 U_t^\mu(x)\coloneqq\int_{\mathbb R^d}|x-y|^{-t}\,\mathrm{d}\mu(y)
\]
...
记 \(M=\mu(\mathbb R^d)\)。由
\[
 u^{-t}=\int_u^\infty t r^{-t-1}\,\mathrm{d}r
\]
及 Tonelli 定理，包括 \(u=0\) 的无穷值情形，得到
\begin{equation}\label{b1:eq:energy-ball-integral}
 I_t(\mu)=t\int_0^\infty r^{-t-1}
       \int_{\mathbb R^d}\mu\bigl(B(x,r)\bigr)\,\mathrm{d}\mu(x)\,\mathrm{d}r.
\end{equation}
在 \(0<r<1\) 用增长估计，在 \(r\geq1\) 用总质量估计，即得
\[
 I_t(\mu)\leq tAM\int_0^1r^{s-t-1}\,\mathrm{d}r+
 tM^2\int_1^\infty r^{-t-1}\,\mathrm{d}r
...
\[
 \nu\bigl(B(z,r)\bigr)
 \leq(2r)^t\int_{B(z,r)}|x-y|^{-t}\,\mathrm{d}\mu(y)
 \leq 2^t n r^t.
\]
若交集为空，左边为零，同一估计仍成立。
对正质量测度 \(\nu\) 使用质量分布原理，便得
...
\(\int_0^1r^{-1}\,\mathrm{d}r\) 当成有限会得出错误结论。

...
\[
h^t_\delta(E)\leq\delta^{t-s}\cdot h^s_\delta(E),
\quad
...
\[
h^t_\delta(E)\leq\delta^{t-s}\cdot h^s(E)\longrightarrow0
...
\[
\dim_{\mathrm H}E
\coloneqq\inf\{s\geq0:\mathcal H^s(E)=0\},
\]
约定一般意义下 \(\inf\varnothing=+\infty\)。特别地，由于
\(\mathcal H^s(\varnothing)=0\) 对所有 \(s\geq0\) 成立，定义中取下确界的集合为
\([0,\infty)\)，故 \(\dim_{\mathrm H}\varnothing=0\)。
...
\]
第二个断言在 \(d_E<\infty\) 时由下确界定义证明：给定 \(t>d_E\)，可取 \(u<t\) 使 \(\mathcal H^u(E)=0\)，再用\cref{b1:prop:hausdorff-critical-exponent}。若第一个断言失败，即某个 \(s<d_E\) 的测度有限，则在 \(s<t<d_E\) 处测度已经为零，与 \(d_E\) 的定义矛盾。这也说明
\[
\dim_{\mathrm H}E
...
Lipschitz 映射不增大 Hausdorff 维数。事实上，若 \(t>\dim_{\mathrm H}E\)，则 \(\mathcal H^t(E)=0\)，\cref{b1:prop:hausdorff-lipschitz}给出 \(\mathcal H^t\bigl(f(E)\bigr)=0\)，再令 \(t\) 下降到原集合的维数即可；常值映射单独由像至多为单点处理。给定单射 \(f:E\to Y\)，若存在 \(0<a\leq b<\infty\) 满足
\[
a\,\mathrm{d}_X(x,y)\leq \mathrm{d}_Y\bigl(f(x),f(y)\bigr)
\leq b\,\mathrm{d}_X(x,y)
\quad(x,y\in E),
\]
...
\[
m_d^*(E)\leq2^d \cdot h^d_\delta(E),
\quad
\mathcal H^d(E)\geq c_d2^{-d}\cdot m_d^*(E).
\]
...
\cref{b1:prop:cantor-null}已经证明三分 Cantor 集不可数而 Lebesgue 测度为零。现在考虑一族类似集合。固定 \(0<\lambda<1/2\)，从 \([0,1]\) 开始，在每个保留区间的左右两端各保留原长度 \(\lambda\) 倍的闭区间。记第 \(n\) 轮的并集为 \(K_{\lambda,n}\)，并令
\[
...
则
\[
\mathcal H^s(E)\geq\frac{c_s\cdot\nu(E)}{C}>0,
\quad
...
若希望继续计算三分 Cantor 集在原始归一化下的精确测度值，可阅读 Fremlin 的《Measure Theory》\cite[264J]{Fremlin2016Measure}。本节保留足以确定维数的上下界，使同一证明能处理全部 \(0<\lambda<1/2\)。下一节转向欧氏空间的精确测度识别；\secref{b1:sec:1504}则进一步讨论，什么时候可以从集合的覆盖性质反过来构造具有球质量控制的测度。

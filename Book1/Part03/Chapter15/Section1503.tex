...
% 264I的球覆盖比较改接本书13.1 Vitali，另由\chapref{b1:ch:03}立方体覆盖补全零测遗漏。
...
前两个小节给出等直径不等式的完整几何证明，属于第二遍阅读模块。
首次阅读可以先接受\cref{b1:thm:isodiametric}，从\subsecref{b1:subsec:150303}继续；
后两小节对 \(\mathcal H^d\) 的归一化与后文面积公式仍是主线内容。

\OptionalSubsection{等直径问题}{b1:subsec:150301}

沿用\secref{b1:sec:1303}的记号
...
下面按 Fremlin 的《Measure Theory》\cite[264H and notes to \S264]{Fremlin2016Measure}，
把集合逐坐标对称化。每一步都保持或增大体积，同时不增加直径。
只需 \(d\) 步，就能得到一个关于原点中心对称的集合。

\OptionalSubsection{欧氏球的极值性质}{b1:subsec:150302}
...
\cref{b1:fig:steiner-symmetrization}画出若干代表性截面。对称化只把每条竖直截面平移为
以 \(t=0\) 为中心的等长区间；证明的任务是说明这一操作保持体积且不增加直径。

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[scale=0.92,line cap=round,line join=round]
  \begin{scope}[xshift=-3.5cm]
    \draw[thick,smooth cycle]
      plot coordinates {(-1.55,-0.20) (-1.20,0.95) (-0.25,1.15)
                        (0.55,0.45) (1.35,0.62) (1.58,-0.38)
                        (0.62,-1.02) (-0.45,-0.78)};
    \draw[dashed] (-1.8,0)--(1.8,0);
    \draw[very thick] (-1,0.00)--(-1,0.90);
    \draw[very thick] (0,-0.70)--(0,0.50);
    \draw[very thick] (1,-0.80)--(1,0.50);
    \node[above] at (0,1.45) {原集合 \(F\)};
    \node[below] at (0,-1.38) {位置不同的竖直截面 \(F_y\)};
  \end{scope}
  \draw[->,thick] (-1.25,0)--(1.25,0)
    node[midway,above,align=center] {保持每条\\截面的长度};
  \begin{scope}[xshift=3.5cm]
    \draw[thick,smooth cycle]
      plot coordinates {(-1.55,0) (-1.20,0.62) (-0.25,0.78)
                        (0.60,0.82) (1.35,0.58) (1.58,0)
                        (1.35,-0.58) (0.60,-0.82) (-0.25,-0.78)
                        (-1.20,-0.62)};
    \draw[dashed] (-1.8,0)--(1.8,0);
    \draw[very thick] (-1,-0.45)--(-1,0.45);
    \draw[very thick] (0,-0.60)--(0,0.60);
    \draw[very thick] (1,-0.65)--(1,0.65);
    \node[above] at (0,1.45) {对称化后的集合 \(D\)};
    \node[below] at (0,-1.38) {以 \(t=0\) 为中心的截面 \(D_y\)};
  \end{scope}
\end{tikzpicture}
\caption[Steiner 对称化]{Steiner 对称化。图中粗线仅示意三条截面：右侧截面分别与左侧对应截面等长，但都以 \(t=0\) 为中心。}
\label{b1:fig:steiner-symmetrization}
\end{figure}
...
\begin{proof}[\cref{b1:thm:isodiametric}的证明]
...
先使用\secref{b1:sec:1501}未归一化的覆盖量 \(h_\delta^d\) 与 \(h^d\)。
...
\secref{b1:sec:1501}对所有实数 \(s\geq0\) 统一采用
\[
 \mathcal H^s=c_sh^s,\quad
 c_s=\frac{\pi^{s/2}}{2^s\cdot \Gamma(1+s/2)}.
\]
...
对每个正整数 \(d\)，有
\[
 \omega_d\cdot \Gamma(1+d/2)=\pi^{d/2},\quad c_d=\frac{\omega_d}{2^d}.
\]
\end{proposition}
\begin{proof}
\subsecref{b1:subsec:070504}已用二维换元证明

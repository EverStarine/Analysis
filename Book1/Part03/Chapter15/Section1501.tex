...
例如，\(\mathbb R\) 上的 \(\mathcal H^0\) 在任何非空开区间上都为无穷。故这个测度是 Borel 可测的，这并不意味着局部有限，更不能据此直接把任意 Hausdorff 测度称为第\ref{b1:ch:11}章意义下的 Radon 测度。另一方面，覆盖允许使用任意集合并不会妨碍从外部找到一个具有相同测度的 Borel 集。

...
本节的度量空间构造可与 Fremlin 的《Measure Theory》\cite[471A--E and 471J--L]{Fremlin2016Measure}对读。该书使用直径幂的原始归一化；本书稍后乘一个只依赖指数的正常数，以便整数维的测度与通常的长度、面积、体积相衔接。两种约定不会改变零测集和维数，但比较具体测度值时必须保留这个常数。

\subsection{直径覆盖与尺度}
\label{b1:subsec:150101}

固定度量空间 \((X,\mathrm{d})\)。集合 \(U\subseteq X\) 的直径为 \(\operatorname{diam}U=\sup\{\mathrm{d}(x,y):x,y\in U\}\)，约定空集直径为零，允许无界集合的直径为无穷。下面的覆盖集不要求开、闭或可测；它们只承担几何覆盖的作用。
...
\[
h^s_\delta(E)
\coloneqq\inf\Bigl\{\sum_{i=1}^{\infty}(\operatorname{diam}U_i)^s:
...
由尺度的单调性，可以定义极限
\[
h^s(E)\coloneqq\lim_{\delta\downarrow0}h^s_\delta(E)
...
\[
\Gamma(a)\coloneqq\int_0^\infty t^{a-1}e^{-t}\,\mathrm dt.
\]
...
给定 \(s\geq0\)，令
\[
c_s\coloneqq\frac{\pi^{s/2}}{2^s\cdot \Gamma(1+s/2)},
\quad
\mathcal H^s_\delta(E)\coloneqq c_s \cdot h^s_\delta(E),
\quad
\mathcal H^s(E)\coloneqq c_s \cdot h^s(E).
\]
称 \(\mathcal H^s\) 为 \(s\) 维\allowbreak\term[hausdorff-ce-du]{Hausdorff 外测度}[Hausdorff outer measure]。把它限制到 Carathéodory 可测集上得到的测度，称为 \(s\) 维\allowbreak\term[hausdorff-cedu]{Hausdorff 测度}[Hausdorff measure]。下面将证明所有 Borel 集都在其可测域内。讨论 Borel 测度时，我们也把相应的限制记为 \(\mathcal H^s\)。
\symidx{\(\mathcal H^s\)：归一化的 Hausdorff 测度及其外测度}
\symidx{\(\mathcal H^s_\delta\)：归一化的尺度覆盖量}
\symidx{\(c_s\)：Hausdorff 测度的归一化常数}
\end{definition}

对任意集合写 \(\mathcal H^s(E)\) 时，尚未声明其可测性也有意义，此时指外测度值。整数 \(d\geq1\) 时，常数将满足 \(c_d=\omega_d/2^d\)，其中 \(\omega_d\) 是 \(\mathbb R^d\) 中单位球的体积；这一识别及 \(\mathcal H^d=m_d\) 的证明放在\secref{b1:sec:1503}。当前构造只需要 \(0<c_s<\infty\) 与 \(c_0=1\)。
...
\[
\sum_{i=1}^{\infty}\theta(A_i)\leq2\cdot \theta(E)<\infty.
\]
...
\]
闭集因此 Carathéodory 可测。由\cref{b1:thm:caratheodory}，可测集组成 \(\sigma\)-代数，包含闭集便包含全部 Borel 集；同一定理也给出可数可加性与完备性。取 \(\theta=\mathcal H^s\) 即得结论。
\end{proof}
...
例如，\(\mathbb R\) 上的 \(\mathcal H^0\) 在任何非空开区间上都为无穷。故这个测度是 Borel 可测的，这并不意味着局部有限，更不能据此直接把任意 Hausdorff 测度称为\chapref{b1:ch:11}意义下的 Radon 测度。另一方面，覆盖允许使用任意集合并不会妨碍从外部找到一个具有相同测度的 Borel 集。
...
\[
h^s_{L\delta,Y}\bigl(f(E)\bigr)\leq L^s \cdot h^s_{\delta,X}(E).
\]
...
尺度与覆盖代价的关系可由\cref{b1:fig:visual-f10}中的有限阶段观察。
\input{Figures/F10}

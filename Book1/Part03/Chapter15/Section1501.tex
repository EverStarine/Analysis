...
每个固定的 \(h^s_\delta\) 都是外测度。空集与单调性直接来自定义。为说明可数次次可加性，设 \(E=\bigcup_jE_j\)。若 \(\sum_jh^s_\delta(E_j)=\infty\)，不等式自动成立；否则，对每个 \(j\) 取代价不超过 \(h^s_\delta(E_j)+\varepsilon2^{-j}\) 的覆盖，将所有这些覆盖合并，得到
\[
...
\leq\sum_jh^s(E_j),
\]
再令 \(\delta\downarrow0\)。这里直接核对了可数次次可加性，并未使用“一族外测度的上确界总是外测度”这样的断言。
...
给定 \(s\geq0\)，令
\[
c_s\coloneqq\frac{\pi^{s/2}}{2^s\Gamma(1+s/2)},
\quad
\mathcal H^s_\delta(E)\coloneqq c_s h^s_\delta(E),
\quad
\mathcal H^s(E)\coloneqq c_s h^s(E).
\]

...
后者提供平面上密度为 \(1\) 的基准。\chapref{b1:ch:12}给出了概率测度弱紧性的更一般理论；
为使本章的首次阅读路线自足，Frostman 证明所需的紧集版本将在\secref{b1:sec:1504}内单独证明。
首次阅读可以先沿“测度定义—Cantor 维数—质量分布—Frostman 引理”这一顺序掌握维数估计。
等直径不等式的对称化证明标为第二遍阅读内容；接受其结论后，可直接进入整数维归一化，
再按需要回读几何证明及密度估计中的常数。

\ProseParagraphBreak

Fremlin 的《Measure Theory》\cite[\S264]{Fremlin2016Measure}与 Simon 的
《Lectures on Geometric Measure Theory》\cite[\S\S3--4]{Simon1983Lectures}%
分别提供测度构造与几何密度的参考。
...
% \chapref{b1:ch:15}习题临时稿；合入章聚合文件已有的 BookExercises 之后。
% 十一题均自拟，来源是本章工具的变式；不冒称取自尚未读到全文的教材习题。
% 统一采用 H^s=c_s h^s，H^s_infty 同系数；不另建标题或书签。

\begin{exercise}[覆盖量与基本维数]\label{b1:ex:15-basic-covering-dimensions}
令 \(I=[0,1]\)。
\begin{enumerate}
\item\label{b1:item:15-interval-one-content}
证明对每个 \(0<\delta\leq1\)，都有 \(h^1_\delta(I)=1\)。
\item\label{b1:item:15-interval-critical-exponent}
证明当 \(0<s<1\) 时，\(h^s_\delta(I)\geq\delta^{s-1}\)；
当 \(s>1\) 时，\(h^s_{1/n}(I)\leq n^{1-s}\)。由此求出
\(\mathcal H^s(I)\) 在 \(s<1\)、\(s=1\)、\(s>1\) 时的值，并求 \(\dim_{\mathrm H}I\)。
\item\label{b1:item:15-finite-countable-dimension}
求非空有限集和可数无限集的 Hausdorff 维数，并分别说明临界指数 \(s=0\) 处的测度值。
\end{enumerate}
\end{exercise}
% 来源：自拟；直接训练固定尺度覆盖量、临界指数与零维计数约定。
\begin{solution}
任取 \(I\) 的可数 \(\delta\)-覆盖 \((U_j)\)。每个有界非空 \(U_j\subset\mathbb R\)
都包含在一个长度为 \(\operatorname{diam}U_j\) 的闭区间中，故 Lebesgue 外测度的次可加性给出
\[
 1=m_1(I)\leq\sum_j\operatorname{diam}U_j.
\]
另一方面，取整数 \(N\) 使 \(1/N\leq\delta\)，用 \(N\) 个长度为 \(1/N\) 的闭区间覆盖 \(I\)，
总代价为 \(1\)。因此 \(h^1_\delta(I)=1\)。

若 \(0<s<1\)，则对每个直径不超过 \(\delta\) 的非空覆盖块，
\[
 (\operatorname{diam}U_j)^s
 =\operatorname{diam}U_j\,(\operatorname{diam}U_j)^{s-1}
 \geq\delta^{s-1}\operatorname{diam}U_j.
\]
对覆盖求和并取下确界，得到 \(h^s_\delta(I)\geq\delta^{s-1}\)，
所以 \(\mathcal H^s(I)=\infty\)。若 \(s>1\)，把 \(I\) 等分为 \(n\) 段即得
\(h^s_{1/n}(I)\leq n(1/n)^s=n^{1-s}\)，从而 \(\mathcal H^s(I)=0\)。
又因 \(c_1=1\)，第一问给出 \(\mathcal H^1(I)=1\)，故 \(\dim_{\mathrm H}I=1\)。

由\cref{b1:prop:hausdorff-counting}，含 \(N\) 个点的集合满足 \(\mathcal H^0=N\)，
可数无限集满足 \(\mathcal H^0=\infty\)，而二者在每个 \(s>0\) 下都为零测集。
因此所有非空有限集和可数无限集的 Hausdorff 维数均为零。
\end{solution}

\begin{exercise}[一次质量分布估计]\label{b1:ex:15-basic-mass-distribution}
令 \(\lambda=m_1\llcorner[0,1]\)。证明对任意 \(x\in\mathbb R\)、\(r>0\)，
\[
 \lambda\bigl(\overline B(x,r)\bigr)\leq2r.
\]
用质量分布原理推出 \(\mathcal H^1([0,1])\geq1/2\) 与
\(\dim_{\mathrm H}[0,1]\geq1\)，再用显式覆盖证明反向维数不等式。
说明这里的常数为何没有给出精确的一维测度，却仍给出了精确维数。
\end{exercise}
% 来源：自拟；在最简单的 Lebesgue 测度上直接使用质量分布原理。
\begin{solution}
闭球 \(\overline B(x,r)=[x-r,x+r]\) 的长度为 \(2r\)，故其与 \([0,1]\) 的交集长度至多为 \(2r\)。
质量分布原理取 \(s=1\)、\(C=2\)、\(c_1=1\)，得到
\[
 \mathcal H^1([0,1])\geq\frac{\lambda([0,1])}{2}=\frac12,
 \qquad \dim_{\mathrm H}[0,1]\geq1.
\]
对任意 \(t>1\)，用 \(n\) 个长度为 \(1/n\) 的区间覆盖，总代价
\(c_tn^{1-t}\to0\)，所以 \(\mathcal H^t([0,1])=0\) 且维数不超过 \(1\)。
球估计使用了对所有中心都成立的粗常数 \(2\)，因而只给出测度的非锐下界；
但常数不影响增长指数，所以维数下界仍然锐利。
\end{solution}

\begin{exercise}[上密度、下密度与密度极限]\label{b1:ex:15-basic-density-oscillation}
令
\[
 E=\bigcup_{j=0}^{\infty}\bigl(2^{-(2j+1)},2^{-2j}\bigr]\subset(0,1],
 \qquad \mu=m_1\llcorner E.
\]
证明
\[
 \Theta_*^1(\mu,0)=\frac16,
 \qquad
 \Theta^{*1}(\mu,0)=\frac13,
\]
从而 \(\Theta^1(\mu,0)\) 不存在。证明中不仅要计算半径 \(2^{-N}\) 的两列比值，
还要控制位于相邻两个二进尺度之间的所有半径。
\end{exercise}
% 来源：自拟；用交替二进环带区分上、下密度与密度极限存在性。
\begin{solution}
由于 \(\alpha_1=2\) 且 \(E\subset(0,1]\)，所需比值为
\[
 R(r)=\frac{m_1(E\cap(0,r])}{2r}.
\]
固定 \(N\geq0\)，令 \(a=2^{-(N+1)}\)，并取 \(a\leq r\leq2^{-N}\)。
若 \(N\) 为偶数，则区间 \((a,2^{-N}]\) 包含于 \(E\)，而更小尺度处的总质量为 \(a/3\)。因此
\[
 R(r)=\frac{r-a+a/3}{2r}
 =\frac12-\frac{2^{-N}}{6r},
\]
它在该区间上从 \(1/6\) 增至 \(1/3\)。若 \(N\) 为奇数，则
\((a,2^{-N}]\cap E=\varnothing\)，而 \((0,a]\) 中的总质量为 \(2^{-N}/3\)，故
\[
 R(r)=\frac{2^{-N}}{6r},
\]
它从 \(1/3\) 减至 \(1/6\)。所以所有充分小半径的比值都在
\([1/6,1/3]\) 内，而偶数、奇数 \(N\) 的端点尺度分别达到两个端值。
这便给出所述上、下密度，并说明共同极限不存在。
\end{solution}
...
以下 \(\overline D=[0,1]\)。任意可数集合覆盖 \([0,1]\) 时，每个有界非空覆盖集都可放进同直径的闭区间；由 Lebesgue 外测度的次可加性，这些直径之和至少为 \(1\)。
...
\begin{optionalexercise}\label{b1:ex:15-snowflake-metric}
...
这并不违反\cref{b1:ex:15-bilipschitz-norms}：恒等映射虽然是同胚，却不满足双边 Lipschitz 估计。
...
\begin{optionalexercise}\label{b1:ex:15-holder-graphs}
...
\begin{optionalexercise}\label{b1:ex:15-zero-dimensional-compact}
令
\[
...
这证明连续性。而 \([0,1]\) 中每个数都有二进展开，因而 \(F\) 满射；不同点可能由于二进展开的非唯一性而有相同像，但不影响满射结论。
...
\begin{optionalexercise}\label{b1:ex:15-energy-choice}
...
\begin{optionalexercise}\label{b1:ex:15-separated-similarities}
...
\[
 \Phi_n(t)=S_{w_n(t)}(x_*).
\]
每个 \(\Phi_n\) 在有限个半开区间上分别取常值，故是 Borel 可测的。
...
因此 \(\Phi\) 是这些 Borel 映射的一致极限，也是 Borel 可测的，且取值于 \(K\)。

定义 \(\mu=\Phi_\#(m_1\llcorner[0,1))\)。这是集中在 \(K\) 上的 Borel 概率测度。
...
\begin{optionalexercise}\label{b1:ex:15-cantor-product}
...
这时 \(\mathbb R^{k+1}\) 中半径为 \(r\) 的球包含于它的各坐标投影的直积，而后 \(k\) 个坐标的投影包含在一个边长 \(2r\) 的立方体内，故
\[
 \nu\bigl(B((x,y),r)\bigr)

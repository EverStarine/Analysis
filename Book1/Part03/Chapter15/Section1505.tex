...
沿用 \(c_s\) 的约定，记
\[
 \alpha_s\coloneqq2^s c_s=\frac{\pi^{s/2}}{\Gamma(1+s/2)}.
\]
对正整数 \(k\)，\secref{b1:sec:1503}证明了 \(\alpha_k=\omega_k\)，
...
\[
 \Theta^{*s}(\mu,x)\coloneqq
 \limsup_{r\downarrow0}\frac{\mu\bigl(\overline B(x,r)\bigr)}{\alpha_s r^s},
 \quad
 \Theta_*^s(\mu,x)\coloneqq
 \liminf_{r\downarrow0}\frac{\mu\bigl(\overline B(x,r)\bigr)}{\alpha_s r^s}.
\]
若二者相等，则把共同值记为 \(\Theta^s(\mu,x)\)，称为 \(s\) 维密度。
对 Borel 集 \(E\) 及局部有限测度 \(\mathcal H^s\llcorner E\)，还记
\[
 \Theta^{*s}(E,x)\coloneqq\Theta^{*s}(\mathcal H^s\llcorner E,x),
 \quad
 \Theta_*^s(E,x)\coloneqq\Theta_*^s(\mathcal H^s\llcorner E,x).
\]
\end{definition}
...
\end{theorem}
\begin{proof}
令 \(\mu=\mathcal H^s\llcorner E\)。它是有限 Borel 测度，
由欧氏空间中有限 Borel 测度的正则性，亦为 Radon 测度。
若 \(\mu(E)=0\)，结论自动成立，以下允许 \(\mu(E)>0\)。

证明分成两个彼此独立的方向。上界从平均过大的坏点出发，借助开集外逼近和
Radon 细覆盖抽取不交球，再把局部不等式求和；下界则把所有充分小球质量都过低的点分组，
直接对任意 Hausdorff 细覆盖求和。前者依赖选择出一族好球，后者回到覆盖量的定义。
...
\]
这些集是 Borel 集。固定有理数 \(r>0\) 时，
\(x\mapsto\mu\bigl(\overline B(x,r)\bigr)\) 是 Borel 函数，
所以只要求上式对 \(0<r<1/n\) 的有理半径成立，得到的是可数个 Borel 集的交。
这一有理半径条件与原条件等价：给定任意 \(0<r<1/n\)，可取有理数
\(r_j\downarrow r\)，且 \(r<r_j<1/n\)。这时
\[
 \overline B(x,r_j)\downarrow\overline B(x,r).
\]
由于 \(\mu\) 有限，测度的从上连续性给出
\(\mu(\overline B(x,r_j))\downarrow\mu(\overline B(x,r))\)，
同时 \(r_j^s\to r^s\)，故有理半径上的不等式可传到每个半径 \(r\)。
...
于是
\[
 \mu(A_n)\leq t\cdot2^s\cdot c_s\cdot\sum_j a_j^s.
\]
先对覆盖取下确界，再令 \(\delta\downarrow0\)，得到
\[
 \mu(A_n)\leq t\cdot2^s\cdot\mathcal H^s(A_n)=t\cdot2^s\cdot\mu(A_n).
\]
由 \(t\cdot2^s<1\) 及 \(\mu(A_n)<\infty\)，知 \(\mu(A_n)=0\)。
...
\cite[\S3]{Simon1983Lectures}讨论了这类密度与覆盖的联系；
上面的证明利用本书前两章已有的 Radon 细覆盖，把所需有限性与可测性条件单独列明。

\ProseParagraphBreak

若只知 \(\mathcal H^s\llcorner E\) 局部有限，同一结论仍可局部使用。
...
若 \(E\subset P\) 为 Borel 集，\(\mathcal H^k\llcorner E\) 局部有限，
...
\(\nu=g\,(\mathcal H^k\llcorner P)\)，则由 \(k\) 维 Lebesgue 微分定理，
\[
 \Theta^k(\nu,x)=g(x)
...
\chapref{b1:ch:19}将正式讨论可整流性；这里先说明为什么后面还需要映射的微分。
...
再把点态线性近似整合为面积公式。这分别是\chapref{b1:ch:16}与\chapref{b1:ch:17}的任务。

...
面积公式使曲线和曲面的积分有了共同的测度论解释。本节先把曲线的行进长度与像集长度区分开，再计算球面面积，最后讨论参数选择和图像。计算中始终使用\chapref{b1:ch:15}归一化的 \(\mathcal H^k\)；参数块的选取可以改变，测量单位不能随参数化改变。
...
\OptionalSubsection{曲线长度}{b1:subsec:170401}

本小节是\secref{b1:sec:1605}度量曲线理论的选读应用。设 \(\gamma:[a,b]\to\mathbb R^n\) 为 Lipschitz 曲线。该节已经由区间分割证明
\[
...
\[
 ...
 =\int_{\mathbb R^n}h(y)\cdot N(\gamma,[a,b],y)\,
       \mathrm d\mathcal H^1(y).
\]
特别地，若 \(\gamma\) 单射，则 \(\ell(\gamma)=\mathcal H^1\bigl(\gamma([a,b])\bigr)\)。
\end{proposition}
\begin{proof}
在区间两端把曲线延为常值，得到全实线上的 Lipschitz 映射。由 \(J_1\gamma=|\gamma'|\) 几乎处处，将\cref{b1:thm:area-formula}与\cref{b1:thm:weighted-area-formula}应用于参数集 \([a,b]\)，就得到前两个等式。单射时重数是像集的示性函数，最后一个结论随即成立。
\end{proof}
...
对一般绝对连续的单射欧氏曲线，像集长度仍等于行进长度。事实上，由\cref{b1:prop:arc-length-parameterization}写成 \(\gamma=\eta\circ s\)，其中 \(s\) 是累计变差。单射性保证 \(s\) 严格递增，否则某个非退化区间上的变差为零，\(\gamma\) 将在其上为常值。因此弧长参数化 \(\eta\) 也单射，与 \(\gamma\) 有相同的像；对 \(1\)-Lipschitz 的 \(\eta\) 使用上面的结论即可。这一步依赖\secref{b1:sec:1605}的弧长参数化，用它消除速度不有界的问题，而不是直接删去 Lipschitz 面积公式的假设。
...
\[
 J_2X=\sqrt{|X_u|^2\cdot|X_v|^2-\langle X_u,X_v\rangle^2}
      =|X_u\times X_v|.
\]
若 \(X\) 单射，且 \(E\subseteq U\) 是 Lebesgue 可测的，则
\[
 \mathcal H^2\bigl(X(E)\bigr)
 =\int_E|X_u\times X_v|\,\mathrm du\,\mathrm dv.
\]
这正是熟悉的曲面积分因子。若去掉单射条件，积分仍有意义，但它给出按重数累计的面积。而 \(X_u\times X_v\ne0\) 只说明微分满秩，并不能排除全局重复覆盖。
...
\[
 J_kF_\pm(x)=\sqrt{1+\frac{|x|^2}{1-|x|^2}}
            =\frac1{\sqrt{1-|x|^2}}.
\]
由\cref{b1:cor:local-area-formulas}，面积公式可先用于 \(B(0,1-1/j)\)，再由单调收敛取遍开球；不要求在整个开球上有统一 Lipschitz 常数。
...
\[
 ...
 \end{aligned}
\]
最后利用\chapref{b1:ch:15}的 \(\omega_j=\pi^{j/2}/\Gamma(1+j/2)\) 及 \(\Gamma(t+1)=t\cdot \Gamma(t)\)，得到 \(2\pi\omega_{k-1}=(k+1)\omega_{k+1}\)。一般半径的结论由 Hausdorff 测度的相似缩放性质得到。
\end{proof}
...
\[
 \mathcal A(f;E)\coloneqq\int_EJ_kf(x)\,\mathrm dx.
\]
这里 \(\mathcal A(f;E)\) 是带有参数化的数据，并非只由集合 \(f(E)\) 决定。局部面积公式\cref{b1:cor:local-area-formulas}给出
\[
...
\end{proposition}
\begin{proof}
先说明 \(f\) 不可微的点构成零测集。由于 \(\phi^{-1}\) 局部 Lipschitz，这个零测集在 \(\phi^{-1}\) 下的像仍为零测集；所以在几乎每个 \(z\) 都可以应用普通链式法则，得到
\[
 D(f\circ\phi)(z)=Df\bigl(\phi(z)\bigr)D\phi(z).
\]
对右边的可逆方阵使用\eqref{b1:eq:linear-jacobian-composition}之后的参数变换形式，即得 Jacobi 因子等式。再用\chapref{b1:ch:07}的 \(C^1\) 换元公式，对非负函数 \(J_kf\) 积分，得到第二个结论。若积分无穷，先截断或直接用非负版本，结论不变。
\end{proof}

这个命题只改变同一组参数点的坐标，不增加其重数。若换成一个多对一的参数映射，就不能再使用“微分同胚”版本；此时应重新计算重数。例如圆柱的参数化
\[
 X(u,v)=(R\cdot \cos u,R\cdot \sin u,v)
\]
...
本节的“Lipschitz 参数曲面”只指由上述参数映射描述的像及其参数化。是否容许可数个参数片、如何忽略零测余项、何时存在近似切平面，将在\chapref{b1:ch:19}用可整流集的定义统一处理；这里不把一个像集未经说明地当作全局嵌入的光滑流形。
...
\]
例如把 \(h\) 取为位置坐标、温度或质量密度时，应同时保留图像的面积因子；只在参数域直接积分 \(h(x,u(x))\)，计算的是对底面体积的积分。Simon 的《Lectures on Geometric Measure Theory》\cite[8.6 EXAMPLES]{Simon1983Lectures}把曲线、参数曲面和图像并列为面积公式的基本应用。下一章将改变切分方式：不再从低维参数块生成曲面，而是按高维映射的水平集分解积分。

% !TeX root = ../../Book1.tex
\section{线性映射的 Jacobi 因子}
\label{b1:sec:1701}

\chapref{b1:ch:03}的线性换元公式适用于方阵：体积按行列式的绝对值改变。参数曲面却经常由非方阵描述，例如一个二维参数块可以映入三维空间。这时应比较二维面积，而不是把像看作三维零测集后停止计算。本节从 Gram 矩阵构造所需的非负伸缩因子，并把它与\chapref{b1:ch:15}归一化的 Hausdorff 测度接起来。

% 实读：Fremlin2016Measure 作者公开chap26.tex，265B--C全文。
% 采用像平面的规范正交坐标，将非方阵化为方阵；\chapref{b1:ch:03}已有任意集合的线性外测度换元。
% Gram、奇异值、子式和外积的有限维代数在此展开，不依赖尚未写出的附录 B。
...
\]
因此 \(G\) 的特征值非负；其行列式是这些特征值的乘积，且严格为正当且仅当 \(T\) 单射。这里使用实对称矩阵的正交对角化，也可由\chapref{b1:ch:08}自伴算子的有限维情形得到。
...
\cref{b1:fig:gram-parallelogram}把 Gram 行列式的平方根解释为平行四边形面积：底边长度乘以另一条边在垂直方向上的分量。
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[scale=1.1,>=stealth]
    \coordinate (O) at (0,0);
    \coordinate (V) at (3.2,0);
    \coordinate (W) at (1.15,1.9);
    \coordinate (S) at (4.35,1.9);
    \fill[gray!18] (O)--(V)--(S)--(W)--cycle;
    \draw[thick,->] (O)--(V) node[midway,below] {$v$};
    \draw[thick,->] (O)--(W) node[midway,left] {$w$};
    \draw[thick] (V)--(S)--(W);
    \draw[dashed] (W)--(1.15,0) node[midway,right] {$|w|\,|\sin\theta|$};
    \draw (1.15,0)--(1.15,0.20)--(1.35,0.20)--(1.35,0);
    \draw (0.62,0) arc[start angle=0,end angle=58.8,radius=0.62];
    \node at (0.76,0.30) {$\theta$};
  \end{tikzpicture}
  \caption[Gram 面积]{Gram 面积。由 \(v,w\) 张成的平行四边形面积为 \(\sqrt{\det G}=|v|\,|w|\,|\sin\theta|\)。}
  \label{b1:fig:gram-parallelogram}
\end{figure}

\subsection{多维 Jacobi 因子}
\label{b1:subsec:170102}

\begin{definition}\label{b1:def:linear-jacobian}
设 \(1\leq k\leq n\)，\(T:\mathbb R^k\to\mathbb R^n\) 为线性映射。定义它的\term[kwei-jacobi-yinzi]{\(k\) 维 Jacobi 因子}[\(k\)-dimensional Jacobian]为
\[
 J_k(T)\coloneqq\sqrt{\det(T^{\mathsf T}T)}.
\]
\end{definition}
...
\]
若 \(T\) 不单射，则对任意 \(E\) 都有 \(\mathcal H^k\bigl(T(E)\bigr)=0\)。这里 \(\mathcal H^k\) 先按外测度解释；若 \(E\) 是 Lebesgue 可测的，则 \(T(E)\) 是完备 \(\mathcal H^k\) 测度下的可测集。
\end{theorem}
\begin{proof}
先设 \(T\) 单射。仍取像平面的规范正交坐标，写 \(T=QB\)，其中 \(Q:\mathbb R^k\to\mathbb R^n\) 等距，\(B\) 为可逆方阵。由\cref{b1:cor:hausdorff-flat-subspace}与\cref{b1:thm:invertible-linear-change}，
\[
...
若 \(T\) 的秩为 \(r<k\)，像落在一个 \(r\) 维平面中。由\chapref{b1:ch:15}的临界维数性质，这个平面的有界块的 \(k\) 维 Hausdorff 测度为零；取可数并，整个平面也为零测集。当 \(r=0\) 时像为单点，同一结论因 \(k>0\) 成立。因此不论 \(E\) 是否可测或有界，其像的外测度都是零。

最后设 \(E\) 是 Lebesgue 可测的。对每个正整数 \(m\)，集合
\[
 E_m=E\cap B(0,m)
\]
具有有限测度。由 Lebesgue 测度的紧内正则性，可以在每个 \(E_m\) 中取一列紧集，使其并与 \(E_m\) 只差零测集。把这些双指标紧集枚举为 \(K_1,K_2,\ldots\)，便有
\[
 K_j\subseteq E,
 \qquad
 m_k\!\left(E\setminus\bigcup_{j=1}^{\infty}K_j\right)=0.
\]
各 \(T(K_j)\) 紧，而零测余项的像由刚证的等式或秩亏结论为 \(\mathcal H^k\) 零测集。完备性于是保证 \(T(E)\) 可测。
\end{proof}

把秩亏情形单独陈述，是为了不在 \(m_k^*(E)=\infty\) 时使用没有解释的 \(0\cdot\infty\)。对于有限体积参数块，当然可以简写成同一个乘法公式。上述证明也表明，等式中的常数来自\chapref{b1:ch:15}的归一化，而不是另行选择的“曲面单位”。Fremlin 的《Measure Theory》\cite[265B--C]{Fremlin2016Measure}给出了相同的线性测度比较。
...
因此 \(J_k(T)\) 是一个外积的长度；它不记录外积的方向。当 \(k=2,n=3\) 时，三个二阶子式就是叉积的三个坐标（其中一个按既定次序带负号），所以 \(J_2(T)=|v_1\times v_2|\)。当 \(k=n\) 时只有一个子式，长度退化为行列式的绝对值。

...
线性面积公式已经给出一个线性映射的精确体积伸缩率。对于 Lipschitz 映射，应在每一点使用 \(J_kf\)，再把局部贡献积起来；若不同参数点落到同一像点，还须按覆盖次数重复计入。本节的\term[mianji-gongshi]{面积公式}[area formula]把这两件事联系起来，先证明单射版本，再引入重数。证明的近线性比较与秩亏处理参考 Fremlin 的《Measure Theory》\cite[265D--E]{Fremlin2016Measure}，并把本书所需的可测分片与一般重数版本展开。
...
单射面积公式的证明分成六步：先保证可测参数集的像可测，再比较一块上接近固定线性映射的面积，随后把满秩点分成可数个这样的片；满秩部分由逐片比较处理，秩亏部分则另用增广映射 \((f,\eta\operatorname{id})\) 压回原像，最后合并两部分。也就是
\[
 \begin{aligned}
 \text{像的可测性}
 &\longrightarrow\text{近线性比较}
 \longrightarrow\text{满秩分片}\\
 &\longrightarrow\text{满秩面积比较}
 \longrightarrow\text{秩亏像零测}
 \longrightarrow\text{单射面积公式}.
 \end{aligned}
\]
满秩部分依靠近线性分片的双向距离控制；秩亏部分没有这样的下界，不能用同一个估计替代增广映射。
...
\[
 \left|\mathcal H^k\bigl(u(A)\bigr)-J_k(T)\cdot m_k^*(A)\right|
 \leq\varepsilon\cdot m_k^*(A).
\]
这里像的测度先按外测度解释。
\end{lemma}
\begin{proof}
...
若 \(E\subseteq G\) 是 Lebesgue 可测的，则
\[
...
\[
 \left|\mathcal H^k\bigl(f(E_j)\bigr)-J_j\cdot m_k(E_j)\right|
 \leq\varepsilon\cdot m_k(E_j).
\]
所有像都由\cref{b1:lem:lipschitz-image-hausdorff-measurable}保证可测。集合 \(f(E_j)\) 一般会互相重叠，所以第一步只能使用 \(\mathcal H^k\) 的可数次可加性；参数片 \(E_j\) 的两两不交性则用于合并参数域上的测度与积分：
\[
 \begin{aligned}
 \mathcal H^k\bigl(f(E)\bigr)
 &\leq\sum_j\mathcal H^k\bigl(f(E_j)\bigr)\\
 &\leq\sum_jJ_j\cdot m_k(E_j)+\varepsilon\cdot m_k(E)\\
 &\leq\int_EJ_kf+2\varepsilon\cdot m_k(E).
 \end{aligned}
\]
...
\[
 ...
 \int_EJ_kf
 &\leq\sum_jJ_j\cdot m_k(E_j)+\varepsilon\cdot m_k(E)\\
 &\leq\sum_j\mathcal H^k\bigl(f(E_j)\bigr)+2\varepsilon\cdot m_k(E)\\
 &=\mathcal H^k\bigl(f(E)\bigr)+2\varepsilon\cdot m_k(E).
 \end{aligned}
\]
...
设 \(E\subseteq\mathbb R^k\) 是 Lebesgue 可测的，且 \(f|_E\) 单射。则
\[
...
若 \(E\) 是 Lebesgue 可测的，则 \(N(f,E,\cdot)\) 为 \(\mathcal H^k\) 可测函数。除去一个 \(\mathcal H^k\) 零测集后，每个纤维至多可数。
\end{proposition}
\begin{proof}
...
设 \(1\leq k\leq n\)，\(f:\mathbb R^k\rightarrow\mathbb R^n\) 为 Lipschitz 映射，\(E\subseteq\mathbb R^k\) 是 Lebesgue 可测的。则
\[
...
在\cref{b1:thm:area-formula}的条件下，设 \(g:E\rightarrow[0,\infty]\) 是 Lebesgue 可测的。则纤维权重和
\[
 S_g(y)=\sum_{x\in E\cap f^{-1}(\{y\})}g(x)
\]
为 \(\mathcal H^k\) 可测函数，并满足
\[
 \int_E g(x)\cdot J_kf(x)\,\mathrm dx
 =
 \int_{\mathbb R^n}
...
\end{theorem}
\begin{proof}
若 \(g=\sum_{i=1}^ra_i\cdot\mathbf1_{A_i}\) 为非负简单函数，其中 \(A_i\subseteq E\) 可测且两两不交，则
\[
 S_g(y)=\sum_{i=1}^r a_i\cdot N(f,A_i,y).
\]
...
\begin{corollary}[局部面积公式]\label{b1:cor:local-area-formulas}
设 \(U\subseteq\mathbb R^k\) 为开集，\(f:U\rightarrow\mathbb R^n\) 为局部 Lipschitz 映射，其中 \(1\leq k\leq n\)，并设 \(E\subseteq U\) 是 Lebesgue 可测的。则重数函数 \(N(f,E,\cdot)\) 是 \(\mathcal H^k\) 可测的，并且
\[
 \int_EJ_kf(x)\,\mathrm dx
 =
 \int_{\mathbb R^n}N(f,E,y)\,\mathrm d\mathcal H^k(y).
\]
若 \(g:E\rightarrow[0,\infty]\) 是 Lebesgue 可测的，则相应的加权公式
\[
 \int_Eg(x)\cdot J_kf(x)\,\mathrm dx
 =
 \int_{\mathbb R^n}
 \sum_{x\in E\cap f^{-1}(\{y\})}g(x)\,
 \mathrm d\mathcal H^k(y)
...
\]
则 \(A_j\) 是两两不交的 Borel 集，\(A_j\subseteq B_j\)，且其并为 \(U\)。把 \(f|_{B_j}\) 的各坐标分别作 Lipschitz 延拓，得到全空间上的 Lipschitz 映射 \(f_j:\mathbb R^k\rightarrow\mathbb R^n\)。在开球 \(B_j\) 内，\(f_j\) 与 \(f\) 局部相同，因而二者的可微点及 Jacobi 因子在那里一致。

置 \(E_j=E\cap A_j\)。分别对 \(f_j|_{E_j}\) 使用\cref{b1:thm:area-formula}与\cref{b1:thm:weighted-area-formula}。由于参数片不交，对每个 \(y\) 都有
\[
 N(f,E,y)=\sum_jN(f_j,E_j,y),
\]
加权纤维和也逐片相加。再对这些非负可测函数及参数域积分使用单调收敛定理，便得到两式及重数的可测性。这一局部化不要求 \(U\) 有规则边界，也不要求 \(f\) 在整个 \(U\) 上具有同一个 Lipschitz 常数。
\end{proof}
...
单射时纤维只有一个参数点，加权公式便退化为沿参数化转移积分。非单射时必须保留求和：折回、重叠或重复经过同一区域，都会改变右端的权重。Simon 的《Lectures on Geometric Measure Theory》\cite[8.4--8.5]{Simon1983Lectures}用计数测度 \(\mathcal H^0\) 写出同一结构；这里的纤维求和正是该计数积分。下一节将把这个公式用于曲线、图像与 Lipschitz 参数曲面。

面积公式的重数项有一个有限分支模型，见\cref{b1:fig:visual-f12}及\cref{b1:ex:16-retracing-multiplicity}。
\input{Figures/F12}

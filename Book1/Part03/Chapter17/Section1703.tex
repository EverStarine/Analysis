...
\]
所有像都由\cref{b1:lem:lipschitz-image-hausdorff-measurable}保证可测。集合 \(f(E_j)\) 一般会互相重叠，所以第一步只能使用 \(\mathcal H^k\) 的可数次次可加性；参数片 \(E_j\) 的两两不交性则用于合并参数域上的测度与积分：
\[
 \begin{aligned}
...
置 \(E_j=E\cap A_j\)。分别对 \(f_j|_{E_j}\) 使用\cref{b1:thm:area-formula,b1:thm:weighted-area-formula}。由于参数片不交，对每个 \(y\) 都有
\[

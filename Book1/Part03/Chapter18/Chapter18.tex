...
把高维区域按映射值分层，得到的不是离散重数，而是每条纤维自身的低维面积。本章从水平集的线性化和正则坐标出发，证明 Lipschitz 映射的余面积公式。零测集的切片、临界部分的贡献和切片积分的可测性分别处理；应用中再讨论球壳积分、水平集面积的平均控制，以及分布密度公式为什么不能无条件除以梯度。
\end{chapterabstract}

% 本章目标（不计入编号）
\begin{learninggoals}
\begin{itemize}
\item\label{b1:item:ch18-goal-level}理解正则水平集的切空间与法空间，区分光滑局部坐标和一般 Lipschitz 水平集。
...
本章固定定义域维数 \(m\) 不小于目标维数 \(n\)。公式把 \(m\) 维体积拆成 \((m-n)\) 维纤维面积与对 \(n\) 维参数的积分；当两维相等，纤维上的计数测度使它回到前章的面积公式。读者应先熟悉 Tonelli 定理、\chapref{b1:ch:13}的 Besicovitch 覆盖及\chapref{b1:ch:17}的单射面积公式。\secref{b1:sec:1801}把光滑正则坐标所需的局部求逆补证出来；一般 Lipschitz 证明则不假定这些坐标存在于整块开邻域中，而在可数个双 Lipschitz 片上进行。

\ProseParagraphBreak

Simon 的《Lectures on Geometric Measure Theory》\cite[\S10]{Simon1983Lectures}可供核对线性余面积及水平集的几何表述；它把部分逼近论证交给外部文献，本书则利用前章工具补齐 Lipschitz 主线。Fremlin 的《Measure Theory》\cite[265G]{Fremlin2016Measure}提供球壳积分的参数化处理，便于与本章的径向特例比较。Federer 的《Geometric Measure Theory》\cite{Federer1996Geometric}提供更系统的余面积理论与几何应用；本章证明所用的具体工具及其出处已在前文列明。
...
水平集的预期维数由微分的核给出；当映射为 \(C^1\) 且微分满秩时，局部求逆把这个线性预期实现为真实的图像坐标。余面积 Jacobi 因子 \(J_nf=\sqrt{\det(DfDf^{\mathsf T})}\) 则记录法向的体积伸缩，与前章对列向量构造的 Jacobi 因子相区别。
...
% \chapref{b1:ch:18}习题临时稿，不另设 BookExercises 标题。
% 九题均为依据本章及前文工具组织的自拟题；不冒认特定教材题号。
% 不使用\chapref{b1:ch:19}的可整流集理论或尚未建立的曲面余面积公式。
...
求 \(F_\#(m_3\llcorner B)\) 关于 \(m_2\) 的密度，并直接积分核对其总质量。若将 \(m_3\llcorner B\) 换成球上的均匀概率测度，密度怎样改变？
\end{enumerate}
\end{exercise}
...
\[
 ...
 =\int_{\mathbb R^2}h(a,b)\cdot\rho(a,b)\,\mathrm da\,\mathrm db,
\]
其中
\[
 \rho(a,b)=
...
在单位方形 \(Q=(0,1)^2\) 上令 \(u(x,y)=xy\)。用余面积公式而不是先对矩形作逐次积分，求 \(u_\#(m_2\llcorner Q)\) 的密度和分布函数。
进一步计算
\[
...
\]
故密度为 \((-\log t)\cdot\mathbf1_{(0,1)}(t)\)。方形面积为 \(1\)，它已经是概率密度。

由 \(t\cdot \log t\to0\) 可知分布函数为
\[
 G(t)=
 \begin{cases}
 0,&t\le0,\\
 ...
 \end{cases}
\]
最后取 \(h(t)=t^a\)。分部积分及 \(t^{a+1}\cdot \log t\to0\) 给出
\[
...
求 \(0<t<1\) 时水平集的两个分支及其梯度大小，继而计算 \(u_\#(m_2\llcorner D)\) 的密度。
...
\[
 \rho(t)=\frac{2\pi}{\sqrt t}\cdot\mathbf1_{(0,1)}(t),
\]
它的总积分为 \(4\pi=m_2(D)\)。
...
\]
临界水平的原像是单位圆，二维面积为零，所以 \(t=0\) 没有原子。两支上径向导数的符号相反，但余面积 Jacobi 因子是梯度的模；两片原像各自携带正面积，必须相加。
\end{solution}
...
\[
 F_\#(m_3\llcorner\Omega),
 \quad F_\#\bigl(|z|(m_3\llcorner\Omega)\bigr).
\]
说明临界集在推前中是否留下额外质量。
\end{exercise}
...
\[
 \rho_0(s,t)=\frac{\pi}{\sqrt t}\cdot\mathbf1_{(0,1)^2}(s,t),
 \quad \rho_1(s,t)=\pi\cdot\mathbf1_{(0,1)^2}(s,t).
\]
...
设 \(L>0\)，\(S=[0,L]\times\{0\}\subset\mathbb R^2\)，令 \(\mathrm{d}(x)=\operatorname{dist}(x,S)\)。对 \(R>0\)，记 \(S_R=\{\,x\mid \mathrm{d}(x)<R\,\}\)。
\begin{enumerate}
\item\label{b1:item:18-segment-level-length}
逐段描述 \(\mathrm{d}^{-1}(t)\)，计算其长度，并用余面积公式求 \(m_2(S_R)\)。
\item\label{b1:item:18-segment-weighted-layer}
对非负 Borel 函数 \(h:(0,R)\rightarrow[0,\infty]\)，求 \(\int_{S_R\setminus S}h\bigl(\mathrm{d}(x)\bigr)\,\mathrm dm_2(x)\)。特别地，求 \(\int_{S_R}\mathrm{d}(x)\,\mathrm dm_2(x)\)，并核对 \(m_2(S_R)\) 对 \(R\) 的导数与边界长度的关系。
\end{enumerate}
\end{exercise}
% 来源：自拟；以非球形平行集计算水平长度、距离矩和薄层体积，不调用周长或有界变差函数理论。
\begin{solution}
...
\[
 \mathcal H^1\bigl(\mathrm{d}^{-1}(t)\bigr)=2L+2\pi t.
\]
又 \(m_2(S)=0\)，所以余面积公式给出
\[
...
同样，由加权版本得到
\[
 \int_{S_R\setminus S}h\bigl(\mathrm{d}(x)\bigr)\,\mathrm dm_2(x)
 =\int_0^Rh(t)(2L+2\pi t)\,\mathrm dt.
\]
非负性允许两端同时为无穷。取 \(h(t)=t\)，得到
\[
 \int_{S_R}\mathrm{d}(x)\,\mathrm dm_2(x)
 =LR^2+\frac{2\pi}{3}R^3.
\]
因为 \(\partial S_R=\mathrm{d}^{-1}(R)\)，上述面积表达式还给出
\[
...
\[
 \mathcal H^2\bigl(C\cap u^{-1}(t)\bigr)=\sqrt3\cdot V'(t)
\]
对几乎每个 \(t\) 成立。
...
\[
 ...
 \quad (a,b)\in D_t,
\]
其中 \(D_t=\{\,(a,b)\in[0,1]^2\mid t-1\le a+b\le t\,\}\)。该参数映射的 Jacobi 因子为 \(\sqrt3\)，故截面积等于 \(\sqrt3\cdot m_2(D_t)\)。若 \(t_n\to t\)，则除 \(a+b=t-1\) 和 \(a+b=t\) 两条零测直线外，\(\mathbf1_{D_{t_n}}\to\mathbf1_{D_t}\)。控制收敛定理给出 \(m_2(D_{t_n})\to m_2(D_t)\)。截面积与上面的 \(\sqrt3\cdot V'\) 都连续，而它们几乎处处相等，因而处处相等。
...
证明
\[
 u_\#(m_2\llcorner Q)=m_1\llcorner[0,\ell]+\nu,
 \quad \nu=f_\#(m_1\llcorner K),
\]
其中 \(\nu\) 是无原子的奇异测度，总质量为 \(a\)。
...
\end{enumerate}
\end{exercise}
% 来源：自拟；把\chapref{b1:ch:17}的单射压缩构造用于临界推前，展示奇异部分不必由平台原子组成。
\begin{solution}
由于 \(u\) 不依赖第二坐标，而该坐标区间长度为 \(1\)，Fubini 定理给出
\[
 u_\#(m_2\llcorner Q)=f_\#(m_1\llcorner[0,1]).
\]
...
\]
最后一步用了 \([0,\ell]\setminus f(E)=f(K)\) 为零测集。因此 \(f_\#(m_1\llcorner E)=m_1\llcorner[0,\ell]\)，分解即得。
...
\begin{exercise}\label{b1:ex:18-coarea-proof-interfaces}
设 \(m>n\geq1\)，\(q=m-n\)，并设 \(f:\mathbb R^m\rightarrow\mathbb R^n\) 为 Lipschitz 映射，\(E\subseteq\mathbb R^m\) Lebesgue 可测。记
\[
 N=\mathbb R^m\setminus\mathcal D_f,
 \qquad
 C=\{x\in\mathcal D_f:\operatorname{rank}Df(x)<n\},
 \qquad
 G=\mathcal D_f\setminus C.
\]
下面分步复原余面积公式证明中三类集合的不同作用。
\begin{enumerate}
\item\label{b1:item:18-proof-null-critical}
使用零测集切片引理与临界集切片引理证明：对几乎每个 \(y\in\mathbb R^n\)，
\[
 \mathcal H^q\bigl(E\cap(N\cup C)\cap f^{-1}(\{y\})\bigr)=0.
\]
并说明为什么这一步没有断言临界纤维为空。

\item\label{b1:item:18-proof-full-rank}
设 \(A\subseteq E\cap G\) Lebesgue 可测，并存在坐标投影 \(P:\mathbb R^m\rightarrow\mathbb R^q\)，使
\[
 \Phi=(P,f):A\longrightarrow Z=\Phi(A)
\]
为双 Lipschitz 映射。记其逆为 \(H:Z\rightarrow A\)，并把 \(H\) 逐坐标 Lipschitz 延拓为 \(\widetilde H:\mathbb R^m\rightarrow\mathbb R^m\)。利用切片坐标的 Jacobi 因子恒等式、\cref{b1:thm:area-formula}与\cref{b1:thm:weighted-area-formula}以及 Tonelli 定理证明
\[
 \int_AJ_nf(x)\,\mathrm dx
 =\int_{\mathbb R^n}
 \mathcal H^q\bigl(A\cap f^{-1}(\{y\})\bigr)\,\mathrm dy.
\]

\item\label{b1:item:18-proof-assembly}
若 \(G\) 可写成两两不交的 Borel 坐标片 \(A_j\) 的可数并，说明如何合并前两问而得到 \(E\) 上的余面积公式。指出可数性在例外参数集和纤维测度求和中分别起什么作用。
\end{enumerate}
\end{exercise}
% 来源：自拟；把余面积主证明中零测部分、临界部分和满秩坐标片的接口改写成分步复原题。
\begin{solution}
不可微点集 \(N\) 为 Lebesgue 零测集，故由\cref{b1:lem:null-set-slices}，对几乎每个 \(y\) 有
\[
 \mathcal H^q\bigl(E\cap N\cap f^{-1}(\{y\})\bigr)=0.
\]
由\cref{b1:lem:critical-set-slices}，同样有
\[
 \mathcal H^q\bigl(E\cap C\cap f^{-1}(\{y\})\bigr)=0
\]
对几乎每个 \(y\) 成立。合并两个例外零测集并使用次可加性，就得到第一问。结论只说这些点在几乎每张纤维中的 \(q\) 维测度为零；零测集合仍可非空，所以不能把它加强成纤维不经过临界点。

对第二问，由\cref{b1:lem:lipschitz-image-hausdorff-measurable}及 \(\mathcal H^m=m_m\)，集合 \(Z\) Lebesgue 可测。对几乎每个 \(u=(z,y)\in Z\)，点 \(u\) 是 \(Z\) 的密度点，\(\widetilde H\) 在 \(u\) 处可微，而且密度点上的恒等式 \(\Phi\circ H=\operatorname{id}_Z\) 给出
\[
 D\widetilde H(u)=D\Phi\bigl(H(u)\bigr)^{-1}.
\]
写 \(B(x)=D\Phi(x)\)。由\cref{b1:prop:coarea-coordinate-identity}，
\[
 J_q\bigl(D_z\widetilde H(z,y)\bigr)
 =\frac{J_nf\bigl(H(z,y)\bigr)}
 {\lvert\det B\bigl(H(z,y)\bigr)\rvert}
 \quad\text{在 }Z\text{ 中几乎处处}.
\]
映射 \(\Phi|_A\) 单射。对它使用\cref{b1:thm:weighted-area-formula}，得到
\[
 \int_AJ_nf(x)\,\mathrm dx
 =\int_Z
 \frac{J_nf\bigl(H(u)\bigr)}
 {\lvert\det B\bigl(H(u)\bigr)\rvert}\,\mathrm du.
\]
另一方面，Fubini 定理和\cref{b1:thm:area-formula}的单射情形说明，对几乎每个 \(y\)，
\[
 \begin{aligned}
 ...
 \end{aligned}
\]
把前两个等式代入，再对 \(Z\subseteq\mathbb R^q\times\mathbb R^n\) 使用 Tonelli 定理，即得第二问。

最后把 \(E\cap G\) 写成两两不交的 \(E\cap A_j\)，逐片应用第二问。每片都有一个参数例外零测集；可数性保证这些例外集的并仍为零测集。在其补集上，各纤维片两两不交且可测，故 \(\mathcal H^q\) 的可数可加性给出
\[
 \mathcal H^q\bigl(E\cap G\cap f^{-1}(\{y\})\bigr)
 =\sum_j\mathcal H^q\bigl(E\cap A_j\cap f^{-1}(\{y\})\bigr).
\]
定义域上的非负积分也可逐片求和。再由第一问补回 \(E\cap(N\cup C)\)，并注意 \(J_nf=0\) 于该集合，便得到 \(E\) 上的余面积公式。
\end{solution}

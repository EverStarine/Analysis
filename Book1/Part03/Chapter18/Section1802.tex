...
\begin{proof}[定理\ref{b1:thm:coarea-formula}及其标量版本的证明]
...
\[
 ...
 \mathrm d\mathcal H^{m-n}(x)\right)\,\mathrm dy.
\]
两式中的纤维积分及其可测性按\cref{b1:thm:coarea-formula,b1:thm:weighted-coarea}的约定理解。
\end{corollary}
\begin{proof}
...
置 \(E_j=E\cap A_j\)，分别对 \(f_j|_{E_j}\) 应用\cref{b1:thm:coarea-formula,b1:thm:weighted-coarea}。对可数个片的例外参数集取并后，几乎每张纤维中的 \(E_j\cap f^{-1}(\{y\})\) 都可测；它们彼此不交，故可数可加性与单调收敛定理允许逐片求和，得到两式。
\end{proof}

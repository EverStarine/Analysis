...
面积公式沿参数方向把低维区域映入高维空间；\term[yumianji-gongshi]{余面积公式}[coarea formula]则把高维区域按映射值分层。正交投影时，这就是 Fubini 定理。一般 Lipschitz 映射的水平集不再平行，补偿因子也由常数变成余面积 Jacobi 因子。
线性情形与几何表述可对照 Simon 的《Lectures on Geometric Measure Theory》\cite[\S10, pp. 53--57]{Simon1983Lectures}；本节从零测集的切片、满秩坐标分片和前章面积公式出发，证明 Lipschitz 版本。
...
设 \(f:\mathbb R^m\rightarrow\mathbb R\) 为 Lipschitz 函数，且 \(E\subseteq\mathbb R^m\) 是 Lebesgue 可测的。则
\[
 \int_E|\nabla f(x)|\,\mathrm dx
 =
...
\]
为 Borel 可测函数。若 \(E\subseteq\mathbb R^m\) 仅是 Lebesgue 可测的，则 \(E_y\) 对几乎每个 \(y\) 都是 \(\mathcal H^q\) 可测集，且 \(y\mapsto\mathcal H^q(E_y)\) 为 Lebesgue 可测函数。
\end{lemma}
\begin{proof}
...
\]
并在不可微点取零。它非负、Borel 可测，且不超过 \(L^n\)。先处理余面积 Jacobi 因子消失的部分；与前章不同，这里不要求该部分的像在目标空间中零测。
...
设 \(m\geq n\geq1\)，\(f:\mathbb R^m\rightarrow\mathbb R^n\) 为 Lipschitz 映射，且 \(E\subseteq\mathbb R^m\) 是 Lebesgue 可测的。则
\[
...
证明分为五步：先分别排除不可微点与秩亏点在几乎每张纤维上的贡献，再把满秩集分成可数个双 Lipschitz 坐标片；在每片上使用切片坐标的 Jacobi 因子恒等式，最后以面积公式和 Tonelli 定理合并切片积分。也就是
\[
 \begin{aligned}
 \text{零测集切片与临界集切片}
 &\longrightarrow\text{满秩集的双 Lipschitz 分片}\\
 &\longrightarrow\text{Jacobi 因子恒等式}
 \longrightarrow\text{面积公式与 Tonelli 定理}.
 \end{aligned}
\]
...
令 \(G=\{x\in\mathcal D_f:\operatorname{rank}Df(x)=n\}\)。在 \(G\) 外，余面积 Jacobi 因子为零；不可微点由\cref{b1:lem:null-set-slices}处理，秩亏点由\cref{b1:lem:critical-set-slices}处理。因此只需证明 \(E\cap G\) 上的公式。
...
\]
由\cref{b1:lem:lipschitz-image-hausdorff-measurable}及同维归一化 \(\mathcal H^m=m_m\)，同维 Lipschitz 映射把 Lebesgue 可测集 \(A\) 送到 Lebesgue 可测集，故 \(Z\) 可测。片上的逆映射 \(H:Z\rightarrow A\) 为 Lipschitz 映射。逐坐标延拓，得到全空间的 Lipschitz 映射 \(\widetilde H:\mathbb R^m\rightarrow\mathbb R^m\)。
...
先说明如何限制一个 Lebesgue 可测函数到纤维。设 \(E\subseteq\mathbb R^m\) Lebesgue 可测，\(g:E\rightarrow[0,\infty]\) Lebesgue 可测；把 \(g\) 在 \(E^c\) 上补零，得到全空间函数 \(\widehat g\)。由\cref{b1:thm:lebesgue-borel-representative}，可取与 \(\widehat g\) 几乎处处相等的 Borel 函数，再以其正部替代之，从而得到取值于 \([0,\infty]\) 的 Borel 代表 \(g_0\)。记两者不同的零测集为 \(N\)。若 \(m>n\)，由\cref{b1:lem:null-set-slices}，\(N\) 在几乎每张纤维上的 \(\mathcal H^q\) 测度为零；所以这些纤维上 \(g\) 对完备测度 \(\mathcal H^q\) 可测，并与 \(g_0\) 给出相同的非负扩展积分。若 \(m=n\)，则由\cref{b1:lem:lipschitz-image-hausdorff-measurable}及 \(\mathcal H^m=m_m\) 有 \(m_n(f(N))=0\)；当 \(y\notin f(N)\) 时，纤维根本不遇到改值点。这个构造已经直接允许 \(g=+\infty\)，无须再用水平集作额外约定。

\begin{theorem}[加权余面积公式]\label{b1:thm:weighted-coarea}
在\cref{b1:thm:coarea-formula}的条件下，设 \(g:E\rightarrow[0,\infty]\) 是 Lebesgue 可测的。则几乎每个纤维上都有定义良好的非负积分，并且
\[
 \int_Eg(x)\cdot J_nf(x)\,\mathrm dx
 =
 \int_{\mathbb R^n}
...
\begin{corollary}[局部余面积公式]\label{b1:cor:local-coarea-formulas}
设 \(U\subseteq\mathbb R^m\) 为开集，\(m\geq n\geq1\)，\(f:U\rightarrow\mathbb R^n\) 为局部 Lipschitz 映射，且 \(E\subseteq U\) Lebesgue 可测。则
\[
 \int_EJ_nf(x)\,\mathrm dx
 =
 \int_{\mathbb R^n}
 \mathcal H^{m-n}\bigl(E\cap f^{-1}(\{y\})\bigr)\,\mathrm dy.
\]
若 \(g:E\rightarrow[0,\infty]\) Lebesgue 可测，则还有
\[
 \int_Eg(x)\cdot J_nf(x)\,\mathrm dx
 =
 \int_{\mathbb R^n}
 ...
 \mathrm d\mathcal H^{m-n}(x)\right)\,\mathrm dy.
\]
两式中的纤维积分及其可测性按\cref{b1:thm:coarea-formula}与\cref{b1:thm:weighted-coarea}的约定理解。
\end{corollary}
\begin{proof}
取可数个开球 \(B_j\) 覆盖 \(U\)，使 \(f|_{B_j}\) 都具有有限的 Lipschitz 常数，并令
\[
 A_1=B_1,
 \qquad
 A_j=B_j\setminus\bigcup_{i<j}B_i\quad(j\geq2).
\]
则 \(A_j\) 是两两不交的 Borel 集，且 \(A_j\subseteq B_j\)。由\cref{b1:thm:mcshane-extension}逐坐标延拓 \(f|_{B_j}\)，得到全空间 Lipschitz 映射 \(f_j:\mathbb R^m\rightarrow\mathbb R^n\)。它与 \(f\) 在开球 \(B_j\) 内局部相同，故二者的可微点、微分及余面积 Jacobi 因子在那里一致。

置 \(E_j=E\cap A_j\)，分别对 \(f_j|_{E_j}\) 应用\cref{b1:thm:coarea-formula}与\cref{b1:thm:weighted-coarea}。对可数个片的例外参数集取并后，几乎每张纤维中的 \(E_j\cap f^{-1}(\{y\})\) 都可测；它们彼此不交，故可数可加性与单调收敛定理允许逐片求和，得到两式。
\end{proof}
...
若 \(n=1\) 且 \(\nabla f(x)\ne0\)，微分核为与 \(\nabla f(x)\) 正交的超平面，\(|\nabla f(x)|\) 测量函数值沿法向变化的速率。沿函数值 \(t\) 积分时，变化快的地方需要更大的补偿因子，才能恢复原空间中的体积。这个解释只涉及正则点的局部微分，不把所有水平集都说成光滑超曲面。
...
使用 Jacobi 因子之前，先固定源、目标和纤维的维数，见\cref{b1:tab:visual-t10}。
\input{Tables/T10}

...
本章已经从参数化推出切平面与密度。现在反过来问：如果不知道参数化，只观察小球内的质量，能否判断集合或测度可整流？这一逆问题远比顺向计算困难。本选读节先证明带权密度的顺向结论和可整流／纯不可整流分解，再陈述作为深层输入的 Preiss 定理，最后把集合的密度判据作为推论。深层几何步骤明确交由文献，不能把它们看作前面面积公式证明的一部分。

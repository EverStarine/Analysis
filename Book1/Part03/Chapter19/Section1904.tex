...
切平面与切测度的区别也可以在最熟悉的例子上看出。若 \(\mu=\theta(\mathcal H^k\llcorner V)\)，其中 \(V\) 为平面、\(\theta>0\) 为常数，则在 \(x\in V\) 处，\(r^{-k}(T_{x,r})_\#\mu=\theta(\mathcal H^k\llcorner(V-x))\)。平面给出方向，测度另外保留质量常数；更一般的缩放极限还可能保留重数和非平坦形状。
...
\item\label{b1:item:density-criterion-one}\(\Theta^k(\mathcal H^k\llcorner E,x)=1\) 对 \(\mathcal H^k\)-几乎每个 \(x\in E\) 成立；
\item\label{b1:item:density-criterion-exists}\(\Theta^k(\mathcal H^k\llcorner E,x)\) 对 \(\mathcal H^k\)-几乎每个 \(x\in E\) 存在。
\end{equivalences}
\end{theorem}
...
\[
 2^{-k}\leq\Theta^{*k}(\mathcal H^k\llcorner E,x)\leq1
\]
在 \(E\) 中几乎处处成立。因此一旦密度极限存在，它必正且有限。将\cref{b1:thm:preiss-rectifiability}应用于有限 Radon 测度 \(\mu=\mathcal H^k\llcorner E\)，得到 \(\mu\) 集中在可数个 Lipschitz 像上。因而这些像在 \(E\) 中遗漏的 \(\mathcal H^k\) 测度为零，这正是第一项。
\end{proof}

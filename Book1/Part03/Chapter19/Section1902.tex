...
局部有限性保证上式左端的每次积分有限。这一定义要求缩放后的质量趋于平面上的 \(\mathcal H^k\)，没有额外的重数因子；一般带权测度的缩放极限将在\secref{b1:sec:1904}另行讨论。改变 \(E\) 的一个 \(\mathcal H^k\) 零测子集不会改变积分条件，但是否把某个点列为 \(E\) 的元素，仍取决于所选集合。
...
\end{lemma}
\begin{proof}
采用\secref{b1:sec:1702}的 Borel Jacobi 行列式，置 \(h={\bf1}_D J_kF\)。它是有界可测函数。删去一个参数零测集后，可同时要求 \(F\) 在 \(a\in D\) 处可微、\(A=DF(a)\) 满秩，以及 \(a\) 为 \(h\) 的 Lebesgue 点，且 \(h(a)=J_k(A)\)。被删集合的像也是 \(\mathcal H^k\) 零测集。

...
Lipschitz 参数化在几乎每一点有线性微分，但一个可整流集可能由许多相交的参数像组成。要把参数空间中的微分解释成集合自身的切平面，既要看清单个片的局部形状，也要控制其余各片进入同一个小球的质量。本节采用缩放测度的定义；它同时记录方向与单位面积密度。Simon 的《Lectures on Geometric Measure Theory》\cite[11.2 DEFINITION, 11.3 REMARK, 11.4 DEFINITION, 11.5 REMARK, and 11.6 THEOREM]{Simon1983Lectures}以这种方式组织近似切空间。本节直接从 Lipschitz 片和已证的面积公式出发，不另引入光滑逼近定理。

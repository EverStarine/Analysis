...
以下先固定 \(1\leq k\leq n\)，并设 \(E\subseteq\mathbb R^n\) 为 Borel 集，限制测度 \(\mu=\mathcal H^k\llcorner E\) 局部有限。这里 \((\mathcal H^k\llcorner E)(A)=\mathcal H^k(E\cap A)\)，不是把整个空间上的 \(\mathcal H^k\) 假定为局部有限。对 \(k<n\)，后一个假定通常不成立。
...
\[
 ...
 &\hspace{1em}=
 \int_{\mathbb R^k}
   \varphi\left(\frac{F(a+rv)-F(a)}r\right)\cdot h(a+rv)\,\mathrm dv.
 \end{aligned}
\]
...
设 \(E\subseteq\mathbb R^n\) 为 Borel 的可数 \(k\)-可整流集，且 \(\mathcal H^k\llcorner E\) 局部有限。则对 \(\mathcal H^k\)-几乎每个 \(x\in E\)，存在唯一的 \(k\) 维平面 \(T_x\) 使单位密度平面缩放极限\eqref{b1:eq:approximate-tangent-blowup}成立。特别地，\(T_x\) 是 \(E\) 在 \(x\) 处的近似切平面，记为 \(\operatorname{Tan}^k(E,x)\)，并且
\[
 \lim_{r\downarrow0}
 \frac{\mathcal H^k\bigl(E\cap\overline B(x,r)\bigr)}
      {\omega_k r^k}=1
...
现在置
\[
 \mu=\mathcal H^k\llcorner E,\quad
 \nu_j=\mathcal H^k\llcorner M_j,\quad
 \sigma_j=\mathcal H^k\llcorner(E\setminus M_j).
\]
...
在 \(g_j\) 可微的点，\(D\psi_j(v)=v+Dg_j(v)\) 的 \(P_j\) 分量就是 \(v\)，故它满秩。对 \(A_j\) 应用\cref{b1:lem:bilipschitz-piece-blowup}，再像前一定理那样对 \(\mathcal H^k\llcorner(E\setminus M_j)\) 与 \(\mathcal H^k\llcorner M_j\) 使用奇异测度微分，得到\eqref{b1:eq:graph-approximate-tangent}。参数零测集由 Lipschitz 性映为面积零测集；反向对零测子集使用逆投影的 Lipschitz 性即可。
...
若另考虑 \(k=0\)，局部有限的 \(\mathcal H^0\llcorner E\) 意味着每个有界区域只含有限个点。每个 \(x\in E\) 因而在 \(E\) 中孤立，其缩放测度趋于原点的单位点质量；对应的切空间为 \(\{0\}\)，法空间为 \(\mathbb R^n\)。以上需要 \(r^k\) 质量估计的论证主要服务于正维数情形。

\cref{b1:fig:visual-f14}把同一切线与单位重数的区别分开显示。
\input{Figures/F14}

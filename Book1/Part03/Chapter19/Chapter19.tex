...
面积与余面积公式已经说明，Lipschitz 参数片可以承载微分和积分。本章转而从集合与测度本身出发，定义可数可整流性和纯不可整流性，证明局部有限面积的可数可整流集合几乎处处具有近似切空间与单位密度，再建立切向微分、面积公式及带权推前。最后在选读部分讨论可整流／纯不可整流分解，并以明确的文献边界介绍密度判据与 Preiss 定理。
\end{chapterabstract}
...
本章前 3 节构成后文会调用的可整流性主线，但不是第十七、十八章已经完成的面积与余面积证明的补充假设。读者应先掌握可整流集、近似切平面和图上的切向积分；\secref{b1:sec:1904}的结构分解、密度判据与 Preiss 理论标为选读，可留作第二遍阅读。关于近似切平面的主线，先修是 Rademacher、面积公式、Lebesgue 点，以及\chapref{b1:ch:14}对一般 Radon 测度的微分；后一个工具用来说明相邻参数片为何不会在典型点留下额外的主阶质量。
...
密度的顺向结论已在本章证明；由正有限密度反推可整流性则属于 Preiss 定理的深层内容，这里只概述其方法与边界。几何测度论的这一组工具为后面的弱导数、Sobolev 空间和有界变差函数理论准备了低维集合及积分的语言。
\end{chaptersummary}

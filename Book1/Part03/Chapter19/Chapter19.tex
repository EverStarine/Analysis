% !TeX root = ../../Book1.tex
\chapter{可整流集的切向结构与密度}
\label{b1:ch:19}% 本章摘要（不计入编号）
\BookChapterNumber{b1:ch:19}
\begin{chapterabstract}
面积与余面积公式已经说明，Lipschitz 参数片可以承载微分和积分。本章转而从集合与测度本身出发，定义可数可整流性和纯不可整流性，证明局部有限面积的可数可整流集合几乎处处具有近似切空间与单位密度，再建立切向微分、面积公式、带权推前及可整流测度的密度公式。最后在选读部分讨论可整流／纯不可整流分解、密度判据与 Preiss 定理，并注明逆向理论所需的深层输入。
\end{chapterabstract}
...
\item\label{b1:item:ch19-goal-density}理解集合的单位密度、带权可整流测度的密度、纯不可整流分解与 Preiss 逆向判据各自需要哪些工具。
\end{itemize}
\end{learninggoals}

本章前 3 节构成后文会调用的可整流性主线。它们不是第十七、十八章面积与余面积证明的补充假设。读者应先掌握可整流集、近似切平面、图上的切向积分以及带权可整流测度的密度。\secref{b1:sec:1904}的结构分解、密度逆判据与 Preiss 理论标为选读，可留作第二遍阅读。近似切平面的主线以 Rademacher 定理、面积公式、Lebesgue 点及\chapref{b1:ch:14}对一般 Radon 测度的微分为前置；最后一项用来说明相邻参数片为何不会在典型点留下额外的主阶质量，并识别带权密度。

\ProseParagraphBreak

Simon 的《Lectures on Geometric Measure Theory》\cite[\S\S11--12]{Simon1983Lectures}是切空间与切向面积公式的主要参照。本书改用前章已经建立的 Lipschitz 分片，把参数独立性与片间测度比较展开。Mattila 的《Rectifiability: A Survey》\cite{Mattila2023Rectifiability}提供集合、测度与密度判据的系统对照；其测度定义有不要求相对于 Hausdorff 测度绝对连续的版本，本书会明确区分。Preiss 定理在本章只陈述结论并给出证明概要；深层证明不在本章展开。
...
可数可整流性允许可数参数片和面积零测的余项，纯不可整流性则排除了与任何参数片的正测相交。四角 Cantor 集说明，一个集合可以有正且有限的长度，却没有可由曲线捕捉的正长度部分。在选读部分，有限 \(\mathcal H^k\) 测度的 Borel 集被证明能在零测意义下唯一分成这两类部分。

\ProseParagraphBreak

局部有限的可整流集合在几乎处处具有单位重数的平面缩放极限。证明先把一张双 Lipschitz 片上的积分拉回参数域，再以测度微分控制其余片的质量。图参数化随后给出切向微分，重叠参数域的密度点保证它与坐标和延拓的选择无关。切向面积公式计算的是面积伸缩后的质量；原质量的推前还须保留被临界映射压缩的部分。主线最后用有限面积分片和 Radon 测度微分证明，权重 \(\theta\) 正是带权可整流测度的几乎处处密度。

\ProseParagraphBreak

密度的顺向结论已在主线中证明；由正有限密度反推可整流性则属于 Preiss 定理的深层内容，这里只在选读部分概述其方法与边界。几何测度论的这一组工具为后面的弱导数、Sobolev 空间和有界变差函数理论准备了低维集合及积分的语言。
\end{chaptersummary}


\BookExercises

% \chapref{b1:ch:19}习题来源与设计说明。
% 十一题均为依据本章及前文工具组织的自拟变式；不冒认教材题号。
% 不以四角 Cantor 集的纯不可整流性或尚未证明的 Preiss 定理作为解题前置。

\begin{exercise}\label{b1:ex:19-linear-plane-basics}
设 \(V\subseteq\mathbb R^n\) 为 \(k\) 维线性子空间，\(E=x_0+V\)。证明 \(\mathcal H^k\llcorner E\) 局部有限，并对每个 \(x\in E\) 验证：
\begin{enumerate}
\item \(V\) 是 \(E\) 在 \(x\) 处的近似切平面，且单位密度平面缩放极限\eqref{b1:eq:approximate-tangent-blowup}成立；
\item \(E\) 在 \(x\) 处的 \(k\) 维密度为 \(1\)，近似法空间为 \(V^\perp\)。
\end{enumerate}
\end{exercise}
% 来源：自拟；直接操作近似切平面、缩放积分、密度和法空间四个基本定义。
\begin{solution}
正交等距变换把 \(E\cap\overline B(x,R)\) 送到 \(\mathbb R^k\) 中半径为 \(R\) 的闭球，所以
\[
 \mathcal H^k\bigl(E\cap\overline B(x,R)\bigr)=\omega_kR^k<\infty.
\]
这证明局部有限性，也立刻给出每一点的密度为 \(1\)。

若 \(y\in E\)，则 \(y-x\in V\)。因此\cref{b1:def:approximate-tangent-plane}中的锥外集合至多含点 \(x\)，其 \(\mathcal H^k\) 测度为零；正上密度又等于 \(1\)，故 \(V\) 是近似切平面。对任意 \(\varphi\in C_c(\mathbb R^n)\)，在线性空间 \(V\) 上作变量代换 \(y=x+rz\)，得到
\[
 \frac1{r^k}\int_E\varphi\left(\frac{y-x}{r}\right)
     \,\mathrm d\mathcal H^k(y)
 =\int_V\varphi(z)\,\mathrm d\mathcal H^k(z).
\]
这不只给出极限，而是对每个 \(r>0\) 都成立。最后，近似法空间按定义就是 \(V^\perp\)。
\end{solution}

\begin{exercise}\label{b1:ex:19-parabola-tangent-jacobian}
令 \(\gamma(t)=(t,t^2)\)、\(E=\gamma(\mathbb R)\)，并固定 \(p=\gamma(a)\)。直接验证 \(E\) 在 \(p\) 处具有单位密度平面缩放极限，求 \(\operatorname{Tan}^1(E,p)\) 与 \(\operatorname{Nor}^1(E,p)\)。再令 \(f(x,y)=x+y\)，计算 \(\mathrm{d}^Ef(p)\) 及切向 Jacobi 因子 \(J_Ef(p)\)。
\end{exercise}
% 来源：自拟；在一张显式光滑图上从缩放定义过渡到切向微分和 Jacobi 因子。
\begin{solution}
曲线的速度为 \(\gamma'(a)=(1,2a)\)，故候选切线为
\[
 T=\operatorname{span}\{(1,2a)\}.
\]
对紧支集连续函数 \(\varphi\)，以 \(t=a+rs\) 参数化。曲线弧长公式给出
\[
 \frac1r\int_E\varphi\left(\frac{y-p}{r}\right)\,\mathrm d\mathcal H^1(y)
 =\int_{\mathbb R}
 \varphi\bigl(s,2as+rs^2\bigr)
 \sqrt{1+4(a+rs)^2}\,\mathrm ds.
\]
当 \(r\downarrow0\) 时，被积函数趋于
\(\varphi\bigl(s(1,2a)\bigr)\sqrt{1+4a^2}\)。因第一坐标就是 \(s\)，\(\varphi\) 的紧支集把所有非零项限制在同一个有界 \(s\) 区间内，故支配收敛定理可用。极限恰为 \(\int_T\varphi\,\mathrm d\mathcal H^1\)，所以强缩放结论成立，并由\cref{b1:prop:approximate-tangent-properties}得到近似切线与单位密度。

因此
\[
 \operatorname{Tan}^1(E,p)=T,
 \qquad
 \operatorname{Nor}^1(E,p)=\operatorname{span}\{(-2a,1)\}.
\]
对 \(v\in T\)，切向微分就是环境线性泛函的限制：
\[
 \mathrm{d}^Ef(p)[v]=(1,1)\mathbin{\boldsymbol\cdot}v.
\]
取单位切向量 \(\tau=(1,2a)/\sqrt{1+4a^2}\)，得到
\[
 J_Ef(p)=|\mathrm{d}^Ef(p)[\tau]|
 =\frac{|1+2a|}{\sqrt{1+4a^2}}.
\]
\end{solution}

\begin{exercise}\label{b1:ex:19-definition-comparisons}
在 \(\mathbb R^2\) 中比较以下三个对象：直线 \(L=\mathbb R\times\{0\}\)、\cref{b1:prop:four-corner-pure-unrectifiable}中的四角 Cantor 集 \(P\)，以及点质量 \(\delta_0\)。
\begin{enumerate}
\item 判断 \(L\) 与 \(P\) 是否可数 \(1\) 维可整流，以及 \(\mathcal H^1\llcorner L\)、\(\mathcal H^1\llcorner P\) 是否局部有限。
\item 判断 \(\mathcal H^1\llcorner L\)、\(\mathcal H^1\llcorner P\) 与 \(\delta_0\) 是否为本书意义下的 \(1\) 维可整流测度。
\end{enumerate}
据此说明“承载集可整流”“承载集局部有限面积”与“测度关于 \(\mathcal H^1\) 绝对连续”是三个不能互相替代的条件。
\end{exercise}
% 来源：自拟；用本章已经建立的三个模型辨析集合可整流性、局部有限面积和可整流测度。
\begin{solution}
直线 \(L\) 是全空间 Lipschitz 映射 \(t\mapsto(t,0)\) 的像，故可数 \(1\) 维可整流；每个有界球只截出有限长度，所以 \(\mathcal H^1\llcorner L\) 局部有限。四角 Cantor 集 \(P\) 为紧集且 \(0<\mathcal H^1(P)<\infty\)，故其限制测度也局部有限；但该集纯 \(1\) 维不可整流，所以不是可数 \(1\) 维可整流集。

测度 \(\mathcal H^1\llcorner L\) 是 Radon 测度，集中在可整流集 \(L\) 上，并显然绝对连续于 \(\mathcal H^1\)，所以是 \(1\) 维可整流测度。若 \(\mathcal H^1\llcorner P\) 是 \(1\) 维可整流测度，它就会集中在某个可数 \(1\) 维可整流集 \(R\) 上；纯不可整流性却给出 \(\mathcal H^1(P\cap R)=0\)，与 \(\mathcal H^1(P)>0\) 矛盾。故它不是可整流测度。

点质量 \(\delta_0\) 虽集中在直线 \(L\) 上，却不绝对连续于 \(\mathcal H^1\)，因为 \(\mathcal H^1(\{0\})=0\) 而 \(\delta_0(\{0\})=1\)。所以它也不是 \(1\) 维可整流测度。三个例子分别表明，集合条件、局部有限性和测度绝对连续性必须在使用处逐项核对。
\end{solution}
...
\]
证明 \(E\) 是可数 \(k\)-可整流 Borel 集，\(\mathcal H^k\llcorner E\) 为 \(\sigma\) 有限测度，却在每个非空开球上取无穷值。
...
求图像内部点 \(G(x,y)\) 的切平面及一个单位法向量，计算 \(J_2G\) 和切向 Jacobi 因子 \(J_EP\)。
\item\label{b1:item:19-saddle-critical-fiber}
描述 \(P(E)\) 及投影的各纤维，指出切向 Jacobi 因子为零的集合。用可整流集上的面积公式计算
\[
...
\]
再求 \(P_\#(\mathcal H^2\llcorner S^2)\) 关于平面面积的密度，并说明单位圆周是否承受额外的推前质量。
\end{exercise}
% 来源：自拟；球面积在前文已知，此处训练切向 Jacobi 因子、二重投影与推前密度，不重复球面积的参数化证明。
\begin{solution}
...
\[
 E=L_1\cup L_2,
 \quad \mu=2(\mathcal H^1\llcorner L_1)+3(\mathcal H^1\llcorner L_2).
\]
计算每个 \(x\in E\) 处的极限
\[
...
集合 \(E\) 可数 \(1\)-可整流，且 \(\mathcal H^1\llcorner E\) 局部有限。令 \(\theta=2\) 于 \(L_1\setminus\{0\}\)，\(\theta=3\) 于 \(L_2\setminus\{0\}\)，在原点任取有限值，便有
\[
 \mu=\theta\mathcal H^1\llcorner E.
\]
因此 \(\mu\) 是 \(1\)-可整流 Radon 测度。原点的权值怎样修改都不改变 \(\mu\)，却不能改变原点处计算出的密度 \(5\)；密度等于权值的结论只要求几乎处处成立。

另一方面，\(\mathcal H^1(\{0\})=0\)，而 \((\mu+c\delta_0)(\{0\})=c>0\)。任何 \(\theta\mathcal H^1\llcorner R\) 形式的测度都不可能在这个单点上赋正质量，故 \(\mu+c\delta_0\) 不是 \(1\)-可整流测度。它在原点的上述密度为无穷。两种测度的支撑都是 \(E\)，可见支撑是可整流集合并不足以代替关于 \(\mathcal H^1\) 的绝对连续性。
\end{solution}
...
集合 \(E\) 可数 \(1\)-可整流，且 \(\mathcal H^1\llcorner E=\mathcal H^1\llcorner S^1\)。两集合的近似切线在 \(\mathcal H^1\) 几乎处处意义下相同。
\item\label{b1:item:19-null-addition-closure}
有 \(\overline E=\mathbb R^2\)，但测度 \(\mathcal H^1\llcorner E\) 的支撑是 \(S^1\)，并且 \(\overline E\) 不是可数 \(1\)-可整流集。
\end{enumerate}
\end{exercise}
...
由于 \(\mathbb Q^2\) 稠密，\(\overline E=\mathbb R^2\)。但是圆周外每个点都有一个不交圆周的小球，其 \(\mathcal H^1\llcorner E\) 测度为零；圆周上每个点的任意小球都含正长度圆弧。于是测度的支撑恰为 \(S^1\)。
...
现按可数可整流性的定义，选全空间 Lipschitz 映射 \(f_j:\mathbb R^k\rightarrow\mathbb R^m\)，使
\[
 N=E\setminus\bigcup_jf_j(\mathbb R^k),
 \quad \mathcal H^k(N)=0.
\]
对每个 \(j\) 定义全空间 Lipschitz 映射
\[
 F_j:\mathbb R^{k+l}\longrightarrow\mathbb R^{m+l},
 \quad F_j(a,b)=(f_j(a),b).
\]
映射 \(F_j\) 的 Lipschitz 常数不超过 \(\max\{\operatorname{Lip}(f_j),1\}\)。其全空间像当然包含 \(f_j(\mathbb R^k)\times[0,1]^l\)，所以这些 \((k+l)\) 维参数片覆盖 \((E\setminus N)\times[0,1]^l\)；剩余的 \(N\times[0,1]^l\) 已由第一问证明为零测集。这就严格符合本书以全空间参数映射定义可数可整流性的约定。证明中立方体的网格数 \(O(r_i^{-l})\) 抵消了额外的 \(l\) 次尺度因子。
\end{solution}

\begin{optionalexercise}\label{b1:ex:19-pure-bilipschitz-decomposition}
...
同时，若 \(R\) 可数 \(k\)-可整流，将它的各参数映射与 \(F\) 复合即可覆盖 \(F(R)\)，原来的零测余项仍为零测。因此 \(F(R)\) 可数 \(k\)-可整流。又 \(F\) 单射保证两部分不交，且它们覆盖 \(F(E)\)。若 \(E\) 被可数个有限 \(\mathcal H^k\) 测度集覆盖，Lipschitz 测度估计也保证 \(F(E)\) 仍为 \(\sigma\) 有限。故本章结构分解的唯一性适用，任何另一分解都只在 \(\mathcal H^k\) 零测集上与 \(F(R),F(P)\) 不同。

这里同时使用了两个方向的 Lipschitz 控制：前向控制保持零测集，反向控制把目标中的每个测试参数片拉回为合法的测试参数片。
\end{solution}

\begin{optionalexercise}\label{b1:ex:19-rectifiable-weak-limit-dimension}
对整数 \(N\ge1\)，令
\[
 S_{j,N}=[0,1]\times\left\{\frac{j-1/2}{N}\right\},
 \quad j=1,\ldots,N,
 \quad \mu_N=\frac1N\sum_{j=1}^N(\mathcal H^1\llcorner S_{j,N}).
\]
\begin{enumerate}
\item\label{b1:item:19-grid-weak-limit}
证明 \(\mu_N\) 是 \(1\)-可整流概率测度，并且弱收敛到 \(\mu=m_2\llcorner[0,1]^2\)。证明极限 \(\mu\) 不是 \(1\)-可整流测度。

...
面积与余面积公式已经说明，Lipschitz 参数片可以承载微分和积分。本章转而从集合与测度本身出发，定义可数可整流性和纯不可整流性，证明局部有限面积的可数可整流集合几乎处处具有近似切空间与单位密度，再建立切向微分、面积公式、带权推前及可整流测度的密度公式。最后在选读部分讨论可整流／纯不可整流分解，并以明确的文献边界介绍密度判据与 Preiss 定理。
\end{chapterabstract}
...
本章前 3 节构成后文会调用的可整流性主线，但不是第十七、十八章已经完成的面积与余面积证明的补充假设。读者应先掌握可整流集、近似切平面、图上的切向积分以及带权可整流测度的密度；\secref{b1:sec:1904}的结构分解、密度逆判据与 Preiss 理论标为选读，可留作第二遍阅读。关于近似切平面的主线，先修是 Rademacher、面积公式、Lebesgue 点，以及\chapref{b1:ch:14}对一般 Radon 测度的微分；后一个工具用来说明相邻参数片为何不会在典型点留下额外的主阶质量，并识别带权密度。

Simon 的《Lectures on Geometric Measure Theory》\cite[\S\S11--12]{Simon1983Lectures}是切空间与切向面积公式的主要参照。本书改用前章已经建立的 Lipschitz 分片，把参数独立性与片间测度比较展开。Mattila 的《Rectifiability: A Survey》\cite{Mattila2023Rectifiability}提供集合、测度与密度判据的系统对照；其测度定义有不要求相对于 Hausdorff 测度绝对连续的版本，本书会明确区分。Preiss 定理只给有出处的陈述与证明概要，其深层证明不作为本章已自行证完的内容。
...
设 \(V\subseteq\mathbb R^n\) 为 \(k\) 维线性子空间，\(E=x_0+V\)。证明 \(\mathcal H^k|_E\) 局部有限，并对每个 \(x\in E\) 验证：
\begin{enumerate}
...
\begin{enumerate}
\item 判断 \(L\) 与 \(P\) 是否可数 \(1\) 维可整流，以及 \(\mathcal H^1|_L\)、\(\mathcal H^1|_P\) 是否局部有限。
\item 判断 \(\mathcal H^1|_L\)、\(\mathcal H^1|_P\) 与 \(\delta_0\) 是否为本书意义下的 \(1\) 维可整流测度。
\end{enumerate}
据此说明“承载集可整流”“承载集局部有限面积”与“测度关于 \(\mathcal H^1\) 绝对连续”是三个不能互相替代的条件。
\end{exercise}
% 来源：自拟；用本章已经建立的三个模型辨析集合可整流性、局部有限面积和可整流测度。
\begin{solution}
直线 \(L\) 是全空间 Lipschitz 映射 \(t\mapsto(t,0)\) 的像，故可数 \(1\) 维可整流；每个有界球只截出有限长度，所以 \(\mathcal H^1|_L\) 局部有限。四角 Cantor 集 \(P\) 为紧集且 \(0<\mathcal H^1(P)<\infty\)，故其限制测度也局部有限；但该集纯 \(1\) 维不可整流，所以不是可数 \(1\) 维可整流集。

测度 \(\mathcal H^1|_L\) 是 Radon 测度，集中在可整流集 \(L\) 上，并显然绝对连续于 \(\mathcal H^1\)，所以是 \(1\) 维可整流测度。若 \(\mathcal H^1|_P\) 是 \(1\) 维可整流测度，它就会集中在某个可数 \(1\) 维可整流集 \(R\) 上；纯不可整流性却给出 \(\mathcal H^1(P\cap R)=0\)，与 \(\mathcal H^1(P)>0\) 矛盾。故它不是可整流测度。

...
令 \(\gamma(t)=(t,t^2)\)、\(E=\gamma(\mathbb R)\)，并固定 \(p=\gamma(a)\)。直接验证 \(E\) 在 \(p\) 处具有单位密度平面缩放极限，求 \(\operatorname{Tan}^1(E,p)\) 与 \(\operatorname{Nor}^1(E,p)\)。再令 \(f(x,y)=x+y\)，计算 \(d^Ef(p)\) 及切向 Jacobi 因子 \(J_Ef(p)\)。
\end{exercise}
...
\[
 d^Ef(p)[v]=(1,1)\mathbin{\boldsymbol\cdot}v.
\]

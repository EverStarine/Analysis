...
本章直接使用\chapref{b1:ch:13}的覆盖选择与极大估计，并调用
\chapref{b1:ch:06}的 Radon--Nikodym 分解和\chapref{b1:ch:11}的 Radon 测度框架。
首次阅读可按\secref{b1:sec:1401}、\secref{b1:sec:1402}、\secref{b1:sec:1403}
依次完成“定义比值—恢复函数—恢复测度密度”的主线，再读后两节关于平均形状、积分核、
精确代表与近似连续性的辨析。

\ProseParagraphBreak

Axler 的《Measure, Integration \& Real Analysis》适合对照 Lebesgue 测度下的函数微分，
Fremlin 的《Measure Theory》适合核对一般 Radon 测度的版本
\cite{Axler2020Measure,Fremlin2016Measure}；那汤松的《实变函数论》第5版中译本
\cite{Natanson2010RealChinese}可作为密度点与近似连续性的中文伴读。各资料采用的球、代表
与基准测度约定并不完全相同，阅读时应以本章每条定理的明确假设为准。
...
% 十题均为自拟，以下注释只记录前置工具与题目目的，不冒称参考书原题。
% 正文来源对照：Fremlin2016Measure 472D（已实际读取）的一般Radon微分；
% Cantor测度使用本书0603已构造的数字映射，不预借第16章或未读书中的证明。

\begin{exercise}[绝对连续部分与奇异部分的球比值]\label{b1:ex:14-ac-singular-ball-ratios}
在实线上令
\[
 \mu=m_1,\qquad
 \nu=(1+x^2)m_1+2\delta_0+3\delta_1.
\]
\begin{enumerate}
\item\label{b1:item:14-explicit-decomposition}
写出 \(\nu\) 相对于 \(\mu\) 的绝对连续部分与奇异部分，并说明它们为何构成 Lebesgue 分解。
\item\label{b1:item:14-explicit-ball-ratios}
直接计算 \(x\notin\{0,1\}\)、\(x=0\) 与 \(x=1\) 三种情形的小区间质量比
\[
 \frac{\nu([x-r,x+r])}{\mu([x-r,x+r])},
\]
并求相应极限。
\item\label{b1:item:14-explicit-recovery}
据此辨认绝对连续密度与奇异质量在球比值中的不同表现，并说明为什么
\[
 \int_E D_{m_1}\nu\,\mathrm dm_1
\]
对每个 Borel 集 \(E\) 恢复的是 \(\nu\) 的绝对连续部分，而不是整个 \(\nu\)。
\end{enumerate}
\end{exercise}
% 来源：自拟；用光滑密度加两个原子，把绝对连续恢复、奇异侧发散和积分只恢复绝对连续部分放在同一显式模型中。
\begin{solution}
函数 \(x\mapsto1+x^2\) 非负且局部可积，所以
\[
 \nu_a=(1+x^2)m_1,\qquad \nu_s=2\delta_0+3\delta_1
\]
都是 Radon 测度。第一项绝对连续于 \(m_1\)，第二项集中在 \(m_1\)-零测集 \(\{0,1\}\) 上，故 \(\nu=\nu_a+\nu_s\) 正是所需分解。

若 \(x\notin\{0,1\}\)，充分小的 \([x-r,x+r]\) 不含两个原子，因而
\[
 \frac{\nu([x-r,x+r])}{2r}
 =\frac1{2r}\int_{x-r}^{x+r}(1+t^2)\,\mathrm dt
 =1+x^2+\frac{r^2}{3}
 \longrightarrow1+x^2.
\]
若 \(0<r<1\)，在原点和一点分别有
\[
 \frac{\nu([-r,r])}{2r}
 =1+\frac{r^2}{3}+\frac1r\longrightarrow+\infty,
\]
以及
\[
 \frac{\nu([1-r,1+r])}{2r}
 =2+\frac{r^2}{3}+\frac3{2r}\longrightarrow+\infty.
\]
因此绝对连续部分在普通点给出密度 \(1+x^2\)，奇异原子则在自己的位置产生发散项。特别地，
\[
 D_{m_1}\nu(x)=1+x^2
 \quad\text{对 }m_1\text{-几乎处处的 }x.
\]
原子处的无穷值落在 \(m_1\)-零测集上，不改变非负积分，所以对每个 Borel 集 \(E\)，
\[
 \int_E D_{m_1}\nu\,\mathrm dm_1
 =\int_E(1+x^2)\,\mathrm dm_1
 =\nu_a(E).
\]
右端不含 \(2\cdot \delta_0(E)+3\cdot \delta_1(E)\)，故球比值的 \(m_1\)-几乎处处积分不能恢复奇异部分。
\end{solution}

\begin{exercise}[密度一集合上的普通极限]\label{b1:ex:14-density-one-criterion}
在平面上用极坐标定义
\[
 S=\{\,(\rho\cdot \cos\theta,\rho\cdot \sin\theta):0<\rho<1,\ |\theta|<\rho\,\},
\]
其中 \(-\pi<\theta\leq\pi\)。令 \(u(0)=0\)，并对 \(z\ne0\) 定义
\[
 u(z)=
 \begin{cases}
 |z|^{-1},&z\in S,\\
...
\]
\begin{enumerate}
\item\label{b1:item:14-polar-density-one}
证明 \(m_2(S\cap B(0,r))=2r^3/3\) 对 \(0<r<1\) 成立，并由此说明 \(E=\mathbb R^2\setminus S\) 在原点的密度为 \(1\)。
\item\label{b1:item:14-polar-approximate-limit}
在 \(E\) 上求 \(u(z)\) 当 \(z\to0\) 时的普通极限，再用\cref{b1:prop:approximate-limit-density-one}求 \(u\) 在原点的近似极限。
\item\label{b1:item:14-polar-not-lebesgue}
证明 \(u\) 在原点没有普通极限，而且原点不是它的 Lebesgue 点。
\end{enumerate}
\end{exercise}
% 来源：自拟；用角宽随半径缩小的极坐标尖域直接训练密度一集合判据，并把近似极限与普通极限、Lebesgue点分开。
\begin{solution}
当 \(0<r<1\) 时，极坐标换元给出
\[
 m_2\bigl(S\cap B(0,r)\bigr)
 =\int_0^r\int_{-\rho}^{\rho}\rho\,\mathrm d\theta\,\mathrm d\rho
 =\frac23r^3.
\]
除以 \(m_2(B(0,r))=\pi r^2\)，所得比例为 \(2r/(3\pi)\)，趋于零。因此 \(S\) 在原点的密度为零，补集 \(E\) 在原点的密度为 \(1\)。

在 \(E\) 上有 \(u(z)=|z|\)，故 \(u(z)\to0\) 当 \(z\to0\)、\(z\in E\setminus\{0\}\)。由\cref{b1:prop:approximate-limit-density-one}，
\[
 \operatorname*{ap\,lim}_{z\to0}u(z)=0=u(0),
\]
所以 \(u\) 在原点近似连续。

沿正横轴取 \(z=(\rho,0)\)、\(0<\rho<1\)，则 \(z\in S\) 且 \(u(z)=1/\rho\to+\infty\)，所以普通极限不存在。另一方面，\(u\) 非负且局部可积，并且
\[
 \begin{aligned}
 \frac1{\pi r^2}\int_{B(0,r)}|u(z)-u(0)|\,\mathrm dm_2(z)
 &\geq\frac1{\pi r^2}\int_{S\cap B(0,r)}\frac1{|z|}\,\mathrm dm_2(z)\\
 &=\frac1{\pi r^2}\int_0^r\int_{-\rho}^{\rho}1\,\mathrm d\theta\,\mathrm d\rho
 =\frac1\pi.
 \end{aligned}
\]
平均绝对偏差不趋于零，故原点不是 Lebesgue 点。密度一集合判据只忽略坏集的相对体积，并不控制其中函数值的大小。
\end{solution}
...
\end{enumerate}
\end{exercise}
% 来源：自拟；把本章Lebesgue点的绝对偏差与\chapref{b1:ch:05}积分绝对连续性结合。
...
令 \(\rho_C\) 为\subsecref{b1:subsec:060303}用二进数字构造的 Cantor 概率测度，定义
\[
...
% 从该已证测度构造分布函数，并与上一题的L1原函数比较；不调用有界变差函数理论。
\begin{solution}
单调性来自半直线的包含关系。
由\subsecref{b1:subsec:060303}，\(\rho_C\) 无原子且全部质量在三分 Cantor 集 \(C\) 上。
测度从上、从下连续性分别给出
\[
...
证明对每个 \(x\)，恰有一个数 \(a\) 满足
\[
 \lim_{r\downarrow0}\frac1{m_d\bigl(B(x,r)\bigr)}
...
连续性给出
\[
 \frac1{m_d\bigl(B(x,r)\bigr)}\int_{B(x,r)}|f(y)-u(x)|\,\mathrm dy
 =\frac1{m_d\bigl(B(x,r)\bigr)}\int_{B(x,r)}|u(y)-u(x)|\,\mathrm dy
 \longrightarrow0.
\]
若 \(a\) 也满足题述极限，三角不等式给出
\[
 |a-u(x)|
 \leq\frac1{m_d\bigl(B(x,r)\bigr)}\int_{B(x,r)}|a-f(y)|\,\mathrm dy
 +\frac1{m_d\bigl(B(x,r)\bigr)}\int_{B(x,r)}|f(y)-u(x)|\,\mathrm dy.
\]
...
\[
 \begin{aligned}
 \frac1{m_d\bigl(B(x,r)\bigr)}\int_{B(x,r)}|f(y)-f(x)|^p\,\mathrm dy&\longrightarrow0,\\
 \frac1{m_d\bigl(B(x,r)\bigr)}\int_{B(x,r)}|g(y)-g(x)|^q\,\mathrm dy&\longrightarrow0,
 \end{aligned}
\]
...
\begin{solution}
对球 \(B=B(x,r)\)，把 \(m_d(B)^{-1}(m_d\llcorner B)\) 视为概率测度。
由 \(f\cdot g-f(x)\cdot g(x)=(f-f(x))\cdot g+f(x)\cdot(g-g(x))\) 及 Hölder 不等式，
\[
 \begin{aligned}
 \frac1{m_d(B)}\int_B|f\cdot g-f(x)\cdot g(x)|
 &\leq
 \left(\frac1{m_d(B)}\int_B|f-f(x)|^p\right)^{1/p}
 \left(\frac1{m_d(B)}\int_B|g|^q\right)^{1/q}\\
 &\quad+|f(x)|\left(\frac1{m_d(B)}\int_B|g-g(x)|^q\right)^{1/q}.
 \end{aligned}
\]
Minkowski 不等式还给出
\[
 \left(\frac1{m_d(B)}\int_B|g|^q\right)^{1/q}
 \leq|g(x)|+
 \left(\frac1{m_d(B)}\int_B|g-g(x)|^q\right)^{1/q},
\]
...
\[
 ...
 h=\sum_{n=2}^{\infty}2^n\cdot\mathbf1_{I_n}.
\]
于是 \(h(0)=0\)，且
\[
...
\[
 ...
 &\quad\leq
 \frac1{|\det A|\,m_d\bigl(B(x,r)\bigr)}
    \int_{B(y_0,cr)}|f(y)-f(y_0)|\,\mathrm dy\\
 &\quad=
 \frac{c^d}{|\det A|}\,
 \frac1{m_d\bigl(B(y_0,cr)\bigr)}
    \int_{B(y_0,cr)}|f(y)-f(y_0)|\,\mathrm dy.
 \end{aligned}
\]

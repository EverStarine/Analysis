...
以下 \(\mu,\nu\) 都是 \(\mathbb R^d\) 上的正 Radon 测度，沿用\secref{b1:sec:1102}的 Borel 约定。
...
记 \(S_\mu=\operatorname{supp}\mu\)。\secref{b1:sec:1401}已经说明
...
先补齐总质量可能无穷时所需的分解。
\secref{b1:sec:0604}的符号测度版本采用有限质量约定，不能不加说明就直接用于这里的 \(\nu\)。
...
\end{theorem}
\begin{proof}
先说明证明路线。对每组有理阈值 \(a<b\)，把“点值不超过 \(a\)、但小球平均的上极限超过 \(b\)”的点组成坏集；用开集从外逼近它，再以 Radon Vitali 定理选取不交坏球。把各球上的平均不等式求和，便迫使这个坏集为 \(\mu\)-零集。最后对阈值和局部穷竭作可数并，并对 \(-h\) 重复论证；绝对偏差则由可数稠密的标量族得到。
...
\]
说明接下来所用坏集的可测性。
若 \(g\geq0\) 局部可积，则 \(\bigl(g\cdot\mathbf1_{\overline B(0,n)}\bigr)\mu\) 是有限 Radon 测度，
这由\subsecref{b1:subsec:110204}的可积密度结论得到。
用这些限制穷竭可知 \(g\mu\) 局部有限且紧内正则，因而也是 Radon 测度。
分别取 \(g=h_+,h_-\)，\cref{b1:prop:relative-density-borel}的固定半径可测性便给出 \(A_rh\) 的 Borel 可测性。
...
\(\int_Bh\,\mathrm d\mu>b\cdot\mu(B)\)。
...
每个 \(x\in E\) 都有任意小的中心闭球 \(B\subseteq G\)，满足 \(\sigma(B)>t\cdot\mu(B)\)。
由\cref{b1:cor:radon-vitali-covering}，可选取这种球的可数不交族 \((B_j)\)，使其并在 \(\mu\)-意义下覆盖 \(E\)。
因此
\[
 t\cdot\mu(E)\leq t\sum_j\mu(B_j)
 \leq\sum_j\sigma(B_j)
...
\]
这里的分母始终为正。为了把反向结论与\cref{b1:def:relative-density}的定义域严格分开，对任意正 Radon 测度 \(\tau\) 及 \(x\in S_\tau\)，定义扩展球质量比
\[
 \widetilde Q_r^{\mu,\tau}(x)=
 \begin{cases}
 \dfrac{\tau\bigl(\overline B(x,r)\bigr)}
       {\mu\bigl(\overline B(x,r)\bigr)},
       &\mu\bigl(\overline B(x,r)\bigr)>0,\\[6pt]
 +\infty,&\mu\bigl(\overline B(x,r)\bigr)=0.
 \end{cases}
\]
分子在 \(S_\tau\) 上始终为正，所以第二行不是 \(0/0\) 的约定。这个扩展比值不称为 \(D_\mu\tau\)：后者只在 \(S_\mu\) 上按\cref{b1:def:relative-density}定义。用这一记号，反向结论是
\[
 \widetilde Q_r^{\mu,\sigma}(x)\longrightarrow+\infty
 \quad\text{对 }\sigma\text{-几乎处处的 }x\in S_\sigma.
\]
具体地，对任意 \(M>0\)，当 \(r\) 足够小时，上面的 \(\mu/\sigma\) 比值小于 \(1/M\)；
若 \(\mu\) 的球质量为正，就取倒数，若它为零，扩展比值已经是 \(+\infty\)。
...
最后，上一小节已证 \(\widetilde Q_r^{\mu,\nu_s}\) 在 \(\nu_s\)-几乎处处趋于无穷，
而 \(\nu\geq\nu_s\)，故 \(\widetilde Q_r^{\mu,\nu}\) 也趋于无穷。
\end{proof}

例如在实线上取 \(\mu=m_1\)、\(\nu=m_1+\delta_0\)。当 \(x\ne0\) 时，足够小的闭区间不含原点，
故比值最终等于 \(1\)；在原点则有
\[
 \frac{\nu([-r,r])}{m_1([-r,r])}=1+\frac1{2r}\longrightarrow+\infty.
\]
于是 \(D_{m_1}\nu=1\) 在 \(m_1\)-几乎处处成立，但 \(\nu\) 并不绝对连续于 \(m_1\)。
...
本节也说明了\secref{b1:sec:1402}结论中哪些内容依赖于 Lebesgue 测度，哪些不依赖。

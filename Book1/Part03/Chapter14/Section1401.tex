...
本节先定义这些比值，说明它们何时有意义，以及哪些结论只需测度连续性便能得到。几乎处处存在极限是后面两节的任务。集合密度的初步例子可对照 Axler 的《Measure, Integration \& Real Analysis》\cite[4.22 Definition density, 4.23 Example density of an interval, and 4.24 Lebesgue Density Theorem]{Axler2020Measure}；相对于一般 Radon 测度的球平均及支集条件，可结合 Fremlin 的《Measure Theory》\cite[472D--E]{Fremlin2016Measure}阅读。后者用闭球，本章的测度比也采用这一约定。

\subsection{上、下密度}
\label{b1:subsec:140101}

本节固定在 \(\mathbb R^d\) 上讨论，\(m_d\) 表示 \(d\) 维 Lebesgue 测度。由伸缩性质，正半径球的体积有限且为正。对 Radon 测度 \(\nu\)，有界闭球的质量也有限，因此下面每个固定半径的比值都是非负实数；趋于零尺度时的极限却可以为无穷。
...
\[
\underline D_{m_d}\nu(x)
\coloneqq\liminf_{r\downarrow0}
\frac{\nu\bigl(\overline B(x,r)\bigr)}{m_d\bigl(\overline B(x,r)\bigr)},
\quad
\overline D_{m_d}\nu(x)
\coloneqq\limsup_{r\downarrow0}
\frac{\nu\bigl(\overline B(x,r)\bigr)}{m_d\bigl(\overline B(x,r)\bigr)}
\]
分别称为 \(\nu\) 在 \(x\) 处相对于 Lebesgue 测度的\term[xia-midu]{下密度}[lower density]与\term[shang-midu]{上密度}[upper density]。若二者相等，则把共同值记作 \(D_{m_d}\nu(x)\)，称为密度，允许其值为 \(+\infty\)。
\end{definition}
...
\underline\theta(E,x)
&\coloneqq\liminf_{r\downarrow0}
\frac{m_d\bigl(E\cap\overline B(x,r)\bigr)}{m_d\bigl(\overline B(x,r)\bigr)},\\
\overline\theta(E,x)
&\coloneqq\limsup_{r\downarrow0}
\frac{m_d\bigl(E\cap\overline B(x,r)\bigr)}{m_d\bigl(\overline B(x,r)\bigr)}.
\end{aligned}
...
\]
这些等式来自每个球中 \(E\) 与其补集的质量之和等于整个球的质量。若 \(m_d(E\mathbin{\triangle} F)=0\)，则 \(E\) 与 \(F\) 在每个球中的质量相同，所以它们在每一点的上、下密度都相同，而不只是几乎处处相同。
...
\cref{b1:fig:local-density-cases}用同一观察球对照三种典型局部占比。图只表示某个小尺度上的几何直观；密度的定义仍要求考察全部充分小的半径。
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[every node/.style={font=\small}]
    \begin{scope}[shift={(-3.2,0)}]
      \fill[gray!65] (0,0) circle (0.9);
      \draw[thick] (0,0) circle (0.9);
      \fill (0,0) circle (1.3pt);
      \node[below=1.05cm] at (0,0) {密度接近 \(1\)};
    \end{scope}
    \begin{scope}
      \begin{scope}
        \clip (0,0) circle (0.9);
        \fill[gray!65] (-1,-1) rectangle (1,0);
      \end{scope}
      \draw[thick] (-1,0)--(1,0);
      \draw[thick] (0,0) circle (0.9);
      \fill (0,0) circle (1.3pt);
      \node[below=1.05cm] at (0,0) {半空间边界：\(1/2\)};
    \end{scope}
    \begin{scope}[shift={(3.2,0)}]
      \fill[gray!65] (-0.62,0.35) circle (0.08);
      \fill[gray!65] (0.53,-0.43) circle (0.06);
      \fill[gray!65] (0.30,0.62) circle (0.05);
      \draw[thick] (0,0) circle (0.9);
      \fill (0,0) circle (1.3pt);
      \node[below=1.05cm] at (0,0) {密度接近 \(0\)};
    \end{scope}
    \node[above] at (-3.2,1.05) {$E\cap B(x,r)$};
    \node[above] at (0,1.05) {$E\cap B(x,r)$};
    \node[above] at (3.2,1.05) {$E\cap B(x,r)$};
  \end{tikzpicture}
  \caption[局部密度]{局部密度。灰色表示 \(E\) 在观察球中所占的部分；三幅图分别示意占比趋近 \(1\)、保持在中间值和趋近 \(0\) 的情形。}
  \label{b1:fig:local-density-cases}
\end{figure}
...
这些都是逐点计算，尚未限制异常点集的大小。\cref{b1:thm:lebesgue-density}将证明：对任意 Lebesgue 可测集 \(E\)，几乎每个属于 \(E\) 的点都是密度点，而在补集内密度几乎处处为零。该结论将在下一节从积分的微分定理推出，这里不把它作为已知性质使用。中文伴读可参看那汤松的《实变函数论》第5版中译本\cite[第九章, 第6节]{Natanson2010RealChinese}，该书以“稠密点、近似连续”为这一话题的节名；本书使用“密度点”，将它与拓扑意义下的稠密性分开。
...
\coloneqq\{\,x\in\mathbb R^d\mid \mu\bigl(B(x,r)\bigr)>0\ \forall r>0\,\}
\]
...
\[
\underline D_\mu\nu(x)\coloneqq\liminf_{r\downarrow0}Q_r^{\mu,\nu}(x),
\quad
\overline D_\mu\nu(x)\coloneqq\limsup_{r\downarrow0}Q_r^{\mu,\nu}(x).
\]
...
当 \(\mu=m_d\) 时，这就回到第一小节的密度。这里的导数由空间中的小球定义，暂时还不是\secref{b1:sec:0604}中由积分等式确定的 Radon--Nikodym 导数。将二者联系起来，既要证明球比值几乎处处收敛，也要辨别奇异部分在极限中的表现。
...
在 \(\operatorname{supp}\mu\) 上，固定半径的质量比 \(Q_r^{\mu,\nu}\) 是 Borel 可测函数。上、下密度也是扩展实值 Borel 可测函数；它们的定义中可把半径限制为正有理数。导数存在的点集以及导数存在且有限的点集均为 Borel 集，导数在这些定义域上是 Borel 可测的。
\end{proposition}
\begin{proof}
...
例如，取 \(\mu=\delta_0+m_d\)、\(\nu=2\delta_0+3m_d\)。在原点，测度导数为 \(2\)；在任意 \(x\ne0\) 处，足够小的球避开原点，质量比恒为 \(3\)。此例中的原点不是可任意修改的 \(\mu\)-零测点，因而密度函数在原点的值也由测度决定。

\ProseParagraphBreak

另一方面，若分母为 \(m_d\)、分子为 \(\delta_0\)，则原点处的比值为 \(1/m_d\bigl(\overline B(0,r)\bigr)\)，趋于 \(+\infty\)；在其他点处，小球质量最终为零。因此 \(D_{m_d}\delta_0\) 在 \(m_d\)-几乎处处等于零。把 \(\delta_0\) 换成 \(2\delta_0\)，甚至会得到处处相同的扩展实值导数，而两测度并不相等。这说明测度比的极限不能未经条件地代回积分，宣称它恢复整个原测度。\cref{b1:sec:1403}将用 Lebesgue 分解精确说明它恢复哪一部分。

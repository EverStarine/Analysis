...
本节采用连续稠密类与极大估计的路线，可对照 Axler 的《Measure, Integration \& Real Analysis》\cite[4.10 Lebesgue Differentiation Theorem, first version]{Axler2020Measure}。先证明通常平均的微分，再把它加强为绝对偏差的平均趋零。两者不能只凭积分符号相似就视为同一陈述。
...
\[
 A_rf(x)\coloneqq\frac1{\omega_dr^d}\int_{B(x,r)}f(y)\,\mathrm dy.
\]
...
\term[lebesgue-weifen-dingli]{Lebesgue 微分定理}[Lebesgue differentiation theorem]说明，局部可积性已经足以保证通常平均在几乎处处恢复函数。其结论涉及选定的有限值可测代表，但任何两个代表得到的结论只会在一个零测集上不同。
...
设 \(\Omega\subseteq\mathbb R^d\) 开，并从 \(L^1_{\mathrm{loc}}(\Omega)\) 的一个等价类中选定有限值可测代表 \(f\)。则对几乎每个 \(x\in\Omega\)，
\[
...
\]
约定到空集的距离为 \(\infty\)。此时 \(K_n\) 是紧集，且 \(\Omega=\bigcup_nK_n\)。若 \(K_n\ne\varnothing\)，令 \(\rho_n=1/(3n)\)，并设
\[
 H_n=\{\,y:\operatorname{dist}(y,K_n)\leq\rho_n\,\}.
\]
它是包含于 \(\Omega\) 的紧集。把 \(f\cdot\mathbf1_{H_n}\) 延为整个空间上的零函数，记为 \(f_n\)，则 \(f_n\in L^1(\mathbb R^d)\)。对 \(x\in K_n\)、\(0<r<\rho_n\)，有 \(A_rf_n(x)=A_rf(x)\)，且 \(f_n(x)=f(x)\)。已证情形因而在 \(K_n\) 的一个零测集之外给出结论。最后对这些零测集作可数并，覆盖整个 \(\Omega\)。空的 \(K_n\) 不需处理。
\end{proof}
...
密度点无需是内点。一个正测度、内点为空的闭集仍在几乎每个自身点具有密度 \(1\)；\chapref{b1:ch:03}的正测度 Cantor 型集合便属此类。定理也没有说每个边界点密度均为 \(0\) 或 \(1\)：半空间的边界点密度为 \(1/2\)，而上一节的环带例子甚至没有密度极限。它限制的是这些异常行为能够占据的 Lebesgue 测度。
...
设 \(\Omega\subseteq\mathbb R^d\) 开，\(1\leq p<\infty\)，并从 \(L^p_{\mathrm{loc}}(\Omega)\) 的一个等价类中选定有限值可测代表 \(f\)。则对几乎每个 \(x\in\Omega\)，
\[
...
\end{equation}
这同样对 \(1\leq p<\infty\) 成立。取 \(K\) 的一个仍在 \(\Omega\) 内的紧邻域 \(H\)，把 \(f\cdot\mathbf1_H\) 延为零。对充分小的 \(r\)，其球平均在 \(K\) 上与 \(A_rf\) 一致；而全空间的平均是核 \(\mathbf1_{B(0,1)}/\omega_d\) 生成的卷积逼近，故由\cref{b1:prop:approximate-identity-lp}得到\eqref{b1:eq:local-average-lp-convergence}。

...
\secref{b1:sec:1602}是本章主线：\chapref{b1:ch:15}的覆盖测度尚不足以精确计算一个映射如何改变体积，
Rademacher 定理提供的几乎处处微分矩阵才使这种计算有了局部对象。
近似微分一节可以在首次读完该定理后继续阅读；
\secref{b1:sec:1604}虽标为选读，但其中的实值绝对连续函数微积分也是后续 Sobolev 理论会用到的基础，
...
又 \(f'(0)=1\) 排除了递减的可能；但一个递增函数在可微点的导数不可能为负，与区间内某个 \(x_n\) 的导数矛盾。

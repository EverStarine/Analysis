...
\item\label{b1:item:ch16-goal-ac}掌握实值绝对连续函数、全变差与微积分基本定理，说明它们为何构成后续一维 Sobolev 代表理论的前置。
...
\secref{b1:sec:1604}建立后续 Sobolev 理论会直接使用的实值绝对连续函数微积分，属于主线；
...
\input{Part03/Chapter16/Section1605}
% 本章小结（不计入编号）
...
其余习题只使用本章主线内容。最后一题把 Rademacher 证明中的三个关键接口拆开重做。
...
\[
 ...
 \end{cases}
\]
证明 \(f\) 是 Lipschitz 函数，在 \((-1,1)\) 处处可微，且 \(f'(0)=1\)，但它在 \(0\) 的任何邻域内都不是单射。
...
\begin{exercise}[\(\star\) 选读]\label{b1:ex:16-banach-and-metric-derivatives}
...
\begin{exercise}[\(\star\) 选读]\label{b1:ex:16-retracing-multiplicity}
...
\begin{exercise}[Rademacher 证明的三个接口]
...
平移换元给出
\[
 \int\Delta_{v,t}f(x)\phi(x)\,\mathrm dx

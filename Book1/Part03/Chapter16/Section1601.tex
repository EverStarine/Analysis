...
\[
 f(a)+L\cdot d(x,a)
 \geq f(a_0)-L\cdot d(a,a_0)+L\cdot d(x,a)
 \geq f(a_0)-L\cdot d(x,a_0).
\]
...
对每个 \(a\in A\)，有
\[
 f(a)+L\cdot d(x,a)\leq f(a)+L\cdot d(y,a)+L\cdot d(x,y).
\]
取下确界得 \(F_+(x)\leq F_+(y)+L\cdot d(x,y)\)，交换两点得到 Lipschitz 条件。
对 \(F_-\) 取上确界可作同样证明。
最后，对任意 \(a,b\in A\)，
\[
 f(a)-L\cdot d(x,a)
 \leq f(b)+L\cdot d(a,b)-L\cdot d(x,a)
 \leq f(b)+L\cdot d(x,b).
\]
